       ...
       public class LicenseTypeRecords_View : PXSelectOrderBy<LicenseTypeGridFilter,
                                                        ...
                                                        PXFilter<StaffSelectionFilter> filter)
        {
            ...
            else 
            {
				...
				for (int i = 0; i < employeeIDList.Count; i += batchSize)
				{
					var batch = employeeIDList.Skip(i).Take(batchSize).ToList();

					allStaffSkillList.AddRange(SharedFunctions.GetItemWithList<FSEmployeeSkill,
																				FSEmployeeSkill.employeeID,
																				FSEmployeeSkill.skillID>(graphFilter, batch));

					allStaffLicenseTypeList.AddRange(SharedFunctions.GetItemWithList<FSLicense,
																						...
																						And<
																							Where<
																								FSLicense.expirationDate, GreaterEqual<Current<StaffSelectionFilter.scheduledDateTimeBegin>>,
																									Or<FSLicense.expirationDate, IsNull>>>>>>>(graphFilter, batch));
				}

				foreach (BAccountStaffMember staffRow in staffSet)
				{
					var employeeSkillList = allStaffSkillList.FirstOrDefault(y => y.itemID == staffRow.BAccountID);
					var employeeLicenseTypeList = allStaffLicenseTypeList.FirstOrDefault(y => y.itemID == staffRow.BAccountID);

					if ((HasEmployeeItemsSelected(employeeSkillList, skillsSelection) == false && staffRow.Selected == false) ||
						(HasEmployeeItemsSelected(employeeLicenseTypeList, licenseTypesSelection) == false && staffRow.Selected == false))
					{
						continue;
					}
