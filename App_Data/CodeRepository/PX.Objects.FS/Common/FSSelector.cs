...
namespace PX.Objects.FS
{
            ...
            : base(BqlTemplate.OfCommand<
                   ...
                   On<
                       FSServiceInventoryItem.inventoryID, Equal<InventoryItem.inventoryID>,
                           And<Current<LineType>, Equal<ListField_LineType_Pickup_Delivery.Pickup_Delivery>>>>>,
                   Where2<
    ...
    public class FSSelector_Driver_RouteDocumentRouteIDAttribute : PXSelectorAttribute
    {
            ...
            : base(
                typeof(Search<FSSODet.sODetID,
                       Where<
                           ...
                           And<FSSODet.status, NotEqual<FSSODet.status.Scheduled>,
    ...
    {
        [PXHidden]
        public partial class FSSOLine : PXBqlTable, IBqlTable
        {
            #region LineNbr
    ...
    {
        [PXHidden]
        public partial class ARTranExt : PXBqlTable, IBqlTable
        {
            #region LineNbr
            public abstract class lineNbr : PX.Data.BQL.BqlInt.Field<lineNbr> { }
            [PXInt]
        ...
        protected virtual IEnumerable GetRecords()
        {
            var rows = PXSelectJoin<ARTran,
                       ...
                       Where<
                           FSxEquipmentModel.equipmentItemClass, Equal<ListField_EquipmentItemClass.ModelEquipment>,
                       And<
                           FSxARTran.equipmentAction, Equal<ListField_EquipmentActionBase.SellingTargetEquipment>,
                       And<
                           Where<
            ...
            if (rows?.Count > 0)
            {
                foreach (PXResult<ARTran, InventoryItem> row in rows)
                {
                    ...
                    InventoryItem inventoryItemRow = (InventoryItem)row;
                    yield return new ARTranExt { LineNbr = ARTranRow.LineNbr, SortOrder = ARTranRow.SortOrder, InventoryID = inventoryItemRow.InventoryCD };
                }
            }
        }
    }
    ...
    public class FSINLotSerialNbrAttribute : PXCustomSelectorAttribute
    {
        ...
        public class ApptINLotSerialStatus : PXBqlTable, PX.Data.IBqlTable, IStatus
        {
            #region Keys
    ...
    public class FSPostBatchAttribute : PXCustomSelectorAttribute
    {
        ...
        [Serializable]
        [PXVirtual]
        public class PostBatch : PXBqlTable, PX.Data.IBqlTable
        {
            #region BatchID
		...
		protected virtual IEnumerable GetRecords()
		{
			...
			FSBillHistory currentFSBillHistory = PXView.Currents.OfType<FSBillHistory>().FirstOrDefault() ?? (FSBillHistory)cache.Current;

			IEnumerable<PostBatch> result;

			if (string.IsNullOrEmpty(currentFSBillHistory.ServiceContractRefNbr))
			{
				result = PXSelect<FSPostBatch,
						Where<FSPostBatch.batchID, Equal<Required<FSPostBatch.batchID>>>>
					.Select(_Graph, currentFSBillHistory.BatchID)
					.RowCast<FSPostBatch>()
					.Select(batch => new PostBatch
					{
						BatchID = batch.BatchID,
						BatchNbr = batch.BatchNbr,
						IsContractBatch = false
					});
			}
			else
			{
				result = PXSelect<FSContractPostBatch,
						Where<FSContractPostBatch.contractPostBatchID, Equal<Required<FSContractPostBatch.contractPostBatchID>>>>
					.Select(_Graph, currentFSBillHistory.BatchID)
					.RowCast<FSContractPostBatch>()
					.Select(contractBatch => new PostBatch
					{
