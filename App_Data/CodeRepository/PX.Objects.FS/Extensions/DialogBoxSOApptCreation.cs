...
namespace PX.Objects.FS
{
        ...
        where TMain : class, IBqlTable, new()
    {
        ...
        protected virtual void _(Events.RowSelected<DBoxDocSettings> e)
        {
			...
			PXUIFieldAttribute.SetEnabled<DBoxDocSettings.projectID>
					(e.Cache, e.Row, ProjectSelectorEnabled);
			PXUIFieldAttribute.SetRequired<DBoxDocSettings.projectTaskID>
					(e.Cache, ProjectDefaultAttribute.IsProject(Base, e.Row.ProjectID));
			PXDefaultAttribute.SetPersistingCheck<DBoxDocSettings.projectTaskID>
					(e.Cache, e.Row, ProjectDefaultAttribute.IsProject(Base, e.Row.ProjectID) ?
												PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
        }
        #endregion
		#endregion
            ...
            bool createAppointment)
        {
            ...
            foreach (DBoxDetails line in details)
            {
                if (line.InventoryID != null)
                {
					...
					if (line.ProjectID != null && line.ProjectTaskID != null)
					{
					...
					{
						srvOrdLine.ProjectID = header.ProjectID;
						srvOrdLine.ProjectTaskID = header.ProjectTaskID;
					}
