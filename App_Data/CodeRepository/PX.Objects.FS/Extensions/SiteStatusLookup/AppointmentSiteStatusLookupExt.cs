...
namespace PX.Objects.FS.SiteStatusLookup
{
	...
	public class AppointmentSiteStatusLookupExt : FSSiteStatusLookupExt<AppointmentEntry, FSAppointment, FSAppointmentDet>
	{
		protected override FSAppointmentDet CreateNewLine(FSSiteStatusSelected line)
		{
			...
			FSAppointmentDet newline = PXCache<FSAppointmentDet>.CreateCopy(Base.AppointmentDetails.Insert(new FSAppointmentDet()));
			...
			newline = PXCache<FSAppointmentDet>.CreateCopy(Base.AppointmentDetails.Update(newline));

			if (line.BillingRule == ID.BillingRule.TIME)
			{
				newline.EstimatedDuration = Base.IsMobile == true ? line.EstimatedDuration : line.DurationSelected;
			}
			else
			{
				newline.EstimatedQty = line.QtySelected;

				if (Base.AppointmentRecords.Current.AreActualFieldsActive == true)
					newline.ActualQty = line.QtySelected;
			}

			return Base.AppointmentDetails.Update(newline);
		}
	}
}
