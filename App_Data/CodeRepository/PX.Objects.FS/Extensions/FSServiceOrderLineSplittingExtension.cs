...
namespace PX.Objects.FS
{
	...
	public class FSServiceOrderLineSplittingExtension : LineSplittingExtension<ServiceOrderEntry, FSServiceOrder, FSSODet, FSSODetSplit>
	{
		...
		protected override void SubscribeForSplitEvents()
		{
			...
			ManualEvent.Row<FSSODetSplit>.Deleting.Subscribe(Base, EventHandler);
		...
		protected virtual void EventHandler(ManualEvent.Row<FSSODetSplit>.Deleting.Args e)
		{
			if (e.Row == null)
			{
				return;
			...
			if ((e.Row.POCreate == true &&
				(string.IsNullOrEmpty(e.Row.PONbr) == false || string.IsNullOrEmpty(e.Row.POType) == false))
				|| (string.IsNullOrEmpty(e.Row.POReceiptNbr) == false || string.IsNullOrEmpty(e.Row.POReceiptType) == false))
			{
				throw new PXException(TX.Error.CannotDeleteLineBecausePOIsAlreadyCreated);
			}
		}
		...
		{
			FSSODetSplit split = base.EnsureSplit(row);
