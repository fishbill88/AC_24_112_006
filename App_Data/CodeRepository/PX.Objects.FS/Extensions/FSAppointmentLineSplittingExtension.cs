...
namespace PX.Objects.FS
{
	...
	public class FSAppointmentLineSplittingExtension : LineSplittingExtension<AppointmentEntry, FSAppointment, FSAppointmentDet, FSApptLineSplit>
	{
		...
		protected override void SubscribeForSplitEvents()
		{
			...
			ManualEvent.Row<FSAppointment>.Selected.Subscribe(Base, EventHandler);
		}

		protected virtual void EventHandler(ManualEvent.Row<FSAppointment>.Selected.Args e)
		{
			showSplits.SetEnabled(Base.AppointmentDetails.Any());
		}
		...
		public virtual bool VerifyLotSerialTotalQty(FSAppointmentDet apptLine, decimal newIncrease, bool runningFieldVerifying)
		{
			...
			{
			PXResult<InventoryItem, INLotSerClass> item = ReadInventoryItem(apptLine.InventoryID);

			if (item == null)
				throw new PXException(TX.Error.RECORD_X_NOT_FOUND, DACHelper.GetDisplayName(typeof(InventoryItem)));

			newSplitTotalQty = INUnitAttribute.ConvertFromTo<FSAppointmentDet.inventoryID>
				(LineCache, apptLine, ((InventoryItem)item).BaseUnit, apptLine.UOM, newSplitTotalQty, INPrecision.QUANTITY);
			}
			...
			if (pendingQty > 0m)
			{
				...
				foreach (FSSODetSplit soSplit in soSplitsWithLotSerial)
				{
					...
					if (soSplitBalance > 0m)
					{
						...
						decimal adjustedSOSplitBalance = INUnitAttribute.ConvertFromTo<FSApptLineSplit.inventoryID>(SplitCache, apptSplit, apptSplit.UOM, apptLine.UOM, soSplitBalance, INPrecision.NOROUND);

						if (adjustedSOSplitBalance > pendingQty)
							soSplitBalance = INUnitAttribute.ConvertFromTo<FSApptLineSplit.inventoryID>(SplitCache, apptSplit, apptLine.UOM, apptSplit.UOM, pendingQty, INPrecision.QUANTITY);
						...
						pendingQty -= adjustedSOSplitBalance > pendingQty ? pendingQty : adjustedSOSplitBalance;

						SplitCache.Update(apptSplit);
					}

					decimal roundedPendingQuantity = pendingQty;

					if (apptSplit != null)
						INUnitAttribute.ConvertFromTo<FSApptLineSplit.inventoryID>(SplitCache, apptSplit, apptLine.UOM, apptSplit.UOM, pendingQty, INPrecision.QUANTITY);

					if (roundedPendingQuantity <= 0m)
						break;
				}
			}
		}
		...
		protected virtual void UpdateLotSerialSplitsBasedOnLineLotSerial(FSAppointmentDet apptLine, string lotSerTrack, bool? lotSerTrackExpiration)
		{
			...
			{
				lotSerialSplit = SplitCache.Rows.CreateCopy(SplitCache.Insert(new FSApptLineSplit()));
		...
		{
			FSApptLineSplit split = base.EnsureSplit(row);

			if (split != null && split.InventoryID != null && split.SubItemID == null)
			{
				split.SubItemID = InventoryItem.PK.Find(Base, split.InventoryID).DefaultSubItemID;
			}
