...
namespace PX.Objects.FS
{
    public class RouteScheduleProcess : ContractGenerationEnqBase<RouteScheduleProcess, FSRouteContractScheduleFSServiceContract, RouteServiceContractFilter, FSContractGenerationHistory.recordType.RouteServiceContract>
    {
        ...
        [CustomerActive(Visibility = PXUIVisibility.SelectorVisible, BqlField = typeof(FSServiceContract.customerID), DescriptionField = typeof(Customer.acctName), Filterable = true)]
        [PXDefault()]
        protected virtual void FSRouteContractScheduleFSServiceContract_CustomerID_CacheAttached(PXCache sender)
        {
        }
        
        [LocationActive(typeof(Where<Location.bAccountID, Equal<Optional<ARRegister.customerID>>,
            And<MatchWithBranch<Location.cBranchID>>>), BqlField = typeof(FSServiceContract.customerLocationID), DescriptionField = typeof(Location.descr), Visibility = PXUIVisibility.SelectorVisible)]
        protected virtual void FSRouteContractScheduleFSServiceContract_CustomerLocationID_CacheAttached(PXCache sender)
        {
        }
