...
namespace PX.Objects.FS
{
    ...
    public class ServiceOrderBase<TGraph, TPrimary> : PXGraph<TGraph, TPrimary>, ISiteStatusLookupCompatible
        ...
        where TPrimary : class, PX.Data.IBqlTable, new()
    {
		...
		public virtual string GetDefaultTaxZone(int? billCustomerID, int? billLocationID, int? branchID, int? projectId)
		{
			string result = null;
			...
			var customerLocation = (Location)PXSelect<Location,
												 ...
												 .Select(this, billCustomerID, billLocationID);
			...
			if (result == null)
			{
				if (this.Caches<FSAddress>().Current is FSAddress fsAddress)
				{
		...
		public virtual void FSAddress_RowUpdated_Handler(Events.RowUpdated<FSAddress> e, PXCache cache)
		{
			if (e.Row == null)
			{
				return;
			...
			var newAddress = e.Row;
			var oldAddress = e.OldRow;
			
			if (oldAddress.PostalCode == newAddress.PostalCode &&
				oldAddress.CountryID == newAddress.CountryID &&
			...
			switch (cache.Current)
			{
				case FSServiceOrder fsServiceOrder:
					cache.SetDefaultExt<FSServiceOrder.taxZoneID>(fsServiceOrder);
					break;
				case FSAppointment fsAppointment:
					cache.SetDefaultExt<FSAppointment.taxZoneID>(fsAppointment);
					break;
			}
		}
				...
				if ((curyChanged || projectChanged || branchChanged || projectTasksChanged || billServiceContractChanged || detailsForUpdate.Any()) && serviceOrderAppointments.Any())
                    {
						...
						if (fsAppointmentRow_local.ProjectID != fsServiceOrderRow.ProjectID)
						{
		...
		protected bool HaveAnyBilledAppointmentsInServiceOrder(PXGraph graph, int? SOID )
		{
			return PXSelect<FSAppointment,
				...
				SelectWindowed(graph, 0, 1, SOID).Any();
		}
                ...
                where DAC : class, IBqlTable, IFSSODetBase, new()
        {
			...
			isEnabled = isEnabled && row.IsFree == false;

			SetEnabledAndPersistingCheck<FSSODet.manualDisc>(cache, eRow, eventType, isEnabled, persistingCheck: null);

			if (fsAppointmentDetRow != null)
			{
				SetEnabledAndPersistingCheck<FSAppointmentDet.manualDisc>(cache, eRow, eventType, isEnabled, persistingCheck: null);
			}
                                                     ...
                                                     FSServiceOrder fsServiceOrder)
        {
            ...
            if (bqlResultSet_SODet.Count > 0)
            {
				throw PXRedirectToBoardRequiredException.GenerateMultiEmployeeRedirect(
					SiteMapProvider,
					GetServiceOrderUrlArguments(fsServiceOrder),
					new MainAppointmentFilter() { InitialSORefNbr = fsServiceOrder.RefNbr },
                                                              ...
                                                              FSAppointment fsAppointmentRow)
        {
			...
			bool enableTravel = fsAppointmentRow == null ? false : fsAppointmentRow.NotStarted == false && fsAppointmentRow.Hold == false;
            ...
            where SODetType : FSSODet, new()
        {
			...
			PXCache cacheSODet = cacheAppointmentDet.Graph.Caches<FSSODet>();
        ...
        public virtual IEnumerable<FSProfitability> ProfitabilityRecords_INItems(PXGraph graph, FSServiceOrder serviceOrder, FSAppointment appointment)
        {
            if (appointment != null) 
            {
				return PXSelect<FSAppointmentDet,
					Where<
            ...
            { 
				...
				return PXSelect<FSSODet,
						...
						.RowCast<FSSODet>()
						.Where(x => IsValidForProfitability(x))
						.Select(x => PrepareProfitabilityRow(x, billingBy));
            }
			else
			{
