        ...
        where TGraph : PXGraph
    {
        ...
        public virtual void CleanPostingInfoLinkedToDoc(object createdDoc)
        {
            ...
            if (createdDoc is SOOrder)
            {
                ...
                createdRefNbr = soOrder.OrderNbr;
        ...
        {
			PXCache cacheARTran = graph.Caches[typeof(ARTran)];

			var serviceContractGroups = PXSelect<ARTran,
					Where<ARTran.tranType, Equal<Required<ARTran.tranType>>,
						And<ARTran.refNbr, Equal<Required<ARTran.refNbr>>>>>
							  ...
							  {
								  arTran = x,
								  fsxARTran = cacheARTran.GetExtension<FSxARTran>(x)
							  })
			...
			if (serviceContractGroups.Count() > 0)
            {
                ...
                foreach (var serviceContractGroup in serviceContractGroups)
                {
                    ...
                    if (serviceContract.fsxARTran.ServiceContractRefNbr != null)
                    {
						var tempARTran = (ARTran)cacheARTran.CreateCopy(serviceContract.arTran);
						var tempFSxARTran = cacheARTran.GetExtension<FSxARTran>(tempARTran);
						tempFSxARTran.SrvOrdType = null;
						tempFSxARTran.ServiceOrderRefNbr = null;
						tempFSxARTran.AppointmentRefNbr = null;

                        CreateBillHistoryRowsFromARTran(cacheFSBillHistory, tempARTran, tempFSxARTran,
                        ...
                        if (parentEntityType != null && parentDocType != null && parentRefNbr != null)
                        {
							...
							var newARTran = new ARTran();
							var newFSxARTran = cacheARTran.GetExtension<FSxARTran>(newARTran);
							newFSxARTran.ServiceContractRefNbr = tempFSxARTran.ServiceContractRefNbr;

                            foreach (FSBillHistory billHistory in billHistoryRows)
                            {
								newFSxARTran.SrvOrdType = billHistory.SrvOrdType;
								newFSxARTran.ServiceOrderRefNbr = billHistory.ServiceOrderRefNbr;
								newFSxARTran.AppointmentRefNbr = billHistory.AppointmentRefNbr;

                                CreateBillHistoryRowsFromARTran(cacheFSBillHistory, newARTran, newFSxARTran,
                                        childEntityType, childDocType, childRefNbr,
                                        parentEntityType, parentDocType, parentRefNbr,
                                        false);
                            }
                    ...
                    {
                        var fsDocuments = serviceContractGroup.Distinct(x => (x.fsxARTran.SrvOrdType, x.fsxARTran.ServiceOrderRefNbr, x.fsxARTran.AppointmentRefNbr));

                        foreach (var fsDocument in fsDocuments)
                        {
                            CreateBillHistoryRowsFromARTran(cacheFSBillHistory, fsDocument.arTran, fsDocument.fsxARTran,
                                    childEntityType, childDocType, childRefNbr,
                                    parentEntityType, parentDocType, parentRefNbr,
                                    true);
                        }
        ...
        public virtual FSBillHistory CreateBillHistoryRowsFromARTran(PXCache billHistoryCache, ARTran arTran, FSxARTran fsxARTranRow,
                                        string childEntityType, string childDocType, string childRefNbr,
                                        string parentEntityType, string parentDocType, string parentRefNbr,
                                        bool verifyIfExists)
        {
            ...
            {
                if (arTran.SOOrderType == null || arTran.SOOrderNbr == null)
                {
                    return null;
                }
            }
			...
			FSBillHistory newRow = new FSBillHistory
			{
				...
				ParentDocType = parentEntityType == FSEntityType.SalesOrder && parentDocType == null ? arTran.SOOrderType : parentDocType,
				ParentRefNbr = parentEntityType == FSEntityType.SalesOrder && parentRefNbr == null ? arTran.SOOrderNbr : parentRefNbr,
