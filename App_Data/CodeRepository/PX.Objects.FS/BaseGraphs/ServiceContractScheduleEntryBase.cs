...
{
    public class ServiceContractScheduleEntryBase<TGraph, TPrimary, TScheduleID, TEntityID, TCustomerID> : PXGraph<TGraph, TPrimary>, ISiteStatusLookupCompatible
        ...
        where TCustomerID : PX.Data.IBqlField
    {
		...
		/// <exclude/>
		public void SyncFSSalesPrice(PXCache cache, int? serviceContractID, PXDBOperation operation, FSScheduleDet modifiedRow)
        {
            ...
            if (operation == PXDBOperation.Insert || operation == PXDBOperation.Update)
            {
                ...
                if (fsServiceContractRow != null)
                {
                    ...
                    foreach (PXResult<FSSchedule, FSScheduleDet, FSServiceTemplateDet, FSSalesPrice> result in list)
                    {
						...
						if (fsScheduleDetRow.LineType == ID.LineType_ALL.SERVICE_TEMPLATE &&
							modifiedRow != null &&
							!cache.ObjectsEqual(fsScheduleDetRow, modifiedRow))
						{
							continue;
						}
                        ...
                        if (inventoryItemRow != null)
                        {
                            ...
                            fsSalesPriceRow.UOM = fsScheduleDetRow?.UOM ?? fsServiceTemplateRow?.UOM;
            ...
            if (operation == PXDBOperation.Delete || operation == PXDBOperation.Update)
            {
                ...
                foreach (PXResult<FSServiceContract, FSSchedule, FSScheduleDet, FSServiceTemplateDet> existingInventory in existingInventoryList)
                {
					...
					string uom = fsSCheduleDetRow.LineType == ID.LineType_ALL.SERVICE_TEMPLATE ? fsServiceTemplateDetRow.UOM : fsSCheduleDetRow.UOM;

					if (inventoryID != null)
                    {
                        FSSalesPrice existingSalesPriceRow = existingSalesPrice
							.RowCast<FSSalesPrice>()
							.Where(sp =>
							{
								if (fsSCheduleDetRow.LineType == ID.LineType_ALL.SERVICE_TEMPLATE && modifiedRow != null)
								{
									return !cache.ObjectsEqual(modifiedRow, fsSCheduleDetRow);
								}

								return sp.InventoryID == inventoryID && sp.UOM == uom;
							})
							.FirstOrDefault();
        ...
        public virtual void EnableDisable_LineType(PXCache cache, FSScheduleDet fsScheduleDetRow)
        {
			...
			PXUIFieldAttribute.SetEnabled<FSScheduleDet.uOM>(cache, fsScheduleDetRow, isInventoryItem);
        ...
        protected virtual void _(Events.RowPersisted<FSSchedule> e)
        {
            ...
            if (e.TranStatus == PXTranStatus.Open)
            {
                ...
                {
                    FSSchedule fsScheduleRow = (FSSchedule)e.Row;
					// Acuminator disable once PX1043 SavingChangesInEventHandlers [Legacy code refactoring]
					// Acuminator disable once PX1073 ExceptionsInRowPersisted [Legacy code refactoring]
					SyncFSSalesPrice(e.Cache, CurrentServiceContract.Current.ServiceContractID, e.Operation, null);
					// Acuminator disable once PX1043 SavingChangesInEventHandlers [Legacy code refactoring]
					// Acuminator disable once PX1073 ExceptionsInRowPersisted [Legacy code refactoring]
		...
		protected virtual void _(Events.FieldUpdated<FSScheduleDet, FSScheduleDet.inventoryID> e)
        {
			...
			e.Cache.SetDefaultExt<FSScheduleDet.uOM>(e.Row);
		}
        ...
        protected virtual void _(Events.RowPersisted<FSScheduleDet> e)
        {
            ...
            if (e.TranStatus == PXTranStatus.Open)
            {
				// Acuminator disable once PX1043 SavingChangesInEventHandlers [Legacy code refactoring]
				// Acuminator disable once PX1073 ExceptionsInRowPersisted [Legacy code refactoring]
				SyncFSSalesPrice(e.Cache, CurrentServiceContract.Current.ServiceContractID, e.Operation, e.Row);
				// Acuminator disable once PX1043 SavingChangesInEventHandlers [Legacy code refactoring]
				// Acuminator disable once PX1073 ExceptionsInRowPersisted [Legacy code refactoring]
