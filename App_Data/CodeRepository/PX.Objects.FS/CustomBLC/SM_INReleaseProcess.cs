            ...
            if (fsPostRegisterRow.PostedTO == ID.Batch_PostTo.SI)
            {
                if (fsPostRegisterRow.EntityType == ID.PostDoc_EntityType.SERVICE_ORDER)
                {
                    var soInvoiceINTranRows = PXSelectJoin<INTran,
                                        InnerJoin<ARTran,
                                            On<INTran.aRDocType, Equal<ARTran.tranType>,
                                            And<INTran.aRRefNbr, Equal<ARTran.refNbr>,
                                            And<INTran.aRLineNbr, Equal<ARTran.lineNbr>>>>,
                                        LeftJoin<FSSODet,
                                                On<FSSODet.srvOrdType, Equal<FSxARTran.srvOrdType>,
                                                   And<FSSODet.refNbr, Equal<FSxARTran.serviceOrderRefNbr>,
                                                   And<FSSODet.lineNbr, Equal<FSxARTran.serviceOrderLineNbr>,
                                                   And<FSxARTran.appointmentRefNbr, IsNull>>>>,
                                        LeftJoin<FSPostDet,
                    ...
                    foreach (PXResult<INTran, ARTran, FSSODet, FSPostDet> bqlResult in soInvoiceINTranRows)
                    {
                        inTranProcessList.Add(new INTranProcess()
                        {
                            appDet = null,
                ...
                else if(fsPostRegisterRow.EntityType == ID.PostDoc_EntityType.APPOINTMENT)
                {
                    var soInvoiceINTranRows = PXSelectJoin<INTran,
                                        InnerJoin<ARTran,
                                            ...
                                            And<INTran.aRLineNbr, Equal<ARTran.lineNbr>>>>,
                                        LeftJoin<FSAppointmentDet,
                                                On<FSAppointmentDet.srvOrdType, Equal<FSxARTran.srvOrdType>,
                                                   And<FSAppointmentDet.refNbr, Equal<FSxARTran.appointmentRefNbr>,
                                                   And<FSAppointmentDet.lineNbr, Equal<FSxARTran.appointmentLineNbr>>>>,
                                        LeftJoin<FSPostDet,
                    ...
                    foreach (PXResult<INTran, ARTran, FSAppointmentDet, FSPostDet> bqlResult in soInvoiceINTranRows)
                    {
                        inTranProcessList.Add(new INTranProcess()
                        {
                            appDet = (FSAppointmentDet)bqlResult,
