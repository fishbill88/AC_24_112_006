...
namespace PX.Objects.FS
{
    public class SM_SOInvoiceEntry : FSPostingBase<SOInvoiceEntry>
    {
		...
		protected virtual void _(Events.FieldDefaulting<ARTran, ARTran.uOM> e)
		{
			if (e.Row == null || e.Args.NewValue != null)
			{
				return;
			}
			...
			e.NewValue = SharedFunctions.IsLotSerialRequired(e.Cache, e.Row.InventoryID) ?
				inventoryItem?.BaseUnit : inventoryItem?.SalesUnit;
		}
        ...
        protected virtual void _(Events.RowUpdating<ARTran> e)
        {
			...
			var fsRow = e.Cache.GetExtension<FSxARTran>(e.Row);
        ...
        protected virtual void _(Events.RowPersisted<ARTran> e)
        {
            ...
            if (e.TranStatus == PXTranStatus.Open)
            {
                ...
                {
					var fsxARTran = e.Cache.GetExtension<FSxARTran>(e.Row);

					var fsBillHistory = CreateBillHistoryRowsFromARTran(BillHistoryRecords.Cache,
							e.Row, fsxARTran, FSEntityType.SOInvoice, soInvoiceRow.DocType, soInvoiceRow.RefNbr,
                            FSEntityType.SalesOrder, null, null, true);
            ...
            try
            {
				...
				foreach (DocLineExt docLineExt in docLines)
				{
					...
					if (isLotSerialRequired)
					{
						...
						{
							var qtyInsertedAdjusted = INUnitAttribute.ConvertFromTo<ARTran.inventoryID>(
							...
							if (qtyInsertedAdjusted != docLineExt.docLine.GetQty(FieldType.BillableField))
							{
					...
					foreach (SharedClasses.SOARLineEquipmentComponent currLineModel in componentList.Where(x => x.equipmentAction == ID.Equipment_Action.SELLING_TARGET_EQUIPMENT))
					{
																														...
																														|| x.equipmentAction == ID.Equipment_Action.NONE)))
						{
							if (currLineComponent.sourceNewTargetEquipmentLineNbr == currLineModel.sourceLineRef)
							{
								...
								currLineComponent.fsxARTranRow.NewEquipmentLineNbr = currLineModel.currentLineRef;
								Base.Transactions.Update(currLineComponent.arTranRow);
							}
						}
					}
				}
												  ...
												  bool firstInADoc /*indicates if the docLineExt is the first one in current Service Order or Appointment*/)
        {
            ...
            if (docLine.SubID != null)
            {
                try
                {
					...
					Base.Transactions.Cache.SetDefaultExt<ARTran.expenseSubID>(arTranRow);
                }
                catch (PXException)
                {
            ...
            Base.Transactions.Cache.SetValueExtIfDifferent<ARTran.manualPrice>(arTranRow, docLine.ManualPrice);
				...
				decimal curyUnitPrice = (docLine.CuryUnitPrice * invtMult) ?? 0m;

				if (arTranRow.UOM != docLine.UOM)
				{
					curyUnitPrice = INUnitAttribute.ConvertFromTo<ARTran.inventoryID>
						(Base.Transactions.Cache, arTranRow, arTranRow.UOM, docLine.UOM, curyUnitPrice, INPrecision.UNITCOST);
				}

				Base.Transactions.Cache.SetValueExtIfDifferent<ARTran.curyUnitPrice>(arTranRow, curyUnitPrice);
			...
			decimal qtyForPriceCalculation = arTranRow.Qty ?? 0m;

			if (arTranRow.UOM != docLine.UOM)
			{
				qtyForPriceCalculation = INUnitAttribute.ConvertFromTo<ARTran.inventoryID>
					(Base.Transactions.Cache, arTranRow, arTranRow.UOM, docLine.UOM, qtyForPriceCalculation, INPrecision.NOROUND);
			}

			decimal? curyExtPrice = docLine.GetQty(FieldType.BillableField) != 0 ? ((docLine.CuryBillableExtPrice * invtMult) / docLine.GetQty(FieldType.BillableField)) * qtyForPriceCalculation : 0m;
            ...
            if (PXAccess.FeatureInstalled<FeaturesSet.equipmentManagementModule>())
            {
                ...
                {
                    fsxARTranRow.EquipmentAction = docLine.EquipmentAction;
                    fsxARTranRow.SMEquipmentID = docLine.SMEquipmentID;
                    fsxARTranRow.EquipmentComponentLineNbr = docLine.EquipmentLineRef;
