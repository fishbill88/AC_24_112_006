	...
	public class ServiceOrderEntry : ServiceOrderBase<ServiceOrderEntry, FSServiceOrder>, IGraphWithInitialization
	{
		...
		public override void Persist()
		{
			...
			if (!RecalculateExternalTaxesSync && order?.SkipExternalTaxCalculation == false) //When the calling process is the 'UI' thread.
			{
				RecalculateExternalTaxes();
				this.SelectTimeStamp();
				ServiceOrderRecords.Current.tstamp = this.TimeStamp;
			}
		...
		[PXCopyPasteHiddenFields(typeof(FSSODet.status), typeof(FSSODet.curyBillableExtPrice), typeof(FSSODet.curyBillableTranAmt))]
		...
		{
			var historyList = PXSelect<FSBillHistory,
				Where<
					FSBillHistory.srvOrdType, Equal<Current<FSServiceOrder.srvOrdType>>,
					And<FSBillHistory.serviceOrderRefNbr, Equal<Current<FSServiceOrder.refNbr>>>>,
				OrderBy<Desc<FSBillHistory.createdDateTime>>>
			.Select(this)
			.AsEnumerable()
			.Where(rec =>
			{
		...
		public virtual IEnumerable GetProfitabilityRecords(FSServiceOrder row)
		{
			...
			var INItems = ProfitabilityRecords_INItems(dummyGraph, row, null);
		...
		public virtual decimal CalcEffectiveCostTotal(FSServiceOrder order)
		{
			...
			var inItems = ProfitabilityRecords_INItems(dummyGraph, order, null);
		...
		// Acuminator disable once PX1013 PXActionHandlerInvalidReturnType -- legacy code
				...
				case ID.SrvOrdType_SalesAcctSource.CUSTOMER_LOCATION:
					...
					Location customerLocationRow = PXSelect<Location,
								   Where<
										Location.bAccountID, Equal<Required<Location.bAccountID>>,
										And<Location.locationID, Equal<Required<Location.locationID>>>>>
								   .Select(graph, fsServiceOrderRow.BillCustomerID, fsServiceOrderRow.BillLocationID);
		...
		public static void UpdateServiceOrderUnboundFields(PXGraph graph, FSServiceOrder fsServiceOrderRow, FSAppointment fsAppointmentRow, string billingBy, bool disableServiceOrderUnboundFieldCalc, bool calcPrepaymentAmount)
		{
			...
			if (calcPrepaymentAmount == true)
			{
				...
				fsServiceOrderRow.SOCuryUnpaidBalanace = fsServiceOrderRow.CuryDocTotal ?? 0;
				fsServiceOrderRow.SOCuryBillableUnpaidBalanace = fsServiceOrderRow.CuryEffectiveBillableDocTotal ?? 0;
				...
				foreach (PXResult<ARPayment> row in resultSet)
				{
					...
					if (arPaymentRow.Status != ARDocStatus.Voided)
					{
		...
		protected virtual void _(Events.RowDeleting<FSServiceOrder> e)
		{
			...
			var unDeletedBillingDocs =
				SelectFrom<FSBillHistory>
					.Where<FSBillHistory.srvOrdType.IsEqual<FSServiceOrder.srvOrdType.FromCurrent>
						.And<FSBillHistory.appointmentRefNbr.IsEqual<FSServiceOrder.refNbr.FromCurrent>>>
				.View
				.Select(this)
				.RowCast<FSBillHistory>()
				.Where(x => x.IsChildDocDeleted == false);

			if (unDeletedBillingDocs.Any())
			{
				e.Cancel = true;
		...
		protected virtual void _(Events.RowDeleting<FSSODet> e)
		{
			...
			if (fsSODetRow.EnablePO == true &&
				(string.IsNullOrEmpty(fsSODetRow.PONbr) == false || string.IsNullOrEmpty(fsSODetRow.POType) == false))
			{
				throw new PXException(TX.Error.CannotDeleteLineBecausePOIsAlreadyCreated, fsSODetRow.SourceLineNbr);
			}
		...
		public class SalesTax : TaxGraph<ServiceOrderEntry, FSServiceOrder>
		{
			...
			{
				Base.FSAddress_RowUpdated_Handler(e, Base.ServiceOrderRecords.Cache);
			}
			#endregion
		}
		...
		/// </summary>
		public class PerUnitTaxDisableExt : PX.Objects.Extensions.PerUnitTax.PerUnitTaxDataEntryGraphExtension<ServiceOrderEntry, FSServiceOrderTaxTran>
		{
		...
		{
			return GetDefaultTaxZone(row.BillCustomerID, row.BillLocationID, row.BranchID, row.ProjectID);
		}
