...
namespace PX.Objects.FS
{
    public class AppointmentEntry : ServiceOrderBase<AppointmentEntry, FSAppointment>, IGraphWithInitialization
    {
		...
		public bool SkipEarningTypeCheck = false;
        ...
        public virtual void SaveBeforeApplyAction(PXCache cache, FSAppointment row)
        {
            ...
            {
				using (new PX.SM.SuppressErpTransactionsScope(true))
				{
                SkipTaxCalcAndSave();
            ...
            : base()
        {
            ...
            if (!PXGraph.ProxyIsActive && TimeCardHelper.CanCurrentUserEnterTimeCards(this, this.Accessinfo.ScreenID) == false)
            {
		...
		protected virtual void _(Events.CacheAttached<FSSchedule.recurrenceDescription> e) { }
		#endregion
                    ...
                    try
                    {
                        ...
                        var errorWaitingPORows = AppointmentDetails.Select().RowCast<FSAppointmentDet>()
                                                                    .Where(r => r.EnablePO == true
                                                                            && r.POCompleted != true
                                                                            && r.Status == FSAppointmentDet.status.WaitingForPO);
        ...
        public virtual IEnumerable OpenEmployeeBoard(PXAdapter adapter)
        {
            ...
            if (!adapter.MassProcess && list.Count() > 0)
            {
				...
				throw PXRedirectToBoardRequiredException.GenerateMultiEmployeeRedirect(
					SiteMapProvider,
					GetAppointmentUrlArguments(AppointmentRecords.Current),
					new MainAppointmentFilter() { InitialRefNbr = AppointmentRecords.Current.RefNbr },
		...
		public PXSelectReadonly2<ARPayment,
				...
				Where<FSAdjust.adjdOrderType, Equal<Current<FSServiceOrder.srvOrdType>>,
					And<FSAdjust.adjdOrderNbr, Equal<Current<FSServiceOrder.refNbr>>,
					And<ARPayment.status, NotEqual<ARDocStatus.voided>>>>> Adjustments;
		...
		{
			var historyList = SelectFrom<FSBillHistory>
				...
				.OrderBy<FSBillHistory.createdDateTime.Desc>
			.View
			.Select(this)
			.AsEnumerable()
			.Where(rec =>
			{
				var hst = rec.GetItem<FSBillHistory>();

				return hst.IsChildDocDeleted != true;
			})
			.ToList();
        ...
        {
            var INItems = ProfitabilityRecords_INItems(this, null, AppointmentRecords.Current);
		...
		private void UpdateSrvOrdSplits(AppointmentEntry apptGraph, FSAppointmentDet apptLine, List<FSApptLineSplit> apptSplits, ServiceOrderEntry soGraph)
        {
            ...
            decimal origEstimatedQty = (decimal)soLine.BaseEstimatedQty;
            ...
            decimal surplusQuantity = (decimal)soLine.BaseEstimatedQty > origEstimatedQty ? (decimal)soLine.BaseEstimatedQty - origEstimatedQty : 0m;
            while (surplusQuantity > 0m)
            {
                ...
                if (soSplit != null)
                {
                    ...
                    if (soSplitCopy.BaseQty >= surplusQuantity)
                    {
                        soSplitCopy.BaseQty -= surplusQuantity;
                        surplusQuantity = 0m;
                    }
                    else
                    {
                        surplusQuantity -= (decimal)soSplitCopy.BaseQty;
                        soSplitCopy.BaseQty = 0m;
                    }

                    if (soSplitCopy.BaseQty == 0m)
                    {
                        soGraph.Splits.Delete(soSplit);
                    }
                    else
                    {
                        soSplitCopy.Qty = INUnitAttribute.ConvertFromBase(soGraph.Splits.Cache, soSplitCopy.InventoryID, soSplitCopy.UOM, soSplitCopy.BaseQty.Value, INPrecision.QUANTITY);
            ...
            if (origEstimatedQty != (decimal)soLine.BaseEstimatedQty)
            {
        ...
        {
			var signed =
				PXSelectJoin<NoteDoc,
						InnerJoin<UploadFile, On<UploadFile.fileID, Equal<NoteDoc.fileID>,
							And<UploadFile.name, Like<Required<UploadFile.name>>>>>,
						Where<NoteDoc.noteID, Equal<Required<NoteDoc.noteID>>>>
					.Select(this, "%signature%", fsAppointmentRow.NoteID)
					.Any();
        ...
        public virtual void VerifySrvOrdLineQty(PXCache cache, FSAppointmentDet apptLine, object newValue, Type QtyField, bool runningFieldVerifying)
        {
            if (newValue == null 
                ...
                || apptLine.IsCanceledNotPerformed == true)
            {
                return;
            }
        ...
        {
            if (newStatus != FSAppointmentDet.status.CANCELED
                ...
                && newStatus != FSAppointmentDet.status.RequestForPO)
            {
                ...
                {
                    return FSAppointmentDet.status.WaitingForPO;
                }
                else if (row.ShouldBeRequestPO == true)
                {
                    return FSAppointmentDet.status.RequestForPO;
                }
            }
        ...
        public virtual void SendNotification(AppointmentEntry graphAppointmentEntry, PXCache sourceCache, string notificationCD, int? branchID, IDictionary<string, string> parameters, IList<Guid?> attachments = null)
        {
            ...
            var setups = new NotificationUtility(graphAppointmentEntry).SearchSetup(FSNotificationSource.Appointment, notificationCD, branchID);
            NotificationSetup setup = setups.SetupWithBranch ?? setups.SetupWithoutBranch;

            Guid? setupID = setup?.SetupID;
        ...
        public virtual void Send(AppointmentEntry graphAppointmentEntry, PXCache sourceCache, Guid setupID, int? branchID, IDictionary<string, string> reportParams, IList<Guid?> attachments = null)
        {
            ...
            if (source.NotificationID != null)
            {
                ...
                sender.BodyFormat = source.ReportID != null ? NotificationFormat.Html : source.Format;
        ...
        protected virtual void _(Events.RowDeleting<FSAppointment> e)
        {
			...
			bool unDeletedDocumentsExist =
				SelectFrom<FSBillHistory>
					.Where<FSBillHistory.srvOrdType.IsEqual<FSAppointment.srvOrdType.FromCurrent>
						.And<FSBillHistory.appointmentRefNbr.IsEqual<FSAppointment.refNbr.FromCurrent>>>
				.View
				.Select(this)
				.Any(bh => ((FSBillHistory)bh).IsChildDocDeleted == false);

			if (unDeletedDocumentsExist)
			{
				e.Cancel = true;
		...
		#endregion
		#region FieldVerifying
		protected virtual void _(Events.FieldVerifying<FSAppointmentEmployee, FSAppointmentEmployee.earningType> e)
		{
			if (e.Row == null)
			{
				return;
			...
			// TODO: We definety don't need to use default verification every time - it creates cache (and AppDomains!) for EPEarningType twice (> 300msec)
			// But are there cases in the wild were we need to verify the value of earningType?
			// Or should we check if it's a known earningType and skip checking only in that case?

			e.Cancel = SkipEarningTypeCheck;
		}
		#endregion
		#region FieldUpdated
        ...
        protected virtual void _(Events.FieldUpdated<FSAppointmentDet, FSAppointmentDet.status> e)
        {
            ...
            {
                if (e.Row.Status == FSAppointmentDet.status.RequestForPO
                    || e.Row.Status == FSAppointmentDet.status.WaitingForPO)
                {
        ...
        protected virtual void _(Events.RowSelected<FSAppointmentDet> e)
        {
            ...
            bool includeIN = InventoryItemsAreIncluded();
        ...
        protected virtual void _(Events.RowInserted<FSAppointmentDet> e)
        {
			...
			if (fsAppointmentDetRow.SMEquipmentID != null)
			{
				UpdateWarrantyFlag(e.Cache, fsAppointmentDetRow, AppointmentRecords.Current.ExecutionDate);
			}
            ...
            where SourceRowType : class, IBqlTable, IFSSODetBase, new()
        {
            ...
            if (targetRow.InventoryID != null)
            {
                ...
                if (sourceRow.EnablePO == true)
                {
					...
					targetRow.POLineNbr = sourceRow.POLineNbr;
        ...
        public class SalesTax : TaxGraph<AppointmentEntry, FSAppointment>
        {
            ...
            {
				FSAppointment fsAppointment = e.Row;
				...
				FSServiceOrder fsServiceOrderRow = Base.ServiceOrderRelated.Current;

				if (fsServiceOrderRow == null)
				{
					soRelatedView.Cache.SetValueExt<FSServiceOrder.taxZoneID>(soRelatedView.Current, null);
					return;
				}

				e.NewValue = Base.GetDefaultTaxZone(fsAppointment);
			...
			protected virtual void _(Events.RowUpdated<FSAddress> e)
					{
				Base.FSAddress_RowUpdated_Handler(e, Base.AppointmentRecords.Cache);
			}
			#endregion
		}
		...
		/// </summary>
		public class PerUnitTaxDisableExt : PX.Objects.Extensions.PerUnitTax.PerUnitTaxDataEntryGraphExtension<AppointmentEntry, FSAppointmentTaxTran>
		{
		...
		{
			FSServiceOrder fsServiceOrderRow = ServiceOrderRelated.Current;
			return GetDefaultTaxZone(row.BillCustomerID, fsServiceOrderRow.BillLocationID, row.BranchID, row.ProjectID);
		}
