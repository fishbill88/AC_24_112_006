...
namespace PX.Objects.RQ
{
							...
							if (item.VendorID != null && item.VendorLocationID != null)
							{
									...
									.SelectMultiBound(this, new object[] { item }))
								{
									...
									if (order != null)
									{
										...
										order.ShipContactID = item.ShipContactID;
										order.FOBPoint = biddier.FOBPoint ?? item.FOBPoint;
										order.ShipVia = biddier.ShipVia ?? item.ShipVia;
				...
				if (this._overrideVendorCuryOrRate == false && rVendor != null)
				{
					...
					{ 
						...
						try
						{
							...
							if (CurrencyCollection.GetCurrency(copyInfo.BaseCuryID)?.CuryInfoID == copyInfo.CuryInfoID || CurrencyCollection.GetBaseCurrency()?.CuryInfoID == copyInfo.CuryInfoID)
							{
					...
					if (update)
					{
						...
						foreach (RQRequisitionLine line in this.Lines.View.SelectMultiBound(new object[] { e.Row }))
						{
							...
							else
							{
								...
								decimal unitCost;
								PXCurrencyAttribute.CuryConvBase<RQRequisitionLine.curyInfoID>(this.Lines.Cache, upd, upd.CuryEstUnitCost ?? 0, out unitCost);

								if (unitCost != upd.EstUnitCost)
								{
									decimal curyUnitCost;
									PXCurrencyAttribute.CuryConvCury<RQRequisitionLine.curyInfoID>(this.Lines.Cache, upd, upd.EstUnitCost ?? 0, out curyUnitCost, true);
									upd.CuryEstUnitCost = curyUnitCost;
								}
	...
	public partial class RQRequisitionStatic : PXBqlTable, IBqlTable
	{
		#region SourceReqNbr
	...
	public partial class RQRequestLineSelect : PXBqlTable, IBqlTable
	{
		#region Selected
