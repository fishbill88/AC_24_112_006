...
namespace PX.Objects.RQ
{
	...
	public partial class RQRequestLine : PXBqlTable, PX.Data.IBqlTable
	{
		#region Keys
	...
	public static class RQRequestIssueType
	{
		public class ListAttribute : PXStringListAttribute, IPXRowSelectedSubscriber
		{
			...
			protected virtual void SetIssueStatus(PXCache cache, RQRequestLine row)
			{
				...
				{
					// Acuminator disable once PX1047 RowChangesInEventHandlersForbiddenForArgs [Legacy code moved to extension]
					row.IssueStatus = Canceled;
				}
				else if (row.IssuedQty >= row.OrderQty)
				{
					// Acuminator disable once PX1047 RowChangesInEventHandlersForbiddenForArgs [Legacy code moved to extension]
					row.IssueStatus = Closed;
				}
				else if (row.OpenQty == 0)
				{
					bool ordered = true;
					bool received = true;
					foreach (RQRequisitionLineReceived line in GetRequisitionReceivedLines(cache.Graph, row))
					{
						...
						if (line.Status.IsIn(RQRequisitionReceivedStatus.Partially, RQRequisitionReceivedStatus.Ordered))
							received = false;
					}
					...
					if (GetRequestClass(cache.Graph, row) is RQRequestClass reqClass && reqClass.IssueRequestor == true && row.IssueStatus == Closed)
					{
						// Acuminator disable once PX1047 RowChangesInEventHandlersForbiddenForArgs [Legacy code moved to extension]
						row.IssueStatus = Received;
					}
				}
				else if (row.IssuedQty > 0)
				{
					// Acuminator disable once PX1047 RowChangesInEventHandlersForbiddenForArgs [Legacy code moved to extension]
					row.IssueStatus = PartiallyIssued;
				}
				else
				{
					// Acuminator disable once PX1047 RowChangesInEventHandlersForbiddenForArgs [Legacy code moved to extension]
					row.IssueStatus = Open;
				}
			}

			protected virtual RQRequestClass GetRequestClass(PXGraph graph, RQRequestLine row) =>
				(PXResult<RQRequest, RQRequestClass>)
				SelectFrom<RQRequest>.
				InnerJoin<RQRequestClass>.On<RQRequest.FK.RequestClass>.
				Where<RQRequest.orderNbr.IsEqual<@P.AsString>>.
				View.Select(graph, row.OrderNbr);

			protected virtual IEnumerable<RQRequisitionLineReceived> GetRequisitionReceivedLines(PXGraph graph, RQRequestLine row) =>
				SelectFrom<RQRequisitionContent>.
				InnerJoin<RQRequisitionLineReceived>.On<
					RQRequisitionLineReceived.reqNbr.IsEqual<RQRequisitionContent.reqNbr>.
					And<RQRequisitionLineReceived.lineNbr.IsEqual<RQRequisitionContent.reqLineNbr>>>.
				Where<
					RQRequisitionContent.orderNbr.IsEqual<@P.AsString>.
					And<RQRequisitionContent.lineNbr.IsEqual<@P.AsInt>>>.
				View.Select(graph, row.OrderNbr, row.LineNbr).RowCast<RQRequisitionLineReceived>();
		}
