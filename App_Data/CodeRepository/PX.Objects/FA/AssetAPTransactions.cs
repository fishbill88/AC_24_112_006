...
namespace PX.Objects.FA
{
	...
	public class AssetGLTransactions : PXGraph<AssetGLTransactions>
	{
		...
		public partial class Error : PXBqlTable, IBqlTable
		{
			...
			out int? assetID)
		{
			...
			try
			{
				...
				foreach (FABookSettings bookSetting in PXSelect<FABookSettings,
						Where<FABookSettings.assetID, Equal<Current<FixedAsset.classID>>>,
						OrderBy<Desc<FABookSettings.updateGL>>>.Select(graph))
				{
					FABookBalance bookBalance = (FABookBalance)cache.Insert(new FABookBalance
					{
						AssetID = asset.AssetID,
						ClassID = _classID,
						BookID = bookSetting.BookID,
						UsefulLife = bookSetting.UsefulLife,
					});

					cache.RaiseFieldDefaulting<FABookBalance.deprToPeriod>(bookBalance, out object deprtoPeriod);
					bookBalance.DeprToPeriod = (string)deprtoPeriod;

					if (postingBookID == null || bookBalance.UpdateGL == true)
					{
						postingBookID = bookBalance.BookID;
					}
					if (string.IsNullOrEmpty(location.PeriodID))
					{
						location.PeriodID = asset.UnderConstruction == true ? graph.GetService<IFABookPeriodRepository>().GetFABookPeriodIDOfDate(det.ReceiptDate, postingBookID, asset.AssetID, check: false) : bookBalance.DeprFromPeriod;
					}
				}
			}
			catch (PXException ex)
	...
	[Serializable]
	public partial class GLTranFilter : PXBqlTable, IBqlTable
	{
		#region AccountID
