	...
	public class AssetClassMaint : PXGraph<AssetClassMaint, FixedAsset>
	{
		...
		public override void Persist()
		{
			foreach (FABookSettings set in DepreciationSettings.Cache.Inserted.Cast<FABookSettings>().Concat<FABookSettings>(DepreciationSettings.Cache.Updated.Cast<FABookSettings>()))
			{
				...
				if (method != null && method.IsTableMethod == true && method.AveragingConvention != set.AveragingConvention)
				{
					DepreciationSettings.Cache.RaiseExceptionHandling<FABookSettings.averagingConvention>(set, set.AveragingConvention, new PXSetPropertyException<FABookSettings.averagingConvention>(Messages.AveragingConventionNotMatchDeprMethod));
				}
			}
			base.Persist();
		}
