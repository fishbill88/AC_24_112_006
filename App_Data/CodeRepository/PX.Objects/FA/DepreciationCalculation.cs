...
namespace PX.Objects.FA
{
	public class DepreciationCalculation : PXGraph<DepreciationCalculation, FADepreciationMethod>
	{
		...
		public virtual void CalculateDepreciation(FABookBalance assetBalance, string maxPeriodID)
		{
			...
			PXResult<FixedAsset, FAClass> result = (PXResult<FixedAsset, FAClass>)PXSelectJoin<FixedAsset,
			...
			FAClass cls = result.GetItem<FAClass>();
			FixedAsset asset = result.GetItem<FixedAsset>();

			Currency cury = PXSelect<Currency, Where<Currency.curyID.IsEqual<@P.AsString>>>.Select(this, asset.BaseCuryID);
			...
			if(assetBalance.UpdateGL == true)
			{
				...
				{
					AssetProcess.SetLastDeprPeriod(asset, BookBalance, assetBalance, lastClosedPeriod.FinPeriodID);
		...
		protected virtual IEnumerable<DepreciationMethodBase> GetDepreciationMethods()
		{
			...
			yield return new NewZealandStraightLineMethod();
			yield return new DutchMethod2();
			//yield break;
		}
		...
		private bool PerformCalculation(FABookBalance bookBalance, string maxPeriodID = null, PXGraph uiGraph = null)
		{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				this.FADetails.Cache.MarkUpdated(details);
				Save.Press();
				CloseHistoryInClosedPeriods(asset, depreciationMethod);
				ts.Complete();
			}
			return true;
		}
		...
		private void CloseHistoryInClosedPeriods(FixedAsset asset, DepreciationMethodBase depreciationMethod)
		{
			FABookBalance bookBalance = depreciationMethod.IncomingParameters.BookBalance;
			...
			if (bookBalance.UpdateGL == true)
			{
				...
				{
				AssetProcess.SetLastDeprPeriod(asset, BookBalance, bookBalance, maxClosedPeriod);
				}
