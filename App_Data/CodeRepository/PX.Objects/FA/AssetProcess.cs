...
namespace PX.Objects.FA
{
	...
	public class AssetTranRelease : PXGraph<AssetTranRelease>
	{
		...
		[Serializable]
		public partial class ReleaseFilter : PXBqlTable, IBqlTable
		{
			#region Origin
	...
	public class AssetProcess : PXGraph<AssetProcess>
	{
		...
		public static void SuspendBalanceForPeriods(TransactionEntry graph, FABookBalance bookbal, int Periods)
		{
			...
			{
				copy.CurrDeprPeriod = ExtendPeriod(graph, copy.CurrDeprPeriod, Periods, copy.BookID, copy.AssetID);
			}
			if (!string.IsNullOrEmpty(copy.LastDeprPeriod))
			{
				copy.LastDeprPeriod = ExtendPeriod(graph, copy.LastDeprPeriod, Periods, copy.BookID, copy.AssetID);
			}
			if (!string.IsNullOrEmpty(copy.DeprToPeriod))
			{
				copy.DeprToPeriod = ExtendPeriod(graph, copy.DeprToPeriod, Periods, copy.BookID, copy.AssetID);
		...
		public static string ExtendPeriod(TransactionEntry graph, string finPeriodID, int counter, int? bookID, int? assetID)
		{
			IFABookPeriodUtils fABookPeriodUtils = graph.GetService<IFABookPeriodUtils>();
			string extendedPeriodID = finPeriodID;

			try
			{
				extendedPeriodID = fABookPeriodUtils.PeriodPlusPeriodsCount(finPeriodID, counter, bookID, assetID);
			}
			catch (PXFABookPeriodException)
			{
				IFABookPeriodRepository faBookPeriodRepository = graph.GetService<IFABookPeriodRepository>();
				IPeriodSetup periodsInYear = faBookPeriodRepository.FindFABookPeriodSetup(bookID).LastOrDefault();

				if (periodsInYear != null && periodsInYear.PeriodNbr != null)
				{
					string maxPeriodID = GL.FinPeriods.FinPeriodUtils.OffsetPeriod(finPeriodID, counter, Convert.ToInt32(periodsInYear.PeriodNbr));
					throw new PXSetPropertyException(
						Messages.PeriodDoesNotExistForBookNeedToGeneratePeriods,
						FABookPeriodIDAttribute.FormatForError(maxPeriodID),
						BookMaint.FindByID(graph, bookID).BookCode);
				}
			}
			return extendedPeriodID;
		}
		...
		public static void DisposeAsset(TransactionEntry docgraph, FixedAsset asset, IEnumerable<FABookBalance> books, FASetup setup, string reason)
		{
			...
			foreach (FABookBalance bal in books)
			{
				...
				if (string.CompareOrdinal(bookbal.DeprFromPeriod, period) > 0)
				{
					throw new PXException(Messages.DisposalPeriodCannotBeEarlierthanPlacedInServiceDatePeriod, FinPeriodIDFormattingAttribute.FormatForError(period), FinPeriodIDFormattingAttribute.FormatForError(bookbal.DeprFromPeriod));
				}
				if (hist == null)
				{
		...
		public static void AcquireAsset(TransactionEntry docgraph, int BranchID, IDictionary<FABookBalance, List<OperableTran>> booktrn, FARegister register)
		{
			if (register != null)
			{
			...
			foreach (KeyValuePair<FABookBalance, List<OperableTran>> book in booktrn)
			{
				...
				foreach (OperableTran bookValue in book.Value)
				{
					if (bookValue.Op == PXDBOperation.Update && (!docgraph.UpdateGL || register.Released == false))
					{
						...
						bookValue.Op = PXDBOperation.Insert;
						docgraph.Document.Update(docgraph.Document.Current);
					}

					if (string.IsNullOrEmpty(book.Key.LastDeprPeriod))
					{
						if (bookValue.Op == PXDBOperation.Delete || bookValue.Op == PXDBOperation.Update)
						{
							PXDatabase.Delete<FABookHistory>(
								new PXDataFieldRestrict<FABookHistory.assetID>(book.Key.AssetID),
								new PXDataFieldRestrict<FABookHistory.bookID>(book.Key.BookID));

							if (bookValue.Op != PXDBOperation.Delete)
							{
								bookBal.MaxHistoryPeriodID = null;
								docgraph.EnsureRowPersistence(bookBal);
							}
						}
						if ((bookValue.Op == PXDBOperation.Insert && asset.SplittedFrom == null) || bookValue.Op == PXDBOperation.Update)
						{
							FABookHist hist = new FABookHist
							{
					...
					switch (bookValue.Op)
					{
						case PXDBOperation.Insert:
							...
							if (!docgraph.UpdateGL)
							{
								...
								KeyValuePair<FABookBalance, List<OperableTran>> b = book;
						...
						case PXDBOperation.Update:
							FATran copy = PXCache<FATran>.CreateCopy(bookValue.Tran);
						...
						case PXDBOperation.Delete:
							docgraph.Trans.Delete(bookValue.Tran);
							break;
					}
				}
			}
		...
		public static void SetLastDeprPeriod(FixedAsset asset, PXSelectBase<FABookBalance> bookBalances, FABookBalance bookBal, string LastDeprPeriod)
		{
			if (LastDeprPeriod == null || asset.Depreciable != true) return;
		...
		{
			PXGraph graph = bookBalances.Cache.Graph;
			FixedAsset asset = graph.Caches<FixedAsset>().InternalCurrent as FixedAsset;
			if (asset == null || asset.AssetID != bookBal.AssetID)
			{
				asset = SelectFrom<FixedAsset>
					.Where<FixedAsset.assetID.IsEqual<@P.AsInt>>
					.View.ReadOnly
					.SelectSingleBound(graph, null, bookBal.AssetID);
			}
			SetLastDeprPeriod(asset, bookBalances, bookBal, LastDeprPeriod);
		}
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
																																	...
																																	And<FATran.released, Equal<False>>>>.Select(this, doc.RefNbr))
				{
					...
					if (closedPeriods != null)
					{
						...
						SetLastDeprPeriod(asset, bookbalance, bookbal, maxClosedPeriod);
					}
