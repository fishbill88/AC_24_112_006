...
namespace PX.Objects.FA
{
	...
	public class AssetMaint : PXGraph<AssetMaint, FixedAsset>, IGraphWithInitialization
	{
		...
		public class AdditionsViewExtension : AdditionsViewExtensionBase<AssetMaint>
		{
			...
			public virtual IEnumerable processAdditions(PXAdapter adapter)
			{
				...
				bool isInterBranch = PXAccess.FeatureInstalled<FeaturesSet.interBranch>();
				foreach (DsplFAATran dspl in DsplAdditions.Cache.Inserted)
				{
					if (dspl.Selected == true)
					{
						...
						bool isSameParentOrganization = PXAccess.IsSameParentOrganization(gltran.BranchID, asset.BranchID);
						Organization parentOrganization = OrganizationMaint.FindOrganizationByID(Base, PXAccess.GetParentOrganizationID(gltran.BranchID));

						if (!isInterBranch && gltran.BranchID != asset.BranchID
							&& (!isSameParentOrganization
								|| (isSameParentOrganization && parentOrganization.OrganizationType != OrganizationTypes.WithBranchesNotBalancing)))
						{
							throw new PXException(Common.Messages.InterBranchFeatureIsDisabled);
						}
			...
			protected virtual void DsplFAATran_Selected_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
			{
				...
				if (tran.Selected == true && filter.UnreconciledAmt == 0m)
				{
					tran.SelectedAmt = tran.OpenAmt;
				}
		...
		public override void Persist()
		{
			...
			{
				foreach (FATran tran in cmdsel.View.SelectMultiBound(new object[] { bookbal }))
				{
					if (tran != null && (tran.Origin == FARegister.origin.Purchasing || tran.Origin == FARegister.origin.Reconcilliation) && tran.Released == false)
					{
						if (!booktrn.ContainsKey(bookbal))
						{
							booktrn.Add(bookbal, new List<OperableTran>());
						}
						booktrn[bookbal].Add(new OperableTran(PXCache<FATran>.CreateCopy(tran), PXDBOperation.Delete));
					}
				}
			}
			...
			foreach (FABookBalance bookbal in AssetBalance.Cache.Updated)
			{
				...
				{
					booktrn.Add(bookbal, new List<OperableTran>());
					booktrn[bookbal].Add(new OperableTran(PXCache<FATran>.CreateCopy(res), PXDBOperation.Update));
				}

				if (res == null)
				{
					booktrn.Add(bookbal, new List<OperableTran>());
					booktrn[bookbal].Add(new OperableTran(null, PXDBOperation.Insert));
				}
			}
			...
			if (adapter.MassProcess)
			{
				...
				foreach (SplitParams split in list)
				{
					...
					foreach (FABookBalance bookbal in PXSelect<FABookBalance, Where<FABookBalance.assetID, Equal<Current<FixedAsset.assetID>>>>.SelectMultiBound(this, new object[] { asset }))
					{
						...
						split_bal.BonusRate = bookbal.BonusRate;
						...
						decimal? originalAssetBonusAmount =  bookbal.BonusAmount - split_bal.BonusAmount;
						bookbal.Tax179Amount -= split_bal.Tax179Amount;

						AssetBalance.Update(bookbal);
						AssetBalance.SetValueExt<FABookBalance.bonusAmount>(bookbal, originalAssetBonusAmount); // to ovveride calculated value from formula
					}
		...
		protected virtual void FixedAsset_RowUpdating(PXCache sender, PXRowUpdatingEventArgs e)
		{
				...
				)
			{
				...
				try
				{
					...
					if (newLocation.PeriodID == null && IsImport)
					{
						int? organizationID = PXAccess.GetParentOrganizationID(CurrentAsset.Current.BranchID);
		...
		public virtual void _(Events.FieldUpdated<FABookBalance.usefulLife> e)
		{
				...
				string.CompareOrdinal(balance.DeprToPeriod, balance.LastDeprPeriod) == 0)
			{
				...
				if (AssetBalance.Select().RowCast<FABookBalance>().All(_ => _.Status == FixedAssetStatus.FullyDepreciated))
					FixedAsset.Events
						.Select(ev => ev.FullyDepreciateAsset)
						.FireOn(this, Asset.Current);
			}
		}
		...
		{
			FixedAsset asset = PXParentAttribute.SelectParent<FixedAsset>(sender, e.Row);
		...
		protected virtual void FABookBalance_DeprFromPeriod_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (bal == null ||
				...
				(string.IsNullOrEmpty(oldValue) && !isUnderConstruction) ||
			...
			if (isUnderConstruction)
			{
				...
				origFromPeriod = purchasingHistory?.FinPeriodID ?? bal.DeprFromPeriod;
				if(string.IsNullOrEmpty(oldValue))
				{
					oldValue = origFromPeriod;
				}
			}
					...
					toPeriod))
			{
				...
				string originalFromPeriod = (string)sender.GetValueOriginal<FABookBalance.deprFromPeriod>(bal);

				if(string.IsNullOrEmpty(originalFromPeriod) && isUnderConstruction)
				{
					originalFromPeriod = origFromPeriod;
				}

				if ((originalFromPeriod != null) && isPeriodGreater && originalFromPeriod != fromPeriod  &&
					bookPeriod.FinPeriodID == fromPeriod)
				{
					origHist = hist;// to fix AC-293866 which causes issue when there is intermediate Placed in Date change before final
				}
	...
	[Serializable]
	[PXHidden]
	public class FABookBalance2 : PXBqlTable, IBqlTable
	{
		#region ExactLife
	...
	public partial class FAHistory : PXBqlTable, IBqlTable
	{
		#region AssetID
	...
	[Serializable]
	public partial class FASheetHistory : PXBqlTable, IBqlTable
	{
		#region AssetID
		public abstract class assetID : PX.Data.BQL.BqlInt.Field<assetID> { }
		protected Int32? _AssetID;
	...
	[Serializable]
	public partial class TranBookFilter : PXBqlTable, IBqlTable
	{
		#region BookID
	...
	[Serializable]
	public partial class DeprBookFilter : PXBqlTable, IBqlTable
	{
		#region BookID
		public abstract class bookID : PX.Data.BQL.BqlInt.Field<bookID> { }
		protected Int32? _BookID;
	...
	public partial class DisposeParams : PXBqlTable, IBqlTable
	{
		#region DisposalDate
	...
	[Serializable]
	public partial class SuspendParams : PXBqlTable, IBqlTable
	{
	...
	[Serializable]
	public partial class DsplFAATran : PXBqlTable, IBqlTable
	{
		#region Selected
