    ...
    internal class CRMTaskAction: TemplateMessageAction
    {
        ...
        public override void Process(MatchedRow[] eventRows, CancellationToken cancellation)
        {
            ...
            using (new PXLocaleScope(taskTemplate.LocaleName ?? PXLocalesProvider.GetCurrentLocale()))
            {
				...
				var forms = GetFormFields(taskTemplate.ScreenID);
				SetFormValues(eventRows[0], entityGraph, taskTemplate.ScreenID, forms);
                activity.Subject = PXTemplateContentParser.Instance.Process(taskTemplate.Summary, parameters, entityGraph, primaryView, forms);
                
                SyFormulaFinalDelegate fieldValueGetter;
                if (entityGraph is PXGenericInqGrph)
                {
                    if (Guid.TryParse(PXTemplateContentParser.Instance.Process(taskTemplate.RefNoteID, parameters, entityGraph, primaryView, forms), out Guid refNoteID))
                ...
                else
                {
                    ...
                    fieldValueGetter = (names) => 
                    {
                        ...
                        if (names.Length == 2)
                        {
							...
							if (entityGraph.Views.ContainsKey(viewName))
							{
							...
							var paramName = string.Join(SeparatorConstants.ViewField, names);
							if (parameters[0].Item1?.TryGetValue(paramName, out var value) == true)
							{
								return ValueWithInternal.UnwrapInternalValue(value);
							}
                ...
                if (!string.IsNullOrEmpty(taskTemplate.OwnerName))
                {
	                ...
	                ownerID = PXTemplateContentParser.NullableInstance.Process(ownerID, parameters, entityGraph, primaryView, forms);
	                if (ownerID != string.Empty)
						activity.OwnerID = int.Parse(ownerID);
				}

				var activityBody = PXTemplateContentParser.ScriptInstance.Process(taskTemplate.Body, parameters, entityGraph, primaryView, forms);
