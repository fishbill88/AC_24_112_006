...
namespace PX.Objects.CS
{
	public class DimensionMaint : PXGraph<DimensionMaint, Dimension>
	{
		...
		protected virtual void Segment_ConsolNumChar_FieldVerifying(PXCache cache, PXFieldVerifyingEventArgs e)
		{
			...
			if (newValue > 0)
			{
				foreach (SegmentValue val in PXSelect<SegmentValue, Where<SegmentValue.dimensionID, Equal<Optional<Segment.dimensionID>>,
							And<SegmentValue.segmentID, Equal<Optional<Segment.segmentID>>>>>.Select(this, Detail.Current.DimensionID, Detail.Current.SegmentID))
				{
					if (val.MappedSegValue?.ToString().Length > 0
						&& val.MappedSegValue?.ToString().Length > newValue)
					{
						throw new PXSetPropertyException(Messages.NumberOfCharactersExceedMappedSegValueLength, val.MappedSegValue?.ToString().Length);
					}
				}
			}
		}
