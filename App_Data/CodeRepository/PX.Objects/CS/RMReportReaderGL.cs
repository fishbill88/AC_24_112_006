...
namespace PX.Objects.CS
{
	...
	public partial class RMReportReaderGL : PXGraphExtension<RMReportReader>
    {
		...
		private void LoadHistory(int ledgerID, int year)
		{
			...
			{
				ProcessGLResultset(SelectFrom<GLHistory>
					.InnerJoin<Account>.On<GLHistory.accountID.IsEqual<Account.accountID>>
					.InnerJoin<ArmGLHistoryByPeriod>
					.On<GLHistory.ledgerID.IsEqual<ArmGLHistoryByPeriod.ledgerID>
					.And<GLHistory.branchID.IsEqual<ArmGLHistoryByPeriod.branchID>
					.And<GLHistory.accountID.IsEqual<ArmGLHistoryByPeriod.accountID>
					.And<GLHistory.subID.IsEqual<ArmGLHistoryByPeriod.subID>
					.And<GLHistory.finPeriodID.IsEqual<ArmGLHistoryByPeriod.lastActivityPeriod>>>>>>
					.Where<Account.type.IsEqual<AccountType.asset>.Or<Account.type.IsEqual<AccountType.liability>>>
					.View.Select(Base)
					.Where(_ => _.GetItem<ArmGLHistoryByPeriod>().LedgerID == ledgerID && _.GetItem<ArmGLHistoryByPeriod>().FinPeriodID == lastPeriodOfPrevYear.FinPeriodID)
					.Select(_ => _.GetItem<GLHistory>()));
			}
		}

		private void ProcessGLResultset(IEnumerable<GLHistory> resultset) => resultset.ForEach(ProcessGLHistoryItem);
