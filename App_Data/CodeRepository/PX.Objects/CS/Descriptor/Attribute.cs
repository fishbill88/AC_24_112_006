...
namespace PX.Objects.CS
{
	...
	public class PXDimensionMaskAttribute : PXDimensionAttribute
	{
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			var maskSegmentViewName = "_" + _Mask + "_Segments_";
			if (!sender.Graph.Views.ContainsKey(maskSegmentViewName))
				sender.Graph.Views[maskSegmentViewName] =
	...
	public class TermsAttribute : PXEventSubscriberAttribute, IPXFieldUpdatedSubscriber, IPXRowSelectedSubscriber
	{
		...
		public TermsAttribute(Type DocDate, Type DueDate, Type DiscDate, Type CuryDocBal, Type CuryDiscBal, Type CuryTaxTotal, Type BranchID)
		{
			_DocDate = DocDate;
			_DueDate = DueDate;
			_DiscDate = DiscDate;
		...
		public TermsAttribute(Type DocDate, Type DueDate, Type DiscDate, Type CuryDocBal, Type CuryDiscBal, Type CuryTaxTotal, Type CuryTaxDiscountTotal, Type BranchID)
		{
			_DocDate = DocDate;
			_DueDate = DueDate;
			_DiscDate = DiscDate;
			...
			_CuryDocBal = CuryDocBal;
			_CuryTaxTotal = CuryTaxTotal;
			_CuryTaxDiscountTotal = CuryTaxDiscountTotal;
			_BranchID = BranchID;
		}
		...
		protected virtual void CalcDisc(PXCache sender, PXFieldUpdatedEventArgs e)
		{
				...
				!sender.Graph.IsCopyPasteContext)
			{
				...
				string cashDiscountBase = null;
				if (_BranchID != null)
				{
					int? branchID = sender.GetValue(e.Row, _BranchID.Name) as int?;
					cashDiscountBase = OrganizationMaint.GetCashDiscountBase(sender.Graph, branchID);
				}

				if (cashDiscountBase == CashDiscountBases.DocumentAmountLessTaxes)
				{
					if (_CuryTaxTotal != null && sender.GetValue(e.Row, _CuryTaxTotal.Name) != null)
					{
						CuryTaxTotal = (decimal)sender.GetValue(e.Row, _CuryTaxTotal.Name);
					}
					if (_CuryTaxDiscountTotal != null && sender.GetValue(e.Row, _CuryTaxDiscountTotal.Name) != null)
					{
						CuryTaxDiscountTotal = (decimal)sender.GetValue(e.Row, _CuryTaxDiscountTotal.Name);
					}
					CuryDocBal -= CuryTaxTotal + CuryTaxDiscountTotal;
				}

				Terms terms = SelectTerms(sender.Graph, TermsID);
				if (terms != null && terms.InstallmentType == TermsInstallmentType.Single && CuryDocBal > 0m && terms.DiscPercent != 0m)
				{
					if ((e.Row as AR.ARInvoice)?.DocType != ARDocType.PrepaymentInvoice)
					{
						CuryDiscBal = CM.TemporaryHelpers.MultiCurrencyCalculator.RoundCury(sender, e.Row, CuryDocBal * (decimal)terms.DiscPercent / 100);
					}
					else
					{
						sender.RaiseExceptionHandling(_FieldName, e.Row, TermsID, new PXSetPropertyException(AR.Messages.CannotSelectCreditTermsWithDiscountForPrepaymentInvoice, PXErrorLevel.Warning));
					}
				}
		...
		protected virtual void UnsubscribeCalcDisc(PXCache sender)
		{
			...
			if (_BranchID != null)
			{
				sender.Graph.FieldUpdated.RemoveHandler(BqlCommand.GetItemType(_BranchID), _BranchID.Name, CalcDisc);
			}
			if (_CuryTaxTotal != null)
			{
				sender.Graph.FieldUpdated.RemoveHandler(BqlCommand.GetItemType(_CuryTaxTotal), _CuryTaxTotal.Name, CalcDisc);
			}
		}
		...
		protected virtual void SubscribeCalcDisc(PXCache sender)
		{
			...
			if (_BranchID != null)
			{
				sender.Graph.FieldUpdated.AddHandler(BqlCommand.GetItemType(_BranchID), _BranchID.Name, CalcDisc);
			}
			if (_CuryTaxTotal != null)
			{
				sender.Graph.FieldUpdated.AddHandler(BqlCommand.GetItemType(_CuryTaxTotal), _CuryTaxTotal.Name, CalcDisc);
			}
		}
