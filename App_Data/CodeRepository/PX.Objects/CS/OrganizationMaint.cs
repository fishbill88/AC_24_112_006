...
namespace PX.Objects.CS
{
	public class OrganizationMaint : OrganizationUnitMaintBase<OrganizationBAccount, Where<Match<Current<AccessInfo.userName>>>>
	{
		...
		public static string GetCashDiscountBase(PXGraph graph, int? branchID)
		{
			...
			Organization organization = FindOrganizationByID(graph, organizationID);
			return organization?.CashDiscountBase;
		}
		...
		[Serializable]
		public class State : PXBqlTable, IBqlTable
		{
		...
		protected virtual void _(Events.FieldUpdated<Organization, Organization.organizationLocalizationCode> e)
		{
			if (e.Row == null)
				return;

			if (LocalizationServiceExtensions.LocalizationEnabled<FeaturesSet.canadianLocalization>(e.Row.OrganizationLocalizationCode))
			{
				e.Row.CashDiscountBase = CashDiscountBases.DocumentAmountLessTaxes;
			}
		}

		protected virtual void _(Events.FieldVerifying<Organization, Organization.cashDiscountBase> e)
		{
			if (e.Row == null)
				return;
			
			if (LocalizationServiceExtensions.LocalizationEnabled<FeaturesSet.canadianLocalization>(e.Row.OrganizationLocalizationCode)
				&& e.NewValue.ToString() == CashDiscountBases.DocumentAmount)
			{
				e.Cache.RaiseExceptionHandling<Organization.cashDiscountBase>(e.Row, e.Row.CashDiscountBase,
					new PXSetPropertyException(Messages.ShouldSelectDocumentAmountLessTaxesForCA, PXErrorLevel.Warning));
			}
		}
		#endregion
		...
		protected virtual void ProcessPublicableToBranchesFieldsChanging(Organization origOrganization, Organization organization, PXEntryStatus status)
		{
			...
			if (forceCopy || shouldAdjustRoleName || shouldAdjustActive || shouldAdjustLedgerID || shouldAdjustCountryID || shouldLogoNameReport
                || shouldLogoName || shouldInvokeSyncEx || shouldAdjustBaseCuryID)
			{
		...
		[Serializable]
		public class LedgerCreateParameters : PXBqlTable, IBqlTable
		{
