	...
	public class Prepayments : PXGraphExtension<APInvoiceEntry.MultiCurrency, APInvoiceEntry>
	{
		...
		public virtual bool AddPOOrderLines(IEnumerable<POLineRS> lines)
		{
			...
			foreach (POLineRS line in lines.Where(l =>
				(!IsNegativeNonStockPrice(l) && l.CuryExtCost + l.CuryRetainageAmt > l.CuryReqPrepaidAmt
				|| IsNegativeNonStockPrice(l) && l.CuryExtCost + l.CuryRetainageAmt < l.CuryReqPrepaidAmt)
				...
				&& (l.Billed == false || l.LineType == POLineType.Service)))
			{
				...
				else if (!IsNegativeNonStockPrice(line) && line.CuryExtCost + line.CuryRetainageAmt <= billedAndPrepaidAmt
				|| (IsNegativeNonStockPrice(line) && line.CuryExtCost + line.CuryRetainageAmt >= billedAndPrepaidAmt))
				{
					tran.CuryLineAmt = 0m;
					tran.CuryDiscAmt = 0m;
		...
		private bool IsNegativeNonStockPrice(POLineRS line) => POLineType.IsNonStock(line.LineType) && line.CuryExtCost + line.CuryRetainageAmt < 0;
		...
		protected virtual void _(Events.RowPersisting<APTran> e)
		{
			...
			if (tran.Released != true && tran.TranType == APDocType.Prepayment && !string.IsNullOrEmpty(tran.PONbr) && tran.POLineNbr != null)
			{
				...
				bool isNegativeNonStock = POLineType.IsNonStock(poLine.LineType) && poLine.CuryExtCost + poLine.CuryRetainageAmt < 0;
				...
				{
					if ((poLine.CuryReqPrepaidAmt > poLine.CuryBilledAmt ? poLine.CuryBilledAmt : poLine.CuryReqPrepaidAmt)
						+ tran.CuryTranAmt + tran.CuryRetainageAmt
						< poLine.CuryExtCost + poLine.CuryRetainageAmt)
					{
						e.Cache.RaiseExceptionHandling<APTran.curyTranAmt>(tran, tran.CuryTranAmt,
							new PXSetPropertyException(Messages.PrepaidAmtCantBeLessThanNegativePOLine));
					}
					else if (poLine.CuryReqPrepaidAmt + poLine.CuryBilledAmt
						+ tran.CuryTranAmt + tran.CuryRetainageAmt
						< poLine.CuryExtCost + poLine.CuryRetainageAmt)
					{
						e.Cache.RaiseExceptionHandling<APTran.curyTranAmt>(tran, tran.CuryTranAmt,
							new PXSetPropertyException(Messages.PrepaidAmtMayBeLessThanNegativePOLine, PXErrorLevel.Warning, poLine.OrderNbr, poLine.OrderType, poLine.LineNbr));
					}
				}
			}
		}
