...
namespace PX.Objects.PO
{
	...
	public class POReceiptEntry : PXGraph<POReceiptEntry, POReceipt>, IGraphWithInitialization
	{
		...
		protected virtual void _(Events.CacheAttached<POReceiptLine.unbilledQty> e) { }

		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXParent(typeof(POReceiptLine.FK.OrderLine))]
		protected virtual void _(Events.CacheAttached<POReceiptLine.pOLineNbr> e) { }

		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXFormula(null, typeof(SumCalc<POLine.baseReceivedQty>), ValidateAggregateCalculation = true)]
		protected virtual void _(Events.CacheAttached<POReceiptLine.baseMultReceiptQty> e) { }

		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXRemoveBaseAttribute(typeof(PXDefaultAttribute))]
		[PXDBDefault(typeof(POReceipt.receiptType))]
		protected virtual void _(Events.CacheAttached<POReceiptLine.receiptType> e) { }

		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXRemoveBaseAttribute(typeof(PXDefaultAttribute))]
		[PXDBDefault(typeof(POReceipt.receiptNbr))]
		protected virtual void _(Events.CacheAttached<POReceiptLine.receiptNbr> e) { }

		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXRemoveBaseAttribute(typeof(PXDefaultAttribute))]
		[PXDBDefault(typeof(POReceipt.vendorID))]
		protected virtual void _(Events.CacheAttached<POReceiptLine.vendorID> e) { }

		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXRemoveBaseAttribute(typeof(PXDefaultAttribute))]
		[PXDBDefault(typeof(POReceipt.receiptDate))]
		protected virtual void _(Events.CacheAttached<POReceiptLine.receiptDate> e) { }

		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXCustomizeBaseAttribute(typeof(DBConditionalQuantityAttribute), nameof(PXDBQuantityAttribute.DecimalVerifyUnits), InventoryUnitType.PurchaseUnit)]
		[PXCustomizeBaseAttribute(typeof(DBConditionalQuantityAttribute), nameof(PXDBQuantityAttribute.ConvertToDecimalVerifyUnits), false)]
		protected virtual void _(Events.CacheAttached<POReceiptLine.receiptQty> e) { }

		#endregion POReceiptLine
		...
		[PXDBQuantity]
		protected virtual void _(Events.CacheAttached<POLine.receivedQty> e) { }
		...
		public
			PXSelect<POReceiptLineSplit,
			Where<
				POReceiptLineSplit.receiptType, Equal<Current<POReceiptLine.receiptType>>,
				...
				And<Where<POLineType.Goods.Contains<POReceiptLine.lineType.FromCurrent>>>>>>>
			splits;
		...
		public virtual IEnumerable AddPOOrder(PXAdapter adapter)
		{
			...
			openOrders.Cache.ClearQueryCache();
			return adapter.Get();
		}

		public PXAction<POReceipt> addPOOrder2;
		[PXUIField(DisplayName = Messages.AddPOOrder, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		[PXLookupButton]
        ...
        [PXUIField(DisplayName = Messages.AddTransfer, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
        [PXLookupButton]
		...
		[PXUIField(DisplayName = Messages.AddReceipt, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		[PXLookupButton]
		...
		[PXUIField(DisplayName = Messages.AddReceiptLine, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		[PXLookupButton]
		...
		public virtual void SetOriginalReceiptAmounts(POReceiptLine returnLine, POReceiptLine receiptLine)
		{
			...
			returnLine.AllowEditUnitCost = false;

			returnLine.DiscPct = receiptLine.DiscPct;

			decimal baseReceiptQty = receiptLine.InventoryID == null? (decimal)receiptLine.ReceiptQty : INUnitAttribute.ConvertToBase(transactions.Cache, receiptLine.InventoryID, receiptLine.UOM, (decimal)receiptLine.ReceiptQty, INPrecision.QUANTITY);
			decimal baseReturnQty = returnLine.InventoryID == null ? (decimal)returnLine.ReceiptQty : INUnitAttribute.ConvertToBase(transactions.Cache, receiptLine.InventoryID, returnLine.UOM, (decimal)returnLine.ReceiptQty, INPrecision.QUANTITY);

			bool qtySame = baseReceiptQty == baseReturnQty;

			decimal unitRatio = baseReturnQty / baseReceiptQty;

			returnLine.CuryDiscAmt = qtySame ? receiptLine.CuryDiscAmt : currencyInfo.RoundCury(receiptLine.CuryDiscAmt * unitRatio ?? 0m);

			returnLine.CuryUnitCost = INUnitAttribute.ConvertFromTo<POReceiptLine.inventoryID>(transactions.Cache, receiptLine.InventoryID, returnLine.UOM, receiptLine.UOM, (decimal)receiptLine.CuryUnitCost, INPrecision.UNITCOST);
			transactions.Cache.SetValueExt<POReceiptLine.curyUnitCost>(returnLine, returnLine.CuryUnitCost);

			returnLine.TranUnitCost = receiptLine.TranCostFinal / receiptLine.ReceiptQty;
			if (returnLine.TranUnitCost != null)
			{
					returnLine.CuryTranUnitCost = Math.Round(currencyInfo.CuryConvCuryRaw(returnLine.TranUnitCost ?? 0m), POReceiptLine.curyTranUnitCost.Precision, MidpointRounding.AwayFromZero);
			}

			returnLine.CuryExtCost = currencyInfo.RoundCury(receiptLine.CuryExtCost * unitRatio ?? 0m);
			transactions.Cache.SetValueExt<POReceiptLine.curyExtCost>(returnLine, returnLine.CuryExtCost);

			returnLine.TranCost = qtySame ? receiptLine.TranCostFinal ?? receiptLine.TranCost : currencyInfo.RoundCury((receiptLine.TranCostFinal ?? receiptLine.TranCost) * unitRatio ?? 0m);
			if (returnLine.TranCost != null)
			{
				returnLine.CuryTranCost = currencyInfo.CuryConvCury(returnLine.TranCost ?? 0m);
			}
		}
		...
		protected virtual void POReceipt_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
        {
            ...
            if (doc.Released == true)
            {
				...
				PXNoteAttribute.ForcePassThrow<POReceiptLine.noteID>(transactions.Cache);
            }
            else
            {
		...
		protected virtual void _(Events.RowSelecting<POReceiptLine> e)
		{
			...
			if (fromPO)
			{
				...
				POLine src = GetReferencedPOLine(e.Row.POType, e.Row.PONbr, e.Row.POLineNbr);
		...
		protected virtual void POReceiptLine_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			ValidateUomConversionNotAdjusted(row);
		...
		protected virtual void POReceiptLine_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			POLine poOriginLine = POLine.PK.Find(this, row.POType, row.PONbr, row.POLineNbr);
		...
		protected virtual void POReceiptLine_RowInserted(PXCache sender, PXRowInsertedEventArgs e)
        {
            ...
            if (row.PONbr != null && row.POType != null && row.POLineNbr != null)
			{
				poOriginLine = POLine.PK.Find(this, row.POType, row.PONbr, row.POLineNbr);
				this.UpdatePOLineCompleteFlag(row, false, poOriginLine);
			}
		...
		public virtual POLine GetReferencedPOLine(string poType, string poNbr, int? poLineNbr)
		{
			...
			poLine = poline.Locate(poLine);
			if (poLine == null)
			{
				poLine = POLine.PK.Find(this, poType, poNbr, poLineNbr, PKFindOptions.IncludeDirty);
				if (poLine != null)
				{
					poline.Cache.Hold(poLine);
				}
			}
			return poLine;
		}
        ...
        public class AddReceiptPopupHandler : PopupHandler
        {
            ...
            public override void TryGetSourceItem(POReceiptLineS filter)
            {
                ...
                {
                    POLine poLine = _graph.GetReferencedPOLine(itemsource.OrderType, itemsource.OrderNbr, itemsource.LineNbr);
                    if (poLine == null || itemsource.OrderQty - poLine.ReceivedQty <= 0)
                    {
            ...
            public override object GetSourceItem()
        {
                ...
                foreach (POLineS itemsource in polineResultSet)
                {
                    POLine poLine = _graph.GetReferencedPOLine(itemsource.OrderType, itemsource.OrderNbr, itemsource.LineNbr);
                    if (poLine == null ||
                        (_graph.Document.Current.ReceiptType == POReceiptType.POReceipt && itemsource.OrderQty - poLine.ReceivedQty <= 0) ||
                        (_graph.Document.Current.ReceiptType == POReceiptType.POReturn && poLine.ReceivedQty <= 0))
                    {
                        itemsource.Selected = false;
                        continue;
                    }
            ...
            public override void SetFilterToSource(PXCache sender, POReceiptLineS filter, object _source)
                ...
                if (source != null)
                {
                    POLine poLine = source != null
                                        ? _graph.GetReferencedPOLine(source.OrderType, source.OrderNbr, source.LineNbr)
                                        : null;

                    if (poLine != null)
                    {
                        openQty = _graph.Document.Current.ReceiptType == POReceiptType.POReceipt
                                    ? source.OrderQty - poLine.ReceivedQty
                                    : poLine.ReceivedQty;
                        baseOpenQty = _graph.Document.Current.ReceiptType == POReceiptType.POReceipt
                                        ? source.BaseOrderQty - poLine.BaseReceivedQty
				...
				foreach (POReceiptLineSplit split in splits.Select())
				{
					if (split.LineType == POLineType.GoodsForDropShip && LineSplittingExt.IsTrackSerial(split))
					{
						...
						if (uniqueSerials.Contains(key))
						{
							...
							POReceiptLine detail = PXParentAttribute.SelectParent<POReceiptLine>(splits.Cache, split);
		...
		protected virtual void ValidateUomConversionNotAdjusted(POReceiptLine rctLine)
		{
			if (rctLine.ReceiptType != POReceiptType.POReturn || rctLine.ReceiptQty <= 0m
				|| rctLine.OrigReceiptLineNbr == null || rctLine.Released == true)
				return;

			POReceiptLine origLine = POReceiptLine.FK.OriginalReceiptLine.FindParent(this, rctLine);
			if (origLine?.InventoryID == null || origLine?.UOM == null)
				return;

			var tmpRctLine = new POReceiptLine
			{
				InventoryID = origLine.InventoryID,
				ReceiptQty = origLine.ReceiptQty,
				UOM = origLine.UOM,
			};
			PXDBQuantityAttribute.CalcBaseQty<POReceiptLine.receiptQty>(transactions.Cache, tmpRctLine);
			if (tmpRctLine.BaseReceiptQty.IsNotIn(null, origLine.BaseReceiptQty))
			{
				var item = InventoryItem.PK.Find(this, origLine.InventoryID);
				if (item?.BaseUnit != null && !string.Equals(item.BaseUnit, origLine.UOM, StringComparison.OrdinalIgnoreCase))
				{
					transactions.Cache.RaiseExceptionHandling<POReceiptLine.baseReceiptQty>(rctLine, rctLine.BaseReceiptQty,
						new PXSetPropertyException(Messages.UomConversionModified, PXErrorLevel.Error,
							origLine.UOM.TrimEnd(), item.BaseUnit.TrimEnd(), item.InventoryCD.TrimEnd(), origLine.ReceiptNbr.TrimEnd()));
				}
			}
		}
		...
		protected virtual void CheckRctForPOQuantityRule(PXCache sender, POReceiptLine row, bool displayAsWarning, POLine aOriginLine)
		{
			...
			{
				poLineOrigin = POLine.PK.Find(this, row.POType, row.PONbr, row.POLineNbr);
			}
			if (poLineOrigin?.RcptQtyAction.IsIn(POReceiptQtyAction.Reject, POReceiptQtyAction.AcceptButWarn) == true)
			{
				if (poLineOrigin.OrderQty != Decimal.Zero)
				{
					decimal qty = (decimal)row.ReceiptQty;
					if (row.InventoryID != null)
					{
						if (!string.IsNullOrEmpty(poLineOrigin.UOM) && !string.IsNullOrEmpty(row.UOM) && poLineOrigin.UOM != row.UOM)
						{
							qty = INUnitAttribute.ConvertFromTo<POReceiptLine.inventoryID>(sender, row, row.UOM, poLineOrigin.UOM, qty, INPrecision.QUANTITY);
						}
						foreach (POReceiptLine iLine in PXSelect<POReceiptLine,
							Where<POReceiptLine.receiptType, Equal<Required<POReceiptLine.receiptType>>,
								And<POReceiptLine.receiptNbr, Equal<Required<POReceiptLine.receiptNbr>>,
							...
							.Select(this, row.ReceiptType, row.ReceiptNbr, row.LineNbr, row.POType, row.PONbr, row.POLineNbr))
						{
							if (!string.IsNullOrEmpty(poLineOrigin.UOM) && !string.IsNullOrEmpty(iLine.UOM) && poLineOrigin.UOM != iLine.UOM && iLine.Qty.HasValue)
							{
								qty += INUnitAttribute.ConvertFromTo<POReceiptLine.inventoryID>(sender, iLine, iLine.UOM, poLineOrigin.UOM, iLine.Qty.Value, INPrecision.QUANTITY);
							}
						}
					}
					decimal minQty = PXDBQuantityAttribute.Round(poLineOrigin.RcptQtyMin.Value / 100.0m * poLineOrigin.OpenQty.Value);
					decimal ratio = qty / poLineOrigin.OrderQty.Value * 100.0m;
					decimal maxRatio = ratio;
					POLine poLineUpdated = GetReferencedPOLine(row.POType, row.PONbr, row.POLineNbr);
					if (poLineUpdated != null)
						maxRatio = poLineUpdated.ReceivedQty.Value / poLineOrigin.OrderQty.Value * 100.0m;

					if (row.ReceiptType == POReceiptType.POReceipt)
					{
						var errorLevel = (!displayAsWarning && (poLineOrigin.RcptQtyAction == POReceiptQtyAction.Reject))
							? PXErrorLevel.Error : PXErrorLevel.Warning;
						if (qty < minQty)
						{
							sender.RaiseExceptionHandling<POReceiptLine.receiptQty>(row, row.ReceiptQty,
								new PXSetPropertyException( Messages.ReceiptLineQtyDoesNotMatchMinPOQuantityRules, errorLevel));
						}
						if (maxRatio > poLineOrigin.RcptQtyMax)
						{
							sender.RaiseExceptionHandling<POReceiptLine.receiptQty>(row, row.ReceiptQty,
								new PXSetPropertyException(Messages.ReceiptLineQtyDoesNotMatchMaxPOQuantityRules, errorLevel));
						}
					}
					else if (poLineUpdated != null && poLineUpdated.ReceivedQty < 0)
					{
						sender.RaiseExceptionHandling<POReceiptLine.receiptQty>(row, row.ReceiptQty,
							new PXSetPropertyException(Messages.ReceiptLineQtyGoNegative, PXErrorLevel.Error));
					}
				}
			}
		}
		...
		protected virtual void UpdatePOLineCompleteFlag(POReceiptLine row, bool isDeleted, POLine aOriginLine)
		{
			...
			POLine poLineCurrent = GetReferencedPOLine(row.POType, row.PONbr, row.POLineNbr);
			POLine poLineOrigin = aOriginLine;
			if (poLineOrigin == null || poLineOrigin.OrderType != row.POType || poLineOrigin.OrderNbr != row.PONbr || poLineOrigin.LineNbr != row.POLineNbr)
			{
				poLineOrigin = POLine.PK.Find(this, row.POType, row.PONbr, row.POLineNbr);
			...
			if (row.ReceiptType == POReceiptType.POReceipt)
			{
				...
				if (needToCompletePOByQty != poLineCurrent.AllowComplete)
				{
					...
					poLineCurrent = poline.Update(poLineCurrent);
					this.transactions.View.RequestRefresh();
				}
		...
		protected virtual void DeleteUnusedReference(POReceiptLine aLine, bool skipReceiptUpdate)
		{
			...
			foreach (POReceiptLine iLine in PXSelect<POReceiptLine,
				Where<POReceiptLine.receiptType, Equal<Required<POReceipt.receiptType>>,
					...
					And<POReceiptLine.pOType, Equal<Required<POReceiptLine.pOType>>,
					And<POReceiptLine.pONbr, Equal<Required<POReceiptLine.pONbr>>>>>>>
				.Select(this, aLine.ReceiptType, aLine.ReceiptNbr, aOrderType, aOrderNbr))
			{
			...
			if (!skipReceiptUpdate)
			{
				if (doc.POType == aLine.POType)
				{
					...
					foreach (PXResult<POReceiptLine, POOrder> it in
						PXSelectJoin<POReceiptLine,
							...
							Where<POReceiptLine.receiptType, Equal<Required<POReceipt.receiptType>>,
								And<POReceiptLine.receiptNbr, Equal<Required<POReceipt.receiptNbr>>,
								And<POReceiptLine.pOType, Equal<Required<POReceiptLine.pOType>>>>>>
						.Select(this, aLine.ReceiptType, aLine.ReceiptNbr, aOrderType))
					{
        ...
        public virtual void ReleaseReceipt(INReceiptEntry docgraph, AP.APInvoiceEntry invoiceGraph, POReceipt aDoc, DocumentList<INRegister> aINCreated, DocumentList<AP.APInvoice> aAPCreated, bool aIsMassProcess)
		{
			...
			POLine loadPOLine(string poType, string poNbr, int? poLineNbr)
			{
				if (string.IsNullOrEmpty(poType) || string.IsNullOrEmpty(poNbr) || poLineNbr == null)
					return null;
				//Need special select for correct updating data merging in the cache
				return PXSelect<POLine, Where<POLine.orderType, Equal<Required<POLine.orderType>>,
					And<POLine.orderNbr, Equal<Required<POLine.orderNbr>>,
						And<POLine.lineNbr, Equal<Required<POLine.lineNbr>>>>>>.Select(this, poType, poNbr, poLineNbr);
			};
			...
			foreach (PXResult<POReceiptLine> res in GetLinesToReleaseQuery().Select(aDoc.ReceiptType, aDoc.ReceiptNbr))
			{
				...
				POLine poLine = loadPOLine(line.POType, line.PONbr, line.POLineNbr);
				...
				if (rctLineChange || isNonStockKit)
				{
					...
					poLine = poline.Locate(poLine) ?? poLine;
		...
		protected virtual void UpdateReceiptLineOnRelease(PXResult<POReceiptLine> row, POLine poLine)
		{
		}

		protected virtual void UpdateSOOrderLink(INTran newtran, POLine poLine, POReceiptLine line)
		{
		}
		...
		protected virtual void ReduceSOAllocationOnReleaseReturn(PXResult<POReceiptLine> row, POReceiptLineSplit posplit, POLine poLine)
		{
		}

		public virtual void CollectDemandOrdersMarkedForPO(POReceiptLine line, POLine poLine,
			List<INItemPlan> posupply, HashSet<PXResult<INItemPlan, INPlanType>> podemand)
		{
			...
			foreach (PXResult<INItemPlan, INPlanType> demandres in
				...
				.View.SelectMultiBound(this, new[] { poLine }).AsEnumerable()
		...
		public virtual void UpdateDemandOrdersMarkedForDropship(POReceiptLine line, POLine poLine, POAddress poAddress)
		{
			if (poLine == null ||
		...
		protected virtual bool TryProcessReceiptLinkedAllocation(INTran newline, POReceipt receiptDoc, POLine poLine, HashSet<PXResult<INItemPlan, INPlanType>> podemand, List<INItemPlan> posupply)
		{
			//Non-Stock Item
		...
		protected virtual void ReleaseReturnLine(POReceiptLine line, POLine poLine, POReceiptLine2 origLine)
		{
		...
		public virtual void ReleaseReturn(INIssueEntry docGraph, APInvoiceEntry invoiceGraph, POReceipt aDoc, DocumentList<INRegister> aCreated, DocumentList<APInvoice> aInvoiceCreated, bool aIsMassProcess)
		{
			...
			var receiptLineSplits = PXSelectJoin<POReceiptLine,
				...
				.Cast<PXResult<POReceiptLine, POReceiptLineSplit, POLine, INTran, INItemPlan, POReceiptLine2>>()
				.ToList();
			...
			for (int i = 0; i < receiptLineSplits.Count; i++)
			{
				...
				INItemPlan plan = PXCache<INItemPlan>.CreateCopy(res.GetItem<INItemPlan>());
				INPlanType plantype = INPlanType.PK.Find(this, plan.PlanType) ?? new INPlanType();
				POLine poLine = res.GetItem<POLine>();
		...
		protected virtual void UpdateReturnOrdersMarkedForDropship(POReceiptLine line, POLine poLine)
		{
		}
        ...
        [Serializable]
		public partial class POReceiptLineS : PXBqlTable, PX.Data.IBqlTable
		{
			#region BarCode
