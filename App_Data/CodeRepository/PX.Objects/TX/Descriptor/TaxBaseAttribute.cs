...
namespace PX.Objects.TX
{
													 ...
													 IComparable
	{
		...
		internal static void InvokeRecalcTaxes(PXCache sender)
			=> sender.Graph.FindImplementation<Extensions.SalesTax.ITaxRecalculator>()?.RecalcTaxes();
		...
		protected List<object> SelectLvl1Taxes(PXGraph graph, object row)
		{
			return
				IsExemptTaxCategory(graph, row)
					? new List<object>()
					: SelectTaxes<Where<Tax.taxCalcLevel2Exclude, Equal<False>,
				And<Where<Tax.taxCalcLevel, Equal<CSTaxCalcLevel.calcOnItemAmt>, Or<Tax.taxCalcLevel, Equal<CSTaxCalcLevel.inclusive>>>>>>(graph, row, PXTaxCheck.Line);
		}
		...
		protected virtual void ParentRowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (ParentBranchIDField != null)
			{
				...
				cashDiscountBase = OrganizationMaint.GetCashDiscountBase(sender.Graph, newBranchID);
			}
			...
			if (newDiscAmt != oldDiscAmt && !DiscPercentsDict.ContainsKey(e.Row))
			{
				...
				using (new TermsAttribute.UnsubscribeCalcDiscScope(sender))
				{
					try
					{
						...
						else if (cashDiscountBase == CashDiscountBases.DocumentAmountLessTaxes && reducedTaxAmt != 0m)
						{
							decimal curyDocBal = ((decimal?)sender.GetValue(e.Row, _CuryDocBal)).GetValueOrDefault();
							DiscPercentsDict[e.Row] = 100 * newDiscAmt / (curyDocBal - reducedTaxAmt);
						}
				...
				tax.TaxCalcLevel == CSTaxCalcLevel.Inclusive && (!_isTaxCalcModeEnabled || GetTaxCalcMode(sender.Graph) != TaxCalculationMode.Net)))
			{
				...
				if (docLines.Any())
				{
					...
					var taxLines = inclusiveTaxes.Select(_ => new
					{
						...
						TaxRateMultiplier = PXResult.Unwrap<Tax>(_).TaxType == CSTaxType.Withholding ? 0m : (PXResult.Unwrap<Tax>(_).ReverseTax == true ? -1.0M : 1.0M),
