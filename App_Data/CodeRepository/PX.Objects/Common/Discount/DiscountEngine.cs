...
namespace PX.Objects.Common.Discount
{
		...
		where TDiscountDetail : class, IBqlTable, IDiscountDetail, new()
	{
			...
			RecalcDiscountsParamFilter recalcFilter = null)
		{
			...
			if (documentDetails.Count != 0)
			{
				...
				CalculateGroupDiscountRate(cache, documentDetails, currentLine, groupDiscountCodesWithApplicableLines, discountDetails, false, true, discountCalculationOptions.HasFlag(DiscountCalculationOptions.ForceFormulaRecalculation));
			}
			else
			{
		...
		/// <summary>
		/// Calculates DocumentDiscountRate and updates array of discounts applied to document details lines (DiscountsAppliedToLine field)
		/// GroupDiscountRate affects Taxes and Commissions calculation. Allows to force formula recalculation.
		/// </summary>
		public virtual void CalculateGroupDiscountRate(
			PXCache cache,
			List<TLine> documentDetails,
		...
		{
			CalculateGroupDiscountRate(cache, documentDetails, currentLine, MatchLinesWithDiscounts(cache, newGroupDiscountCodesWithApplicableLines, documentDetails, discountDetails, true, false), recalculateTaxes, recalcAll, forceFormulaCalculation);
		}
	...
	/// Calculates DocumentDiscountRate and updates array of discounts applied to document details lines (DiscountsAppliedToLine field)
			...
			RecalcDiscountsParamFilter recalcFilter = null)
		{
			...
			CalculateDocumentDiscountRate(cache, documentDetails, currentLine, discountDetails, discountCalculationOptions.HasFlag(DiscountCalculationOptions.ForceFormulaRecalculation));
		}
