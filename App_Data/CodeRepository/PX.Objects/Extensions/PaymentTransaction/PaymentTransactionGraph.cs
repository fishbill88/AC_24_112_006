...
namespace PX.Objects.Extensions.PaymentTransaction
{
		...
		where TPrimary : class, IBqlTable, new()
	{
		...
		private TranRecordData FormatRecordData(V2.TransactionData info)
		{
			...
			// The value of the Subtotal field is not set since it is calculated in the getter 
			tranRecord.Level3Support = info.Level3Support;
			tranRecord.TerminalID = info.TerminalID;
		...
		protected virtual void ValidateTransactionData(ICCPayment doc, V2.TransactionData tranData)
		{
			...
			if (!((paymentMethod.PaymentType == PaymentMethodType.EFT && tranData.PaymentMethodType == V2.MeansOfPayment.EFT) ||
				...
				(paymentMethod.PaymentType == PaymentMethodType.POSTerminal && tranData.PaymentMethodType == V2.MeansOfPayment.CreditCard)))
			{
		...
		{
			StackTrace sTrace = PXStackTrace.GetStackTrace();
