...
namespace PX.Objects.Extensions.SalesPrice
{
			...
			where TPrimary : class, IBqlTable, new()
	{
		...
		{
			var item = (InventoryItem) PXSelectorAttribute.Select<Detail.inventoryID>(Details.Cache, e.Row);

			e.NewValue = PXDBLocalizableStringAttribute.GetTranslation<InventoryItem.descr>(
				Base.Caches<InventoryItem>(),
				item);

			var baseRow = e.Row?.Base;
			if (!string.IsNullOrEmpty(item?.Descr) && baseRow != null)
			{
				PXDBLocalizableStringAttribute.CopyTranslations<InventoryItem.descr, Detail.descr>(
					Base.Caches<InventoryItem>(),
					item,
					// Can't use e.Cache here since PXCache._Originals is null for it
					// and PXCache.SetSlot fails at lock (((ICollection)_Originals).SyncRoot).
					Base.Caches[baseRow.GetType()],
					baseRow);
			}
		}
		...
		protected virtual void _(Events.RowUpdated<Detail> e)
		{
			...
			)
			{
				...
				// Acuminator disable once PX1045 PXGraphCreateInstanceInEventHandlers - method of the graph instance is used for price calculation
