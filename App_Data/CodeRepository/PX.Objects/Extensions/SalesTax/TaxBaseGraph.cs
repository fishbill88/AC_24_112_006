...
namespace PX.Objects.Extensions.SalesTax
{
    ...
    public abstract partial class TaxBaseGraph<TGraph, TPrimary> : PXGraphExtension<TGraph>, ITaxRecalculator
         ...
         where TPrimary : class, IBqlTable, new()
    {
		...
		protected class TaxDetailMapping : IBqlMapping
        {
			...
			/// <summary>
			/// Field, TaxDetail.TaxUOM is mapped to
			/// </summary>
			public Type TaxUOM = typeof(TaxDetail.taxUOM);

			/// <summary>
			/// Field, TaxDetail.TaxableQty is mapped to
			/// </summary>
			public Type TaxableQty = typeof(TaxDetail.taxableQty);
		...
		protected class TaxTotalMapping : IBqlMapping
        {
			...
			/// Field, TaxDetail.TaxUOM is mapped to
			/// </summary>
			public Type TaxUOM = typeof(TaxTotal.taxUOM);

			/// <summary>
			/// Field, TaxDetail.TaxableQty is mapped to
			/// </summary>
			public Type TaxableQty = typeof(TaxTotal.taxableQty);
		}
        ...
        private void TaxSetLineDefault(object taxrow, Detail row)
        {
			...
			if (IsPerUnitTax(tax))
			{
				TaxSetLineDefaultForPerUnitTaxes(row, tax, taxrev, taxdet);
				return;
			}
			...
			switch (tax.TaxCalcLevel)
            {
                case CSTaxCalcLevel.Inclusive:
					(CuryTaxableAmt, CuryTaxAmt) = CalculateInclusiveTaxAmounts(row, taxdet, incltaxes, calculatedTaxRate);
					SetTaxableAmt(row, CuryTaxableAmt);
                    SetTaxAmt(row, CuryInclTaxAmt);
                    break;
				case CSTaxCalcLevel.CalcOnItemAmt:
					{
						decimal curyPerUnitTaxAmount = GetPerUnitTaxAmountForTaxableAdjustmentCalculation(tax, taxdet,  row);
		...
		private (decimal InclTaxTaxable, decimal InclTaxAmount) CalculateInclusiveTaxAmounts(Detail row, TaxDetail nonPerUnitTaxDetail, List<object> inclusiveTaxes, in decimal calculatedTaxRate)
		{
			...
			decimal totalInclusiveNonPerUnitTaxRate = SumWithReverseAdjustment(inclusiveNonPerUnitTaxes, typeof(TaxRev), typeof(TaxRev.taxRate)) / 100;

			if (GetTaxDetailMapping().CuryTaxAmt != null)
			{
				curyInclusivePerUnitTaxAmountIncludedInTaxOnTaxCalc =
					SumWithReverseAdjustment(inclusivePerUnitTaxesIncludedInTaxOnTaxCalc, GetTaxDetailMapping().Table, GetTaxDetailMapping().CuryTaxAmt);

				curyPerUnitTaxAmountExcludedFromTaxOnTaxCalc =
					SumWithReverseAdjustment(inclusivePerUnitTaxesExcludedFromTaxOnTaxCalc, GetTaxDetailMapping().Table, GetTaxDetailMapping().CuryTaxAmt);
			}
			...
			decimal curyRealTaxableAmt =
				((row.CuryTranAmt.Value - curyPerUnitTaxAmountExcludedFromTaxOnTaxCalc) / (1 + totalInclusiveNonPerUnitTaxRate)) - curyInclusivePerUnitTaxAmountIncludedInTaxOnTaxCalc;
			...
			CurrencyInfo currencyInfo = Base.FindImplementation<IPXCurrencyHelper>().GetCurrencyInfo(nonPerUnitTaxDetail.CuryInfoID);
			...
			decimal curyNonPerUnitTaxAmount = currencyInfo.RoundCury(nonPerUnitTaxAmount);
			...
			var taxRevCache = Base.Caches[typeof(TaxRev)];
			...
			foreach (PXResult inclusiveNonPerUnitTaxRow in inclusiveNonPerUnitTaxes)
			{
				...
				curyCurrentInclusiveTaxAmount = currencyInfo.RoundCury(curyCurrentInclusiveTaxAmount) * multiplier;

				curyTotalInclusiveTaxAmt += curyCurrentInclusiveTaxAmount;
			}

			curyRealTaxableAmt = row.CuryTranAmt.Value - curyTotalInclusiveTaxAmt;
        ...
        protected virtual TaxTotal CalculateTaxSum(object taxrow, object row)
        {
			...
			if (IsPerUnitTax(tax))
			{
				taxdet = FillAggregatedTaxDetailForPerUnitTax(row, tax, taxrev, taxdet, taxitems);
			}
		...
		{			
            ...
            if (taxCalc == TaxCalc.Calc)
			{
				...
				if (ShouldRecalculateTaxesOnRowUpdate(row, oldRow))
				{
					CalcTaxes(row, PXTaxCheck.Line);
					calculated = true;
				}
		...
		private static bool ShouldRecalculateTaxesOnRowUpdate(Detail row, Detail oldRow)
		{
			...
			{
				if (oldRow.Qty != row.Qty)
					return true;

				if (oldRow.UOM != row.UOM)
					return true;
			}
