        ...
        where TPrimary : class, IBqlTable, new()
    {
        ...
        protected decimal CalculateTaxDiscrepancy(object row)
        {
            ...
            SelectTaxes(row, PXTaxCheck.RecalcTotals).
                ...
                ForEach(taxrow =>
                {
					object taxDet = taxrow[0];
					PXCache taxDetCache = Base.Caches[taxDet.GetType()];
					decimal CuryTaxAmt = (decimal)taxDetCache.GetValue(taxDet, GetTaxDetailMapping().CuryTaxAmt.Name);
					decimal CuryExpenseAmt = (decimal)taxDetCache.GetValue(taxDet, GetTaxDetailMapping().CuryExpenseAmt.Name);
					TaxTotal taxSum = CalculateTaxSum(taxrow, row);
                    if (taxSum != null)
                    {
                        PXCache sumcache = TaxTotals.Cache;
                        discrepancy += CuryTaxAmt + CuryExpenseAmt - taxSum.CuryTaxAmt.Value - taxSum.CuryExpenseAmt.Value;
                    }
                });
            return discrepancy;
        }
