...
namespace PX.Objects.Extensions.MultiCurrency
{
			...
			where TPrimary : class, IBqlTable, new()
	{
		...
		protected virtual void _(Events.RowPersisting<CurrencyInfo> e)
		{
			...
			ValidateCurrencyRate(e.Row);
		}

		public virtual void ValidateCurrencyRate(CurrencyInfo info)
		{
			PXCache primaryCache = Base.Caches[typeof(TPrimary)];

			if (info.CuryRate == null)
			{
				primaryCache.RaiseExceptionHandling<CurrencyInfo.curyID>(primaryCache.Current, info.CuryID,
					new PXSetPropertyException(CM.Messages.CurrencyRateNotDefined, info.CuryRateTypeID, info.CuryEffDate?.ToShortDateString()));
			}
			else
			{
				string immutableMessagePart = CM.Messages.CurrencyRateNotDefined.Split('{').First();
				bool erreorExist = primaryCache.GetAttributes<CurrencyInfo.curyID>(primaryCache.Current)
					.OfType<IPXInterfaceField>()
					.Any(field => string.IsNullOrWhiteSpace(field.ErrorText) ? false : field.ErrorText.StartsWith(immutableMessagePart));

				if (erreorExist) primaryCache.ClearFieldErrors<CurrencyInfo.curyID>(primaryCache.Current);
			}
		}
