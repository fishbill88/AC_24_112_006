	...
	public class JournalEntryProjectExt : PXGraphExtension<JournalEntry>
	{
		...
		public virtual void ProcessProjectTransaction(ProjectBalance pb, Dictionary<string, PMTask> tasksToAutoAllocate, PMTran pmt, Account acc, PMAccountGroup ag, PMProject project, PMTask task, ARTran arTran)
		{
			...
			if (balance.Status != null)
			{
				...
				{
					var arTranCuryTranAmtPlusCuryRetainageAmt = GetAmountInProjectCurrency(
					arTran,
					project,
					(arTran.CuryTranAmt.GetValueOrDefault() + arTran.CuryRetainageAmt.GetValueOrDefault()) * arSign);

					int signOfActual = balance.Status.CuryActualAmount.GetValueOrDefault() < 0 ? -1 : 1;
					ps.CuryInvoicedAmount -= signOfActual * Math.Abs(arTranCuryTranAmtPlusCuryRetainageAmt.GetValueOrDefault());
			...
			Account acc, GLTran tran, APTran apTran, ARTran arTran)
		{
			...
			if (apTran != null)
			{
				...
				var select = new PXSelectReadonly<PMTran,
						Where<PMTran.origModule, Equal<Required<PMTran.origModule>>,
						And<PMTran.origTranType, Equal<Required<PMTran.origTranType>>,
						And<PMTran.origRefNbr, Equal<Required<PMTran.origRefNbr>>,
						And<PMTran.origLineNbr, Equal<Required<PMTran.origLineNbr>>>>>>,
						OrderBy<Desc<PMTran.tranID>>>(Base);

				PMTran reclassified = select.SelectWindowed(0, 1, BatchModule.AP, apTran.TranType, apTran.RefNbr, apTran.LineNbr);
				if (reclassified != null)
				{
					pmt.ProformaRefNbr = reclassified.ProformaRefNbr;
					pmt.ProformaLineNbr = reclassified.ProformaLineNbr;
				}
			}
