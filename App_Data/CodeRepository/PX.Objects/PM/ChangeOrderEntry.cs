...
using static PX.Objects.CN.Subcontracts.PM.Descriptor.Messages;
...
namespace PX.Objects.PM
{
	public class ChangeOrderEntry : PXGraph<ChangeOrderEntry, PMChangeOrder>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		public class ReversingChangeOrderExt : ChangeOrderExt<ChangeOrderEntry, PMChangeOrder>
		{
			...
			public override PMChangeOrder CurrentChangeOrder => Base.Document.Current;
		...
		public ChangeOrderEntry()
		{
			...
			GetExtension<ReversingChangeOrderExt>().viewChangeOrder.SetVisible(false);
			GetExtension<ReversingChangeOrderExt>().viewCurrentReversingChangeOrder.SetVisible(false);
		}
		...
		{
			if (Document.Current != null && !string.IsNullOrEmpty(Document.Current.OrigRefNbr))
			{
				NavigateChangeOrder(Document.Current.OrigRefNbr, Messages.ViewChangeOrder, PXBaseRedirectException.WindowMode.NewWindow);
			}
		...
		{
			var hasCostCommitmentTracking = Setup.Current.CostCommitmentTracking == true;
			Details.Cache.AllowSelect = hasCostCommitmentTracking;
			if (e.Row == null)
				return;
			...
			string budgetLevelCost = Project.Current?.CostBudgetLevel ?? BudgetLevels.Detail;
			string budgetLevelRevenue = Project.Current?.BudgetLevel ?? BudgetLevels.Detail;

				if (!IsCopyPasteContext && !IsImport && !IsExport)
				{
				PXUIFieldAttribute.SetVisible<PMChangeOrderRevenueBudget.inventoryID>(
					RevenueBudget.Cache, null, BudgetLevelHelper.ContainsItem(budgetLevelRevenue));
				PXUIFieldAttribute.SetVisible<PMChangeOrderCostBudget.inventoryID>(
					CostBudget.Cache, null, BudgetLevelHelper.ContainsItem(budgetLevelCost));
				PXUIFieldAttribute.SetVisible<PMChangeOrderRevenueBudget.costCodeID>(
					RevenueBudget.Cache, null, BudgetLevelHelper.ContainsCostCode(budgetLevelRevenue));
				PXUIFieldAttribute.SetVisible<PMChangeOrderCostBudget.costCodeID>(
					CostBudget.Cache, null, BudgetLevelHelper.ContainsCostCode(budgetLevelCost));
				}

			PXUIFieldAttribute.SetVisible<PMBudget.curyCommittedAmount>(Caches[typeof(PMBudget)], null, hasCostCommitmentTracking);
			PXUIFieldAttribute.SetVisible<PMBudget.committedQty>(Caches[typeof(PMBudget)], null, hasCostCommitmentTracking);
			PXUIFieldAttribute.SetVisible<PMBudget.curyCommittedInvoicedAmount>(Caches[typeof(PMBudget)], null, hasCostCommitmentTracking);
			PXUIFieldAttribute.SetVisible<PMBudget.committedInvoicedQty>(Caches[typeof(PMBudget)], null, hasCostCommitmentTracking);
			PXUIFieldAttribute.SetVisible<PMBudget.curyCommittedOpenAmount>(Caches[typeof(PMBudget)], null, hasCostCommitmentTracking);
			PXUIFieldAttribute.SetVisible<PMBudget.committedOpenQty>(Caches[typeof(PMBudget)], null, hasCostCommitmentTracking);
			PXUIFieldAttribute.SetVisible<PMBudget.committedReceivedQty>(Caches[typeof(PMBudget)], null, hasCostCommitmentTracking);
			PXUIFieldAttribute.SetVisible<PMBudget.committedCOQty>(Caches[typeof(PMBudget)], null, hasCostCommitmentTracking);
			PXUIFieldAttribute.SetVisible<PMBudget.curyCommittedCOAmount>(Caches[typeof(PMBudget)], null, hasCostCommitmentTracking);
			PXUIFieldAttribute.SetVisible<PMChangeOrderCostBudget.committedCOQty>(CostBudget.Cache, null, hasCostCommitmentTracking);
			PXUIFieldAttribute.SetVisible<PMChangeOrderCostBudget.committedCOAmount>(CostBudget.Cache, null, hasCostCommitmentTracking);

			var hasCostCommitmentTrackingVisible = hasCostCommitmentTracking ? PXUIVisibility.Visible : PXUIVisibility.Invisible;
			PXUIFieldAttribute.SetVisibility<PMBudget.curyCommittedAmount>(Caches[typeof(PMBudget)], null, hasCostCommitmentTrackingVisible);
			PXUIFieldAttribute.SetVisibility<PMBudget.committedQty>(Caches[typeof(PMBudget)], null, hasCostCommitmentTrackingVisible);
			PXUIFieldAttribute.SetVisibility<PMBudget.curyCommittedInvoicedAmount>(Caches[typeof(PMBudget)], null, hasCostCommitmentTrackingVisible);
			PXUIFieldAttribute.SetVisibility<PMBudget.committedInvoicedQty>(Caches[typeof(PMBudget)], null, hasCostCommitmentTrackingVisible);
			PXUIFieldAttribute.SetVisibility<PMBudget.curyCommittedOpenAmount>(Caches[typeof(PMBudget)], null, hasCostCommitmentTrackingVisible);
			PXUIFieldAttribute.SetVisibility<PMBudget.committedOpenQty>(Caches[typeof(PMBudget)], null, hasCostCommitmentTrackingVisible);
			PXUIFieldAttribute.SetVisibility<PMBudget.committedReceivedQty>(Caches[typeof(PMBudget)], null, hasCostCommitmentTrackingVisible);

				bool isEditable = CanEditDocument(e.Row);
				Document.Cache.AllowDelete = isEditable;

			var isRevenueBudgetEnabled = ChangeOrderClass.Current?.IsRevenueBudgetEnabled == true;
			addRevenueBudget.SetEnabled(isEditable && isRevenueBudgetEnabled);
			RevenueBudget.Cache.AllowInsert = isEditable && isRevenueBudgetEnabled;
			RevenueBudget.Cache.AllowUpdate = isEditable && isRevenueBudgetEnabled;
			RevenueBudget.Cache.AllowDelete = isEditable && isRevenueBudgetEnabled;

			var isCostBudgetEnabled = ChangeOrderClass.Current?.IsCostBudgetEnabled == true;
			addCostBudget.SetEnabled(isEditable && isCostBudgetEnabled);
			CostBudget.Cache.AllowInsert = isEditable && isCostBudgetEnabled;
			CostBudget.Cache.AllowUpdate = isEditable && isCostBudgetEnabled;
			CostBudget.Cache.AllowDelete = isEditable && isCostBudgetEnabled;

			var isPurchaseOrderEnabled = ChangeOrderClass.Current?.IsPurchaseOrderEnabled == true;
			PXUIFieldAttribute.SetVisible<PMChangeOrder.commitmentTotal>(e.Cache, e.Row, isPurchaseOrderEnabled);
			addPOLines.SetEnabled(isEditable && isPurchaseOrderEnabled);
			Details.Cache.AllowInsert = isEditable && isPurchaseOrderEnabled;
			Details.Cache.AllowUpdate = isEditable && isPurchaseOrderEnabled;
			Details.Cache.AllowDelete = isEditable && isPurchaseOrderEnabled;
			...
			PXUIFieldAttribute.SetEnabled<PMChangeOrder.projectNbr>(e.Cache, e.Row,
				isEditable && e.Row.IsRevenueVisible == true && e.Row.ProjectNbr != Messages.NotAvailable);
		...
		{
			if (IsContractBasedAPI && e.Row != null)
			{
				var pendingValue = e.Cache.GetValuePending<PMChangeOrderLine.description>(e.Row);
				if (pendingValue != null)
				{
					e.NewValue = pendingValue;
					return;
				}
			}
		...
		[PXHidden]
		[Serializable]
		public class POLineFilter : PXBqlTable, IBqlTable
		{
