	...
	public class RegisterEntry : PXGraph<RegisterEntry, PMRegister>, PXImportAttribute.IPXPrepareItems
	{
		public class MultiCurrency : PMTranMultiCurrencyPM<RegisterEntry>
		{
			...
			protected virtual void _(Events.FieldDefaulting<PMTran, PMTran.tranCuryID> e)
			{
				...
				{
					Branch branch = Branch.PK.Find(Base, e.Row.BranchID);
					e.NewValue = branch?.BaseCuryID ?? Base.Company.Current.BaseCuryID;
				}
			}
			...
			protected virtual void _(Events.RowDeleted<PMTran> e)
			{
				...
				var orphanedCurrencyInfo = SelectFrom<CurrencyInfo>
					.Where<CurrencyInfo.curyInfoID.IsIn<P.AsInt>
						.And<NotExists<SelectFrom<PMTran>
							.Where<PMTran.tranID.IsNotEqual<P.AsInt>
								.And<PMTran.baseCuryInfoID.IsEqual<CurrencyInfo.curyInfoID>
									.Or<PMTran.projectCuryInfoID.IsEqual<CurrencyInfo.curyInfoID>>>>>>>
					.View
					.Select(Base, new[] { e.Row.BaseCuryInfoID, e.Row.ProjectCuryInfoID }, e.Row.TranID)
					.FirstTableItems;

				foreach (var item in orphanedCurrencyInfo)
				{
					currencyinfo.Delete(item);
				}
			}
		...
		[PXDimensionSelector("EMPLOYEE",
            ...
            typeof(CR.Standalone.EPEmployee.vStatus),
		...
		public RegisterEntry()
        {
            ...
            PXUIFieldAttribute.SetVisible<CR.Standalone.EPEmployee.vStatus>(this.Caches[typeof(CR.Standalone.EPEmployee)], null, false);
        }
		...
		protected void _(Events.FieldUpdated<PMTran, PMTran.accountGroupID> e)
		{
			if (e.Row != null && e.ExternalCall)
				CheckDebitAccount(e.Row, e.Cache);
		}

		public static void UpdateOffsetAccountId(PXGraph graph, PXCache cache, PMTran tran)
		{
			if (tran != null && tran.IsNonGL != true)
			{
		...
		{
			var tran = e.Row;
			if (tran == null || e.Operation == PXDBOperation.Delete)
				return;

			var project = PMProject.PK.Find(this, tran.ProjectID);
			if (tran.AccountGroupID == null
				&& project?.BaseType == CTPRType.Project
				&& !ProjectDefaultAttribute.IsNonProject(project.ContractID))
			{
				e.Cache.RaiseExceptionHandling<PMTran.accountGroupID>(tran, null,
					new PXSetPropertyException(ErrorMessages.FieldIsEmpty, $"[{nameof(PMTran.accountGroupID)}]"));
			}

			CheckDebitAccount(tran, e.Cache);
		}

		protected void CheckDebitAccount(PMTran tran, PXCache cache)
		{
			if (tran?.AccountID == null || tran?.AccountGroupID == null || cache == null)
				return;

			var account = Account.PK.Find(this, tran.AccountID);
			var accountGroup = PMAccountGroup.PK.Find(this, tran.AccountGroupID);
			if (account == null || accountGroup == null)
				return;

			if (accountGroup.Type != PMAccountType.OffBalance
				&& accountGroup.GroupID != account.AccountGroupID)
			{
				var state = (PXFieldState) cache.GetStateExt<PMTran.accountID>(tran);
				cache.RaiseExceptionHandling<PMTran.accountID>(tran, state.Value,
					new PXSetPropertyException(ErrorMessages.ElementDoesntExist,
						state.DisplayName));
			}
		}
		...
		public virtual void ReverseCreditMemo(ARRegister arDoc, List<PXResult<ARTran, PMTran>> list, List<PMTran> remainders)
		{
			...
			foreach (PXResult<ARTran, PMTran> item in list)
			{
				...
				if (pm.Reverse == PMReverse.Never && pm.RemainderOfTranID == null)
				{
					...
					reversal = Transactions.Insert(reversal);

					if (!IsFinPeriodValid(reversal))
					{
						reversal.FinPeriodID = ar.FinPeriodID;
						Transactions.Update(reversal);
					}
				}
		...
		protected virtual bool IsFinPeriodValid(PMTran tran)
		{
			try
			{
				...
				var attribute = Transactions.Cache
					.GetAttributes(nameof(PMTran.finPeriodID))
		...
		private void AddAllocatedTotal(PMTran tran)
		{
			if (tran.OrigProjectID != null && tran.OrigTaskID != null && tran.OrigAccountGroupID != null)
			{
				...
				tat.Amount += tran.ProjectCuryAmount;
