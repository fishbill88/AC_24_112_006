...
namespace PX.Objects.PM
{
	public class ProjectBalance
	{
		...
		{
			return Calculate(project, tran, ag, offsetAg);
		}

		public virtual List<Result> Calculate(PMProject project, PMTran tran, PMAccountGroup ag, PMAccountGroup offsetAg)
		{
			...
			if (project.NonProject != true && tran.TaskID != null)
			{
				...
				if (!debitcreditcancelout)
				{
					...
					if (ag.Type == AccountType.Income || ag.Type == AccountType.Liability)
						list.Add(Calculate(project, tran, ag, -1, 1));
					else
						list.Add(Calculate(project, tran, ag, 1, 1));


					//CREDIT
					bool containsCreditPart = offsetAg != null && offsetAg.GroupID != null;
					if (containsCreditPart)
					{
						if (offsetAg.Type == AccountType.Income || offsetAg.Type == AccountType.Liability)
							list.Add(Calculate(project, tran, offsetAg, 1, -1));
						else
							list.Add(Calculate(project, tran, offsetAg, -1, -1));
					}
				}
		...
		{
			return Calculate(project, pmt, ag, amountSign, qtySign);
		}

		public virtual Result Calculate(PMProject project, PMTran pmt, PMAccountGroup ag, int amountSign, int qtySign)
		{
