...
{
	public class PMQuoteMaint : PXGraph<PMQuoteMaint, PMQuote>, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess
	{
		...
		[PXHidden]
		[Serializable]
		public partial class CopyQuoteFilter : PXBqlTable, IBqlTable
		{
			#region Description
		...
		[PXHidden]
		[Serializable]
		public partial class ConvertToProjectFilter : PXBqlTable, IBqlTable
		{
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = PX.Objects.EP.Messages.Submit)]
		
		public IEnumerable Submit(PXAdapter adapter)
		{
			foreach (var item in adapter.Get<PMQuote>())
			{
				var cache = Caches[typeof(PMQuote)];
				cache.SetValue<PMQuote.submitCancelled>(item, false);

				if (Approval.GetAssignedMaps(item, QuoteCurrent.Cache).Any())
				{
					Approval.Assign(item, Approval.GetAssignedMaps(item, QuoteCurrent.Cache));
				}
				else
				{
					cache.SetValue<PMQuote.approved>(item, true);
				}
				...
				if (string.IsNullOrWhiteSpace(item.QuoteProjectCD) && DimensionMaint.IsAutonumbered(this, ProjectAttribute.DimensionName, false))
				{
		...
		public IEnumerable ConvertToProject(PXAdapter adapter)
		{
			...
			if (!ConvertQuoteInfo.AskExt().IsPositive())
				return list;
		...
		public virtual IEnumerable CopyQuote(PXAdapter adapter)
		{
			...
			foreach (PMQuote quote in CRQoutes)
			{
				...
				if (!CopyQuoteInfo.AskExt().IsPositive())
					return CRQoutes;
		...
		private void FillDefaultBAccountID(PXCache cache, PMQuote row)
		{
			...
			if (row.ContactID != null)
			{
				...
				if (contact != null)
				{
					...
					cache.SetValueExt<PMQuote.bAccountID>(row, contact.BAccountID);
				}
			}
		...
		public virtual void ConvertQuoteToProject(PMQuote row, ConvertToProjectFilter settings)
		{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				if (settings.MoveActivities == true)
				{
					...
					{
						// CRPMTimeActivity projection left joins CRActivity with PMTimeActivity
						// using PMTimeActivity.refNoteID (TimeActivityRefNoteID).
						// I.e. no TimeActivityRefNoteID means no PMTimeActivity.
						if (activity.TimeActivityRefNoteID != null)
						{
						...
						{
							// Create new PMTimeActivity to link CRActivity to the project.
		...
		public virtual void DefaultFromTemplate(PMQuote row)
		{
			...
			row.BranchID = GetBranchForQuoteAndTemplate(row, template);
		...
		{
			// See AC-290609 for the algorithm.

			var quoteBranchID = quote.BranchID;
			var opportunity = CR.CROpportunity.PK.Find(this, quote.OpportunityID);
			if (opportunity?.BranchID != null)
				return quoteBranchID;

			var templateBranchID = template?.DefaultBranchID;
			if (templateBranchID != null)
				return templateBranchID;

			if (quoteBranchID != null)
				return quoteBranchID;

			return PXAccess.GetBranchID();
		}
		...
		public virtual void AddingBillingInfoToProject(PMQuote row, ProjectEntry projectEntry)
		{
			...
			if (contact != null && billingContact != null)
				...
				billingContact.IsDefaultContact = isDefault;
				projectEntry.Billing_Contact.Update(billingContact);
				}

			CRAddress address = Quote_Address.SelectSingle();
					PMAddress billingAddress = projectEntry.Billing_Address.Select();

			if (address != null && billingAddress != null)
					{
				...
				billingAddress.IsDefaultAddress = isDefault;
				projectEntry.Billing_Address.Update(billingAddress);
			}
		}
		...
		public class SalesTax : TaxGraph<PMQuoteMaint, PMQuote>
		{
			...
			/// </summary>
			public class PerUnitTaxDisableExt : PX.Objects.Extensions.PerUnitTax.PerUnitTaxDataEntryGraphExtension<PMQuoteMaint, CRTaxTran>
			{
				public static bool IsActive() => IsActiveBase();

				protected override void _(Events.RowSelected<CRTaxTran> e)
				{
					if (e.Row == null)
						return;

					if (GetTax(e.Row)?.TaxType == CSTaxType.PerUnit)
