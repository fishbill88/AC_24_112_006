...
namespace PX.Objects.PM
{
	...
	/// </summary>
	[PXDBInt()]
	[PXUIField(DisplayName = PX.Objects.GL.Messages.ProjectContract, Visibility = PXUIVisibility.Invisible, Visible = false, Enabled = false)]
	...
	[PXUIField(DisplayName = "Project", Visibility = PXUIVisibility.Visible)]
	[PXRestrictor(typeof(Where<PMProject.baseType, NotEqual<CT.CTPRType.projectTemplate>,
		And<PMProject.baseType, NotEqual<CT.CTPRType.contractTemplate>>>), Messages.TemplateProject, typeof(PMProject.contractCD))]
	[PXRestrictor(typeof(Where<Contract.baseType, Equal<CT.CTPRType.project>, Or<Contract.nonProject, Equal<True>>>), PX.Objects.PM.Messages.NotAProject, typeof(PMProject.contractCD))]
	[PXRestrictor(typeof(Where<PMProject.isCancelled, Equal<False>>), Messages.CancelledProject, typeof(PMProject.contractCD))]
	[PXRestrictor(typeof(Where<PMProject.isCompleted, Equal<False>>), Messages.CompleteProject, typeof(PMProject.contractCD))]
	[PXRestrictor(typeof(Where<PMProject.isActive, Equal<True>, Or<PMProject.nonProject, Equal<True>>>), Messages.InactiveProject, typeof(PMProject.contractCD))]
	public class ActiveProjectBaseAttribute : PXEntityAttribute, IPXFieldVerifyingSubscriber
	{
		protected Type customerField;

		public ActiveProjectBaseAttribute() : this(null) { }
	...
	public class CAActiveProjectAttribute : ProjectBaseAttribute
	{
		...
		{
			PMProject project = PXSelect<PMProject>.Search<PMProject.contractID>(sender.Graph, e.NewValue);
			if (project != null && project.NonProject != true && project.BaseType == CTPRType.Project && AccountFieldType != null)
			{
	...
	public BaseProjectTaskAttribute(Type projectID, string Module): this(projectID)
		...
		public virtual void FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			if (!SuppressVerify && e.NewValue != null && e.NewValue is Int32)
			{
				...
				{
					VerifyTaskState(sender, projectId, e.NewValue, AllowCompleted, AllowCanceled, AllowInactive);
				}
			}
		}
		...
		{
			VerifyTaskState(sender, projectId, taskId, false, false, false);
		}

		public static void VerifyTaskState(PXCache sender, object projectId, object taskId, bool allowCompleted = false, bool allowCanceled = false, bool allowInactive = false)
		{
			if (projectId == null || taskId == null)
				return;

			PMTask task = PMTask.PK.Find(sender.Graph, (int?)projectId, (int?)taskId);

			if (task == null)
				return;

			bool taskIsNotAuthorized = (!allowCompleted && task.IsCompleted == true)
				|| (!allowCanceled && task.IsCancelled == true)
				|| (!allowInactive && task.IsActive != true);

			if (taskIsNotAuthorized)
			{
	...
	/// </summary>
	[Serializable]
	public class PMInventorySelectorAttribute :
			...
			var searchType = BqlTemplate.OfCommand<
				...
				Or<INUnit.unitType, Equal<INUnitType.global>,
					And<Optional<BqlPlaceholder.A>, IsNull,
						Or<Optional<BqlPlaceholder.A>, Equal<Zero>>>>>>>>
				.Replace<BqlPlaceholder.A>(InventoryType).ToType();

			PMUnitSelectorAttrubute attr = new PMUnitSelectorAttrubute(InventoryType, searchType);
			_Attributes.Add(attr);
			_SelAttrIndex = _Attributes.Count - 1;

			SupportNAInventory = true;
		}
	}
	...
	public class PMUnitSelectorAttrubute : PXCustomSelectorAttribute
	{
		...
		public PMUnitSelectorAttrubute(Type inventory, Type searchType) : base(searchType)
		{
	...
	public class PMBudgetAccumAttribute : PXAccumulatorAttribute
	{
		...
		protected override bool PrepareInsert(PXCache sender, object row, PXAccumulatorCollection columns)
		{
			...
			columns.Update<PMBudget.mode>(ProjectionMode.Auto, PXDataFieldAssign.AssignBehavior.Replace);
