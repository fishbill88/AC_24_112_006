...
namespace PX.Objects.PM
{
	...
	public class ProformaEntry : PXGraph<ProformaEntry, PMProforma>, IGraphWithInitialization
	{
		...
		public IEnumerable Release(PXAdapter adapter)
		{
			...
			var proforma = Document.Current;
			PXLongOperation.StartOperation(this, () =>
			{
				var graph = CreateInstance<ProformaEntry>();
				graph.Document.Current = proforma;
				graph.RecalculateExternalTaxesSync = true;
				graph.ReleaseDocument(proforma);
			});
		...
		public virtual IEnumerable Send(PXAdapter adapter)
		{
			...
			var isMassProcessing = adapter.MassProcess;
			PXLongOperation.StartOperation(this, () =>
			{
				var graph = CreateInstance<ProformaEntry>();
				graph.Document.Current = proforma;
				graph.SendReport(ProformaNotificationCD, proforma, isMassProcessing);
			});
		...
		{
			foreach (PMProforma current in adapter.Get<PMProforma>())
			{
				if (current != null)
				{
					...
					PMShippingAddress shippingAddress = this.Shipping_Address.Select();
					if (shippingAddress != null && shippingAddress.IsDefaultAddress == false && shippingAddress.IsValidated == false)
					{
						PXAddressValidator.Validate<PMShippingAddress>(this, shippingAddress, true, true);
					}
				}
		...
		protected virtual void _(Events.RowDeleted<PMProformaProgressLine> e)
		{
			...
			{
				SubtractValuesToInvoice(e.Row, -e.Row.CuryLineTotal, -e.Row.Qty); //Restoring AmountToInvoice and QtyToInvoice
			}
			
			if (!IsDocumentDeleted() && !RecalculatingContractRetainage)
			{
		...
		protected virtual void OnProformaLineDeleted(PMProformaLine line)
		{
			if (!IsDocumentDeleted())
			{
				...
				foreach (PMTran tran in selectReferencedTransactions.Select(line.RefNbr, line.LineNbr))
				{
					Unbill(tran);
				}
			}
		...
		protected virtual void _(Events.RowSelected<PMProforma> e)
		{
			...
			if (e.Row != null)
			{
				...
				uploadUnbilled.SetEnabled(e.Row.Hold == true && !isMigratedRow);
				...
				TransactionLines.Cache.AllowInsert = e.Row.Hold == true && !isMigratedRow;
				TransactionLines.Cache.AllowUpdate = e.Row.Hold == true;
				TransactionLines.Cache.AllowDelete = e.Row.Hold == true && !isMigratedRow;
				...
				if (e.Row.CustomerID != null)
				{
					...
					bool enableShippingAddressValidation = shippingAddress != null && shippingAddress.IsDefaultAddress == false && shippingAddress.IsValidated == false;
					validateAddresses.SetEnabled(enableAddressValidation || enableShippingAddressValidation);
				}
			}
		}
		...
		public virtual void ReleaseDocument(PMProforma doc)
		{
			...
			else
			{
				...
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					invoice.DueDate = invoice.DocType == ARDocType.CreditMemo && arSetup.Current.TermsInCreditMemos != true ? null : doc.DueDate;
					invoice.DiscDate = invoice.DocType == ARDocType.CreditMemo && arSetup.Current.TermsInCreditMemos != true ? null : doc.DiscDate;
					invoice.TermsID = invoice.DocType == ARDocType.CreditMemo && arSetup.Current.TermsInCreditMemos != true ? null : doc.TermsID;
					...
					{
						invoiceEntry.Document.Cache.SetValueExt<ARInvoice.finPeriodID>(invoice,
							PeriodIDAttribute.FormatForDisplay(doc.FinPeriodID));
					}
					...
					foreach (PXResult<PMProformaProgressLine, PMRevenueBudget> res in ProgressiveLines.View.SelectMultiBound(new[] { doc }))
					{
						...
						ProgressiveLines.Cache.SetValue<PMProformaTransactLine.aRInvoiceLineNbr>(line, artran.LineNbr);
						ProgressiveLines.Cache.MarkUpdated(line, assertError: true);
					...
					doc.ARInvoiceDocType = invoiceEntry.Document.Current.DocType;
					doc.ARInvoiceRefNbr = invoiceEntry.Document.Current.RefNbr;
					reversalDoc.OrigNoteID = invoiceEntry.Document.Current.NoteID;
					doc.Released = true;
					Document.Update(doc);
				...
				if (project.AutomaticReleaseAR == true && ARSetup.Current.HoldEntry != true)
				{
					try
					{
						ARDocumentRelease.ReleaseDoc(new List<ARRegister>(new[] { invoice }), false);
					}
					catch (Exception ex)
					{
		...
		protected virtual void ProcessReversingCreditMemo()
		{
			...
			if (last != null)
			{
				ARInvoice invoice = ARInvoice.PK.Find(this, last.ARInvoiceDocType, last.ARInvoiceRefNbr);
				ARInvoice creditMemo = ARInvoice.PK.Find(this, last.ReversedARInvoiceDocType, last.ReversedARInvoiceRefNbr);
								
				if (invoice != null && invoice.PaymentsByLinesAllowed == true && creditMemo != null && invoice.ApplicationBalance != 0m)
					CreatePayment(invoice, creditMemo);
			}
		}

		private void CreatePayment(ARInvoice invoice, ARInvoice creditMemo)
		{
			ARPaymentEntry pe = CreateInstance<ARPaymentEntry>();
			pe.CreatePayment(invoice, ARDocType.Payment);
			pe.Document.Current.CuryOrigDocAmt = 0;
			pe.Document.Current.Hold = false;
			pe.Document.Current.DontApprove = true;
			pe.Document.Current.AdjFinPeriodID = creditMemo.FinPeriodID;
			pe.Document.UpdateCurrent();
						
			pe.Document.Current.ExtRefNbr = "PFRev";

			var creditMemoLines = SelectFrom<ARTran>
				.Where<
					ARTran.tranType.IsEqual<P.AsString>
					.And<ARTran.refNbr.IsEqual<P.AsString>>>
				.View.Select(this, creditMemo.DocType, creditMemo.RefNbr);

			var lineAmounts = new Dictionary<int, decimal>();
			foreach (ARTran tran in creditMemoLines)
			{
				var adjust = new ARAdjust
				{
					AdjdDocType = creditMemo.DocType,
					AdjdRefNbr = creditMemo.RefNbr,
					AdjdLineNbr = tran.LineNbr
				};
				adjust = pe.Adjustments.Update(adjust);
				var lineNumber = adjust.AdjdLineNbr.GetValueOrDefault();
				if (lineNumber > 0)
				{
					lineAmounts[lineNumber] = tran.CuryOrigTranAmt.GetValueOrDefault();
				}
			}

			foreach (ARAdjust adjust in pe.Adjustments.Select())
			{
				if (lineAmounts.TryGetValue(adjust.AdjdLineNbr.GetValueOrDefault(), out var amount))
				{
					pe.Adjustments.Cache.SetValueExt<ARAdjust.curyAdjgPPDAmt>(adjust, 0m);
					pe.Adjustments.Cache.SetValueExt<ARAdjust.curyAdjgAmt>(adjust, amount);
				}
			}

			pe.Document.Current.CuryApplAmt = 0m;
			pe.Document.Current.CuryUnappliedBal = 0m;

			pe.Actions.PressSave();
			pe.release.Press();
		}
		...
		protected virtual ARInvoice ProcessRevision()
		{
			...
			invoiceEntry.Document.Current.DontApprove = true;
			invoiceEntry.Document.Current.DontPrint = true;
			invoiceEntry.Document.Current.DontEmail = true;
			invoiceEntry.Document.Current.Hold = false;
			invoiceEntry.Document.Current.ProformaExists = true;
			invoiceEntry.Document.UpdateCurrent();

			invoiceEntry.Save.Press();
			invoiceEntry.ReleaseProcess(new List<ARRegister>(new[] { invoiceEntry.Document.Current }));
		...
		public virtual Dictionary<ProformaTotalsCounter.AmountBaseKey, decimal> GetBilledRetainageToDate(DateTime? cutoffDate)
		{
			...
			/*
			Before the feature of consolidated invoices was introduced, invoices resulting from pro formas
			may have pay by line off and retainage invoices from them also had pay by line off (they only have one line).
			Such case is not included in selectBilledRetainageByLine, but is in selectBilledRetainage.
			However, for consolidated invoice we have on more case.
			If we have 2 invoices with pay by line off and release retainage for both of them
			it results in one retainage invoice with pay by line on, but without origLineNbr (=0)
			because original invoices were without pay by line.
			Because of this combination it is excluded from both selectBilledRetainageByLine
			and selectBilledRetainage. So one more select is added: selectBilledRetainageZeroLine.
			*/

			var selectBilledRetainageZeroLine = SelectFrom<ARTran>
				.InnerJoin<ARInvoice>
					.On<ARInvoice.docType.IsEqual<ARTran.tranType>
					.And<ARInvoice.refNbr.IsEqual<ARTran.refNbr>>>
				.Where<
					ARInvoice.isRetainageDocument.IsEqual<True>
					.And<ARInvoice.released.IsEqual<True>>
					.And<ARInvoice.paymentsByLinesAllowed.IsEqual<True>>
					// origLineNbr is IBqlField, not BqlInt.Field<>, so no FBQL here
					.And<Where<ARTran.origLineNbr, Equal<Zero>>>
					.And<ARInvoice.docDate.IsLessEqual<P.AsDateTime>>
					.And<ARInvoice.projectID.IsEqual<PMProforma.projectID.FromCurrent>>>
				.AggregateTo<
				...
				.View.Select(this, cutoffDate);

			foreach (ARTran arTran in selectBilledRetainageZeroLine)
			{
				var sign = ARDocType.SignAmount(arTran.TranType).GetValueOrDefault(1);
				total = total.GetValueOrDefault() + sign * arTran.CuryTranAmt.GetValueOrDefault();
			}
		...
		{
			ClearProformaReference(tran);

			if (tran.Billed == true) //Applicable only to Transactional type.
			{
				tran.Billed = false;
		...
		public class PMProformaOverflow : PXBqlTable, PX.Data.IBqlTable
		{			
			#region CuryInfoID
