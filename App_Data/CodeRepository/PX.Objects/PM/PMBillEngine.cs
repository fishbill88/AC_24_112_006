...
namespace PX.Objects.PM
{
	public class PMBillEngine : PXGraph<PMBillEngine>, IRateTable
	{
		...
		public PMBillEngine()
		{
			...
			revenueBudgetLines = new Dictionary<int, List<PXResult<PMRevenueBudget, PMAccountGroup>>>();
		...
		public virtual BillingResult Bill(int? projectID, DateTime? invoiceDate, string finPeriod)
		{
			...
			PreSelectRevenueBudgetLines(projectID);
			...
			if (invoices.Count > 0)
			{
				...
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					foreach (KeyValuePair<string, Tuple<int?, List<BillingData>>> kv in invoices)
					{
						...
						if (project.CreateProforma == true)
						{
							PMProject copy = this.Project.Cache.CreateCopy(project) as PMProject;
							ProformaEntry.autoApplyPrepayments.Press();
							ProformaEntry.Save.Press();
							project = ProformaEntry.Project.Current;
			...
			DateTime billingDate, string docDesc, int? locationID)
		{
			...
			if (docType == ARDocType.CreditMemo && arSetup.Current.TermsInCreditMemos != true)
				InvoiceEntry.Document.SetValueExt<ARInvoice.termsID>(invoice, null);
		...
		public virtual List<BillingData> BillFixPriceTask(PMProject project, Customer customer, PMTask task, PMBillingRule rule, DateTime billingDate)
		{
			...
			List<BillingData> list = new List<BillingData>();
			if (task.TaskID == null || !revenueBudgetLines.ContainsKey((int)task.TaskID))
				return list;

			var budgetLines = revenueBudgetLines[(int)task.TaskID];
			...
			foreach (var res in budgetLines)
			{
		...
		public virtual List<BillingData> BillPrepayment(PMProject project, Customer customer, PMTask task, PMBillingRule rule, DateTime billingDate)
		{
			...
			var budgetLines = revenueBudgetLines[(int)task.TaskID]
				.Where(x => ((PMRevenueBudget)x).CuryPrepaymentAmount > ((PMRevenueBudget)x).CuryPrepaymentInvoiced);
		...
		{
			revenueBudgetLines.Clear();

			var query = new SelectFrom<PMRevenueBudget>
				.InnerJoin<PMAccountGroup>.On<PMRevenueBudget.accountGroupID.IsEqual<PMAccountGroup.groupID>>
				.Where<PMRevenueBudget.projectID.IsEqual<P.AsInt>
				.And<PMRevenueBudget.type.IsEqual<AccountType.income>>>
				.View(this);

			if (CostCodeAttribute.UseCostCode())
			{
				query.OrderByNew<OrderBy<Asc<PMRevenueBudget.inventoryID, Asc<PMRevenueBudget.costCodeID>>>>();
			}

			revenueBudgetLines = query.Select(projectID)
				.Select(x => (PXResult<PMRevenueBudget, PMAccountGroup>)x)
				.Where(x => x.GetItem<PMRevenueBudget>() != null)
				.AsEnumerable()
				.GroupBy(x => x.GetItem<PMRevenueBudget>().TaskID)
		...
		public virtual PXResultset<PMTran> GetTranFromDatabase(int? projectID, int groupID, DateTime? cuttofDate)
		{
			...
			if (CostCodeAttribute.UseCostCode())
			{
				selectTrans.OrderByNew<OrderBy<Asc<PMTran.costCodeID>>>();
			}
		...
		public virtual PMTran ReverseTranGL(PMTran tran)
		{
			...
			if (tran.IsNonGL == true)
			{
				reversal.AccountID = null;
				reversal.SubID = null;
				reversal.OffsetAccountID = null;
				reversal.OffsetSubID = null;
			}

			if (tran.MigrationOffsetAccountGroupID != null &&
				reversal.OffsetAccountGroupID == null)
			{
				reversal.OffsetAccountGroupID = tran.MigrationOffsetAccountGroupID;
				reversal.MigrationOffsetAccountGroupID = null;
			}
