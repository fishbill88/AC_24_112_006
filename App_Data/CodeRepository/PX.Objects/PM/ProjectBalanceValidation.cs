...
namespace PX.Objects.PM
{
	...
	public class ProjectBalanceValidationProcess : PXGraph<ProjectBalanceValidationProcess>
	{
		...
		{
			//OBSOLETE - there is no dependency on the accounts anymore.
		}
		...
		public virtual void RecalculateBalance(PMProject project, PMValidationFilter options)
		{
				...
				, typeof(PMTran.origTranType)))
			{
				foreach (PMTran tran in select.Select(project.ContractID))
				{
					...
					AccountGroups.TryGetValue(tran.AccountGroupID.Value, out accountGroup);

					if (tran.OffsetAccountGroupID != null)
					{
						AccountGroups.TryGetValue(tran.OffsetAccountGroupID.Value, out offsetAccountGroup);
					}

					if (accountGroup == null)
						accountGroup = new PMAccountGroup();

					if (offsetAccountGroup == null)
					...
					try
					{
						ProcessTransaction(project, tran, accountGroup, offsetAccountGroup, pb);
					}
		...
		public virtual void ProcessTransaction(PMProject project, PMTran tran, PMAccountGroup ag, PMAccountGroup offsetAg, ProjectBalance pb)
		{
			IList<ProjectBalance.Result> balances = pb.Calculate(project, tran, ag, offsetAg);
		...
		public virtual void RebuildAllocationTotals(PMProject project)
		{
			PXSelectBase<PMTran> select = new PXSelectJoinGroupBy<PMTran,
				LeftJoin<PMTranAllocReversal, On<PMTranAllocReversal.origTranID, Equal<PMTran.tranID>>>,
				...
				And<Where<PMTranAllocReversal.tranID, IsNull, Or<PMTranAllocReversal.origDocType, Equal<PMOrigDocType.allocationReversal>>>>>>>,
				Aggregate<GroupBy<PMTran.origProjectID,
					GroupBy<PMTran.origTaskID,
					GroupBy<PMTran.origAccountGroupID,
					GroupBy<PMTran.inventoryID,
					GroupBy<PMTran.costCodeID,
					Sum<PMTran.projectCuryAmount,
					Sum<PMTran.qty>>>>>>>>>(this);
			...
			foreach (PMTran tran in select.Select(project.ContractID))
			{
				...
				tat.Amount += tran.ProjectCuryAmount.GetValueOrDefault();
		...
		public virtual void RecalculateDraftInvoicesAmount(PMProject project, ProjectBalance pb)
		{
			...
			foreach (PXResult<PMProformaLine, Account, PMProforma, GL.Branch> res in selectProforma.Select(project.ContractID))
			{
				...
				// Migrated pro formas should not contain t&m lines.
				// For details see AC-291831.
				if (doc.IsMigratedRecord == true && line.Type == PMProformaLineType.Transaction)
					continue;
		...
		[PXHidden]
		[PXProjection(typeof(Select2<PMTran,
			InnerJoin<PMRegister, On<PMRegister.module, Equal<PMTran.tranType>,
			And<PMRegister.refNbr, Equal<PMTran.refNbr>>>>>))]
		public class PMTranAllocReversal : PXBqlTable, IBqlTable
		{
			#region TranID
	...
	[Serializable]
	[System.Diagnostics.CodeAnalysis.ExcludeFromCodeCoverage]
	public partial class PMValidationFilter : PXBqlTable, IBqlTable
	{
