...
namespace PX.Objects.SO
{
	...
	public partial class SOOrderEntry : PXGraph<SOOrderEntry, SOOrder>, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess, IGraphWithInitialization
	{
		...
		[PXCopyPasteHiddenFields(typeof(SOLine.completed), typeof(SOLine.isLegacyDropShip), typeof(SOLine.curyUnbilledAmt), typeof(SOLine.pOLinkActive))]
		...
		public PXSelectJoin<SOAdjust,
					LeftJoin<ARPayment, On<ARPayment.docType, Equal<SOAdjust.adjgDocType>, And<ARPayment.refNbr, Equal<SOAdjust.adjgRefNbr>>>,
				LeftJoin<ExternalTransaction, On<ExternalTransaction.transactionID, Equal<ARPayment.cCActualExternalTransactionID>>>>>
			Adjustments;

		public PXSelectJoin<SOAdjust,
					LeftJoin<ARRegisterAlias, On<ARRegisterAlias.docType, Equal<SOAdjust.adjgDocType>, And<ARRegisterAlias.refNbr, Equal<SOAdjust.adjgRefNbr>>>,
					LeftJoinSingleTable<ARPayment, On<ARRegisterAlias.docType, Equal<ARPayment.docType>, And<ARRegisterAlias.refNbr, Equal<ARPayment.refNbr>>>,
							LeftJoin<ExternalTransaction, On<ExternalTransaction.transactionID, Equal<ARPayment.cCActualExternalTransactionID>>>>>,
		...
		public PXSelect<SOOwner, Where<SOOwner.defContactID, Equal<Current<SOOrder.ownerID>>>> Employee;
		...
		{
			decimal CuryDocBal = GetPaymentBalance(inv_info, payment, adj);
		...
		public virtual decimal GetPaymentBalance(CurrencyInfo inv_info, ARPayment payment, SOAdjust adj)
		{
		...
		{
			SOAdjust other =
				PXSelectGroupBy<SOAdjust,
				Where<
					SOAdjust.adjgDocType, Equal<Required<SOAdjust.adjgDocType>>,
					And<SOAdjust.adjgRefNbr, Equal<Required<SOAdjust.adjgRefNbr>>,
					And<Where<SOAdjust.adjdOrderType, NotEqual<Required<SOAdjust.adjdOrderType>>,
						Or<SOAdjust.adjdOrderNbr, NotEqual<Required<SOAdjust.adjdOrderNbr>>>>>>>,
				Aggregate<
					GroupBy<SOAdjust.adjgDocType,
					GroupBy<SOAdjust.adjgRefNbr,
					Sum<SOAdjust.curyAdjgAmt,
					Sum<SOAdjust.adjAmt>>>>>>.Select(this, adj.AdjgDocType, adj.AdjgRefNbr, adj.AdjdOrderType, adj.AdjdOrderNbr);
			...
			ARAdjust fromar =
				...
				Aggregate<
					...
					Sum<ARAdjust.adjAmt>>>>>>.Select(this, adj.AdjgDocType, adj.AdjgRefNbr);
		...
		[PXUIField(DisplayName = Messages.ViewOrigOrder, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select, Visible = false)]
		[PXLookupButton]
		public virtual IEnumerable ViewOrigOrder(PXAdapter adapter)
		{
			PXRedirectHelper.TryRedirect(
				Document.Cache,
				SOOrder.PK.Find(this, Transactions.Current?.OrigOrderType, Transactions.Current?.OrigOrderNbr),
				Messages.ViewOrigOrder,
				PXRedirectHelper.WindowMode.NewWindow);
		...
		[DenormalizedFrom(
			new[] { typeof(ARPayment.isCCPayment), typeof(ARPayment.released), typeof(ARPayment.isCCAuthorized), typeof(ARPayment.isCCCaptured), typeof(ARPayment.voided),
					...
					typeof(ARPayment.pendingPayment)},
			new[] { typeof(SOAdjust.isCCPayment), typeof(SOAdjust.paymentReleased), typeof(SOAdjust.isCCAuthorized), typeof(SOAdjust.isCCCaptured), typeof(SOAdjust.voided),
					...
					typeof(SOAdjust.pendingPayment)},
		...
		protected virtual void FillSOAdjustByPayment(SOAdjust adj)
		{
			...
			PXSelectBase<ARPayment> s = new PXSelectReadonly2<ARPayment,
				...
				Where<ARPayment.customerID, In3<Current<SOOrder.customerID>, Current<Customer.consolidatingBAccountID>>,
					And<ARPayment.docType, In3<ARDocType.payment, ARDocType.prepayment, ARDocType.creditMemo, ARDocType.refund, ARDocType.prepaymentInvoice>,
			...
			foreach (PXResult<ARPayment, CurrencyInfo> res in s.Select(adj.AdjgDocType, adj.AdjgRefNbr))
			{
				...
				InitializeAdjustFields(adj, payment, pay_info);
			}
		}
		
		protected virtual void InitializeAdjustFields(SOAdjust adjustToFill, ARPayment paymentToBeAdapted, CurrencyInfo paymentCurrencyInfo)
		{
			adjustToFill.CustomerID = Document.Current.CustomerID;
			adjustToFill.AdjdOrderType = Document.Current.OrderType;
			adjustToFill.AdjdOrderNbr = Document.Current.OrderNbr;
			adjustToFill.AdjgDocType = paymentToBeAdapted.DocType;
			adjustToFill.AdjgRefNbr = paymentToBeAdapted.RefNbr;
			CalculatePaymentBalance(paymentToBeAdapted, adjustToFill);

			if (Adjustments.Cache.Locate(adjustToFill) == null)
			{
				adjustToFill.AdjgCuryInfoID = paymentToBeAdapted.CuryInfoID;
				adjustToFill.AdjdOrigCuryInfoID = Document.Current.CuryInfoID;
				//if LE constraint is removed from payment selection this must be reconsidered
				adjustToFill.AdjdCuryInfoID = Document.Current.CuryInfoID;
				adjustToFill.CuryDocBal = GetConvertedDocumentBalanceFor(paymentToBeAdapted, paymentCurrencyInfo, Adjustments.Cache);
			}
		}
		...
		protected virtual void SOAdjust_CuryAdjdAmt_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			bool recalcBalance = IsBalanceRecalculationRequired(adj);
			if (recalcBalance && !TryRecalculateBalanceAndCuryInfo(adj, sender))
				return;
			...
			decimal balance = (decimal)adj.CuryDocBal + (decimal)adj.CuryAdjdAmt;
			decimal newBalance = NewBalanceCalculation(adj, (decimal)e.NewValue);
			if (recalcBalance && adj.CuryDocBal < 0m)
			{
			...
			if (newBalance < 0)
			{
				throw new PXSetPropertyException(AR.Messages.Entry_LE, balance.ToString());
			}
		}

		protected virtual bool IsBalanceRecalculationRequired(SOAdjust adj)
		{
			return adj.AdjgCuryInfoID == null || adj.CuryDocBal == null;
		}

		protected virtual bool TryRecalculateBalanceAndCuryInfo(SOAdjust adj, PXCache sender)
		{
			PXResult<ARPayment, CurrencyInfo> res = GetRelatedPaymentAndCurrencyInfoFor(adj);
			...
			decimal curyDocBal = GetConvertedDocumentBalanceFor(payment, res, sender);
			adj.CuryDocBal = curyDocBal - adj.CuryAdjdAmt;
		...
		protected virtual PXResult<ARPayment, CurrencyInfo> GetRelatedPaymentAndCurrencyInfoFor(SOAdjust adj)
		{
			return (PXResult<ARPayment, CurrencyInfo>)
				PXSelectJoin<
					ARPayment,
					InnerJoin<CurrencyInfo, On<CurrencyInfo.curyInfoID, Equal<ARPayment.curyInfoID>>>,
					Where<
		...
		protected virtual decimal GetConvertedDocumentBalanceFor(ARPayment payment, CurrencyInfo paymentCuryInfo, PXCache sender)
		{
			CurrencyInfo inv_info = PXSelect<CurrencyInfo, Where<CurrencyInfo.curyInfoID, Equal<Current<SOOrder.curyInfoID>>>>.Select(this);
			decimal result;
			if (string.Equals(paymentCuryInfo.CuryID, inv_info.CuryID))
			{
				result = (decimal)payment.CuryDocBal;
			}
			else
			{
				decimal docBal = ((payment.Released == true) ? payment.DocBal : payment.OrigDocAmt) ?? 0m;

				PXDBCurrencyAttribute.CuryConvCury(sender, inv_info, docBal, out result);
			}
			return result;
		}
		...
		{
			decimal newBalance = (decimal)adj.CuryDocBal + (decimal)adj.CuryAdjdAmt - newValue;
			return newBalance;
		}
		...
		protected virtual void SOAdjust_CuryAdjdAmt_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (adj.AdjgRefNbr == null && sender.Graph.IsContractBasedAPI)
				return;

			decimal CuryAdjgAmt, AdjdAmt, AdjgAmt;
			...
			UpdateBalanceOnCuryAdjdAmtUpdated(sender, e);
		}

		protected virtual void UpdateBalanceOnCuryAdjdAmtUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			SOAdjust adj = (SOAdjust)e.Row;
		...
		protected virtual void SOOrderShipment_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (link?.InvoiceNbr != null)
			{
				PXException warning = ARInvoice.PK.Find(this, link.InvoiceType, link.InvoiceNbr)?.Rejected == true
					? new PXSetPropertyException(Messages.OrderHasRejectedInvoice, PXErrorLevel.Warning, link.InvoiceNbr)
					: null;
				sender.RaiseExceptionHandling<SOOrderShipment.invoiceNbr>(link, link.InvoiceNbr, warning);
			}
		}
		#endregion
		...
		protected virtual void SOOrder_CustomerID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
...

			{
				...
				{
					bool isCreditMemo = soordertype.Current.ARDocType == ARDocType.CreditMemo;
					if (!isCreditMemo || (isCreditMemo && arsetup.Current.TermsInCreditMemos == true))
					{
		...
		protected virtual void SOOrder_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			bool isInvoiceOrderBillSeparately = doc.IsInvoiceOrder == true && doc.BillSeparately == true;
			PXDefaultAttribute.SetPersistingCheck<SOOrder.invoiceDate>(sender, doc, isInvoiceOrderBillSeparately ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
			PXDefaultAttribute.SetPersistingCheck<SOOrder.finPeriodID>(sender, doc, isInvoiceOrderBillSeparately ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
			PXDefaultAttribute.SetPersistingCheck<SOOrder.invoiceNbr>(sender, doc, (isInvoiceOrderBillSeparately  && doc.IsUserInvoiceNumbering == true) ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
			...
			PXDefaultAttribute.SetPersistingCheck<SOOrder.dueDate>(sender, doc, (isInvoiceOrderBillSeparately
							&& doc.ARDocType != ARDocType.CreditMemo) ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
			PXDefaultAttribute.SetPersistingCheck<SOOrder.discDate>(sender, doc, (isInvoiceOrderBillSeparately
							&& doc.ARDocType != ARDocType.CreditMemo) ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
			PXDefaultAttribute.SetPersistingCheck<SOOrder.paymentMethodID>(sender, doc, (isInvoiceOrderBillSeparately && (doc.ARDocType == ARDocType.CashSale
							|| doc.ARDocType == ARDocType.CashReturn)) ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
			PXDefaultAttribute.SetPersistingCheck<SOOrder.cashAccountID>(sender, doc, (isInvoiceOrderBillSeparately && (doc.ARDocType == ARDocType.CashSale
			...
			PXDefaultAttribute.SetPersistingCheck<SOOrder.extRefNbr>(sender, doc, (isInvoiceOrderBillSeparately && (doc.ARDocType == ARDocType.CashSale
		...
		protected virtual void SOOrder_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			if (!object.ReferenceEquals(doc, _LastSelected))
			{
				...
				PXUIFieldAttribute.SetEnabled<SOLine.autoCreateIssueLine>(this.Transactions.Cache, null, soordertype.Current.ActiveOperationsCntr > 1);
			...
			else
			{
				...
				if (soordertype.Current != null)
				{
					...
					PXUIFieldAttribute.SetEnabled<SOOrder.termsID>(cache, doc, !(doc.ARDocType == ARDocType.CreditMemo && arsetup.Current.TermsInCreditMemos != true));
					...
					bool isARCreditMemo = soordertype.Current.ARDocType == ARDocType.CreditMemo;

					PXUIFieldAttribute.SetEnabled<SOOrder.dueDate>(cache, doc, isInvoiceInfoEnabled && (!isARCreditMemo || (isARCreditMemo && doc.TermsID != null)));
					PXUIFieldAttribute.SetRequired<SOOrder.dueDate>(cache, doc.BillSeparately == true
						&& (soordertype.Current.ARDocType == ARDocType.CashSale || soordertype.Current.ARDocType == ARDocType.CashReturn || soordertype.Current.ARDocType == ARDocType.Invoice || (isARCreditMemo && doc.TermsID != null)));
					PXUIFieldAttribute.SetEnabled<SOOrder.discDate>(cache, doc, isInvoiceInfoEnabled && (!isARCreditMemo || (isARCreditMemo && doc.TermsID != null)));
			...
			Transactions.Cache.Adjust<PXUIFieldAttribute>()
				.For<SOLine.origOrderType>(a => a.Visible = doc.Behavior.IsIn(SOBehavior.RM, SOBehavior.CM, SOBehavior.MO, SOBehavior.IN))
				.SameFor<SOLine.origOrderNbr>();
		...
		protected virtual void RQRequisitionOrder_RowDeleted(PXCache sender, PXRowDeletedEventArgs e)
		{
			...
			{
				var req = PXParentAttribute.SelectParent<RQ.RQRequisition>(sender, reqord);
				req.Quoted = false;
		...
		protected virtual void SOOrder_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			bool customerHasChanged = !sender.ObjectsEqual<SOOrder.customerID>(oldRow, row),
				 projectHasChanged = !sender.ObjectsEqual<SOOrder.projectID>(oldRow, row);
			if (customerHasChanged || projectHasChanged)
			{
				foreach (SOLine line in Transactions.Select())
				{
					if (customerHasChanged)
					{
						...
						Transactions.Cache.SetDefaultExt<SOLine.customerLocationID>(line);
					}
					line.ProjectID = Document.Current.ProjectID;
					Transactions.Update(line);
				}
			}
		...
		protected virtual void _(Events.ExceptionHandling<SOAdjust.adjgCuryInfoID> e)
		{
			e.Cancel = true;
		}
		...
		protected virtual void SOLine_CuryUnitPrice_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if (e.NewValue != null)
			{
				using (var calculationScope = CheckSourceChange())
				{
		...
		protected virtual void SOLine_CuryUnitPrice_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			else if (row.InventoryID != null && (row.ManualPrice != true || sender.Graph.IsCopyPasteContext) && row.IsFree != true
				&& sender.Graph.IsCopyPasteContext.Implies(() => ((PXFieldState)sender.GetStateExt<SOLine.curyUnitPrice>(e.Row)).Enabled == false)
				&& isPriceUpdateNeeded)
			{
		...
		protected virtual void SOLine_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetEnabled<SOLine.autoCreateIssueLine>(sender, e.Row,
				row.Operation != SOOperation.Issue && soordertype.Current.ActiveOperationsCntr > 1);
		...
		protected virtual bool CheckPOLinked(SOLineSplit row, PXCache cache)
		{
			...
			{
				SOLine soline = PXParentAttribute.SelectParent<SOLine>(cache, row);
		...
		protected virtual decimal? GetINTranUnitCost(SOLine line, SOLineSplit split, bool isReturnSpecific)
		{
			if (line.Operation == SOOperation.Receipt)
			{
				...
				INTran invoiceLine = SelectFrom<INTran>.
							Where<INTran.sOOrderType.IsEqual<SOLine.origOrderType.FromCurrent>.
							And<INTran.sOOrderNbr.IsEqual<SOLine.origOrderNbr.FromCurrent>>.
							And<INTran.sOOrderLineNbr.IsEqual<SOLine.origLineNbr.FromCurrent>>.
							And<INTran.inventoryID.IsEqual<SOLineSplit.inventoryID.FromCurrent>>>
					.View.SelectSingleBound(this, new object[] { line, split });
				if (invoiceLine != null)
				{
			...
			foreach (SOOrder doc in orders.Where(doc => doc.DeferPriceDiscountRecalculation == true && doc.IsPriceAndDiscountsValid == false))
			{
				...
				try
				{
					_discountEngine.AutoRecalculatePricesAndDiscounts(Transactions.Cache, Transactions, null, DiscountDetails,
						Document.Current.CustomerLocationID, Document.Current.OrderDate, GetDefaultSODiscountCalculationOptions(doc, true));
		...
		public virtual void ImportDone(PXImportAttribute.ImportMode.Value mode)
		{
			...
			if (soOrder != null)
			{
				...
				_discountEngine.AutoRecalculatePricesAndDiscounts(
					...
					locationID: Document.Current.CustomerLocationID, 
					date: Document.Current.OrderDate, 
						discountCalculationOptions: GetDefaultSODiscountCalculationOptions(Document.Current, true));
				}
				finally
				{
					Document.Current.DeferPriceDiscountRecalculation = soordertype.Current.DeferPriceDiscountRecalculation;
		...
		public static void ProcessPOReceipt(PXGraph graph, IEnumerable<PXResult<INItemPlan, INPlanType>> list, string POReceiptType, string POReceiptNbr)
		{
			...
			foreach (PXResult<INItemPlan, INPlanType> res in list)
			{
				...
				if (schedule != null && (schedule.Completed == false || solinesplit.Cache.GetStatus(schedule) == PXEntryStatus.Updated))
				{
					...
					{
						soorder.Current = PXParentAttribute.SelectParent<SOOrder>(solinesplit.Cache, schedule);
						schedule = PXCache<SOLineSplit>.CreateCopy(schedule);
			...
			foreach (SOLineSplit newsplit in insertedSchedules)
			{
				...
				{
					soorder.Current = PXParentAttribute.SelectParent<SOOrder>(solinesplit.Cache, parentschedule);
		...
		private static void UpdateSchedulesFromCompletedPO(PXGraph graph, PXSelect<SOLineSplit> solinesplit, PXSelect<INItemPlan> initemplan, SOLineSplit parentschedule, PXSelect<SOOrder> soorder, INItemPlan demand, bool cancelDropShip = false)
		{
			...
			SOLine line = PXParentAttribute.SelectParent<SOLine>(solinesplit.Cache, parentschedule);
			...
			Dictionary<int?, (SOLine, decimal?, decimal?)> lineOpenQuantities)
		{
			...
			if (line.Completed == true && line.ShippedQty == 0m || line.OpenLine == false && shipLine.ShippedQty > 0m)
			{
				...
				if (line.OrderQty != 0m && (line.IsFree == false || line.ManualDisc == true))
				{
					Document.Current.OpenLineCntr++;
				}
			}
			...
			line = Transactions.Update(line);
			line.OpenLine = (bool?)PXFormulaAttribute.Evaluate<SOLine.openLine>(Transactions.Cache, line);
	...
	[Serializable]
	public partial class SOParamFilter : PXBqlTable, IBqlTable
	{
		#region ShipDate
	...
	[Serializable()]
	public partial class CopyParamFilter : PXBqlTable, IBqlTable
	{
		#region OrderType
