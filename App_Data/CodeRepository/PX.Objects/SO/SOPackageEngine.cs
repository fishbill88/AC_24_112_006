		...
		public virtual IList<PackSet> Pack(OrderInfo orderInfo)
		{
			...
			foreach (ItemStats stat in orderInfo.Stats)
			{
				...
				if (stat.PackOption == INPackageOption.Quantity)
				{
					...
					{
						InventoryItem item = InventoryItem.PK.Find(graph, stat.Lines.FirstOrDefault().InventoryID);
						throw new PXException(Messages.IncorrectConfigPackageOptionAndPackSeparately, item.InventoryCD);
					}
