...
namespace PX.Objects.SO.WMS
{
	...
	public class PaperlessPicking : WorksheetPicking.ScanExtension
	{
		...
		[Obsolete("Use the " + nameof(GetSplitsForRemoval) + " method instead.")]
		public virtual SOPickerListEntry GetSplitForRemoval() => GetSplitsForRemoval().FirstOrDefault();
		public virtual IEnumerable<SOPickerListEntry> GetSplitsForRemoval()
		{
			if (Basis.Remove != true || Basis.InventoryID == null)
				return Array.Empty<SOPickerListEntry>();

			return WSBasis.PickListOfPicker.SelectMain().Where(ent => ent.PickedQty > 0 && ent.ForceCompleted != true && WSBasis.IsSelectedSplit(ent));
		}

		public virtual bool NeedInversedDirection(SOPicker picker, int nearestLocationID)
		{
			if (Basis.Setup.Current.AllowBidirectionalPickLists != true)
				return false;
		...
		[Obsolete("Use the " + nameof(GetWantedSplitsForIncrease) + " method instead.")]
		public virtual SOPickerListEntry GetWantedSplitForIncrease() => GetWantedSplitsForIncrease().FirstOrDefault();
		public virtual IEnumerable<SOPickerListEntry> GetWantedSplitsForIncrease()
		{
			var wantedSplit = GetWantedSplit();
			if (wantedSplit is null)
				return null;

			IEnumerable<SOPickerListEntry> splitsSimilarToWanted =
				WSBasis.PickListOfPicker
				.SelectMain()
				.Where(r =>
					WSBasis.AreSplitsSimilar(r, wantedSplit) &&
					r.ForceCompleted != true)
				.OrderByAccordanceTo(s => s.EntryNbr == wantedSplit.EntryNbr);

			if (Basis.Get<ToteSupport>() is ToteSupport toteBasis && toteBasis.ToteID.IsNotIn(null, wantedSplit.ToteID))
				splitsSimilarToWanted = splitsSimilarToWanted.Where(r => r.ToteID == toteBasis.ToteID);

			return splitsSimilarToWanted;
		}
		...
		public virtual string PromptWantedItem()
		{
			...
			if (wantedSplit != null)
			{
				var similarSplits = GetWantedSplitsForIncrease();
		...
		public sealed class ConfirmPickListAndTakeNextCommand : PickPackShip.ScanCommand
		{
			...
			public class AlterConfirmPickListCommandLogic : WorksheetPicking.ScanExtension<WorksheetPicking.ConfirmPickListCommand.Logic>
			{
					...
					Action<ScanLongRunAwaiter<PickPackShip, SOPicker>.IResultProcessor> base_ConfigureOnSuccessAction)
				{
					...
					if (Basis.CurrentMode.Commands.OfType<ConfirmPickListAndTakeNextCommand>().FirstOrDefault()?._inProcess == true)
		...
		public virtual void InjectNavigationOnLocation(WMSBase.LocationState locState)
		{
			locState
				.Intercept.StateInstructions.ByAppend(basis =>
					basis.Get<WorksheetPicking>().GetEntriesToPick().FirstOrDefault() is SOPickerListEntry split
				...
				.Intercept.Validate.ByAppend((basis, location) =>
					location.LocationID == basis.Get<WorksheetPicking>().GetEntriesToPick().FirstOrDefault()?.LocationID
						? Validation.Ok
		...
		public virtual void InjectNavigationOnLotSerial(WMSBase.LotSerialState lsState)
		{
			lsState
				...
				.Intercept.Validate.ByAppend((basis, lotSerialNbr) =>
					basis.Remove == true || basis.LotSerialTrack.IsEnterable || string.Equals(lotSerialNbr, basis.Get<PaperlessPicking>().GetWantedSplit().LotSerialNbr, StringComparison.OrdinalIgnoreCase)
						? Validation.Ok
		...
		[PXOverride]
		public IEnumerable<PXResult<SOPickerListEntry, INLocation>> GetListEntries(string worksheetNbr, int? pickerNbr,
		...
		{
			if (WSBasis.PickingJob.Current is null)
			...
			Action<SOShipmentEntry.WorkLog, bool> base_UpdateWorkLogOnLogScan)
		{
			...
			if (Basis.CurrentMode is SinglePickMode && !string.IsNullOrEmpty(SingleShipmentNbr))
			{
		...
		public abstract class ScanExtension : PXGraphExtension<PaperlessPicking, WorksheetPicking, PickPackShip, PickPackShip.Host>
		{
			...
			public PaperlessPicking PPBasis { [DebuggerStepThrough, DebuggerStepperBoundary] get => Base3; }
		}
