...
namespace PX.Objects.SO
{
	...
	public partial class SOAdjust : PXBqlTable, PX.Data.IBqlTable, IAdjustment, ICreatePaymentAdjust
	{
		#region Keys
		...
		[PXParent(typeof(Select<ARInvoice, Where<ARInvoice.docType, Equal<Current<SOAdjust.adjgDocType>>, And<ARInvoice.refNbr, Equal<Current<SOAdjust.adjgRefNbr>>>>>))]
		...
		[PXUnboundFormula(
			typeof(Switch<Case<Where<voided, Equal<False>, And<isCCAuthorized, Equal<False>,
									And<Where<paymentReleased, Equal<False>, Or<pendingPayment, Equal<True>>>>>>,
		...
		[PXUnboundFormula(
			typeof(Switch<Case<Where<voided, Equal<False>, And<paymentReleased, Equal<True>,
									And<pendingPayment, Equal<False>>>>,
				curyAdjdAmt>, decimal0>),
		...
		[PXUnboundFormula(
			typeof(IIf<Where<voided, Equal<False>,
				And2<Where<adjgDocType, NotEqual<ARDocType.prepaymentInvoice>,
		...
		//[PXUnboundFormula(typeof(Switch<Case<Where<SOAdjust.curyAdjdAmt, Greater<decimal0>>, int1>, int0>), typeof(SumCalc<SOOrder.paymentCntr>))]
		[PXUIField(DisplayName = "Blanket SO Ref. Nbr.", Visible = false, Enabled = false)]
		...
		#endregion
		#region IsBalanceRecalculationRequired
		/// <summary>
		...
		public abstract class isBalanceRecalculationRequired : Data.BQL.BqlBool.Field<isBalanceRecalculationRequired> { }
		#endregion
		...
		public abstract class pendingPayment : Data.BQL.BqlBool.Field<pendingPayment> { }
