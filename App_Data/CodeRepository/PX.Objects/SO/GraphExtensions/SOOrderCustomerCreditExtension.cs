		...
		where TGraph : PXGraph
	{
		...
		protected override bool? GetCreditCheckError(PXCache sender, SOOrder row)
		{
			return (SOOrderType.PK.Find(sender.Graph, row.OrderType)?.CreditHoldEntry ?? false) ||
				EnsureCustomer(sender, row)?.Status == CustomerStatus.CreditHold;
		}
		...
		public override void Verify(PXCache sender, SOOrder Row, EventArgs e)
		{
			if (!IsLongOperationProcessing(sender.Graph) || VerifyOnLongRun(sender, Row, e))
			{
				...
				if (e is PXRowPersistingEventArgs || Row?.PrepaymentReqSatisfied != true ||
					(customer = EnsureCustomer(sender, Row))?.Status != CustomerStatus.CreditHold)
				{
					base.Verify(sender, Row, e);
				}
				else
				{
					ApplyCreditVerificationResult(sender, Row, e, customer, new CreditVerificationResult()
					{
						Enforce = EnforceType.AdminHold,
						Failed = true,
						ErrorMessage = AR.Messages.CustomerIsOnCreditHold
					});
				}
			}
		}
		...
		{
			if (row.IsFullyPaid == true && SOOrderType.PK.Find(Base, row.OrderType)?.RemoveCreditHoldByPayment == true)
			{
		...
		protected override decimal? GetDocumentSign(PXCache sender, SOOrder row)
		{
