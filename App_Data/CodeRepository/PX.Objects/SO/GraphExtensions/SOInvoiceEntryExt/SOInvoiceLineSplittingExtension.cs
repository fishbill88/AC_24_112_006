...
namespace PX.Objects.SO.GraphExtensions.SOInvoiceEntryExt
{
	...
	{
		protected virtual SOInvoiceItemAvailabilityExtension Availability => Base.FindImplementation<SOInvoiceItemAvailabilityExtension>();
		...
		protected override void EventHandler(ManualEvent.Row<ARTran>.Persisting.Args e)
		{
			...
			if (e.Operation.Command().IsIn(PXDBOperation.Insert, PXDBOperation.Update)
				&& GenerateLotSerialNumberOnPersist(e.Row) && !string.IsNullOrEmpty(e.Row?.LotSerialNbr))
			{
				Base.FindImplementation<ARTranPlan>()?.RaiseRowUpdated(e.Row);
			}
		}
		...
		protected override bool UseAvailabilityToIssueLotSerials(ARTran line, ARTranAsSplit split) => false;
		#endregion
