...
namespace PX.Objects.SO.GraphExtensions.SOInvoiceEntryExt
{
	...
	public class SOInvoiceItemAvailabilityExtension : SOBaseItemAvailabilityExtension<SOInvoiceEntry, ARTran, ARTranAsSplit>
	{
		...
		protected override string GetStatus(ARTran line)
		{
			...
			if (ARTranPlan.IsDirectLineNotLinkedToSO(line)
				&& FetchWithLineUOM(line, excludeCurrent, CostCenter.FreeStock) is IStatus availability)
			{
		...
		{
			return Base.FindImplementation<IItemPlanHandler<ARTran>>()?
				.GetAvailabilitySigns<TStatus>(ARTranAsSplit.ToARTran(split))
				?? new AvailabilitySigns();
		}

		public override IEnumerable<InvoiceSplit> SelectInvoicedRecords(string arDocType, string arRefNbr, int? arLineNbr)
			=> SelectInvoicedRecords(arDocType, arRefNbr, arLineNbr, includeDirectLines: true);
