...
namespace PX.Objects.SO.GraphExtensions
{
		...
		where TPaymentAdjust : class, IBqlTable, new()
	{
		...
		[PXLookupButton(Tooltip = "View Payment", DisplayOnMainToolbar = false, OnClosingPopup = PXSpecialButtonType.Cancel)]
		public virtual IEnumerable ViewPayment(PXAdapter adapter)
		{
			...
			{
				if (adj.AdjgDocType == ARPaymentType.PrepaymentInvoice)
				{
					ARInvoiceEntry ie = PXGraph.CreateInstance<ARInvoiceEntry>();
					ie.Document.Current = ie.Document.Search<ARInvoice.refNbr>(adj.AdjgRefNbr, adj.AdjgDocType);
					throw new PXRedirectRequiredException(ie, true, "Payment")
					{
		...
		protected virtual void _(Events.FieldUpdated<SOQuickPayment, SOQuickPayment.paymentMethodID> eventArgs)
		{
			...
			eventArgs.Cache.SetDefaultExt<SOQuickPayment.terminalID>(eventArgs.Row);
		}
		...
		protected virtual void _(Events.RowSelected<SOQuickPayment> eventArgs)
		{
			...
			bool isCCPaymentMethod = paymentMethod?.PaymentType == PaymentMethodType.CreditCard || paymentMethod?.PaymentType == PaymentMethodType.EFT || paymentMethod?.PaymentType == PaymentMethodType.POSTerminal;
			bool isEft = paymentMethod?.PaymentType == PaymentMethodType.EFT;
			bool isPOSTerminal = paymentMethod?.PaymentType == PaymentMethodType.POSTerminal;
			...
			PXUIFieldAttribute.SetVisible<SOQuickPayment.processingCenterID>(eventArgs.Cache, eventArgs.Row, isCreditCardRequired || isPOSTerminal);
			PXUIFieldAttribute.SetEnabled<SOQuickPayment.processingCenterID>(eventArgs.Cache, eventArgs.Row, isCreditCardRequired && newCard || isPOSTerminal);
			PXUIFieldAttribute.SetRequired<SOQuickPayment.processingCenterID>(eventArgs.Cache, isCreditCardRequired || isPOSTerminal);

			PXUIFieldAttribute.SetVisible<SOQuickPayment.terminalID>(eventArgs.Cache, eventArgs.Row, isPOSTerminal && !isRefund);
			PXUIFieldAttribute.SetEnabled<SOQuickPayment.terminalID>(eventArgs.Cache, eventArgs.Row, isPOSTerminal && !isRefund);
			PXUIFieldAttribute.SetRequired<SOQuickPayment.terminalID>(eventArgs.Cache, isPOSTerminal && !isRefund);
			...
			PXUIFieldAttribute.SetVisible<SOQuickPayment.extRefNbr>(eventArgs.Cache, eventArgs.Row, !isCreditCardRequired && !isPOSTerminal);
			PXUIFieldAttribute.SetEnabled<SOQuickPayment.extRefNbr>(eventArgs.Cache, eventArgs.Row, !isCreditCardRequired && !isPOSTerminal);
			PXUIFieldAttribute.SetRequired<SOQuickPayment.extRefNbr>(eventArgs.Cache,
				!isCreditCardRequired && aRSetup.RequireExtRef == true && !isPOSTerminal);
			...
			if (isPOSTerminal && cCProcessingCenter?.AcceptPOSPayments != true)
			{
				UIState.RaiseOrHideError<SOQuickPayment.processingCenterID>(eventArgs.Cache, eventArgs.Row, true, AR.Messages.ProcessingCenterDoesNotAcceptPOS, PXErrorLevel.Warning, eventArgs.Row.ProcessingCenterID);
			}
		}
