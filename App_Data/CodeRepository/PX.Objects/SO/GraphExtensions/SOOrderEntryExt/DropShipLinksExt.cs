...
namespace PX.Objects.SO.GraphExtensions.SOOrderEntryExt
{
	public class DropShipLinksExt : PXGraphExtension<PurchaseSupplyBaseExt, SOOrderEntry>
	{
		...
		public virtual void _(Events.RowSelecting<SOLine> e)
		{
			// Acuminator disable once PX1042 DatabaseQueriesInRowSelecting [Starting from 23r100 a separate connection scope is no longer required]
			DropShipLink link = Base1.IsOriginalDSLinkVisible(e.Row) ?
									Base1.GetOriginalDropShipLink(e.Row) :
		...
		{
			if (e.Row == null || e.Row.POSource != INReplenishmentSource.DropShipToOrder || e.Row.IsLegacyDropShip == true || Base1.IsDropshipReturn(e.Row))
				return;
		...
		public virtual void _(Events.RowUpdated<SOLine> e)
		{
			if (e.Row.POCreate != true && e.OldRow.POCreate == true
				&& e.OldRow.POSource == INReplenishmentSource.DropShipToOrder && e.OldRow.IsLegacyDropShip != true && !Base1.IsDropshipReturn(e.Row))
			{
			...
			else if (e.Row.POSource == INReplenishmentSource.DropShipToOrder && e.Row.IsLegacyDropShip != true && !Base1.IsDropshipReturn(e.Row))
			{
				DropShipLink link = Base1.GetDropShipLink(e.Row);
			...
			else if (e.Row.POCreate != e.OldRow.POCreate  && Base1.IsDropshipReturn(e.Row))
			{
				if (e.Row.POCreate == true)
				{
					DropShipLink link = Base1.GetOriginalDropShipLink(e.Row);
