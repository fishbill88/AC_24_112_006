...
namespace PX.Objects.SO.GraphExtensions.SOOrderEntryExt
{
	public class DropShipLinkDialog : PXGraphExtension<POLinkDialog, PurchaseSupplyBaseExt, SOOrderEntry>
	{
		...
		public virtual void LinkToSupplyLine(SOLineSplit split, SupplyPOLine supply, bool linkActive = true)
		{
			...
			SOLine line = PXParentAttribute.SelectParent<SOLine>(Base.splits.Cache, split);
			line.POCreate = true;
			line.VendorID = supply.VendorID;
			line.POOrderType = supply.OrderType;
			line.POOrderNbr = supply.OrderNbr;
			line.POLineNbr = supply.LineNbr;
			line.POLinkActive = linkActive;
		...
		protected virtual void UnlinkFromSupplyLine(SupplyPOLine supply, DropShipLink link, List<SOLineSplit> splits, bool? poCreate)
		{
			...
			SupplyPOOrder supplyOrder = SupplyPOLine.FK.SupplyOrder.FindParent(Base, supply);
			SOLineSplit currentSOSplit = splits.First();
			...
			foreach (var split in splits)
			{
					...
					&& split.POType == supply.OrderType && split.PONbr == supply.OrderNbr && split.POLineNbr == supply.LineNbr)
				{
					...
					currentSOSplit = Base.splits.Update(split);
				}
			}

			SOLine line = PXParentAttribute.SelectParent<SOLine>(Base.splits.Cache, currentSOSplit);
			line.POCreate = poCreate;
			line.POOrderType = null;
			line.POOrderNbr = null;
			line.POLineNbr = null;
			line.POLinkActive = false;

			Base1.DropShipLinks.Delete(link);
			supply.LinkedSOLines?.SparseArrayRemove(currentSOSplit.LineNbr.Value);
