...
namespace PX.Objects.SO.GraphExtensions.SOOrderEntryExt
{
	...
	[PXProtectedAccess]
	public abstract class Blanket : PXGraphExtension<SOOrderEntry>
	{
		...
		/// Overrides <see cref="SOOrderEntry.RecalcUnbilledTax()"/>
		[PXOverride]
		public virtual void RecalcUnbilledTax(Action base_RecalcUnbilledTax)
		{
			var orders = Base.Transactions.SelectMain()
						.Where(o => o.BlanketType != null && o.BlanketNbr != null)
						.Select(o => new { o.BlanketType, o.BlanketNbr })
						.Distinct();

			Lazy<SOOrderEntry> _sOOrderEntry = new Lazy<SOOrderEntry>(() => PXGraph.CreateInstance<SOOrderEntry>());

			foreach (var order in orders)
			{
				SOOrderEntry sOOrderEntry = _sOOrderEntry.Value;
				sOOrderEntry.Clear(PXClearOption.ClearAll);
				sOOrderEntry.Document.Current = sOOrderEntry.Document.Search<SOOrder.orderNbr>(order.BlanketNbr, order.BlanketType);
				if (Base.IsExternalTax(sOOrderEntry.Document.Current.TaxZoneID))
		...
		[PXOverride]
		public void CalculatePaymentBalance(ARPayment payment, SOAdjust adj,
			Action<ARPayment, SOAdjust> baseMethod)
		{
			...
			//correct not merged BlanketSOAdjust changes (due to aggregate in query in original method) if any
			{
				SOAdjust originalAdjust = Base.Caches<SOAdjust>().GetOriginal(adj);
				if (originalAdjust?.BlanketNbr != null)
				{
					var originalParent = BlanketSOAdjust.PK.Find(Base, (int)originalAdjust.BlanketRecordID, originalAdjust.BlanketType, originalAdjust.BlanketNbr, originalAdjust.AdjgDocType, originalAdjust.AdjgRefNbr);
					if (Base.Caches<BlanketSOAdjust>().Locate(originalParent) is BlanketSOAdjust actualParent && actualParent != originalParent)
					{
						payment.CuryDocBal -= (actualParent?.CuryAdjgAmt ?? 0m) - (originalParent?.CuryAdjgAmt ?? 0m);
						payment.DocBal -= (actualParent?.AdjAmt ?? 0m) - (originalParent?.AdjAmt ?? 0m);
					}
				}
			}
		}

		/// Overrides <see cref="SOOrderEntry.InitializeAdjustFields(SOAdjust, ARPayment, CurrencyInfo)"/>
		[PXOverride]
		public void InitializeAdjustFields(SOAdjust adjustToFill, ARPayment paymentToBeAdapted, CurrencyInfo paymentCurrencyInfo,
			Action<SOAdjust, ARPayment, CurrencyInfo> baseInitializeAdjustFieldsAndAdaptPayment)
		{
			if (Base.Document.Current?.Behavior == SOBehavior.SO)
			{
				adjustToFill.BlanketNbr = null;
				adjustToFill.BlanketType = null;
				adjustToFill.BlanketRecordID = null;

				var orderDictionary =
					Base.Transactions.View.BqlSelect.WhereAnd<Where<SOLine.blanketNbr.IsNotNull>>()
					.Select<SOLine>(Base, false)
					.GroupBy(soLine => (soLine.BlanketNbr, soLine.BlanketType))
					.ToDictionary(g => g.Key, g => (g.First().SortOrder, g.First().LineNbr));

				if (orderDictionary.Any())
				{
					SOAdjust parentBlanketSOAdjust =
						SelectFrom<SOBlanketOrderLink>
						.InnerJoin<SOAdjust>.On<
								SOAdjust.adjdOrderNbr.IsEqual<SOBlanketOrderLink.blanketNbr>
								.And<SOAdjust.adjdOrderType.IsEqual<SOBlanketOrderLink.blanketType>>>
						.Where<
								SOAdjust.adjgRefNbr.IsEqual<ARPayment.refNbr.FromCurrent>
								.And<SOAdjust.adjgDocType.IsEqual<ARPayment.docType.FromCurrent>>
								.And<SOBlanketOrderLink.orderNbr.IsEqual<SOOrder.orderNbr.FromCurrent>>
								.And<SOBlanketOrderLink.orderType.IsEqual<SOOrder.orderType.FromCurrent>>>
						.View.SelectMultiBound(Base,new object[] { paymentToBeAdapted })
						.RowCast<SOAdjust>().OrderBy(x => x, new BlanketAdjustComparer(orderDictionary)).FirstOrDefault();

					if (parentBlanketSOAdjust != null)
					{
						adjustToFill.BlanketNbr = parentBlanketSOAdjust.AdjdOrderNbr;
						adjustToFill.BlanketType = parentBlanketSOAdjust.AdjdOrderType;
						adjustToFill.BlanketRecordID = parentBlanketSOAdjust.RecordID;
					}
				}
			}
		...
		/// Overrides <see cref="SOOrderEntry.IsBalanceRecalculationRequired(SOAdjust)"/>
		[PXOverride]
		public bool IsBalanceRecalculationRequired(SOAdjust adj,
			Func<SOAdjust, bool> base_IsBalanceRecalculationRequired)
		{
			return base_IsBalanceRecalculationRequired(adj) || adj.IsBalanceRecalculationRequired == true;
		}
		protected class BlanketAdjustComparer : IComparer<SOAdjust>
		{
			private Dictionary<(string, string), (int?, int?)> orderDictionary;
			public BlanketAdjustComparer(Dictionary<(string, string), (int?, int?)> orderDictionary) => this.orderDictionary = orderDictionary;
			public int Compare(SOAdjust x, SOAdjust y)
			{
				bool xExists = orderDictionary.TryGetValue((x.AdjdOrderNbr, x.AdjdOrderType), out (int? sortorder, int? lineNbr) xOrder);
				bool yExists = orderDictionary.TryGetValue((y.AdjdOrderNbr, y.AdjdOrderType), out (int? sortorder, int? lineNbr) yOrder);
				if (xExists && yExists)
				{
					if (xOrder.sortorder < yOrder.sortorder) return -1;
					if (xOrder.sortorder > yOrder.sortorder) return 1;
					if (xOrder.lineNbr < yOrder.lineNbr) return -1;
					if (xOrder.lineNbr > yOrder.lineNbr) return 1;
					return 0;
				}
		...
		protected virtual void _(Events.FieldDefaulting<SOLine, SOLine.shipVia> e)
		{
			if (e.Row?.Behavior == SOBehavior.BL)
			{
				e.NewValue = Base.location.SelectSingle(e.Row.CustomerLocationID)?.CCarrierID;
				e.Cancel = true;
			}
		}

		protected virtual void _(Events.FieldDefaulting<SOLine, SOLine.fOBPoint> e)
		{
			if (e.Row?.Behavior == SOBehavior.BL)
			{
				e.NewValue = Base.location.SelectSingle(e.Row.CustomerLocationID)?.CFOBPointID;
				e.Cancel = true;
			}
		}

		protected virtual void _(Events.FieldDefaulting<SOLine, SOLine.shipTermsID> e)
		{
			if (e.Row?.Behavior == SOBehavior.BL)
			{
				e.NewValue = Base.location.SelectSingle(e.Row.CustomerLocationID)?.CShipTermsID;
				e.Cancel = true;
			}
		}

		protected virtual void _(Events.FieldDefaulting<SOLine, SOLine.shipZoneID> e)
		{
			if (e.Row?.Behavior == SOBehavior.BL)
			{
				e.NewValue = Base.location.SelectSingle(e.Row.CustomerLocationID)?.CShipZoneID;
				e.Cancel = true;
			}
		}
		...
		protected virtual void _(Events.RowDeleted<SOBlanketOrderLink> e)
		{
			SOBlanketOrderLink link = e.Row;
			UpdateAdjustmentsOnBlanketLinkRemoval(link);
		}

		protected virtual void UpdateAdjustmentsOnBlanketLinkRemoval(SOBlanketOrderLink link)
		{
			foreach (SOAdjust relatedAdjustment in Base.Adjustments_Raw.Select().RowCast<SOAdjust>()
				.Where(adj =>
					adj.BlanketNbr == link.BlanketNbr
					&& adj.BlanketType == link.BlanketType))
			{
				FillSOAdjustByPayment(relatedAdjustment);
				Base.Caches<SOAdjust>().Update(relatedAdjustment);
			}
		}
		...
		protected virtual void OnChildSOLineUpdated(SOLine oldLine, SOLine line)
		{
			...
			if (cancelledChanged || completedChanged && !miscLine)
			{
				...
				blanketOrder.IsOpenTaxValid = false;
				Base.Caches<BlanketSOOrder>().Update(blanketOrder);
		...
		{
			if (e.OldRow.BlanketRecordID != null && e.Row.BlanketRecordID == null)
				HandleSOAdjustBlanketReferenceRemoved(e.OldRow, e.Row);
		}
		protected virtual void HandleSOAdjustBlanketReferenceRemoved(SOAdjust oldSOAdjust, SOAdjust newSOAdjust)
		{
			object curyAdjdAmt = newSOAdjust.CuryAdjdAmt;
			newSOAdjust.IsBalanceRecalculationRequired = true;
			PXCache<SOAdjust> cache = Base.Caches<SOAdjust>();
			try
			{
				cache.RaiseFieldVerifying<SOAdjust.curyAdjdAmt>(newSOAdjust, ref curyAdjdAmt);
			}
			catch (PXSetPropertyException ex)
			{
				cache.RaiseExceptionHandling<SOAdjust.curyAdjdAmt>(newSOAdjust, curyAdjdAmt, ex);
			}
		}

		/// Overrides <see cref="SOOrderEntry.UpdateBalanceOnCuryAdjdAmtUpdated(PXCache, PXFieldUpdatedEventArgs)"/>
		[PXOverride]
		...
		{
			SOAdjust adjust = (SOAdjust)e.Row;
			if (adjust.IsBalanceRecalculationRequired == true)
			{
				TryRecalculateBalanceAndCuryInfo(adjust, sender);
				adjust.IsBalanceRecalculationRequired = false;
			}
			else
			{
		...
		/// Uses <see cref="SOOrderEntry.FillSOAdjustByPayment(SOAdjust)"/>
		[PXProtectedAccess] protected abstract void FillSOAdjustByPayment(SOAdjust adj);
		/// Uses <see cref="SOOrderEntry.TryRecalculateBalanceAndCuryInfo(SOAdjust, PXCache)"/>
		[PXProtectedAccess] protected abstract bool TryRecalculateBalanceAndCuryInfo(SOAdjust adj, PXCache sender);
		#endregion
	}
}
