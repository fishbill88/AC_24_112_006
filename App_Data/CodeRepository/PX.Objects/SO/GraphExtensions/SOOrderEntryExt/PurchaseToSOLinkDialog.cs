		...
		public virtual void CollectSupplyPOLines(SOLine currentSOLine, ICollection<SupplyPOLine> supplyPOLines)
		{
			...
			var suppliesQuery = new PXSelectJoin<SupplyPOLine,
				...
				Where<SupplyPOLine.lineType,
					...
					And<SupplyPOLine.vendorID, NotEqual<Current<SOOrder.customerID>>,
					And2<Where<Required<GL.Branch.bAccountID>, IsNull,
						Or<SupplyPOLine.vendorID, NotEqual<Required<GL.Branch.bAccountID>>>>,
					And<Where2<
			...
			GL.Branch orderBranch = GL.Branch.PK.Find(Base, Base.Document.Current.BranchID);
			...
			foreach (PXResult<SupplyPOLine, SOLineSplit> row in suppliesQuery.Select(orderBranch.BAccountID, orderBranch.BAccountID))
			{
			...
			foreach (SupplyPOLine supplyLine in supplyLineDemand.Keys)
			{
				...
				decimal? invtMult = currentSOLine.Behavior != SOBehavior.BL ? (decimal?)currentSOLine.InvtMult : -1m;
				if (invtMult * currentSOLine.OpenQty < 0m && supplyLine.BaseOpenQty - supplyLine.BaseDemandQty > 0m
