...
namespace PX.Objects.SO
{
	public partial class SOShipmentEntry : PXGraph<SOShipmentEntry, SOShipment>, IGraphWithInitialization
	{
		...
		private class SkipShipCompleteValidationScope : Common.Scopes.FlaggedModeScopeBase<SkipShipCompleteValidationScope> { }
		...
		protected virtual void SOShipment_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetVisible<SOShipment.controlQty>(sender, e.Row, (bool)sosetup.Current.RequireShipmentTotal);
		...
		{
			SOOrder order = PXParentAttribute.SelectParent<SOOrder>(sender, row);
			if (order != null)
			{
			...
			SOOrderSite orderSite = PXParentAttribute.SelectParent<SOOrderSite>(sender, row);
		...
		protected virtual void DefaultUnitPrice(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			{
				decimal? unitprice = INUnitAttribute.ConvertFromTo<SOShipLine.inventoryID>(sender, e.Row, ((SOShipLine)e.Row).UOM, ((SOShipLine)e.Row).OrderUOM, (decimal)UnitPrice, INPrecision.NOROUND);
		...
		{
			SOShipLine line = PXParentAttribute.SelectParent<SOShipLine>(sender, e.Row);
			if (line != null )
			{
				InventoryItem item = InventoryItem.PK.Find(this, line.InventoryID);
		...
		public virtual bool CreateShipmentFromSchedules(PXResult<SOShipmentPlan, SOLineSplit, SOLine, InventoryItem, INLotSerClass, INSite, SOShipLine> res, SOShipLine newline, SOOrderType ordertype, string operation, DocumentList<SOShipment> list)
		{
			...
			if (plan.Selected == true || list != null && (!requireAllocationUnallocated || sosetup.Current.AddAllToShipment == true))
			{
				...
				newline.UOM = (line.Operation == SOOperation.Receipt) ? linesplit.UOM : line.UOM;
				...
				using (docgraph.TranSplitPlanExt.ReleaseModeScope())
				{
					...
					foreach (PXResult<SOShipLine, SOShipLineSplit, SOLine, ARTran, INTran, INItemPlan> res in PXSelectJoin<SOShipLine,
						...
						LeftJoin<ARTran,
							...
							And2<Where<ARTran.sOShipmentLineNbr, Equal<SOShipLine.lineNbr>, Or<SOShipLine.isIntercompany, Equal<False>>>,
							And<ARTran.canceled, Equal<False>,
							And<ARTran.isCancellation, Equal<False>>>>>>>>>>,
		...
		protected virtual decimal? GetNSKitComponentUnitCost(SOLine soline, SOShipLine line, SOShipLineSplit split)
		{
			...
			{
				invoiced = SelectFrom<INTran>.
							Where<INTran.sOOrderType.IsEqual<SOShipLineSplit.origOrderType.FromCurrent>.
							And<INTran.sOOrderNbr.IsEqual<SOShipLineSplit.origOrderNbr.FromCurrent>>.
							And<INTran.sOOrderLineNbr.IsEqual<SOShipLineSplit.origLineNbr.FromCurrent>>.
							And<INTran.inventoryID.IsEqual<SOShipLineSplit.inventoryID.FromCurrent>>>
							...
							using (PXTransactionScope ts = new PXTransactionScope())
							{
								...
								if (Document.Current.OverrideFreightAmount != true)
								{
									...
									else
									{
										...
										PXCurrencyAttribute.CuryConvCury<SOShipment.curyFreightAmt>(Document.Cache, Document.Current);
									}
								}
		...
		private void ValidateLineShipComplete(SOShipment shipment, ref string orderType, ref string orderNbr)
		{
			foreach (SOLine2 orderLine in soline.Cache.Updated)
			{
				...
				if (theSameOrder && !object.Equals(soline.Cache.GetValueOriginal<SOLine2.baseShippedQty>(orderLine), orderLine.BaseShippedQty))
				{
				...
				orderLine.LineSign * orderLine.BaseShippedQty < orderLine.LineSign * orderLine.BaseOrderQty * orderLine.CompleteQtyMin / 100m)
			{
				if (orderLine.BaseShippedQty == 0m)
				{
					SOShipLine shipLine = SelectFrom<SOShipLine>
						.Where<SOShipLine.origOrderType.IsEqual<SOLine2.orderType.FromCurrent>
							.And<SOShipLine.origOrderNbr.IsEqual<SOLine2.orderNbr.FromCurrent>>
							.And<SOShipLine.origLineNbr.IsEqual<SOLine2.lineNbr.FromCurrent>>>
		...
		private void ValidateOrderShipComplete(SOShipment shipment, ref string orderType, ref string orderNbr)
		{
			if (!Transactions.Cache.Deleted.Any_() &&
				...
				object.Equals(Document.Cache.GetValueOriginal<SOShipment.shipDate>(shipment), shipment.ShipDate))
			{
				return;
			}

			var shipLines = new Lazy<HashSet<(string orderType, string orderNbr, int? lineNbr)>>(() =>
				Transactions.View.SelectMultiBound(new object[] { shipment })
				.RowCast<SOShipLine>()
				.Select(l => (l.OrigOrderType, l.OrigOrderNbr, l.OrigLineNbr))
				.ToHashSet());

			foreach (var order in OrderList.View.SelectMultiBound(new object[] { shipment }).RowCast<SOOrderShipment>())
			{
		...
		private bool ValidateOrderShipComplete(HashSet<(string orderType, string orderNbr, int? lineNbr)> shiplinesLines, SOOrderShipment order)
		{
			...
			var lines = SelectFrom<SOLine2>
				.Where<SOLine2.orderType.IsEqual<SOOrderShipment.orderType.FromCurrent>
					.And<SOLine2.orderNbr.IsEqual<SOOrderShipment.orderNbr.FromCurrent>>
					.And<SOLine2.siteID.IsEqual<SOOrderShipment.siteID.FromCurrent>>
					.And<SOLine2.operation.IsEqual<SOOrderShipment.operation.FromCurrent>>
					.And<SOLine2.completed.IsNotEqual<True>>
					.And<SOLine2.lineType.IsNotEqual<SOLineType.miscCharge>>
					.And<SOLine2.shipDate.IsLessEqual<SOOrderShipment.shipDate.FromCurrent>>
					.And<SOLine2.pOSource.IsNull.Or<SOLine2.pOSource.IsNotEqual<INReplenishmentSource.dropShipToOrder>>>>
		...
		public class PackageDetail : PXGraphExtension<SOShipmentEntry>
		{
			...
			private class RowPersistingScope : FlaggedModeScopeBase<RowPersistingScope> { }
			...
			protected virtual void _(Events.RowPersisting<SOShipLineSplitPackage> e)
			{
				if (e.Operation.Command().IsIn(PXDBOperation.Insert, PXDBOperation.Update))
				{
					using (new RowPersistingScope())
						e.Cache.VerifyFieldAndRaiseException<SOShipLineSplitPackage.packedQty>(e.Row);
				}
			}
				...
				if (row != null && oldRow != null && row.ShipmentLineNbr == oldRow.ShipmentLineNbr)
				{
					...
					if (shipmentLineSplit != null)
					{
						...
						else
						{
							...
							var oldShipmentLineSplit = PXParentAttribute.SelectParent<SOShipLineSplit>(sender, oldRow);
	...
	public partial class SOLine2 : PXBqlTable, IBqlTable, ISortOrder
	{
		...
		/// <inheritdoc cref="SOLine.Behavior"/>
		...
		/// <inheritdoc cref="SOLine.CuryDiscAmt"/>
		...
		/// <inheritdoc cref="SOLine.DiscAmt"/>
		...
		/// <inheritdoc cref="SOLine.CuryExtPrice"/>
		...
		/// <inheritdoc cref="SOLine.ExtPrice"/>
