...
namespace PX.Objects.SO
{
	...
	public partial class SOInvoiceEntry : ARInvoiceEntry
	{
		...
		public
			SelectFrom<SOSetupInvoiceApproval>.
			Where<SOSetupInvoiceApproval.docType.IsEqual<ARInvoice.docType.FromCurrent>>.
			View SetupInvoiceApproval;

		[PXViewName(EP.Messages.Approval)]
		public EPApprovalAutomationWithoutHoldDefaulting<ARInvoice, ARInvoice.approved, ARInvoice.rejected, ARInvoice.hold, SOSetupInvoiceApproval> SOApproval;
		...
		public PXAction<ARInvoice> completeProcessing;
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Complete Processing")]
			...
			PXLongOperation.StartOperation(this, delegate ()
			{
				...
				try
				{
					foreach (var invoice in invoices)
					{
						if (isMassProcess)
						{
							PXProcessing.SetCurrentItem(invoice);
						}

						invoiceEntry.CompleteProcessingImpl(invoice);
			...
			: base()
		{
			...
			Approval.SuppressInFull = true; /// <see cref="SOApproval"/> instead
		...
		public override void RecalcUnbilledTax()
		{
			...
			foreach (var orderKeys in orders)
			{
				soOrderEntry.Clear(PXClearOption.ClearAll);
				soOrderEntry.Document.Current = soOrderEntry.Document.Search<SOOrder.orderNbr>(orderKeys.SOOrderNbr, orderKeys.SOOrderType);
				if (soOrderEntry.Document.Current.IsUnbilledTaxValid != true)
				{
		...
		protected override void ApplyDocumentState(PXCache cache, ARInvoice doc, ARInvoiceState state)
		{
			...
			if (isCashDocument)
			{
				PXUIFieldAttribute.SetVisible<ARInvoice.curyOrigDocAmt>(cache, doc);
			}
			...
			bool isUnderApproval = state.IsDocumentRejectedOrPendingApproval || state.IsDocumentApprovedBalanced;
			...
			PXUIFieldAttribute.SetEnabled<ARInvoice.curyOrigDocAmt>(cache, doc, doc.Released == false && isUnderApproval == false && !isAuthorizedCashSale && !isRefundedCashReturn);
			PXUIFieldAttribute.SetEnabled<ARInvoice.curyOrigDiscAmt>(cache, doc, doc.Released == false && isUnderApproval == false &&
				(doc.DocType != ARDocType.CreditMemo || (doc.DocType == ARDocType.CreditMemo && arsetup.Current.TermsInCreditMemos == true)) &&
				!isAuthorizedCashSale && !isRefundedCashReturn);
			PXUIFieldAttribute.SetVisible<ARTran.sOOrderNbr>(Transactions.Cache, null, true);
			PXUIFieldAttribute.SetVisible<ARTran.sOOrderType>(Transactions.Cache, null, true);
		...
		{
			bool enabled = doc.DocType.IsNotIn(ARDocType.CreditMemo, ARDocType.CashSale, ARDocType.CashReturn) && !SOApproval.GetAssignedMaps(doc, cache).Any();
			putOnCreditHold.SetEnabled(enabled);
		}

		public override bool IsApprovalRequired(ARInvoice doc)
		{
			return SOSetupInvoiceApproval.EPSettings.ApprovableDocTypes.Contains(doc.DocType);
		}

		// Acuminator disable once PX1016 ExtensionDoesNotDeclareIsActiveMethod extension should be constantly active
		public class Alter_ARInvoiceEntryDocumentExtension : PXGraphExtension<ARInvoiceEntryDocumentExtension, ARInvoiceEntry>
		{
			[PXOverride]
			public void SuppressApproval(Action base_SuppressApproval)
			{
				base_SuppressApproval();

				if (Base is SOInvoiceEntry soBase)
					soBase.SOApproval.SuppressApproval = true;
			}
		}
		...
		protected virtual void SOOrder_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (row != null && oldRow != null
				&& !soorder.Cache.ObjectsEqual<SOOrder.unbilledOrderQty, SOOrder.curyUnbilledLineTotal,
					SOOrder.curyUnbilledMiscTot, SOOrder.curyUnbilledFreightTot, SOOrder.curyUnbilledDiscTotal>(row, oldRow))
			{
		...
		protected virtual List<PXResult<ARPayment, CurrencyInfo, ExternalTransaction>> CollectPaymentsToApply(int applcount)
		{
			...
			else
			{
				...
				{
					cmd.WhereAnd<Where<ARPayment.docType, NotEqual<ARDocType.prepaymentInvoice>,
						Or<ARPayment.pendingPayment, Equal<False>, And<ARPayment.released, Equal<True>>>>>();
				}
			}
		...
		{
			PXCache pxCache = this.Caches<SOOrder>();
			pxCache.ClearQueryCache();
			pxCache.Clear();

			pxCache = this.Caches<SOTaxTran>();
			pxCache.ClearQueryCache();
			pxCache.Clear();
		}
		...
		private void InvoiceOrderImpl(InvoiceOrderArgs args)
		{
			...
			if (args.List != null)
			{
				...
				if (newdoc.RefNbr != null)
				{
					...
					if (newdoc.Released == true)
						throw new PXInvalidOperationException(CA.Messages.DocumentStatusInvalid);
				}
				else
				{
					this.Clear();
					...
					newdoc.Hold = args.QuickProcessFlow != PXQuickProcess.ActionFlow.HasNextInFlow && soOrderType.InvoiceHoldEntry == true || SOSetupInvoiceApproval.EPSettings.ApprovableDocTypes.Contains(newdoc.DocType);

					newdoc.DocDate = orderInvoiceDate;
					newdoc.BranchID = soOrder.BranchID;

					newdoc.IsPaymentsTransferred = !(args.OptimizeExternalTaxCalc && IsExternalTax(soOrder.TaxZoneID));
					...
					if (newdoc.DocType != ARDocType.CreditMemo || newdoc.DocType == ARDocType.CreditMemo && arsetup.Current.TermsInCreditMemos == true)
					{
			...
			else
			{
				...
				else
				{
					...
					{
						throw new PXException(Messages.CurrencyRateDiffersInSO, soOrder.OrderNbr);
					}
				}
			...
			else
			{
				...
				{
					var ext = GetExtension<ARInvoiceEntryExternalTax>();
					try
					{
						this.Document.Current.ApplyPaymentWhenTaxAvailable = true;
						if (ext != null && Document.Current.IsPaymentsTransferred == true)
							ext.forceTaxCalcOnHold = true;
						...
						if (IsExternalTax(Document.Current.TaxZoneID) && Document.Current.IsPaymentsTransferred == true)
						{
		...
		protected virtual void CompleteProcessingImpl(ARInvoice invoice)
		{
			if (invoice.IsPaymentsTransferred == true)
				return;
			...
			SODocument.Current = (SOInvoice)SODocument.Select();
			if (PX.SM.WorkflowAction.HasWorkflowActionEnabled(this, g => g.completeProcessing, Document.Current) == false)
			{
				throw new PXInvalidOperationException(Messages.ActionNotAvailableInCurrentState,
					completeProcessing.GetCaption(), Document.Cache.GetRowDescription(Document.Current));
			}

			if (PXTimeStampScope.GetPersistedRecord(Document.Cache, Document.Current) != Document.Current)
				PXTimeStampScope.PutPersisted(Document.Cache, Document.Current, Document.Current.tstamp);
			...
			if (IsExternalTax(Document.Current.TaxZoneID))
			{
				...
				try
				{
					Document.Current.ApplyPaymentWhenTaxAvailable = true;
					if (ext != null)
						ext.forceTaxCalcOnHold = true;
					this.Save.Press();
				}
				...
				{
					Document.Current.ApplyPaymentWhenTaxAvailable = false;
					if (ext != null)
						ext.forceTaxCalcOnHold = false;
				}
			}

			SOOrderType soOrderType = null;
			PXResultset<SOOrderShipment> invoiceOrderShipments =
				PXSelect<SOOrderShipment,
				Where<SOOrderShipment.invoiceType, Equal<Current<ARInvoice.docType>>,
					And<SOOrderShipment.invoiceNbr, Equal<Current<ARInvoice.refNbr>>>>>
				.Select(this);
			foreach (SOOrderShipment orderShipment in invoiceOrderShipments)
			{
				InsertApplications(orderShipment);

				if (soOrderType == null)
					soOrderType = SOOrderType.PK.Find(this, orderShipment.OrderType);
			}

			ARInvoice.Events
				.Select(ev => ev.ProcessingCompleted)
				.FireOn(this, Document.Current);

			Document.Current.Hold = soOrderType?.InvoiceHoldEntry == true
				|| SOSetupInvoiceApproval.EPSettings.ApprovableDocTypes.Contains(Document.Current.DocType);
		...
		{
			bool severalErrors = false;
			string lastInvoiceWithError = null;
			foreach (var invoice in createdInvoices.RowCast<ARInvoice>())
			{
				try
				{
					CompleteProcessingImpl(invoice);
				}
				catch (Exception e)
				{
					PXTrace.WriteWarning(e);
					PXTrace.WriteWarning(Messages.InvoicePostProcessingFailed, invoice.RefNbr);

					if (lastInvoiceWithError != null)
						severalErrors = true;
					lastInvoiceWithError = invoice.RefNbr;
				}
			}

			if (severalErrors)
			{
				throw new PXOperationCompletedWithWarningException(Messages.InvoicesPostProcessingFailed);
			}
			else if (lastInvoiceWithError != null)
			{
				throw new PXOperationCompletedWithWarningException(
					Messages.InvoicePostProcessingFailed,
					lastInvoiceWithError);
			}
		}
		...
		public virtual void ProcessGroupAndDocumentDiscounts(ARInvoice newdoc, InvoiceOrderArgs args, SOOrderShipment currentOrderShipment, SOOrderType soOrderType)
		{
			...
			if (soOrderType.RecalculateDiscOnPartialShipment == true)
			{
				...
				if (firstLine != null)
				{
					RecalculateDiscounts(this.Transactions.Cache, firstLine, false, true);
				}
			}
		}
		...
		public virtual void InsertApplications(SOOrderShipment orderShipment)
		{
			...
			AfterInsertApplication(orderShipment);
		}
		...
		protected Sign GetTaxSign(ARInvoice newInvoice, SOOrderShipment orderShipment)
		{
			SOOrderType orderType = SOOrderType.PK.Find(this, orderShipment.OrderType);

			if (newInvoice.DrCr == DrCr.Credit && orderType.DefaultOperation == SOOperation.Receipt ||
				newInvoice.DrCr == DrCr.Debit && orderType.DefaultOperation == SOOperation.Issue)
			{
				return Sign.Minus;
			}
			else
		...
		public virtual void AddOrderTaxes(SOOrderShipment orderShipment)
		{
			...
			var orderTaxes = PXSelectJoin<SOTaxTran,
				InnerJoin<Tax, On<SOTaxTran.taxID, Equal<Tax.taxID>>>,
				Where<SOTaxTran.orderType, Equal<Required<SOTaxTran.orderType>>, And<SOTaxTran.orderNbr, Equal<Required<SOTaxTran.orderNbr>>>>>
				.Select(this, orderShipment.OrderType, orderShipment.OrderNbr)
				.Select(m => new { TaxTran = m.GetItem<SOTaxTran>(), Tax = m.GetItem<Tax>() });
			...
			foreach (var item in orderTaxes)
			{
				...
				newtax.TaxID = item.TaxTran.TaxID;
				newtax.TaxRate = 0m;
				newtax.CuryID = Document.Current.CuryID;
				if (item.Tax?.TaxType == CSTaxType.PerUnit)
				{
					newtax.TaxableQty = item.TaxTran.TaxableQty;
				}

				if (Document.Current?.DisableAutomaticTaxCalculation == true)
				{
					var taxSign = GetTaxSign(this.Document.Current, orderShipment);
					newtax.CuryTaxableAmt = item.TaxTran.CuryTaxableAmt * taxSign;
					newtax.CuryTaxAmt = item.TaxTran.CuryTaxAmt * taxSign;
					newtax.TaxRate = item.TaxTran.TaxRate;
					newtax.JurisName = item.TaxTran.JurisName;
					newtax.JurisType = item.TaxTran.JurisType;
				...
				if (Document.Current?.DisableAutomaticTaxCalculation == true)
				{
						...
						&& this.Taxes.Cache.ObjectsEqual<ARTaxTran.module, ARTaxTran.refNbr, ARTaxTran.tranType, ARTaxTran.taxID, ARTaxTran.jurisType, ARTaxTran.jurisName>(newtax, a)))
					{
						...
						if (item.Tax?.TaxType == CSTaxType.PerUnit)
						{
							updatedTaxTran.TaxableQty += newtax.TaxableQty;
						}
				...
				else
				{
						...
						&& this.Taxes.Cache.ObjectsEqual<ARTaxTran.module, ARTaxTran.refNbr, ARTaxTran.tranType, ARTaxTran.taxID>(newtax, a)))
					{
						if (item.Tax?.TaxType == CSTaxType.PerUnit)
						{
							newtax.TaxableQty += existingTaxTran.TaxableQty;
						}
			...
			if (Document.Current?.DisableAutomaticTaxCalculation == true)
			{
				...
				foreach (var t in orderTaxes)
				{
					t.TaxTran.CuryUnbilledTaxableAmt = 0m;
					t.TaxTran.CuryUnbilledTaxAmt = 0m;
					t.TaxTran.CuryUnshippedTaxableAmt = 0m;
					t.TaxTran.CuryUnshippedTaxAmt = 0m;

					this.Caches<SOTaxTran>().Update(t.TaxTran);
				}
		...
		public virtual ARTran InsertInvoiceDirectLine(ARTran tran)
		{
			...
			try
			{
				...
				if (tran.LineType != null)
					Transactions.Cache.Adjust<PXFormulaAttribute>().For<ARTran.lineType>(f => f.CancelCalculation = true);
			...
			finally
			{
				...
				Transactions.Cache.Adjust<PXFormulaAttribute>().For<ARTran.lineType>(f => f.CancelCalculation = false);
			}
		}
		...
		public virtual SOFreightDetail FillFreightDetailsForShipment(SOOrder order, SOOrderShipment orderShipment)
		{
			...
			decimal? curyLineTotal = orderShipment.LineTotal == Document.Current.LineTotal ? Document.Current.CuryLineTotal
				: GetExtension<MultiCurrency>().GetCurrencyInfo(Document.Current.CuryInfoID).CuryConvCury(orderShipment.LineTotal.Value);
		...
		{
			RecalculateDiscounts(sender, line, disableGroupAndDocumentDiscountsCalculation, false);
		}

		public virtual void RecalculateDiscounts(PXCache sender, ARTran line, bool disableGroupAndDocumentDiscountsCalculation, bool forceFormulaRecalculation)
		{
			if (PXAccess.FeatureInstalled<FeaturesSet.customerDiscounts>() && line.InventoryID != null && line.Qty != null && line.CuryTranAmt != null && line.IsFree != true)
			{
				...
				if (forceFormulaRecalculation)
					discountCalculationOptions = discountCalculationOptions | DiscountEngine.DiscountCalculationOptions.ForceFormulaRecalculation;
