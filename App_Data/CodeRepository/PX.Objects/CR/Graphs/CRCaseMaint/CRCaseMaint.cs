...
namespace PX.Objects.CR
{
    public class CRCaseMaint : PXGraph<CRCaseMaint, CRCase>
	{
		...
		[PXHidden]
		[PXCopyPasteHiddenFields(typeof(CRCase.description), typeof(CRCase.solutionActivityNoteID))]
		...
		public virtual IEnumerable pendingCustomer(PXAdapter adapter)
		{
			return adapter.Get();
		}
        ...
        protected virtual List<PMTran> GeneratePMTrans(CRCase @case)
		{
            ...
            DateTime startDate = Accessinfo.BusinessDate ?? @case.ReportedOnDateTime!.Value;
        ...
        protected virtual List<ARTran> GenerateARTrans(CRCase c)
        {
            ...
            DateTime startDate = c.ReportedOnDateTime!.Value;
		...
		private void RecalcDetails(PXCache sender, CRCase row)
		{
			using (new PXConnectionScope())
			{
				...
				{
					TimeSpan createDate = new TimeSpan(row.ReportedOnDateTime!.Value.Ticks);
