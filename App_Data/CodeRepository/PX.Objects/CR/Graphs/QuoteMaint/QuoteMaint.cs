...
namespace PX.Objects.CR
{
    public class QuoteMaint : PXGraph<QuoteMaint>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
    {
		...
		public SelectFrom<CROpportunityProducts>
					.InnerJoin<InventoryItem>
						.On<InventoryItem.inventoryID.IsEqual<CROpportunityProducts.inventoryID>>
					.InnerJoin<INSite>
						.On<INSite.siteID.IsEqual<CROpportunityProducts.siteID>>
					.Where<CROpportunityProducts.quoteID.IsEqual<@P.AsGuid>
						.And<InventoryItem.inventoryCD.IsEqual<@P.AsString>
						.And<INSite.siteCD.IsEqual<@P.AsString>>>>
				.View ProductsByQuoteIDAndInventoryCD;
        ...
        public virtual IEnumerable EditQuote(PXAdapter adapter)
        {
            foreach (CRQuote item in adapter.Get<CRQuote>())
            {
                Approval.ClearRelatedApprovals(item);
        ...
        private void FillDefaultBAccountID(PXCache cache, CRQuote row)
        {
            ...
            if (row.ContactID != null)
            {
                ...
                if (contact != null)
                {
					...
					cache.SetValueExt<CRQuote.bAccountID>(row, contact.BAccountID);
				}
             }
        }
		...
		public virtual bool PrepareImportRow(string viewName, IDictionary keys, IDictionary values)
		{
			if (nameof(this.Products).Equals(viewName, StringComparison.OrdinalIgnoreCase))
			{
				...
				{
					//keys[nameof(CROpportunityProducts.lineNbr)] = Quote.Current.ProductCntr + 1;
				}
				...
				{
					Guid? quoteId = Quote.Current.QuoteID;
		...
		/// </summary>
		public class PerUnitTaxDisableExt : PX.Objects.Extensions.PerUnitTax.PerUnitTaxDataEntryGraphExtension<QuoteMaint, CRTaxTran>
		{
