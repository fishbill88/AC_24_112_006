...
namespace PX.Objects.CR
{
	public class CREmailActivityMaint : CRBaseActivityMaint<CREmailActivityMaint, CRSMEmail>
	{
		...
		protected virtual void _(Events.CacheAttached<CRSMEmail.subject> e) { }
		...
		protected virtual void CRSMEmail_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			cache.AdjustUI(row)
				.For<CRSMEmail.mailFrom>(ui => ui.Visible = isIncome)
				.For<CRSMEmail.mailAccountID>(ui => ui.Visible = !isIncome);
		...
		protected virtual void _(Events.FieldSelecting<CRSMEmail, CRSMEmail.entityDescription> e)
		{
			if (e.Row == null) return;

			// hack: rte in new ui currently doesn't work without html tag
			e.ReturnValue = "<html><body>" + CacheUtility.GetErrorDescription(e.Row.Exception) + GetEntityDescription(e.Row) + "</body></html>";
		}
