		...
		where TMain : class, IBqlTable, INotable, new()
	{
		...
		public virtual IEnumerable duplicates(bool? forceSelect = null)
		{
			...
			foreach (PXResult rec in possibleDuplicates)
			{
				...
				BAccountR duplicateBA = rec.GetItem<BAccountR>();
				...
				{
					ContactTypesAttribute.Person => CRActivityStatistics.PK.Find(Base, duplicateContact.NoteID),
					ContactTypesAttribute.Lead => CRActivityStatistics.PK.Find(Base, duplicateLead.NoteID),
					ContactTypesAttribute.BAccountProperty => CRActivityStatistics.PK.Find(Base, duplicateBA.NoteID)
				};
				...
				if (anyFound)
				{
					...
					if (blocked != null)
					{
						...
						if (blocked.Any(_ => _.IsBlocked && _.BlockType == CreateOnEntryAttribute.Block))
						{
							...
							PXGraph.ThrowWithoutRollback(new PXSetPropertyException(Messages.ErrorSavingWithDuplicates, GetEntityNameByType(duplicateDocument.ContactType)));
						}
