...
namespace PX.Objects.CA
{
    ...
    public class CashAccountMaint : PXGraph<CashAccountMaint, CashAccount>
	{
        ...
        public partial class PaymentMethodAccount2 : PXBqlTable, IBqlTable
        {
        ...
        public CashAccountMaint()
        {
			...
			this.PaymentMethodForRemittance.Cache.AllowInsert = false;
			this.PaymentMethodForRemittance.Cache.AllowDelete = false;

			this.PaymentDetails.Cache.AllowInsert = false;
			this.PaymentDetails.Cache.AllowDelete = false;
		}
		...
		private List<string> GetAccountUsage(Account acct)
		{
			...
			if (SelectFrom<ARInvoice>.Where<ARInvoice.status.IsIn<ARDocStatus.open, ARDocStatus.unapplied>
        ...
        protected virtual void CashAccount_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
        {
			...
			if (cashacct.ClearingAccount == true)
			{
				CAEntryType entryType = (CAEntryType)PXSelect<CAEntryType, Where<CAEntryType.useToReclassifyPayments, Equal<True>,
					And<CAEntryType.branchID, Equal<Required<CAEntryType.branchID>>,
					And<CAEntryType.accountID, Equal<Required<CAEntryType.accountID>>,
					And<CAEntryType.subID, Equal<Required<CAEntryType.subID>>>>>>>
					.Select(this, cashacct.BranchID, cashacct.AccountID, cashacct.SubID);

				if (entryType != null)
				{
					throw new PXRowPersistingException(typeof(CashAccount.clearingAccount).Name, cashacct.ClearingAccount, Messages.ClearingAccountCanNotBeConfiguredAsClearingAccount, cashacct.CashAccountCD, entryType.EntryTypeId);
				}
			}
        ...
        protected virtual void CashAccountPaymentMethodDetail_DetailValue_FieldVerifying(PXCache cache, PXFieldVerifyingEventArgs e)
        {
			...
			if (isRequired && string.IsNullOrEmpty(newDetailValue))
			{
