...
namespace PX.Objects.CA
{
	public class CAReconEntry : PXGraph<CAReconEntry>, ICaptionable
	{
		...
		public virtual IEnumerable createAdjustment(PXAdapter adapter)
		{
			...
			{
				if (AddFilter.Current?.DocumentCreationContext != true)
				{
					CurrencyInfoAttribute.SetDefaults<AddTrxFilter.curyInfoID>(AddFilter.Cache, AddFilter.Cache.Current);
				}

				SetDocumentCreationContext(true);
				...
				if (result == WebDialogResult.OK || result == WebDialogResult.Yes)
				{
					...
					SetDocumentCreationContext(false);
				}
		...
		{
			AddFilter.Current.DocumentCreationContext = isActive;
			AddFilter.UpdateCurrent();
		}
		#endregion
			...
			var transToReconcile = PXSelectJoin<CATranExt,
												...
												And<Where<CATranExt.reconNbr, Equal<Current<CARecon.reconNbr>>,
													...
													And<Where<CATranExt.tranDate, LessEqual<Current2<CARecon.loadDocumentsTill>>,
														...
														Or<CATranExt.reconNbr, Equal<Current2<CARecon.reconNbr>>>>>>>>>>>>.Select(this).AsEnumerable().Select(result => new { CATran = result.GetItem<CATranExt>(), BAccount = result.GetItem<CR.BAccountR>() });
		...
		private void SelectAdditionalTables(CARecon doc, IEnumerable<CATranExt> transToReconcile, out Dictionary<long?, CABankTran> bankTrans, out Dictionary<long?, CATranVoiding> voidingTrans)
		{
			...
			foreach (var batch in idsCollection.BreakInto(batchSize))
			{
				...
				{
					voidingTrans.AddRange(PXSelect<CATranVoiding,
		...
		public override void Persist()
		{
			...
			list.AddRange((IEnumerable<CATran>)this.Caches[typeof(CATranVoiding)].Updated);
		...
		protected virtual bool Skip(CATran voidingTran, CATran voidedTran = null)
		{
			...
			if (SearchInCache)
			{
				voidingTran = SelectFrom<CATranVoiding>
					.Where<CATranVoiding.voidedTranID.IsEqual<@P.AsLong>>
					.OrderBy<CATranVoiding.voidedTranID.Asc, CATranVoiding.tranID.Asc>
					.View.Select(this, voidedTran.TranID);
		...
		protected virtual void CATranExt_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			//In some cases a voiding CATran can be changed in both chaches CATranExt and CATranVoiding.
			//Changes from CATranVoiding is preferred
			if (skipVoids && row.VoidedTranID != null)
			{
				if (VoidingTrans.Cache.Cached.RowCast<CATranVoiding>().Any(tran => tran.TranID == row.TranID))
				{
					e.Cancel = true;
				}
			}
