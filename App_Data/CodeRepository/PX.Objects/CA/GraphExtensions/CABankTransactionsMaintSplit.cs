	...
	public abstract class CABankTransactionsMaintSplit : PXGraphExtension<CABankTransactionsMaint>
	{
		...
		public PXSelect<
				...
				Where<CABankTran.processed, Equal<False>,
					And<CABankTran.cashAccountID, Equal<Current<CABankTransactionsMaint.Filter.cashAccountID>>,
					And<CABankTran.tranType, Equal<Current<CABankTransactionsMaint.Filter.tranType>>>>>,
			OrderBy<Asc<CABankTran.sortOrder, Desc<CABankTranSplit.splitted, Asc<CABankTran.tranID>>>>> Details;
		...
		protected virtual IEnumerable details()
		{
			...
			PXLongRunStatus status = PXLongOperation.GetStatus(Base.UID, out timespan, out ex);

			IEnumerable<CABankTran> recordsInProcessing = null;
			if (status != PXLongRunStatus.NotExists)
			{
				object[] processingList;
				var customInfo = PXLongOperation.GetCustomInfo(Base.UID, out processingList);
		...
		public virtual IEnumerable Split(PXAdapter adapter)
		{
			...
			if (currentExt != null)
			{
				...
				{
					parent = Base.Details.Locate(new CABankTran { TranID = currentExt.ParentTranID }) ?? CABankTran.PK.Find(Base, currentExt.ParentTranID, PKFindOptions.IncludeDirty);
				}
				else
				{
				...
				Base.Views[nameof(Base.Details)].RequestRefresh();
			}
		...
		{
			if (originalTran.DocumentMatched == true || originalTran.Processed == true)
			{
		...
		[PXOverride]
		public virtual IEnumerable<CABankTran> GetUnprocessedTransactions(GetUnprocessedTransactionsDelegate baseMethod)
		{
			CABankTransactionsMaint.Filter current = Base.TranFilter.Current;
			if (current == null || current.CashAccountID == null) yield break;
			var cmd = new PXSelect<
				CABankTran,
				Where2<Where<CABankTran.processed, Equal<False>,
			...
			OrderBy<Asc<CABankTran.sortOrder, Desc<CABankTranSplit.splitted, Asc<CABankTran.tranID>>>>>(Base);
		...
		public virtual void _(Events.RowSelected<CABankTran> e)
		{
			...
			if (isChild)
			{
				...
				{
					CABankTran parentRow = CABankTran.PK.Find(Base, currentExt.ParentTranID, PKFindOptions.IncludeDirty);
				...
				{
					PXUIFieldAttribute.SetWarning<CABankTranSplit.splittedIcon>(e.Cache, row, null);
				}
		...
		public virtual void CABankTran_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			{
				CABankTran parentRow = (CABankTran)sender.Locate(new CABankTran { TranID = rowExt.ParentTranID }) ?? CABankTran.PK.Find(Base, rowExt.ParentTranID, PKFindOptions.IncludeDirty);
		...
		public virtual void _(Events.RowDeleting<CABankTran> e)
		{
			...
			{
				CABankTran parentRow = (CABankTran)Base.Caches[typeof(CABankTran)].Locate(new CABankTran() { TranID = rowExt.ParentTranID }) ?? CABankTran.PK.Find(Base, rowExt.ParentTranID, PKFindOptions.IncludeDirty);
				if (parentRow?.DocumentMatched == true)
				{
					e.Cancel = true;
					throw new PXException(Messages.ChildTransactionCanBeDeletedbecauseItOrParentHasBeenMatched);
		...
		protected virtual PXSetPropertyException VerifyAmout(PXCache cache, CABankTran row, decimal? newValue, decimal? oldValue)
		{
			...
			CABankTran parentRow = (CABankTran)cache.Locate(new CABankTran { TranID = rowExt.ParentTranID }) ?? CABankTran.PK.Find(Base, rowExt.ParentTranID, PKFindOptions.IncludeDirty);
