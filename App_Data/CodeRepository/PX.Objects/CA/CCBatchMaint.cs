...
namespace PX.Objects.CA
{
	public class CCBatchMaint : PXGraph<CCBatchMaint>
	{
		...
		private void CreateDepositProc(bool isMassProcess)
		{
			...
			foreach (PXResult<Light.ARPayment, CADepositDetail, CADeposit, CashAccountDeposit, PaymentMethod, CCBatchTransaction, CCProcessingCenter, BAccount, Location> res in paymentSelect.Select(batch.ProcessingCenterID, batch.BatchID))
			{
				...
				{
					var bAccount = (BAccount)res;
					var location = (Location)res;
					PaymentInfo paymentInfo = cadGraph.Copy(payment, bAccount, location, new PaymentInfo());
		...
		[Obsolete(InternalMessages.MethodIsObsoleteAndWillBeRemoved2024R2, true)]
		...
		{
			CCBatch ccBatch = BatchView.Current;
		...
		private PXResultset<CCBatchTransaction> GetBatchTransactions(int? batchID, params string[] procStatuses)
		{
		...
		private ExternalTransaction GetExternalTransaction(string cCProcessingCenterID, string tranNumber)
		{
		...
		private ExternalTransaction GetExternalTransactionByNoteID(Guid? noteID)
		{
			ExternalTransactions.Cache.Clear();
			ExternalTransactions.Cache.ClearQueryCache();
