...
namespace PX.Objects.CA
{
	public class CABankTransactionsEnq : PXGraph<CABankTransactionsEnq>
    {
		...
		[PXHidden]
		public class CABankTranHistory : PXBqlTable, IBqlTable
		{
			#region TranID
			...
			#region MatchedReferenceCD
			public abstract class matchedReferenceCD : PX.Data.BQL.BqlString.Field<matchedReferenceCD> { }
			...
			public virtual String MatchedReferenceCD
			{
				get;
				set;
			}
			...
			#region MatchedReferenceName
			public abstract class matchedReferenceName : PX.Data.BQL.BqlString.Field<matchedReferenceName> { }
						
			[PXDBString(255, IsUnicode = true)]
			[PXUIField(DisplayName = "Business Account Name", Visibility = PXUIVisibility.SelectorVisible)]
			[PX.Data.EP.PXFieldDescription]
			[PXPersonalDataField]
			public virtual String MatchedReferenceName
			{
				get;
				set;
			}
		...
		protected virtual Dictionary<Type, Type> GetMapperDictionary()
		{
			return new Dictionary<Type, Type>
			{
				...
				{typeof(CABankTranHistory.matchedReferenceCD), typeof(BAccountR.acctCD)},
				{typeof(CABankTranHistory.matchedReferenceName), typeof(BAccountR.acctName)},
		...
		protected virtual IEnumerable result()
		{
			...
			var mapper = new PXResultMapper(this, map, typeof(CABankTranHistory), typeof(CABankTran), typeof(CATran), typeof(BAccountR));
			PXDelegateResult delegateResult = mapper.CreateDelegateResult();
			
			int startRow = PXView.StartRow;
			int totalRows = 0;
			
			foreach (PXResult<CABankTran, CABankTranMatch, CATran, CABatch, BAccountR> res in Views[nameof(CATran_History)].Select(PXView.Currents,null, mapper.Searches,mapper.SortColumns,mapper.Descendings, mapper.Filters, ref startRow, PXView.MaximumRows, ref totalRows))
			{
				...
				BAccountR bAccount = (BAccountR)res;
				...
				if (catran.ReferenceID == null)
				{
					...
					if (bAccount?.BAccountID == null && catran.ReferenceID != null)
					{
						bAccount = BAccountRec.SelectSingle(catran.ReferenceID.Value);
						updateResult = true;
					}
				}
				...
				if (updateResult)
				{
					var newRes = new PXResult<CABankTran, CABankTranMatch, CATran, CABatch, BAccountR>(tran, match, catran, cABatch, bAccount);
					delegateResult.Add(mapper.CreateResult(newRes));
				}
				else
				{
