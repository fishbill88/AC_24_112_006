...
namespace PX.Objects.CA
{
	public class EntryTypeMaint : PXGraph<EntryTypeMaint>
	{
		...
		protected virtual void CAEntryType_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			if ((e.Operation & PXDBOperation.Command) != PXDBOperation.Delete)
			{
				if (row.UseToReclassifyPayments == true && row.AccountID.HasValue)
				{
					...
					else if (cashAcct?.ClearingAccount == true)
					{
						if (sender.RaiseExceptionHandling<CAEntryType.cashAccountID>(e.Row, cashAcct.CashAccountCD, new PXSetPropertyException(Messages.CashAccountCannotBeUsedAsReclassificationAccountBecauseItIsClearingAccount,PXErrorLevel.Error, cashAcct.CashAccountCD)))
						{
							throw new PXRowPersistingException(typeof(CAEntryType.cashAccountID).Name, cashAcct.CashAccountCD, Messages.CashAccountCannotBeUsedAsReclassificationAccountBecauseItIsClearingAccount, cashAcct.CashAccountCD);
						}
					}
				}
			}
		...
		protected virtual void CAEntryType_UseToReclassifyPayments_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			...
			CAEntryType row = e.Row as CAEntryType;
			if (row.UseToReclassifyPayments == false)
			{
			...
			else
			{
				CashAccount cashAcct = PXSelect<CashAccount,
					Where<CashAccount.accountID, Equal<Required<CashAccount.accountID>>,
						And<CashAccount.subID, Equal<Required<CashAccount.subID>>>>>.SelectWindowed(this, 0, 1, row.AccountID, row.SubID);
				if (cashAcct != null)
				{
					cache.SetValueExt<CAEntryType.cashAccountID>(e.Row, cashAcct.CashAccountID);
				}
			}
		}
		#endregion
