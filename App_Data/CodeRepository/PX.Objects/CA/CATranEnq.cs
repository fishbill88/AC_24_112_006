...
namespace PX.Objects.CA
{
	...
	public class CATranEnq : PXGraph<CATranEnq>
	{
		...
		public partial class CATranExt : CATran
		{
			...
			#endregion
			#region CuryClearedDebitTotalAmt
			public abstract class curyClearedDebitTotalAmt : PX.Data.BQL.BqlDecimal.Field<curyClearedDebitTotalAmt> { }

			[PXDecimal]
			[PXUIField(DisplayName = "Receipt Total")]
			public virtual decimal? CuryClearedDebitTotalAmt
			{
				get;
				set;
			}
			#endregion
			#region CuryClearedCreditTotalAmt
			public abstract class curyClearedCreditTotalAmt : PX.Data.BQL.BqlDecimal.Field<curyClearedCreditTotalAmt> { }

			[PXDecimal]
			[PXUIField(DisplayName = "Disbursement Total")]
			public virtual decimal? CuryClearedCreditTotalAmt
			{
				get;
				set;
			}
		...
		[PXFilterable]
		public PXSelectJoinOrderBy<CATranExt, LeftJoin<BAccountR, On<BAccountR.bAccountID, Equal<CATranExt.referenceID>>>,
			OrderBy<Asc<CATranExt.tranDate, Asc<CATranExt.tranID>>>> CATranListRecords;
				...
				if (filter != null)
				{
					...
					if (filter.AccountID != null && filter.StartDate != null)
					{
						...
						foreach (PXResult<CATranExt> res in CATranListRecords.View.Select(PXView.Currents, PXView.Parameters, new object[0], new string[0], new bool[0], CATranListRecords.View.GetExternalFilters(), ref startRow, 0, ref totalRows))
						{
							...
							filter.DebitClearedTotal += filter.ShowSummary == true ? tran.CuryClearedDebitTotalAmt : tran.CuryClearedDebitAmt;
							filter.CreditClearedTotal += filter.ShowSummary == true ? tran.CuryClearedCreditTotalAmt : tran.CuryClearedCreditAmt;
						}
		...
		public virtual IEnumerable cATranListRecords()
		{
			...
			if (filter != null && filter.ShowSummary == true)
			{
				...
				foreach (CADailySummary daily in CATranListSummarized.View.Select(null, null, new object[PXView.SortColumns.Length], PXView.SortColumns, PXView.Descendings, PXView.Filters, ref startRow, 0, ref totalRows))
				{
					...
					tran.CuryClearedDebitTotalAmt = daily.AmtReleasedClearedDr;
					tran.CuryClearedCreditTotalAmt = daily.AmtReleasedClearedCr;
					...
					if (Filter.Current.IncludeUnreleased == true)
					{
						...
						tran.CuryClearedDebitTotalAmt += daily.AmtUnreleasedClearedDr;
						tran.CuryClearedCreditTotalAmt += daily.AmtUnreleasedClearedCr;
					}
			...
			{
				var sortColumns = PXView.SortColumns;
				if (PXView.MaximumRows == 1
				&& PXView.SortColumns?.Length > 0 && PXView.SortColumns[0].Equals(nameof(CATranExt.tranID), StringComparison.OrdinalIgnoreCase)
				&& PXView.Searches?.Length > 0 && PXView.Searches[0] != null)
				{
					var cached = CATranListRecords.Cache.Locate(new CATranExt { TranID = Convert.ToInt32(PXView.Searches[0]) });
					if (cached != null)
					{
						return new[] { cached };
					}
					...
					{
						var newSort = PXView.SortColumns.ToList<string>(PXView.SortColumns.Length);
						newSort.RemoveAt(0);
						newSort.Add(nameof(CATranExt.tranID));

						sortColumns = newSort.ToArray();
					}
				}
				...
				PXSelectBase<CATranExt> cmd = new PXSelectJoin<CATranExt, LeftJoin<ARPayment, On<ARPayment.docType, Equal<CATranExt.origTranType>,
										...
										Where2<Where<Current<CAEnqFilter.includeUnreleased>, Equal<boolTrue>,
											...
											OrderBy<Asc<CATranExt.tranDate, Asc<CATranExt.tranID>>>>(this);
				int startRow = 0;
				int totalRows = 0;
				foreach (PXResult<CATranExt, ARPayment, BAccountR> iRes in cmd.View.Select(null, null, new object[sortColumns.Length], sortColumns, PXView.Descendings, PXView.Filters, ref startRow, 0, ref totalRows))
				{
	...
	[Serializable]
	public partial class CAEnqFilter : PXBqlTable, IBqlTable
	{
		#region AccountID
