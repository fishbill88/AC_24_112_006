...
namespace PX.Objects.CA
{
	public partial class CABankTransactionsMaint : PXGraph<CABankTransactionsMaint>, ICABankTransactionsDataProvider, IAddARTransaction, IAddAPTransaction
	{
		...
		public PXSelect<
			...
			OrderBy<Asc<CABankTran.tranID>>> Details;
		...
		protected virtual IEnumerable<CABankTran> GetUnprocessedTransactions()
		{
			...
			OrderBy<Asc<CABankTran.tranID>>>(this);
			foreach (CABankTran det in cmd.SelectWithViewContext(new object[] { current.CashAccountID, current.TranType }))
			{
				yield return det;
			}
		}
		...
		public virtual IEnumerable LoadInvoices(PXAdapter adapter)
		{
			...
			if (currentDoc.OrigModule == GL.BatchModule.AR)
			{
				...
				if (dialogResult == WebDialogResult.OK || dialogResult == WebDialogResult.Yes)
				{
					...
					{
						if (!receipt && (invoice.DocType == ARDocType.Invoice || invoice.DocType == ARDocType.PrepaymentInvoice || invoice.DocType == ARDocType.DebitMemo || invoice.DocType == ARDocType.FinCharge))
							continue;
			...
			else if (currentDoc.OrigModule == GL.BatchModule.AP)
			{
				...
				payment.BranchID = Accessinfo.BranchID;
		...
		[Serializable]
		public class CreateRuleSettings : PXBqlTable, IBqlTable
		{
		...
		[PXUIField(MapEnableRights = PXCacheRights.Update, MapViewRights = PXCacheRights.Update, DisplayName = "Reset to Default")]
		[PXButton]
		...
		protected virtual void CABankTranInvoiceMatch_IsMatched_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			{
				if (CreatePaymentIsChosen(Details.Current))
				{
					throw new PXSetPropertyException(Messages.AnotherOptionChosenCreateCheckBoxIsSelected, PXErrorLevel.RowWarning);
				}
				else if (Details.Current.DocumentMatched == true)
				{
					if (Details.Current.MatchedToExpenseReceipt == true)
					{
						throw new PXSetPropertyException(Messages.AnotherOptionChosenBankTransactionIsAlreadyMatchedToExpenseReceipt, PXErrorLevel.RowWarning);
					}

					if (Details.Current.MultipleMatching != true && Details.Current.MatchedToInvoice == true)
			{
				throw new PXSetPropertyException(Messages.AnotherOptionChosen, PXErrorLevel.RowWarning);
			}

					if (Details.Current.MatchedToInvoice != true)
					{
						throw new PXSetPropertyException(Messages.AnotherOptionChosenBankTransactionIsAlreadyMatchedToPayment, PXErrorLevel.RowWarning);
					}
				}
			}
		}
		...
		{
			return bankTran.CreateDocument == true && (bankTran.OrigModule != BatchModule.CA || !string.IsNullOrEmpty(bankTran.EntryTypeID));
		}
		...
		protected virtual void CABankTranInvoiceMatch_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (!sender.ObjectsEqual<CABankTranInvoiceMatch.isMatched>(row, oldRow))
			{
				...
				{
					if (currentTran.CreateDocument == true)
					{
						Details.Cache.SetValueExt<CABankTran.createDocument>(currentTran, false);
					}
		...
		protected virtual void _(Events.FieldVerifying<CABankTranExpenseDetailMatch.isMatched> e)
		{
			...
			if ((bool?)e.NewValue == true)
			{
				...
				if (Details.Current.DocumentMatched == true)
				{
					if (Details.Current.MatchedToInvoice == true)
					{
						throw new PXSetPropertyException(Messages.AnotherOptionChosenBankTransactionIsAlreadyMatchedToInvoice, PXErrorLevel.RowWarning);
					}
					else if (Details.Current.MatchedToExpenseReceipt == true)
			{
				throw new PXSetPropertyException(Messages.AnotherOptionChosen, PXErrorLevel.RowWarning);
			}
					else
		...
		{
			sender.SetDefaultExt<CABankTran.curyTaxTotal>(row);

			foreach (var tax in Taxes.Select())
			{
				Taxes.Delete(tax);
			}

			foreach (var taxTran in TaxTrans.Select())
			{
				TaxTrans.Delete(taxTran);
			}
		}
		...
		public static void CABankTranRowUpdated(CABankTransactionsMaint graph, PXCache sender, PXRowUpdatedEventArgs e, PXSelectBase<CABankTranDetail> tranSplit, PXSelectBase<CABankTranMatch> tranMatch, PXSelectBase<CABankTranAdjustment> adjustments, PXSelectBase<CABankTranMatch> tranMatchCharge, bool matchInvoiceProcess)
		{
			...
			if ((row.CreateDocument == false && oldRow.CreateDocument != row.CreateDocument)
				|| (row.OrigModule != GL.BatchModule.CA && row.OrigModule != GL.BatchModule.EP
						&& (oldRow.OrigModule == GL.BatchModule.CA || oldRow.OrigModule == GL.BatchModule.EP)))
			{
				foreach (CABankTranDetail split in tranSplit.Select())
				{
					tranSplit.Delete(split);
		...
		protected virtual void EnableTranFields(PXCache sender, CABankTran row)
		{
			...
			PXUIFieldAttribute.SetEnabled<CABankTran.createDocument>(sender, row, true);
		...
		protected virtual void CATranExt_IsMatched_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if ((bool?)e.NewValue == true)
			{
				...
				else if (Details.Current.DocumentMatched == true)
				{
					...
					if (Details.Current.MultipleMatchingToPayments != true)
			{
				throw new PXSetPropertyException(Messages.AnotherOptionChosen, PXErrorLevel.RowWarning);
			}
		}
		...
		protected virtual void ValidateDataForDocumentCreation(CABankTran aRow)
		{
				...
				if (aRow.EntryTypeID == null)
			{
				...
				if (CASetup.Current.RequireExtRefNbr == true && string.IsNullOrWhiteSpace(aRow.ExtRefNbr))
				{
					throw new PXSetPropertyException(Messages.ExternalReferenceNumberOfTheBankTransactionIsEmpty);
				}
			}
				...
				if (string.IsNullOrEmpty(aRow.PaymentMethodID))
			{
				...
				if (string.IsNullOrWhiteSpace(aRow.ExtRefNbr))
				{
					PaymentMethod pm = PXSelect<PaymentMethod,
						Where<PaymentMethod.paymentMethodID, Equal<Required<PaymentMethod.paymentMethodID>>>>
						.Select(this, aRow.PaymentMethodID);
					if (AP.PaymentRefAttribute.PaymentRefMustBeUnique(pm))
					{
					throw new PXSetPropertyException(Messages.ExternalReferenceNumberOfTheBankTransactionIsEmpty);
				}
					else if (aRow.ExtRefNbr != null && aRow.ExtRefNbr.StartsWith(" "))
				{
					throw new PXSetPropertyException(Messages.ExternalReferenceNumberOfTheBankTransactionCannotStartWithLeadingSpace);
				}
			}
		}
		}
