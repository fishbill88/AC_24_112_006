...
namespace PX.Objects.CA.BankStatementProtoHelpers
{
	[Serializable]
	[PXHidden]
	public partial class CABankTranDocRef : PXBqlTable, IBqlTable, IBankMatchRelevance
	{
		#region Selected
    ...
    public class PXInvoiceSelectorAttribute : PXCustomSelectorAttribute
	{
		...
		{
			PXSelectBase<ARAdjust.ARInvoice> sel = new PXSelectJoin<ARAdjust.ARInvoice,
									...
									Where<ARAdjust.ARInvoice.customerID, In2<Search<PX.Objects.AR.Override.BAccount.bAccountID,
										...
										Or<Where<PX.Objects.AR.Override.BAccount.consolidatingBAccountID, Equal<Required<CABankTran.payeeBAccountID>>,
											And<PX.Objects.AR.Override.BAccount.consolidateToParent, Equal<True>>>>>>>,
									...
									And<Where<CABankTranAdjustment.adjdRefNbr, IsNull, Or<CABankTran.origModule, NotEqual<BatchModule.moduleAR>>>>>>>>>>>(graph);

			if (PXAccess.FeatureInstalled<FeaturesSet.vATRecognitionOnPrepayments>())
			{
				sel.WhereAnd<Where<ARInvoice.docType, NotEqual<ARDocType.prepaymentInvoice>,
										Or<ARInvoice.docType, Equal<ARDocType.prepaymentInvoice>, And<ARInvoice.pendingPayment, Equal<True>>>>>();
			}

			foreach (PXResult<ARAdjust.ARInvoice, CABankTranMatch> result in
								sel.Select(currentBankTran.TranID, AdjdDocType, TranID, AdjNbr, AdjNbr, currentBankTran.PayeeBAccountID, currentBankTran.PayeeBAccountID, AdjdDocType))
			{
		...
		public static IEnumerable<APAdjust.APInvoice> GetRecordsAP(string AdjdDocType, int? TranID, int? AdjNbr, CABankTran currentBankTran, PXCache adjustments, PXGraph graph)
		{
								 ...
								 .Select(graph, currentBankTran.TranID, TranID, AdjNbr, AdjNbr, currentBankTran.PayeeBAccountID, AdjdDocType, currentBankTran.DocType, currentBankTran.DocType, currentBankTran.DocType, currentBankTran.DocType, currentBankTran.MatchingPaymentDate, currentBankTran.MatchingFinPeriodID, currentBankTran.DocType))
			{
				...
				bool hasDiffrentInvoiceBaseCurrency = PXAccess.GetBranch(aPInvoice.BranchID).BaseCuryID != PXAccess.GetBranch(graph.Accessinfo.BranchID).BaseCuryID;

				if (hasDiffrentInvoiceBaseCurrency || ShouldSkipRecord(match, aPInvoice, adjustments, TranID, AdjNbr, graph, BatchModule.AP))
				{
					continue;
				}
	...
	public static class StatementsMatchingProto
	{
			...
			else if (detail.OrigModule == GL.BatchModule.AR)
			{
				...
				else if (detail.DocType == ARDocType.Payment || detail.DocType == ARDocType.VoidPayment)
				{
					...
					PXStringListAttribute.SetList<CABankTranAdjustment.adjdDocType>(cache, null,
						new string[] { ARDocType.Invoice, ARDocType.DebitMemo, ARDocType.CreditMemo, ARDocType.FinCharge, ARDocType.PrepaymentInvoice },
						new string[] { AR.Messages.Invoice, AR.Messages.DebitMemo, AR.Messages.CreditMemo, AR.Messages.FinCharge, AR.Messages.PrepaymentInvoice });
				}
				else
				{
					PXDefaultAttribute.SetDefault<CABankTranAdjustment.adjdDocType>(cache, ARDocType.Invoice);
					PXStringListAttribute.SetList<CABankTranAdjustment.adjdDocType>(cache, null,
						new string[] { ARDocType.Invoice, ARDocType.DebitMemo, ARDocType.FinCharge, ARDocType.PrepaymentInvoice },
						new string[] { AR.Messages.Invoice, AR.Messages.DebitMemo, AR.Messages.FinCharge, AR.Messages.PrepaymentInvoice });
				}
			}
		}
		...
		public static decimal CompareExpenseReceiptAmount(CABankTran aDetail, decimal expenseAmount, decimal diffTreshold)
		{
			return CompareExpenseReceiptAmount(-aDetail.CuryTranAmt.GetValueOrDefault(), expenseAmount, diffTreshold);
		}

		private static decimal CompareExpenseReceiptAmount(decimal tranAmount, decimal expenseAmount, decimal diffTreshold)
		{
			double diff = Convert.ToDouble(tranAmount - expenseAmount);

			double sigma = Convert.ToDouble(diffTreshold * tranAmount) / 100;

			if (sigma == 0)
				return 0m;

			decimal res = (decimal)Math.Exp(-(diff * diff / (2 * sigma * sigma))); //Normal Distribution 
