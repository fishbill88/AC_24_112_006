...
namespace PX.Objects.CA
{
	...
	public class CashFlowEnq : PXGraph<CashFlowEnq>
	{
		...
		[Serializable]
		public partial class CashFlowFilter : PXBqlTable, PX.Data.IBqlTable
		{
		...
		protected virtual void RetrieveARInvoices(Dictionary<FlowKey, CashFlowForecast> flow, CashFlowFilter filter)
		{
			PXSelectBase<Light.ARInvoice> arInvSelect = new PXSelectReadonly2<Light.ARInvoice,
							...
							Where<IIf<Where<Light.ARInvoice.dueDate, IsNull, And<Light.ARInvoice.docType, Equal<ARDocType.creditMemo>>>, Light.ARInvoice.docDate, Light.ARInvoice.dueDate>, Less<Required<Light.ARInvoice.dueDate>>,
								And<Light.ARInvoice.docType, NotEqual<ARDocType.cashSale>,
								And<Light.ARInvoice.docType, NotEqual<ARDocType.cashReturn>,
								And<Light.ARInvoice.docType, NotEqual<ARDocType.prepaymentInvoice>,
								And2<Not<Where<Light.ARInvoice.docType, Equal<ARDocType.creditMemo>,
									And<Light.ARInvoice.origDocType, Equal<ARDocType.prepaymentInvoice>>>>,
			...
			foreach (PXResult<Light.ARInvoice, CurrencyInfo, CashAccount> iRes in arInvSelect.Select(endDate, filter.CashAccountID))
			{
				...
				DateTime date = iDoc.DueDate == null && iDoc.DocType == ARDocType.CreditMemo ? iDoc.DocDate.Value : iDoc.DueDate.Value;		

				rec.CuryID = this.ConvertDocAmount(ref curyValue, ref value, iCuryInfo, iAccount, filter);
				AddAmount(rec, date, curyValue, value, sign);
			}
		}
		...
		protected virtual void RetrieveARPayments(Dictionary<FlowKey, CashFlowForecast> flow, CashFlowFilter filter)
		{
			PXSelectBase<ARPayment> arPaymentSelect = new PXSelectReadonly2<ARPayment, LeftJoin<ARAdjust,
										...
										Where<ARPayment.docDate,
											...
											And<ARPayment.docType, NotEqual<ARPaymentType.prepaymentInvoice>,
											And2<Not<Where<ARPayment.docType, Equal<ARPaymentType.creditMemo>,
												And<ARPayment.origDocType, Equal<ARPaymentType.prepaymentInvoice>>>>,
