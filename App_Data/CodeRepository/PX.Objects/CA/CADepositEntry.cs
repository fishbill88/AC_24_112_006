...
using BAccount = PX.Objects.CA.Light.BAccount;
using Location = PX.Objects.CA.Light.Location;
...
namespace PX.Objects.CA
{
	...
	public class CADepositEntry : PXGraph<CADepositEntry, CADeposit>
	{
		...
		[Serializable]
		public partial class PaymentFilter : PXBqlTable, IBqlTable
		{
		...
		public PXSelectJoin<CADepositDetail, LeftJoin<PaymentInfo, On<CADepositDetail.origDocType, Equal<PaymentInfo.docType>,
						...
						InnerJoin<BAccount, On<BAccount.bAccountID, Equal<PaymentInfo.bAccountID>>,
						InnerJoin<Location, On<Location.locationID, Equal<PaymentInfo.locationID>>>>>,
		...
		public PXSelectJoin<PaymentInfo,
							LeftJoin<BAccount, On<BAccount.bAccountID, Equal<PaymentInfo.bAccountID>>,
							LeftJoin<Location, On<Location.locationID, Equal<PaymentInfo.locationID>>,
		...
		public PXSelect<BAccount> bAccountDummy;
		...
		protected virtual IEnumerable GetAvailablePayments()
		{
			...
			PXCache cache = this.Caches[typeof(PaymentInfo)];
			{
				PXSelectBase<Light.ARPayment> paymentSelect = new PXSelectJoin<Light.ARPayment,
									...
									InnerJoin<BAccount, On<BAccount.bAccountID, Equal<Light.ARPayment.customerID>>,
									InnerJoin<Location, On<Location.locationID, Equal<Light.ARPayment.customerLocationID>>>>>>>>,
									Where<CashAccountDeposit.cashAccountID, Equal<Current<CADeposit.cashAccountID>>,
				...
				foreach (PXResult<Light.ARPayment, CADepositDetail, CADeposit, CashAccountDeposit, PaymentMethod, BAccount, Location> it in paymentSelect.Select())
				{
					...
					if (exist) continue;
					BAccount bAccount = it;
					Location location = it;
					PaymentInfo paymentInfo = Copy(payment, bAccount, location, new PaymentInfo());
					try
					{
...

			{
				PXSelectBase<Light.APPayment> apPaymentSelect = new PXSelectJoin<Light.APPayment,
									...
									InnerJoin<BAccount, On<BAccount.bAccountID, Equal<Light.APPayment.vendorID>>,
									InnerJoin<Location, On<Location.locationID, Equal<Light.APPayment.vendorLocationID>>>>>>>>,
									Where<CashAccountDeposit.cashAccountID, Equal<Current<CADeposit.cashAccountID>>,
									And<Light.APPayment.docType, In3<APDocType.refund, APDocType.voidRefund, APDocType.cashReturn>,
				...
				foreach (PXResult<Light.APPayment, CADepositDetail, CADeposit, CashAccountDeposit, PaymentMethod, BAccount, Location> it in apPaymentSelect.Select())
				{
					Light.APPayment payment = it;
					CADepositDetail detail = it;
					PaymentMethod pmDef = it;
					if (payment.DocType != APPaymentType.Refund && payment.DocType != APPaymentType.VoidRefund && payment.DocType != APDocType.CashReturn) continue;
...

			{
				...
				foreach (PXResult<Light.CAAdj, CADepositDetail, CADeposit, CashAccountDeposit> it in CAAdjSelect.Select())
				{
					...
					yield return paymentInfo;
					//yield return new PXResult<PaymentInfo, BAccount, Location>(paymentInfo, null, null);
				}
			}
		}
		...
		{
			foreach (PXResult<CADepositDetail, Light.ARPayment, BAccount, Location> it in PXSelectJoin<CADepositDetail,
									...
									And<CADepositDetail.tranType, Equal<Current<CADeposit.tranType>>>>>>.Select(this))
			{
				...
				{
					pmInfo = Copy(paymentAR, bAccount, location, pmInfo);
				}
				PXResult<CADepositDetail, PaymentInfo, BAccount, Location> result = new PXResult<CADepositDetail, PaymentInfo, BAccount, Location>(detail, pmInfo, bAccount, location);
				yield return result;
			}

			foreach (PXResult<CADepositDetail, Light.APPayment, BAccount, Location> it in PXSelectJoin<CADepositDetail,
								 ...
								 And<CADepositDetail.tranType, Equal<Current<CADeposit.tranType>>>>>>.Select(this))
			{
				...
				{
					pmInfo = Copy(paymentAP, bAccount, location, pmInfo);
				}
				PXResult<CADepositDetail, PaymentInfo, BAccount, Location> result = new PXResult<CADepositDetail, PaymentInfo, BAccount, Location>(detail, pmInfo, bAccount, location);
				yield return result;
			}
									...
									And<CADepositDetail.tranType, Equal<Current<CADeposit.tranType>>>>>>.Select(this))
			{
				...
				PXResult<CADepositDetail, PaymentInfo, BAccount, Location> result = new PXResult<CADepositDetail, PaymentInfo, BAccount, Location>(detail, pmInfo, null, null);
				yield return result;
			}
		}
		...
		[PXUIField(DisplayName = "Customer/Vendor", Visibility = PXUIVisibility.Visible)]
		protected virtual void _(Events.CacheAttached<Light.BAccount.acctCD> e) { }

		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXUIField(DisplayName = "Account Name", Visibility = PXUIVisibility.Visible)]
		...
		[PXUIField(DisplayName = CR.Messages.Location, Visibility = PXUIVisibility.Visible)]
		...
		public virtual PaymentInfo Copy(Light.ARPayment source, BAccount bAccount, Location location, PaymentInfo destination)
		{
			destination.Module = GL.BatchModule.AR;
			destination.DocType = source.DocType;
			destination.RefNbr = source.RefNbr;
		...
		public virtual PaymentInfo Copy(Light.APPayment source, BAccount bAccount, Location location, PaymentInfo destination)
		{
			destination.Module = GL.BatchModule.AP;
			destination.DocType = source.DocType;
			destination.RefNbr = source.RefNbr;
		...
		public virtual void VoidDepositProc(CADeposit doc)
		{
			...
			if (newdoc.ExtraCashAccountID != null && newdoc.ExtraCashAccountID == newdoc.CashAccountID)
			{
				newdoc.ExtraCashAccountID = null;
			}
