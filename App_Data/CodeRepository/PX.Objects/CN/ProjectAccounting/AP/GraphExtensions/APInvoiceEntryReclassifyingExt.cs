...
namespace PX.Objects.CN.ProjectAccounting.AP.GraphExtensions
{
	...
	public class APInvoiceEntryReclassifyingExt : PXGraphExtension<APInvoiceEntry>
	{
		...
		public PXSelect<PMTran,
			Where<PMTran.origModule, Equal<BatchModule.moduleAP>,
			And<PMTran.origTranType, Equal<Current<APInvoice.docType>>,
			And<PMTran.origRefNbr, Equal<Current<APInvoice.refNbr>>,
			And<PMTran.billed, NotEqual<True>,
			And<PMTran.proformaRefNbr, IsNotNull>>>>>> LinkedProgressiveTransactions;
		...
		private bool MarkChangedLines(APInvoiceEntry graph)
		{
			...
			foreach (PXResult<GLTran> result in glTrans)
			{
				...
				{
					graph.Transactions.Cache.SetValue<APTranExt.projectReclassified>(apTran, true);
				}
		...
		private void ReverseOrigBatch(JournalEntry je, PostGraph pg, APReleaseProcess release, Batch batch)
		{
			...
			//je.BatchModule.Current.Module = batch.Module; //by default reversing sets module to GL, we need to set it back to source
			//
			je.BatchModule.SetValueExt<Batch.module>(je.BatchModule.Current, batch.Module);
			PXCache gltrans = je.Caches[typeof(PX.Objects.GL.GLTran)];
			...
			{
				je.BatchModule.Current.Approved = true;
				je.releaseFromHold.Press();
			}
			je.Save.Press();
			pg.ReleaseBatchProc(je.BatchModule.Current, true);
			if (release.AutoPost)
			{
		...
		private void UpdatePOLine(APReleaseProcess release, APInvoice apDoc, APTran apTran, int poLineNbr, bool open)
		{
			...
			var poRes = (PXResult<POLine, POOrder>)updatePO.poOrderLineUPD.Select(apTran.POOrderType, apTran.PONbr, poLineNbr);
			...
			if (poRes != null)
			{
				...
				updLine = updatePO.UpdatePOLine(apTran, apDoc, (POOrder)poRes, updLine, false);
			}
		}
		...
		{
			foreach (PMTran tran in LinkedProgressiveTransactions.Select())
			{
				bool clearLink = false;

				if (tran.ProformaRefNbr != null)
				{
					APTran key = new APTran();
					key.TranType = tran.OrigTranType;
					key.RefNbr = tran.OrigRefNbr;
					key.LineNbr = tran.OrigLineNbr;

					APTran aPTran = graph.Transactions.Locate(key);
					if (aPTran != null)
					{
						APTranExt extAPTran = graph.Transactions.Cache.GetExtension<APTranExt>(aPTran);
						if (extAPTran != null && extAPTran.ProjectReclassified == true)
						{
							clearLink = true;
						}
					}
				...
				if (clearLink)
				{
					...
					graph.Caches[typeof(PMTran)].Update(tran);
				}
			}
		}
	...
	public sealed class APTranExt : PXCacheExtension<APTran>
	{
		...
		#region ProjectReclassified
		public abstract class projectReclassified : PX.Data.BQL.BqlInt.Field<projectReclassified> { }
		/// <summary>
		/// True if the line's Project was changed
		/// </summary>
		[PXBool]
