...
namespace PX.Objects.CN.Compliance.AP.GraphExtensions
{
    public class ApInvoiceEntryExt : PXGraphExtension<ComplianceViewEntityExtension<APInvoiceEntry, APInvoice>, APInvoiceEntry>
    {
		...
		{
			var projectHasExpiredCompliance = service.ValidateProjectVendorRelatedField<APTran, APTran.projectID>(transaction,
				transaction.ProjectID, transaction.VendorID);
			var taskHasExpiredCompliance = service.ValidateRelatedField<APTran, ComplianceDocument.costTaskID, APTran.taskID>(transaction,
				transaction.TaskID);
			var purchaseOrderOrSubcontractHasExpiredCompliance = IsSubcontract(transaction)
				? service.ValidateRelatedField<APTran, ComplianceDocument.subcontract, ApTranExt.subcontractNbr>(
					transaction, transaction.PONbr)
			...
			service.ValidateRelatedRow<APTran, CacheExtensions.ApTranExt.hasExpiredComplianceDocuments>(transaction,
