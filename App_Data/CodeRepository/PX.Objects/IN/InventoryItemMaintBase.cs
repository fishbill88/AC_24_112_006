...
namespace PX.Objects.IN
{
	public abstract class InventoryItemMaintBase : PXGraph<InventoryItemMaintBase, InventoryItem>
	{
		...
		public
			...
			Where<INKitSpecStkDet.FK.ComponentInventoryItem.SameAsCurrent>.
			View kitspecs;

		public
			SelectFrom<INKitSpecNonStkDet>.
			Where<INKitSpecNonStkDet.FK.ComponentInventoryItem.SameAsCurrent>.
			View kitnonstockdet;
		...
		protected virtual void _(Events.RowDeleting<INUnit> e) => VerifyKitComponentsUOM(e.Row);
		...
		{
			foreach (INKitSpecStkDet kitStockComponentLink in kitspecs.Select())
				if (kitStockComponentLink.UOM == conversion.FromUnit)
					throw new PXException(
						Msg.CannotDeleteUnitSinceItIsReservedByKitComponent,
						GetFormattedInventoryCD(Item.Current),
						GetFormattedInventoryCD(INKitSpecStkDet.FK.KitInventoryItem.FindParent(this, kitStockComponentLink)));

			foreach (INKitSpecNonStkDet kitNonStockComponentLink in kitnonstockdet.Select())
				if (kitNonStockComponentLink.UOM == conversion.FromUnit)
					throw new PXException(
						Msg.CannotDeleteUnitSinceItIsReservedByKitComponent,
						GetFormattedInventoryCD(Item.Current),
						GetFormattedInventoryCD(INKitSpecNonStkDet.FK.KitInventoryItem.FindParent(this, kitNonStockComponentLink)));

			string GetFormattedInventoryCD(InventoryItem item) => this.Caches<InventoryItem>().GetStateExt<InventoryItem.inventoryCD>(item)?.ToString();
		}
