...
namespace PX.Objects.IN.WMS
{
		...
		where TDocType : IConstant, IBqlOperand, new()
	{
		...
		[PXUnboundDefault(typeof(
				 INTranType.transfer.When<RegisterScanHeader.docType.FromCurrent.IsEqual<INDocType.transfer>>.
			Else<INTranType.issue>	.When<RegisterScanHeader.docType.FromCurrent.IsEqual<INDocType.issue>>.
			Else<INTranType.receipt>.When<RegisterScanHeader.docType.FromCurrent.IsEqual<INDocType.receipt>>.
			ElseNull))]
