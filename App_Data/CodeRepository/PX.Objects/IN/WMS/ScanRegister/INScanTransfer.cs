...
namespace PX.Objects.IN.WMS
{
	...
	public class INScanTransfer : WMSBase
	{
		...
		#endregion
		#region SkipAvailabilityWarning
		public bool? SkipAvailabilityWarning
		{
			get => TransferHeader.SkipAvailabilityWarning;
			set => TransferSetter.Set(h => h.SkipAvailabilityWarning, value);
		}
		#endregion
		#endregion
		...
		public sealed class TransferMode : ScanMode
		{
			...
			protected override IEnumerable<ScanQuestion<INScanTransfer>> CreateQuestions()
			{
				yield return new ConfirmState.SkipAvailabilityQuestion();
			}
			...
			public sealed class ConfirmState : ConfirmationState
			{
				...
				public class Logic : ScanExtension
				{
					...
					protected virtual FlowStatus ConfirmAdd()
					{
						...
						if (existTransaction != null)
						{
							...
							using (string.IsNullOrEmpty(Basis.LotSerialNbr) ? null : Graph.LineSplittingExt.SkipUnassignedExceptionScope())
					...
					protected virtual bool HasAvailabilityError(INTran tran, out FlowStatus error)
					{
						...
						bool isWarning = errorInfo != null && PXErrorLevelUtils.IsWarning(errorInfo.ErrorLevel.Value);

						Func<string, object[], FlowStatus> getFlowStatus = (msg, args) =>
							isWarning ? FlowStatus.Warn<SkipAvailabilityQuestion>(msg, args) : FlowStatus.Fail(msg, args);

						if (errorInfo != null && (!isWarning || Basis.SkipAvailabilityWarning != true))
						{
							PXCache lsCache = Basis.Graph.transactions.Cache;
							error = getFlowStatus(errorInfo.MessageFormat, new object[]
							{
					...
					protected virtual INTran FindTransferRow()
					{
						...
						{
							if (Basis.LotSerialTrack.IsTracked && Basis.UseDefaultLotSerialNbr == true)
							{
								if (Basis.Remove == true)
									existTransaction = existTransactions.LastOrDefault();
								else if (Basis.LotSerialTrack.IsEnterable)
									existTransaction = null;
								else
				...
				#region Questions
				public sealed class SkipAvailabilityQuestion : ScanQuestion
				{
					public ConfirmState.Logic Mode => Get<ConfirmState.Logic>();

					public override string Code => "SKIPAVAILABILITY";

					protected override void Confirm()
					{
						Basis.SkipAvailabilityWarning = true;
						Mode.Confirm();
					}
	...
	public sealed class TransferScanHeader : PXCacheExtension<RegisterScanHeader, WMSScanHeader, QtyScanHeader, ScanHeader>
	{
		...
		#region SkipAvailabilityWarning
		/// <summary>
		/// Skip availability warning
		/// </summary>
		[PXBool]
		public bool? SkipAvailabilityWarning { get; set; }
		public abstract class skipAvailabilityWarning : BqlBool.Field<skipAvailabilityWarning> { }
		#endregion
	}
}
