...
namespace PX.Objects.IN
{
	public class INAdjustmentEntry : INRegisterEntryBase
	{
		...
		protected override void _(Events.RowPersisting<INTran> e)
		{
			base._(e);
		...
		public class LineSplittingExtension : GraphExtensions.INRegisterLineSplittingExtension<INAdjustmentEntry>
		{
			...
			protected override bool AllowSplitCreationForLineWithEmptyLotSerialNbr(INLotSerTrack.Mode mode) => true;
		...
		public override bool PrepareImportRow(string viewName, IDictionary keys, IDictionary values)
		{
			...
			{
				if (values.Contains(nameof(InventoryItem.inventoryID)) && values[nameof(InventoryItem.inventoryID)] != null)
				{
					InventoryItem inventoryItem = PXSelect<InventoryItem, Where<InventoryItem.inventoryCD, Equal<Required<InventoryItem.inventoryCD>>>>.Select(this, values[nameof(InventoryItem.inventoryID)].ToString());
					
					if (inventoryItem == null || inventoryItem.ItemStatus == InventoryItemStatus.Inactive || inventoryItem.ItemStatus == InventoryItemStatus.MarkedForDeletion)
					{
						return false;
					}
					return true;
