...
namespace PX.Objects.IN.GraphExtensions
{
		...
		where TSplit : class, IBqlTable, ILSDetail, new()
	{
		...
		public virtual IEnumerable GenerateNumbers(PXAdapter adapter)
		{
			...
			if (opt == null || opt.ExtensionName != TypePrefixed(nameof(LotSerOptions)))
				opt = (LotSerOptions)GetLotSerialOpts().FirstOrDefault_();
			...
			try
			{
				...
				foreach (TSplit split in PXParentAttribute.SelectSiblings(SplitCache, null, typeof(TLine)))
				{
					...
					SplitCache.RaiseFieldUpdated(nameof(ILSDetail.Qty),split,copy.Qty);
		...
		#region Single Split
		protected virtual bool NeedToEnsureSingleSplit(INLotSerTrack.Mode mode, [INLotSerTrack.List] string itemLotSerialTrack, TLine row, TLine oldRow = null)
		{
			bool baseQtyChecks;
			bool oldRowChecks;
			...
			{
				oldRowChecks = true;
				baseQtyChecks = row.BaseQty == 0m || row.BaseQty == 1m;
			}
			else if ((mode & INLotSerTrack.Mode.Issue) > 0)
			{
				baseQtyChecks = row.BaseQty == 1m;
				oldRowChecks = oldRow?.UnassignedQty == 0;
			}
			else return false;

			bool lotSerialNbrChecks = AllowSplitCreationForLineWithEmptyLotSerialNbr(mode) || string.IsNullOrEmpty(row.LotSerialNbr) == false;
			bool itemLotSerialTrackChecks = itemLotSerialTrack != INLotSerTrack.SerialNumbered;

			return lotSerialNbrChecks && (baseQtyChecks || (itemLotSerialTrackChecks && oldRowChecks));
		}

		protected virtual bool AllowSplitCreationForLineWithEmptyLotSerialNbr(INLotSerTrack.Mode mode) => false;
		#endregion
		...
		protected virtual void EventHandlerInternal(ManualEvent.Row<TLine>.Inserted.Args e) // former _Master_RowInserted
		{
			...
			if (item != null && (((InventoryItem)item).StkItem == true || ((InventoryItem)item).KitItem != true))
			{
				...
				if (mode == INLotSerTrack.Mode.None || (mode & INLotSerTrack.Mode.Create) > 0)
				{
					if (NeedToEnsureSingleSplit(mode, ((INLotSerClass)item).LotSerTrack, e.Row))
					{
						EnsureSingleSplit(e.Row, e.Row.BaseQty.Value);
					}
					else
					{
			...
			else
			{
				...
				if (item.StkItem == true || item.KitItem != true)
				{
					...
					if (mode == INLotSerTrack.Mode.None || (mode & INLotSerTrack.Mode.Create) > 0)
					{
						...
						if (NeedToEnsureSingleSplit(mode, itemLotSerTrack, e.Row))
						{
							EnsureSingleSplit(e.Row, e.Row.BaseQty.Value - e.OldRow.BaseQty.Value);
						}
					...
					else if ((mode & INLotSerTrack.Mode.Issue) > 0)
					{
						...
						else if (NeedToEnsureSingleSplit(mode, itemLotSerTrack, e.Row, e.OldRow))
						{
							EnsureSingleSplit(e.Row, e.Row.BaseQty.Value - e.OldRow.BaseQty.Value);
						}
						else if (e.Row.BaseQty > e.OldRow.BaseQty)
		...
		protected virtual (decimal, TSplit) IssueLotSerials(TLine line, decimal deltaBaseQty, PXCache statusCache, PXCache statusAccumCache, PXResult<InventoryItem, INLotSerClass> item)
		{
			...
			if ((GetTranTrackMode(line, lotSerClass) & INLotSerTrack.Mode.Issue) > 0)
			{
				...
				foreach (ILotSerial lsmaster in lotSerialStatuses)
				{
					...
					if (UseAvailabilityToIssueLotSerials(line, split))
					{
		...
		protected virtual bool UseAvailabilityToIssueLotSerials(TLine line, TSplit split)
		{
			...
			return signs.SignQtyAvail.IsMinus;
		}
		...
		public virtual void UpdateParent(TLine line, TSplit newSplit, TSplit oldSplit, out decimal baseQty)
		{
			...
			switch (CurrentCounters.RecordCount)
			{
				...
				default:
					...
					if (CurrentCounters.ProjectID != null)
					{
						line.ProjectID = CurrentCounters.ProjectID;
						line.TaskID = CurrentCounters.TaskID;
					}
