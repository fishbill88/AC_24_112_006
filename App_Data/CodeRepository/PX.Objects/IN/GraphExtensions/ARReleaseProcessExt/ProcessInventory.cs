...
namespace PX.Objects.IN.GraphExtensions.ARReleaseProcessExt
{
	public class ProcessInventory : PXGraphExtension<ARReleaseProcess>
	{
		...
		/// <summary>
		/// Overrides <see cref="ARReleaseProcess.ReleaseInvoiceTransactionPostProcessed(JournalEntry, ARInvoice, ARTran)"/>
		/// </summary>
		[PXOverride]
			...
			if (n.InventoryID != null && (n.LineType.IsIn(null, SOLineType.MiscCharge) || n.LineType.IsIn(SOLineType.Inventory, SO.SOLineType.NonInventory) && !tranCostSet))
			{
				...
				if (result != null)
				{
					...
					if (isStockItem == true)
					{
						...
						{
							n.TranCostOrig = n.BaseQty * itemSite.TranUnitCost;
						}
						else
						{
							INItemCost itemCost = PXSelectReadonly<INItemCost,
								Where<INItemCost.inventoryID, Equal<Required<ARTran.inventoryID>>,
									And<INItemCost.curyID, EqualBaseCuryID<Current<ARRegister.branchID>>>>>
								.Select(Base, n.InventoryID);
							if (itemCost?.TranUnitCost != null)
							{
								n.TranCostOrig = n.BaseQty * itemCost.TranUnitCost;
							}
						}
					}
