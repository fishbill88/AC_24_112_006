...
namespace PX.Objects.IN.Matrix.GraphExtensions
{
		...
		where TMainItem : class, IBqlTable, new()
	{
		protected class EndCreateUpdateOperationCustomInfo : IPXCustomInfo
		{
			...
			public EndCreateUpdateOperationCustomInfo(int? templateID, string columnAttributeID, string rowAttributeID, string[] additionalAttributes)
			{
				_templateID = templateID;
				_columnAttributeID = columnAttributeID;
				_rowAttributeID = rowAttributeID;
			...
			public void Complete(PXLongRunStatus status, PXGraph graph)
			{
				...
				graph.RowSelecting.AddHandler<InventoryItem>(SetFilter);
			}

			private void SetFilter(Events.RowSelecting<InventoryItem> e)
			{
				if (e.Row.InventoryID == _templateID)
				{
					var graph = e.Cache.Graph;
					var createMatrixItemsExt = graph.FindImplementation<CreateMatrixItemsExt<TGraph, TMainItem>>();
					// Acuminator disable once PX1042 DatabaseQueriesInRowSelecting [false positive]
					createMatrixItemsExt?.SetFilter(_templateID, _columnAttributeID, _rowAttributeID, _additionalAttributes);

					graph.RowSelecting.RemoveHandler<InventoryItem>(SetFilter);
				}
			}
		}
		...
		public virtual IEnumerable CreateUpdate(PXAdapter adapter)
		{
			...
			if (templateItem != null && itemsToCreate.Any())
			{
				...
				PXLongOperation.StartOperation(Base, delegate()
				{
					...
					if (!isImportOrApi)
					{
						PXLongOperation.SetCustomInfo(new EndCreateUpdateOperationCustomInfo(
							templateItem.InventoryID, columnAttributeID, rowAttributeID, additionalAttributes));
					}
				});
			}
