...
namespace PX.Objects.IN
{
    ...
    [Serializable]
    public partial class InventoryTranHistEnqFilter : PXBqlTable, PX.Data.IBqlTable
    {
        #region StartDate
...
*/
    ...
    #region ResultSet
    [Serializable]
    public partial class InventoryTranHistEnqResult : PXBqlTable, PX.Data.IBqlTable
    {
        #region GridLineNbr
...
*/
    ...
    {
		private readonly string INTranPrefix = $"{nameof(INTran)}__";
		private readonly string INTranSplitPrefix = $"{nameof(INTranSplit)}__";
		...
		protected virtual IEnumerable resultRecords()
        {
			...
			PXUIFieldAttribute.SetVisible<InventoryTranHistEnqResult.unitCost>(ResultRecords.Cache, null, !summaryByDay);
			...
			if (IsSortedByDateDescending())
			{
				...
				{
					CalculateBegAndEndQty((PXResult<InventoryTranHistEnqResult>)list[i]);
				}
			}
			else
			{
		...
		protected virtual IEnumerable internalResultRecords()
	    {
		    ...
		    if (filter.StartDate != null)
		    {
				...
				if (filterByLotSerial)
                {
					foreach (PXResult<INTranSplit> inTranSplit in cmdBeginBalanceLS.View.Select(new object[0], new object[] { filter.StartDate }, new object[0],
							new string[0], new bool[0], newFilters, ref splitStartRow, 0, ref splitTotalRows))
                    {
						INTranSplit split = inTranSplit;
                ...
                else
                {
					splitStartRow = 0;
					splitTotalRows = 0;
					var begBalanceFilters = GetBeginningBalanceFilters();
					foreach (PXResult<INItemSiteHistByDay, INItemSiteHistDay> res in cmdBegBalanceNew.View.Select(new object[0], new object[] { filter.StartDate }, new object[0],
							new string[0], new bool[0], begBalanceFilters, ref splitStartRow, 0, ref splitTotalRows))
                    {
			...
			RecalculateTotalColumns(summaryByDay, cumulativeQty, resultList);
		...
		private void RecalculateTotalColumns(bool summaryByDay, decimal cumulativeQty, List<PXResult<InventoryTranHistEnqResult, INTran, INTranSplit>> resultList)
		{
			if (IsSortedByDateDescending())
			{
				for(var i = resultList.Count - 1; i >= 0; i--)
				{
					CalculateTotal(resultList[i]);
				}
			}
			else
			{
			...
			void CalculateTotal(PXResult<InventoryTranHistEnqResult, INTran, INTranSplit> item)
			{
				InventoryTranHistEnqResult it = (InventoryTranHistEnqResult)item;
				it.BegQty = cumulativeQty;
				if (summaryByDay)
				{
					cumulativeQty += (it.QtyIn - it.QtyOut) ?? 0m;
				}
				else
				{
					INTranSplit ts_rec = (INTranSplit)item;
					cumulativeQty += (ts_rec.InvtMult * ts_rec.BaseQty) ?? 0m;
				}

				it.EndQty = it.BegQty + it.QtyIn - it.QtyOut;
			}
		}

		private bool IsSortedByDateDescending()
		{
			var dateIndex = Array.FindIndex(PXView.SortColumns, f => f.Equals(INTranPrefix + nameof(INTran.ReleasedDateTime), StringComparison.OrdinalIgnoreCase));
			if (dateIndex == -1)
			{
				dateIndex = Array.FindIndex(PXView.SortColumns, f => f.Equals(nameof(InventoryTranHistEnqResult.TranDate), StringComparison.OrdinalIgnoreCase));
			}
			return dateIndex != -1 && PXView.Descendings.Length > dateIndex && PXView.Descendings[dateIndex] == true;
		}

		private PXFilterRow[] GetBeginningBalanceFilters()
		{
			if (!PXView.Filters.Any_()) return new PXFilterRow[0];
			var filters = new List<PXFilterRow>();
			var fieldsOfHistoryObj = new string[]
						{
							nameof(INItemSiteHistDay.SiteID),
							nameof(INItemSiteHistDay.LocationID),
							nameof(INItemSiteHistDay.SubItemID),
							nameof(INItemSiteHistDay.QtyIn),
							nameof(INItemSiteHistDay.QtyOut)
						};

			foreach (PXFilterRow filter in PXView.Filters)
			{
				string field = filter.DataField;
				if (field.StartsWith(INTranPrefix))
				{
					field = field.Substring(INTranPrefix.Length);
				}
				else if (field.StartsWith(INTranSplitPrefix))
				{
					field = field.Substring(INTranSplitPrefix.Length);
				}

				if (fieldsOfHistoryObj.Contains(field))
				{
					filters.Add(new PXFilterRow(filter)
					{
						DataField = $"{nameof(INItemSiteHistDay)}__{field}"
					});
				}
				else if (field == nameof(INTranSplit.TranDate))
				{
					filters.Add(new PXFilterRow(filter)
					{
						DataField = $"{nameof(INItemSiteHistDay)}__{nameof(INItemSiteHistDay.SDate)}"
					});
				}
			}
