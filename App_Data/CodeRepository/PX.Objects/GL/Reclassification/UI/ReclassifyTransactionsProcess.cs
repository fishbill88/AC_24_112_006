...
namespace PX.Objects.GL.Reclassification.UI
{
	public class ReclassifyTransactionsProcess : ReclassifyTransactionsBase<ReclassifyTransactionsProcess>
	{
		...
		protected virtual void GLTranForReclassification_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetEnabled<GLTranForReclassification.selected>(cache, tran, GetEnabledForSelectedCheckbox(tran, canEdit));
        ...
        protected virtual void GLTranForReclassification_Selected_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			...
			{
				// user unchecked 'Selected' checkbox, so we roll back all modifications to the row
				InitTranForReclassEditableFields(tranForReclass);
				CleanupValidationMessages(tranForReclass);
			}
		...
		protected virtual void GLTranForReclassification_RowUpdated(PXCache cache, PXRowUpdatedEventArgs e)
		{
			...
			bool canEdit = !PXLongOperation.Exists(UID);
			// Acuminator disable once PX1070 UiPresentationLogicInEventHandlers - behavior of this control is not standard,
			// we can decide if the 'Selected' checkbox should be enabled only after it's value is re-calculated
			// depending on values of other fields - which should be done in RowUpdated handler
			PXUIFieldAttribute.SetEnabled<GLTranForReclassification.selected>(cache, tran, GetEnabledForSelectedCheckbox(tran, canEdit));
		}

		private bool GetEnabledForSelectedCheckbox(GLTranForReclassification tran, bool screenIsEditable)
		{
			// Behavior of 'Selected' checkbox on this screen is not standard.
			// Whether it is enabled or not depends on it's value (and several other factors)
			if (!screenIsEditable || tran.Selected != true)
			{
				return false;
			}
			if (tran.IsSplitted && !IsReclassAttrChanged(tran))
			{
				return false;
			}
			return true;
		...
		private void VerifyProjectFields(PXCache cache, PXRowUpdatedEventArgs e)
		{
			...
			if (tran.NewTaskID == null
					&& tran.TaskID != null
					&& tran.NewProjectID != ProjectDefaultAttribute.NonProject())
			{
				PXFieldState state = (PXFieldState) cache.GetStateExt<GLTranForReclassification.newTaskID>(e.Row);
		...
		{
			ValidateField<GLTranForReclassification.newBranchID>(tran.NewBranchID, tran);
			ValidateField<GLTranForReclassification.newAccountID>(tran.NewAccountID, tran);
			ValidateField<GLTranForReclassification.newSubID>(tran.NewSubID, tran);
			ValidateField<GLTranForReclassification.newTranDate>(tran.NewTranDate, tran);
			ValidateField<GLTranForReclassification.newFinPeriodID>(tran.NewFinPeriodID, tran);
			ValidateField<GLTranForReclassification.curyNewAmt>(tran.CuryNewAmt, tran);
			ValidateField<GLTranForReclassification.newProjectID>(tran.NewProjectID, tran);
			ValidateField<GLTranForReclassification.newTaskID>(tran.NewTaskID, tran);
			ValidateField<GLTranForReclassification.newCostCodeID>(tran.NewCostCodeID, tran);
		}

		private void ValidateField<TField>(object newValue, GLTranForReclassification tran) where TField : IBqlField
		{
			// this method is for validation performed in RowSelected event handler.
			// that's why it has to process PXPropertyExceptions in place and do not allow them out

			if (!string.IsNullOrEmpty(PXUIFieldAttribute.GetErrorOnly<TField>(GLTranForReclass.Cache, tran)))
			{
				// it's important not to override errors generated by usual FieldUpdating/FieldVerifying handlers
				return;
			}
		...
		{
			GLTranForReclass.Cache.RaiseExceptionHandling<GLTranForReclassification.newBranchID>(tran, tran.NewBranchID, null);
			GLTranForReclass.Cache.RaiseExceptionHandling<GLTranForReclassification.newAccountID>(tran, tran.NewAccountID, null);
			GLTranForReclass.Cache.RaiseExceptionHandling<GLTranForReclassification.newSubID>(tran, tran.NewSubID, null);
			GLTranForReclass.Cache.RaiseExceptionHandling<GLTranForReclassification.newTranDate>(tran, tran.NewTranDate, null);
			GLTranForReclass.Cache.RaiseExceptionHandling<GLTranForReclassification.newFinPeriodID>(tran, tran.NewFinPeriodID, null);
			GLTranForReclass.Cache.RaiseExceptionHandling<GLTranForReclassification.curyNewAmt>(tran, tran.CuryNewAmt, null);
			GLTranForReclass.Cache.RaiseExceptionHandling<GLTranForReclassification.newProjectID>(tran, tran.NewProjectID, null);
			GLTranForReclass.Cache.RaiseExceptionHandling<GLTranForReclassification.newTaskID>(tran, tran.NewTaskID, null);
			GLTranForReclass.Cache.RaiseExceptionHandling<GLTranForReclassification.newCostCodeID>(tran, tran.NewCostCodeID, null);
		}
		...
		[PXUIField(DisplayName = "Reload", Visible = true, MapEnableRights = PXCacheRights.Update, MapViewRights = PXCacheRights.Update)]
		[PXButton]
		...
		[PXUIField(DisplayName = Messages.Load, Visible = true, MapEnableRights = PXCacheRights.Update, MapViewRights = PXCacheRights.Update)]
		[PXButton]
        ...
        private GLTranForReclassification SplitTransaction(PXCache cache, GLTranForReclassification originalTran)
        {
            ...
            Batch.Current = PXParentAttribute.SelectParent<Batch>(cache, originalTran);
		...
		public partial class ReplaceOptions : PXBqlTable, IBqlTable
		{
