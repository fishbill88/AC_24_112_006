...
using PX.Objects.TX;
using DimensionLookupMode = PX.Data.DimensionLookupMode;
...
namespace PX.Objects.GL
{
    ...
    public class PeriodIDAttribute : PXAggregateAttribute, IPXCommandPreparingSubscriber, IPXRowSelectingSubscriber
    {
        ...
        public class QueryParams : PXBqlTable, IBqlTable
        {
	...
	public abstract class BranchBaseAttribute : PXEntityAttribute, IPXFieldSelectingSubscriber
	{
			...
			: base()
		{
			...
			PXDimensionSelectorAttribute attr =
				new PXDimensionSelectorAttribute(_DimensionName,
													...
													typeof(Branch.branchCD), typeof(Branch.acctName), typeof(Branch.ledgerID), typeof(Organization.organizationName));
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			AddBranchIdInconsistencyValidation(sender);
		}
		...
		protected virtual void AddBranchIdInconsistencyValidation(PXCache sender)
		{
			sender.Graph.OnBeforeCommit += (graph) =>
			{
				...
				{
					ValidateBranchID(sender, row);
				}

				foreach (var row in sender.Updated)
				{
					ValidateBranchID(sender, row);
				}
			};
		...
		{
		    var branchId = sender.GetValue(row, _FieldName) as int?;
			if (branchId != null && PXAccess.GetBranch(branchId) == null)
			{
				Type rowType = sender.GetItemType();
				string name = PXMessages.LocalizeFormatNoPrefix(Messages.BranchIdValidation, rowType.FullName);
				string diagnosticDetails = PXMessages.LocalizeFormatNoPrefix(Messages.BranchIdDoesNotExistInTenant, branchId, rowType.FullName);
				sender.Graph.SetDataConsistencyIssue(name, diagnosticDetails);
			}
		}
	...
	public sealed class PXDimensionValueLookupModeAttribute : PXDimensionAttribute
	{
		...
		public override void FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			if (_AttributeLevel == PXAttributeLevel.Item || e.IsAltered)
			{
				e.ReturnState = PXSegmentedState.CreateInstance(e.ReturnState, _FieldName,
					_Definition != null && _Definition.Dimensions.ContainsKey(_Dimension)
						? _Definition.Dimensions[_Dimension]
						: new PXSegment[0],
					!(e.ReturnState is PXFieldState) || String.IsNullOrEmpty(((PXFieldState)e.ReturnState).ViewName)
						? _Definition.LookupModes[_Dimension] == DimensionLookupMode.BySegmentsAndChildSegmentValues
							? sender.GetItemType().Name + "_" + _FieldName + "_" + _Dimension + "_Segments_"
							: "_" + _Dimension + "_Segments_"
						: null,
					LookupMode,
	...
	public class AccountAnyAttribute : AccountAttribute
	{
		...
		public AccountAnyAttribute(Type branchID, Type SearchType) : base(branchID, SearchType)
		{
			this.Filterable = true;
		}
	...
	public class AccountAttribute : PXEntityAttribute, IPXFieldVerifyingSubscriber, IPXRowPersistingSubscriber
	{
		public class CuryException : PXSetPropertyException
		{
			public CuryException(string accountCD)
				: base(Messages.AccountCuryNotTransactionCury, accountCD.Trim())
			{ 
			}
		...
		public virtual void Verify(PXCache sender, Account item, object row)
		{
			...
			if (ledger?.BalanceType != "R") throw new CuryException(item.AccountCD);
		}
