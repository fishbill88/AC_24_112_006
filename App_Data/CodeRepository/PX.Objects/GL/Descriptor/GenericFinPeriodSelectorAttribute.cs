	...
	public class GenericFinPeriodSelectorAttribute: PXCustomSelectorAttribute
	{
		...
		public override void FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			if (e.ExternalCall && e.Row == null)
				//Belived to only happens with user-defined fields. TableAttribute only subscribe this handler, but non of handlers which normally handle formatting between Dssplay and Storage formats
				e.NewValue = FinPeriodIDFormattingAttribute.FormatForStoring(e.NewValue as string);
		...
		protected virtual IEnumerable GetRecords()
		{
			...
			object extCurrentRow = PXView.Currents.Where(_ => _ != null).FirstOrDefault(c => _CacheType.IsAssignableFrom(c.GetType()));
