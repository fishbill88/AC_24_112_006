...
namespace PX.Objects.GL
{
		...
		where TProcessGraph : PXGraph<TProcessGraph>, IScheduleProcessing, new()
	{
		...
		private void SetMonthlyControlsState(PXCache cache, Schedule s, bool isMonthly)
		{
			...
			PXUIFieldAttribute.SetEnabled<Schedule.monthlyOnDay>(cache, s, isMonthly && s.MonthlyDaySel == GLScheduleMonthlyType.OnDay);
			PXUIFieldAttribute.SetEnabled<Schedule.monthlyOnWeek>(cache, s, isMonthly && s.MonthlyDaySel == GLScheduleMonthlyType.OnDayOfWeek);
			PXUIFieldAttribute.SetEnabled<Schedule.monthlyOnDayOfWeek>(cache, s, isMonthly && s.MonthlyDaySel == GLScheduleMonthlyType.OnDayOfWeek);

			PXUIFieldAttribute.SetVisible<Schedule.monthlyOnDay>(cache, s, isMonthly);
			PXUIFieldAttribute.SetVisible<Schedule.monthlyOnWeek>(cache, s, isMonthly);
			PXUIFieldAttribute.SetVisible<Schedule.monthlyOnDayOfWeek>(cache, s, isMonthly);
			PXUIFieldAttribute.SetVisible<Schedule.months>(cache, s, isMonthly);
		}
		...
		private void SetPeriodicallyControlsState(PXCache cache, Schedule s, bool isPeriodically)
		{
			...
			PXUIFieldAttribute.SetEnabled<Schedule.periodFixedDay>(cache, s, isPeriodically && s.PeriodDateSel == PeriodDateSelOption.PeriodFixedDate);
		}
