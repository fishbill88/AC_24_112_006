...
namespace PX.Objects.GL
{
	#region FilterClass
	[Serializable]
	public partial class AccountByPeriodFilter : PXBqlTable, IBqlTable
	{
		...
		#region OrgBAccountIDIsEmpty
		public abstract class orgBAccountIDIsEmpty : PX.Data.BQL.BqlBool.Field<orgBAccountIDIsEmpty> { }
		...
		public virtual Boolean? OrgBAccountIDIsEmpty { get; set; }
		#endregion
	}
	#endregion
	...
	public class AccountByPeriodEnq : PXGraph<AccountByPeriodEnq>
	{
		...
		protected virtual IEnumerable filter()
		{
			...
			if (filter.AccountID != null && filter.LedgerID != null && filter.StartPeriodID != null && filter.EndPeriodID != null)
			{
				...
				if (account != null)
				{
					if (filter.OrgBAccountIDIsEmpty == true)
					{
						filter.OrgBAccountID = null;
					}
					else if (filter.BranchID != null && filter.OrgBAccountID == null)
					{
					...
					using (new PXReadBranchRestrictedScope(null, branchIDs) { SpecificBranchTable = typeof(GLTran).Name })
					{
						...
						if (filter.ShowSummary == false && filter.IncludeReclassified == false)
							{
							...
							{
							//This is kind of workaround for AC-299201
							if (TransactionFilters.Any())
								turnover = ExecuteCommandForSummary()
									.Select(_ => _.GetItem<GLTranR>())
									.Sum(_ => ((_.DebitAmt ?? 0m) - (_.CreditAmt ?? 0m)) as decimal?) ?? 0m;
							else //AC-299201: request bellow shuld use TransactionFilters
						{
								turnover = Command
										.Select(new object[] { filter.OrgBAccountID })
								.Select(_ => _.GetItem<GLTranR>())
										.Sum(_ => ((_.DebitAmt ?? 0m) - (_.CreditAmt ?? 0m)) as decimal?) ?? 0m;
								this.Caches<GLTranR>().Clear();
							}
							if (AccountRules.IsCreditBalance(account.Type)) turnover *= -1;
						}
	...
	public class APDocGraphCreator : IDocGraphCreator
	{
		...
		public virtual PXGraph Create(string aTranType, string aRefNbr, int? referenceID)
		{
			...
			bool combined = (aTranType == APDocType.QuickCheck) || (aTranType == APDocType.VoidQuickCheck) || (aTranType == APDocType.CashReturn);
			if (combined)
			{
