...
namespace PX.Objects.GL
{
	public class JournalEntry : PXGraph<JournalEntry, Batch>, PXImportAttribute.IPXPrepareItems, PX.Objects.GL.IVoucherEntry
	{
												...
												Func<PXGraph, GLTran, CurrencyInfo, GLTran> buildTran)
		{
			...
			if (GLSetupApproval.EPSetting.ApprovableDocTypes.Contains(batch.BatchType))
			{
				batch.Hold = true;
			}
		...
		protected virtual void Batch_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			if (batch.Status == BatchStatus.Balanced && GLSetupApproval.EPSetting.ApprovableDocTypes.Contains(batch.BatchType))
			{
  ...
  protected virtual void Batch_RowInserting(PXCache cache, PXRowInsertingEventArgs e)
		...
		protected virtual void AssertOnDelete(Events.RowPersisting<Batch> e)
		{
			if (e.Cache.GetStatus(e.Row) == PXEntryStatus.Deleted)
			{
				if (e.Row.Released == true || e.Row.Voided == true)
					throw new PXInvalidOperationException(Common.Messages.ReleasedDocumentCannotBeDeleted, Messages.Batch, e.Row.RefNbr);
			}
		}
	...
	public class PostGraph : PXGraph<PostGraph>
	{
		...
		protected virtual GLTran InsertRoundingTran(Batch b, CurrencyInfo info, Ledger ledger, bool isCurrencyTB)
		{
			...
			if (DoExceedsNegligibleDifference(b.DebitTotal - b.CreditTotal))
			{
				...
				tran.CuryInfoID = CurrencyCollection.GetCurrency(info.BaseCuryID).CuryInfoID;
				tran.CuryCreditAmt = 0;
				tran.CuryDebitAmt = 0;
