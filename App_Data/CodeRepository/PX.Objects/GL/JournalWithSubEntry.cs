...
namespace PX.Objects.GL
{
	...
	public partial class JournalWithSubEntry : PXGraph<JournalWithSubEntry, GLDocBatch>, PXImportAttribute.IPXPrepareItems
	{
		...
		[Serializable]
		public class RefDocKey : PXBqlTable, IBqlTable
		{
			#region BranchID
		...
		[PXHidden]
		public partial class APAdjust3 : PXBqlTable, IBqlTable
		{
			#region AdjgDocType
		...
		[PXHidden]
		public partial class ARAdjust3 : PXBqlTable, IBqlTable
		{
			#region AdjgDocType
			public abstract class adjgDocType : PX.Data.BQL.BqlString.Field<adjgDocType> { }
			protected String _AdjgDocType;
		...
		public PXSelectJoin<CashAccount, LeftJoin<CashAccountETDetail, On<CashAccountETDetail.cashAccountID, Equal<CashAccount.cashAccountID>,
											...
											And2<Where<CashAccount.branchID, Equal<Required<CashAccount.branchID>>, Or<CashAccount.restrictVisibilityWithBranch, NotEqual<True>>>,
		...
		protected virtual void _(Events.FieldUpdated<GLTranDoc, GLTranDoc.termsID> e)
		{
			...
			if ((isDocumentDebitAdjustment || IsARCreditMemo(row)) && terms == null)
			{
				row.CuryDiscAmt = 0m;
				row.DiscDate = null;
				row.DueDate = null;
		...
		protected virtual void GLTranDoc_BAccountID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			GLTranDoc tranDoc = e.Row as GLTranDoc;

			bool isDocumentDebitAdjustment = tranDoc?.DocType == APInvoiceType.DebitAdj;
			// Acuminator disable once PX1074 SetupNotEnteredExceptionInEventHandlers [setup has already been checked in constructor]
			bool isTermsInDebitAdjustments = apsetup.Current?.TermsInDebitAdjustments == true;
			// Acuminator disable once PX1074 SetupNotEnteredExceptionInEventHandlers [setup has already been checked in constructor]
			bool isTermsInCreditMomos = arsetup.Current?.TermsInCreditMemos == true;
			bool isTermsApplicable = !((isDocumentDebitAdjustment && !isTermsInDebitAdjustments) || (IsARCreditMemo(tranDoc) && !isTermsInCreditMomos));
		...
		protected virtual void GLTranDoc_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (row != null)
			{
				...
				bool isDocumentDebitAdjustment = row.DocType == APInvoiceType.DebitAdj;
				if (isDocumentDebitAdjustment || IsARCreditMemo(row))
				{
					bool isTermsInDebitAdjustments = apsetup.Current.TermsInDebitAdjustments == true;
					bool isTermsInCreditMemos = arsetup.Current.TermsInCreditMemos == true;

					PXUIFieldAttribute.SetRequired<GLTranDoc.dueDate>(sender, (isTermsInDebitAdjustments && isDocumentDebitAdjustment || IsARCreditMemo(row) && isTermsInCreditMemos )
						&& row.TermsID != null);
					PXUIFieldAttribute.SetRequired<GLTranDoc.discDate>(sender, (isTermsInDebitAdjustments && isDocumentDebitAdjustment || IsARCreditMemo(row) && isTermsInCreditMemos)
						&& row.TermsID != null);

					PXUIFieldAttribute.SetEnabled<GLTranDoc.termsID>(sender, row, (isTermsInDebitAdjustments && isDocumentDebitAdjustment || IsARCreditMemo(row) && isTermsInCreditMemos)
						&& row.Released == false);
		...
		protected virtual void GLTranDoc_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if ((!sender.ObjectsEqual<GLTranDoc.curyTranTotal>(e.Row, e.OldRow) || !sender.ObjectsEqual<GLTranDoc.curyTaxTotal>(e.Row, e.OldRow))
				&& row?.CuryTranTotal != null)
			{
				//To run TermsAttribute logic with correct tax values set
				sender.RaiseFieldUpdated<GLTranDoc.curyTranTotal>(row, row.CuryTranTotal);
			}
			...
			if (cashAcct == null)
			{
				cashAcct = PXSelectJoin<CashAccount,
											...
											And<PaymentMethodAccount.useForAR, Equal<True>,
												...
												And2<Match<Current<AccessInfo.userName>>,
												And<CashAccount.branchID, Equal<Required<GLTranDoc.branchID>>>>>>>>,
											OrderBy<Desc<PaymentMethodAccount.aRIsDefault>>>.Select(this, aPaymentMethodID, curyID, branchID);
			}
			return cashAcct;
		}
		#endregion
	...
	public class GLBatchDocRelease : PXGraph<GLBatchDocRelease>
	{
		...
		[Serializable]
		public partial class GLTranDocU : PXBqlTable, IBqlTable
		{
			#region BranchID
		...
		protected virtual PXGraph CreateAR(GLTranDoc doc, List<GLTranDoc> aList, List<Batch> toPost)
		{
			...
			if (result == null && JournalWithSubEntry.IsARInvoice(doc))
			{
				...
				if (JournalWithSubEntry.IsARCreditMemo(doc) && qcGraph.ARSetup.Current.TermsInCreditMemos == true)
				{
					copy.TermsID = doc.TermsID;
					copy.DiscDate = doc.DiscDate;
					copy.DueDate = doc.DueDate;
				}
				...
				if (!JournalWithSubEntry.IsARCreditMemo(doc)
					|| (JournalWithSubEntry.IsARCreditMemo(doc) && qcGraph.ARSetup.Current.TermsInCreditMemos == true))
				{
					copy.CuryOrigDiscAmt = doc.CuryDiscAmt;
				}
