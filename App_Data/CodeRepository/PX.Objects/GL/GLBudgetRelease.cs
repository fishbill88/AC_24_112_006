...
namespace PX.Objects.GL
{
	...
	public class GLBudgetRelease : PXGraph<GLBudgetRelease>
	{
		...
		public static void Approve(List<GLBudgetLine> list)
		{
			...
			FinPeriodRepository finPeriodRepository = new FinPeriodRepository(graph);
				...
				try
				{
					...
					foreach (GLBudgetLineDetail alloc in allocations)
					{
						...
						if (delta != 0m)
						{
							...
							AccountHistory accthistMaster = new AccountHistory
							{
								BranchID = alloc.BranchID,
								AccountID = alloc.AccountID,
								FinPeriodID = finPeriodRepository.FindByID(PXAccess.GetParentOrganizationID(alloc.BranchID), alloc.FinPeriodID).MasterFinPeriodID,
								LedgerID = alloc.LedgerID,
								SubID = alloc.SubID,
								CuryID = null,
								BalanceType = ledger.BalanceType
							...
							accthistMaster = (AccountHistory)cacheAccountHistory.Insert(accthistMaster);
							...
							else
							{
								...
								accthistMaster.TranYtdBalance += delta;
								accthistMaster.CuryTranYtdBalance = accthist.TranYtdBalance;
								if (acct.Type == AccountType.Asset || acct.Type == AccountType.Expense)
								{
									...
									accthistMaster.TranPtdDebit += delta;
									accthistMaster.CuryTranPtdDebit = accthistMaster.TranPtdDebit;
									//accthist.FinPtdCredit += 0;
									accthist.CuryFinPtdCredit = accthist.FinPtdCredit;
									//accthist.TranPtdCredit += 0;
									accthistMaster.CuryTranPtdCredit = accthistMaster.TranPtdCredit;
								}
								else
								{
									accthist.FinPtdCredit += delta;
									accthist.CuryFinPtdCredit = accthist.FinPtdCredit;
									accthistMaster.TranPtdCredit += delta;
									accthistMaster.CuryTranPtdCredit = accthistMaster.TranPtdCredit;
								...
								listHistory.Add(accthistMaster);
							}
						}
					}
