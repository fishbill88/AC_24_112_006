...
namespace PX.Objects.AR
{
	...
	public class ARTaxAttribute : TaxAttribute
	{
			...
			: base(ParentType, TaxType, TaxSumType, TaxCalculationMode, parentBranchIDField)
		{
			...
			{
				ForceAggregateRecalculation = PXAccess.FeatureInstalled<FeaturesSet.vATRecognitionOnPrepayments>()
			});
			...
			Case<Where<ARTran.commissionable, Equal<True>, And<Where<ARTran.lineType, NotEqual<SO.SOLineType.discount>,
				And<Where<ARTran.tranType, Equal<ARDocType.creditMemo>, Or<ARTran.tranType, Equal<ARDocType.cashReturn>>>>>>>,
					PX.Data.Minus<Sub<Sub<ARTran.curyTranAmt, Sub<ARTran.curyTranAmt, Mult<Mult<ARTran.curyTranAmt, ARTran.origGroupDiscountRate>, ARTran.origDocumentDiscountRate>>>,
		...
		{
			if ((row as ARTran)?.TranType == ARDocType.PrepaymentInvoice
				&& PXSelect<Tax, Where<Tax.taxID, Equal<Required<Tax.taxID>>>>.Select(sender.Graph, taxID).Select(_ => _.GetItem<Tax>()).SingleOrDefault()?.TaxType == CSTaxType.PerUnit)
				return;
			else
				base.AddTaxTotals(sender, taxID, row);
		}

		protected override void FillTaxDetailValuesForPerUnitTax(PXCache taxDetailCache, Tax tax, TaxRev taxRevision, TaxDetail taxDetail, PXCache rowCache, object row, decimal taxableQty, decimal curyTaxAmount)
		{
			base.FillTaxDetailValuesForPerUnitTax(taxDetailCache, tax, taxRevision, taxDetail, rowCache, row, taxableQty, curyTaxAmount);
			ARTran aRTran = (row as ARTran);
			ARTax aRTax = taxDetail as ARTax;
			if (aRTran?.TranType == ARDocType.PrepaymentInvoice && aRTax?.CuryTaxAmt != null && tax.TaxCalcLevel != CSTaxCalcLevel.Inclusive)
			{
				ARTranVATRecognitionOnPrepayments tranExt = taxDetailCache.Graph.Caches[typeof(ARTran)].GetExtension<ARTranVATRecognitionOnPrepayments>(aRTran);
				aRTran.CuryTranAmt = MultiCurrencyCalculator.RoundCury(taxDetailCache, aRTran, tranExt.PrepaymentAmt.Value + GetPerUnitTaxDetails(taxDetailCache, aRTax, aRTran).Sum(_ => _.CuryTaxAmt ?? 0m) * tranExt.PrepaymentPct.Value / 100);
			}
		}

		protected virtual IEnumerable<ARTax> GetPerUnitTaxDetails(PXCache cache, ARTax taxdetail, ARTran Line)
		{
			IEnumerable<ARTax> alltaxdetails = PXParentAttribute.SelectSiblings(cache, taxdetail, Line.GetType()).Cast<ARTax>();
			foreach (ARTax aRTax in alltaxdetails.Where(_ => _.TaxUOM != null))
			{
				Tax tax = SelectFrom<Tax>.Where<Tax.taxID.IsEqual<P.AsString>>.View.Select(cache.Graph, aRTax.TaxID);
				if (tax.TaxType == CSTaxType.PerUnit && tax.TaxCalcLevel != CSTaxCalcLevel.Inclusive)
					yield return aRTax;
			}
		}


		protected override decimal GetPerUnitTaxAmountForTaxableAdjustmentCalculation(Tax taxForTaxableAdustment, TaxDetail taxDetail, PXCache taxDetailCache, object row, PXCache rowCache)
		{
			if ((row as ARTran)?.TranType == ARDocType.PrepaymentInvoice) return 0;
			else return base.GetPerUnitTaxAmountForTaxableAdjustmentCalculation(taxForTaxableAdustment, taxDetail, taxDetailCache, row, rowCache);
		}
		...
		{
			if ((graph as ARInvoiceEntry)?.Document?.Current?.IsPrepaymentInvoiceDocument() == true)
				return base.SelectInclusiveTaxes(graph, document)
					.Where(taxRow => !IsPerUnitTax(PXResult.Unwrap<Tax>(taxRow))).ToList();
			else
				return
						base.SelectInclusiveTaxes(graph, document);
		}
	...
	public class ARCashTranIDAttribute : CashTranIDAttribute
	{
		...
		public static CATran DefaultValues(PXCache sender, CATran catran_Row, ARPayment orig_Row)
		{
			...
			if (parentDoc.DocType == ARDocType.CreditMemo ||
				...
				parentDoc.DocType == ARDocType.PrepaymentInvoice ||
