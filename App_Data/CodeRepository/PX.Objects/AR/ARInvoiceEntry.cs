...
namespace PX.Objects.AR
{
	...
	public class ARInvoiceEntry : ARDataEntryGraph<ARInvoiceEntry, ARInvoice>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		public class CostAccrual : NonStockAccrualGraph<ARInvoiceEntry, ARInvoice>
		{
			protected virtual void ARTran_SubID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
			{
				...
				if (tran != null && tran.SubID != null && tran.IsStockItem != true && tran.AccrueCost == true)
				{
					InventoryItem item = IN.InventoryItem.PK.Find(Base, tran.InventoryID);
					INPostClass postclass = INPostClass.PK.Find(Base, item?.PostClassID);
					if (postclass?.COGSSubFromSales == true)
		...
		[PXFormula(typeof(
			IIf<Where<ExternalCall, Equal<True>, Or<PendingValue<ARInvoice.termsID>, IsNotPending>>,
				IIf<Where<Current<ARInvoice.docType>, NotEqual<ARDocType.creditMemo>, Or<Current<ARSetup.termsInCreditMemos>, Equal<True>>>,
		...
		{
			ARAdjust2 adj = this.Adjustments.Current;
			if (adj != null)
			{
				ARInvoiceEntry docgraph = CreateInstance<ARInvoiceEntry>();
				docgraph.Document.Current = docgraph.Document.Search<ARInvoice.refNbr>(adj.PPDVATAdjRefNbr, adj.PPDVATAdjDocType);
		...
		public virtual IEnumerable PayInvoice(PXAdapter adapter)
		{
			if (Document.Current != null && Document.Current.Released == true)
			{
				...
				if ((Document.Current.Payable == true || isPayByLineCreditMemo) && Document.Current.OpenDoc == true)
				{
					...
					{
						if (Document.Current.DocType == ARDocType.PrepaymentInvoice && Document.Current.PendingPayment == false)
						{
							...
							Where2<
								...
								And2<
									Where<
										...
										Or<Standalone.ARRegisterAlias.docType, Equal<ARDocType.prepaymentInvoice>>>>
									>,
									...
									And<Standalone.ARRegisterAlias.pendingPayment, Equal<False>,
		...
		[PXCopyPasteHiddenFields(typeof(APTran.defScheduleID))]
		...
		protected virtual ARInvoice CreateReversalARInvoice(ARInvoice doc, ReverseInvoiceArgs reverseArgs)
		{
			...
			if (ARSetup.Current?.TermsInCreditMemos == false)
			{
		...
		public virtual void ReverseInvoiceProc(ARRegister doc, ReverseInvoiceArgs reverseArgs)
		{
			...
			foreach (PXResult<ARInvoice, CurrencyInfo, Terms, Customer> res in ARInvoice_CurrencyInfo_Terms_Customer.Select(this, doc.DocType, doc.RefNbr))
			{
				...
				invoice.CuryOrigDiscAmt = ARSetup.Current?.TermsInCreditMemos == false ? 0m : doc.CuryOrigDiscAmt;
		...
		public virtual void InsertReversedTransactionDetailsBalance(ARRegister doc, ReverseInvoiceArgs reverseArgs)
		{
			...
			insertedTran.TranDesc = reverseArgs.OverrideDocumentDescr;

			ARTran firstLine = PXSelect<ARTran,
				Where<ARTran.tranType, Equal<Required<ARInvoice.docType>>,
					And<ARTran.refNbr, Equal<Required<ARInvoice.refNbr>>,
					And<ARTran.taskID, IsNotNull,
					And<Not<Where<ARTran.lineType, IsNotNull, And<ARTran.lineType, Equal<SOLineType.discount>>>>>>>>,
				OrderBy<Asc<ARTran.lineNbr>>>
				.Select(this, doc.DocType, doc.RefNbr);

			insertedTran.TaskID = firstLine?.TaskID;
		...
		public virtual bool AskUserApprovalIfReversingDocumentAlreadyExists(ARInvoice origDoc)
		{
			...
			if (reversingDoc != null)
			{
				...
				return Document.Ask(Messages.ReversingDocumentCaption, localizedMsg, MessageButtons.OKCancel) == WebDialogResult.OK;
			}
		...
		private void ApplyOriginalDocumentToReversal(ARInvoice origDoc, ARInvoice reversingDoc)
		{
			...
			switch (reversingDoc.DocType)
			{
				...
				case ARDocType.CreditMemo:
					ApplyOriginalDocAdjustmentToCreditMemo(origDoc, reversingDoc);
					break;
			}
		}

		protected virtual void ApplyOriginalDocAdjustmentToCreditMemo(ARInvoice origDoc, ARInvoice reversingDoc)
		{
		...
		public override void Persist()
		{
			...
			foreach (ARInvoice ardoc in Document.Cache.Cached)
			{
				...
				if (status == PXEntryStatus.Deleted && ardoc.PendingPPD == true && (ardoc.DocType == ARDocType.CreditMemo || ardoc.DocType == ARDocType.DebitMemo))
				{
					PXUpdate<Set<ARAdjust.pPDVATAdjRefNbr, Null, Set<ARAdjust.pPDVATAdjDocType, Null>>, ARAdjust,
						Where<ARAdjust.pendingPPD, Equal<True>,
							And<ARAdjust.pPDVATAdjRefNbr, Equal<Required<ARAdjust.pPDVATAdjRefNbr>>,
							And<ARAdjust.pPDVATAdjDocType, Equal<Required<ARAdjust.pPDVATAdjDocType>>>>>>
						.Update(this, ardoc.RefNbr, ardoc.DocType);
				}
		...
		protected virtual void ARShippingAddress_RowUpdated(PXCache sender, PXRowUpdatedEventArgs e)
		{
			...
			if (row != null && oldRow.CustomerID == row.CustomerID)
			{
		...
		protected virtual void ARInvoice_TermsID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (ARSetup.Current.TermsInCreditMemos == true && document?.DocType == ARDocType.CreditMemo && terms == null)
			{
				sender.SetValueExt<ARInvoice.curyOrigDiscAmt>(document, decimal.Zero);
			}
		}
		...
		public virtual ARInvoiceState GetDocumentState(PXCache cache, ARInvoice doc)
		{
			...
			ARInvoiceState res = new ARInvoiceState
			{
				...
				IsDocumentFinCharge = doc.DocType == ARDocType.FinCharge,
				IsDocumentSmallCreditWO = doc.DocType == ARDocType.SmallCreditWO,
			...
			res.AllowDeleteDocument = !res.ShouldDisableHeader || res.IsUnreleasedWO || res.IsUnreleasedPPD || res.IsPrepaymentInvoiceReversing;
			...
			if (PXAccess.FeatureInstalled<FeaturesSet.vATReporting>() &&
				...
				(doc.DocType != ARDocType.CreditMemo || (doc.DocType == ARDocType.CreditMemo && doc.TermsID != null))&&
				doc.DocType != ARDocType.SmallCreditWO)
			{
				Taxes.Select();
			...
			res.ShowCommissionsInfo = doc.DocType != ARDocType.FinCharge && doc.DocType != ARDocType.SmallCreditWO && doc.DocType != ARDocType.PrepaymentInvoice;
		...
		protected virtual void ARInvoice_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			if (doc != null)
				ApplyDocumentState(cache, doc, this.GetDocumentState(cache, doc));
		}
		...
		protected virtual void ApplyDocumentState(PXCache cache, ARInvoice doc, ARInvoiceState state)
		{
			...
			string payInvoiceCaption = doc.DocType switch
			{
				...
				ARDocType.PrepaymentInvoice =>
					doc.Status switch {
						ARDocStatus.Unapplied => AR.Messages.Apply,
			...
			this.salesPerTrans.Cache.AllowSelect = state.ShowCommissionsInfo;
			PXUIFieldAttribute.SetVisible<CustSalesPeople.salesPersonID>(cache, doc, state.ShowCommissionsInfo);
			PXUIFieldAttribute.SetVisible<ARInvoice.curyCommnblAmt>(cache, doc, state.ShowCommissionsInfo);
			PXUIFieldAttribute.SetVisible<ARInvoice.curyCommnAmt>(cache, doc, state.ShowCommissionsInfo);
			...
			PXUIFieldAttribute.SetRequired<ARInvoice.dueDate>(cache, !state.IsDocumentCreditMemo
				|| (state.IsDocumentCreditMemo && ( ARSetup.Current.TermsInCreditMemos == true || doc.TermsID != null)));
			...
			PXUIFieldAttribute.SetVisible<ARTran.sOOrderNbr>(Transactions.Cache, null, false);
			PXUIFieldAttribute.SetVisible<ARTran.sOOrderType>(Transactions.Cache, null, false);
			...
			if (state.ShouldDisableHeader)
			{
				...
				PXUIFieldAttribute.SetEnabled<ARInvoice.dueDate>(cache, doc, ((!state.IsDocumentCreditMemo || (state.IsDocumentCreditMemo && doc.TermsID != null)) &&
					!state.IsDocumentSmallCreditWO && !state.IsDocumentFinCharge) && doc.OpenDoc == true && doc.PendingPPD != true);
				PXUIFieldAttribute.SetEnabled<ARInvoice.discDate>(cache, doc, ((!state.IsDocumentCreditMemo || (state.IsDocumentCreditMemo && doc.TermsID != null)) &&
					!state.IsDocumentSmallCreditWO && !state.IsDocumentFinCharge) && doc.OpenDoc == true && doc.PendingPPD != true);
			...
			else
			{
				...
				PXUIFieldAttribute.SetEnabled<ARInvoice.termsID>(cache, doc, !state.IsDocumentCreditMemo ||
					(state.IsDocumentCreditMemo && ARSetup.Current.TermsInCreditMemos == true));
				PXUIFieldAttribute.SetEnabled<ARInvoice.dueDate>(cache, doc, !state.IsDocumentCreditMemo ||
					(state.IsDocumentCreditMemo && doc.TermsID != null));
				...
				PXUIFieldAttribute.SetEnabled<ARInvoice.curyOrigDiscAmt>(cache, doc, (!state.IsDocumentCreditMemo || (state.IsDocumentCreditMemo && doc.TermsID != null) ) &&
					!state.IsDocumentPrepaymentInvoice && !termsMultiple );
			...
			if (ARSetup.Current != null)
			{
				PXUIFieldAttribute.SetVisible<ARInvoice.commnPct>(cache, doc, false);
				...
				PXUIFieldAttribute.SetEnabled<ARInvoice.workgroupID>(cache, doc, state.IsAssignmentEnabled && !state.IsCancellationDocument);
				PXUIFieldAttribute.SetEnabled<ARInvoice.ownerID>(cache, doc, state.IsAssignmentEnabled && !state.IsCancellationDocument);
			}
			...
			PXUIFieldAttribute.SetEnabled<ARInvoice.taxZoneID>(cache, doc, state.IsTaxZoneIDEnabled && !state.IsPrepaymentInvoiceReversing);

			PXUIFieldAttribute.SetEnabled<ARInvoice.externalTaxExemptionNumber>(cache, doc, state.IsAvalaraCustomerUsageTypeEnabled && !state.IsPrepaymentInvoiceReversing);
			PXUIFieldAttribute.SetEnabled<ARInvoice.avalaraCustomerUsageType>(cache, doc, state.IsAvalaraCustomerUsageTypeEnabled && !state.IsPrepaymentInvoiceReversing);
			PXUIFieldAttribute.SetEnabled<ARInvoice.revoked>(cache, doc, true);
			...
			PXUIFieldAttribute.SetVisible<ARInvoice.curyDiscountedDocTotal>(cache, doc, state.ShowCashDiscountInfo);
			PXUIFieldAttribute.SetVisible<ARInvoice.curyDiscountedTaxableTotal>(cache, doc, state.ShowCashDiscountInfo);
			PXUIFieldAttribute.SetVisible<ARInvoice.curyDiscountedPrice>(cache, doc, state.ShowCashDiscountInfo);
			...
			PXUIFieldAttribute.SetEnabled<ARInvoice.taxCalcMode>(cache, doc, !state.IsRetainageDocument && !(state.IsRetainageApplyDocument && state.IsDocumentReleased) && !state.IsDocumentReleased);
			#endregion
				...
				if (row.Hold != true)
				{
					...
					else if (row.CuryOrigDocAmt < 0m)
					{
						...
						else
						{
							...
							{
								sender.RaiseExceptionHandling<ARInvoice.curyDocUnpaidBal>(row, row.CuryDocUnpaidBal,
									new PXSetPropertyException(Messages.DocumentBalanceNegative));
							}
							else
							{
					...
					else
					{
						...
						sender.RaiseExceptionHandling<ARInvoice.curyDocUnpaidBal>(row, null, null);
					}
				}
		...
		protected virtual void ARTran_CuryUnitPrice_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			if (row != null && row.InventoryID != null && row.UOM != null && row.ManualPrice != true && !skipDefaulting)
			{
				...
				// Acuminator disable once PX1045 PXGraphCreateInstanceInEventHandlers - Legacy code. Method of the graph instance is used for calculations
		...
		protected virtual void _(Events.FieldUpdated<ARTaxTran, ARTaxTran.taxID> e)
		{
			...
			{
				Taxes.Cache.RaiseFieldDefaulting<ARTaxTran.accountID>(arTaxTran, out object newAccountID);
				arTaxTran.AccountID = (int?)newAccountID;
				Taxes.Cache.RaiseFieldDefaulting<ARTaxTran.taxType>(arTaxTran, out object newTaxType);
				arTaxTran.TaxType = (string)newTaxType;
				Taxes.Cache.RaiseFieldDefaulting<ARTaxTran.taxBucketID>(arTaxTran, out object newTaxBucket);
				arTaxTran.TaxBucketID = (int?)newTaxBucket;
			}
		}
		#endregion
		...
		protected virtual void _(Events.RowSelecting<ARAdjust2> e)
		{
			...
			if (adj.PPDVATAdjRefNbr != null)
			{
				adj.PPDVATAdjDescription = ARDocType.GetDisplayName(adj.PPDVATAdjDocType) + ", " + adj.PPDVATAdjRefNbr;
			}
		}
		...
		#region PPDVATAdjustment

		public virtual ARInvoice CreatePPDCreditMemo(ARPPDTaxAdjustmentParameters filter, List<PendingPPDARTaxAdjApp> list, ref int index)
		{
			bool firstApp = true;
			...
			foreach (PendingPPDARTaxAdjApp doc in list)
			{
				if (firstApp)
				{
					...
					invoice = PopulateVATAdjustingDocument(filter, invoice, doc, ARDocType.CreditMemo);
				}

				AddTaxAndVATAdjustmentDocDetails(doc);
			}
			...
			AddPPDCreditMemoApplications(list);

			Document.Cache.SetValueExt<ARInvoice.discDate>(invoice, invoice.DueDate);
			Document.Cache.SetValueExt<ARInvoice.curyOrigDiscAmt>(invoice, 0m);

			Save.Press();

			UpdateARAdjustPPDVATAdjRef(list, invoice);

			return invoice;
		}

		public virtual ARInvoice CreatePPDDebitMemo(ARPPDTaxAdjustmentParameters filter, List<PendingPPDARTaxAdjApp> list, ref int index)
		{
			bool firstApp = true;
			ARInvoice invoice = (ARInvoice)Document.Cache.CreateInstance();
			...
			foreach (PendingPPDARTaxAdjApp doc in list)
			{
				if (firstApp)
				{
					firstApp = false;
					index = doc.Index.Value;
					invoice = PopulateVATAdjustingDocument(filter, invoice, doc, ARDocType.DebitMemo);
				}
			...
			AddPPDDebitMemoApplications(list);
		...
		protected virtual ARInvoice PopulateVATAdjustingDocument(ARPPDTaxAdjustmentParameters filter, ARInvoice invoice, PendingPPDARTaxAdjApp doc, string docType)
		{
			...
			invoice.DocDate = filter.GenerateOnePerCustomer == true ? filter.TaxAdjustmentDate : doc.AdjgDocDate;
		...
		protected virtual void AddPPDDebitMemoApplications(List<PendingPPDARTaxAdjApp> list)
		{
			...
			foreach (ARAdjust2 adj in Adjustments
					...
					.RowCast<ARAdjust2>())
			{
				if (list.Exists(a => a.AdjdDocType == adj.AdjgDocType && a.AdjdRefNbr == adj.AdjgRefNbr))
				{
					Adjustments.Cache.SetDefaultExt<ARAdjust2.adjdHasPPDTaxes>(adj);
					UpdateARAdjustCuryAdjdAmt(adj, adj.CuryDocBal);
				}
			}
		}

		protected virtual void UpdateARAdjustPPDVATAdjRef(List<PendingPPDARTaxAdjApp> list, ARInvoice invoice)
		{
			list.ForEach(doc => PXUpdate<Set<ARAdjust.pPDVATAdjRefNbr, Required<ARAdjust.pPDVATAdjRefNbr>,
					Set<ARAdjust.pPDVATAdjDocType, Required<ARAdjust.pPDVATAdjDocType>>>, ARAdjust,
					...
					.Update(this, invoice.RefNbr, invoice.DocType, doc.AdjdDocType, doc.AdjdRefNbr, doc.AdjgDocType, doc.AdjgRefNbr));
		}
		...
		public virtual void AddTaxAndVATAdjustmentDocDetails(PendingPPDARTaxAdjApp doc)
		{
			...
			AddVATAdjustmentDetails(doc, TaxTotal, InclusiveTotal, taxes);
		}

		public virtual void AddVATAdjustmentDetails(PendingPPDARTaxAdjApp doc, decimal? TaxTotal, decimal? InclusiveTotal, PXResultset<ARTaxTran> taxes)
		{
			...
			{
				// Acuminator disable once PX1050 HardcodedStringInLocalizationMethod [Legacy code. DocType collects constants from ARInvoiceType class. False alert]
				// Acuminator disable once PX1051 NonLocalizableString [Legacy code. DocType collects constants from ARInvoiceType class. False alert]
		...
		{
			return ARSetupApproval.EPSetting.ApprovableDocTypes.Contains(doc.DocType);
		}

		protected virtual void setDontApproveValue(ARInvoice doc, PXCache cache)
		{
			bool DontApprove;
			if (doc.DocType == ARInvoiceType.SmallCreditWO ||
    ...
    public class ARInvoiceEntryProjectFieldVisibilityGraphExtension : PXGraphExtension<ARInvoiceEntry>
    {
        protected virtual void _(Events.RowSelected<ARInvoice> e)
        {
			...
			Boolean contractVisible = PXAccess.FeatureInstalled<FeaturesSet.contractManagement>();
			Boolean projectVisible = PM.ProjectAttribute.IsPMVisible(BatchModule.AR);

			PXUIFieldAttribute.SetVisible<ARInvoice.projectID>(e.Cache, e.Row, contractVisible || projectVisible);
			if (contractVisible && projectVisible) PXUIFieldAttribute.SetDisplayName<ARInvoice.projectID>(e.Cache, GL.Messages.ProjectContract);
			else if (projectVisible) PXUIFieldAttribute.SetDisplayName<ARInvoice.projectID>(e.Cache, PM.Messages.Project);
			else if (contractVisible) PXUIFieldAttribute.SetDisplayName<ARInvoice.projectID>(e.Cache, CT.Messages.Contract);
		}
    }

	[Serializable]
	public partial class DuplicateFilter : PXBqlTable, IBqlTable
	{
		#region RefNbr
