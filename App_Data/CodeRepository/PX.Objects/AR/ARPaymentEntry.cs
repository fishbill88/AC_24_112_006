...
namespace PX.Objects.AR
{
	...
	public partial class ARPaymentEntry : ARDataEntryGraph<ARPaymentEntry, ARPayment>
		...
		[Serializable]
        [PXHidden]
		public partial class LoadOptions : PXBqlTable, IBqlTable
		{
		...
		public virtual IEnumerable ViewPPDVATAdj(PXAdapter adapter)
		{
			...
			ARAdjust adj = PXSelect<ARAdjust, Where<ARAdjust.noteID, Equal<Required<ARTranPostBal.refNoteID>>>>.Select(this, post.RefNoteID);
			if (adj != null)
			{
		...
		public override IEnumerable VoidCheck(PXAdapter adapter)
		{
				...
				&& ARPaymentType.VoidEnabled(payment))
			{
				...
				if (!AskUserApprovalToVoidPayment(payment))
				{
					return adapter.Get();
				}

				CheckDocumentBeforeVoiding(this, payment);
			...
			switch (currentARPayment.DocType)
			{
				case ARDocType.Payment:
					cmd.WhereAnd<Where<ARInvoice.docType, Equal<ARDocType.invoice>,
						Or<ARInvoice.docType, Equal<ARDocType.prepaymentInvoice>,
						And<ARInvoice.pendingPayment, Equal<True>,
			...
			ARTran aTran, ARTran bTran)
		{
			...
			switch (orderBy)
			{
				...
				default:
					aSortOrder += aInvoice.DocType == ARDocType.CreditMemo ? 0 : 100;
					bSortOrder += bInvoice.DocType == ARDocType.CreditMemo ? 0 : 100;
		...
		public virtual void AutoApplyProc(bool checkBalance = true)
		{
						...
						.Where(adj => (adj.CuryAdjgAmt != 0) && (adj.Released != true && adj.Voided != true)))
			{
				...
				decimal totalCuryAdjgAmt = adjustments
					.Cast<PXResult<ARAdjust>>()
					.Sum(adj => ((ARAdjust)adj).CuryAdjgAmt * ((ARAdjust)adj).AdjgBalSign).Value;

				if (totalCuryAdjgAmt < 0)
				{
					totalCuryAdjgAmt = totalCuryAdjgAmt * -1;

					foreach (ARAdjust adj in adjustments
						.Where(adj => ((ARAdjust)adj).AdjdDocType == ARDocType.CreditMemo)
						.OrderBy(adj => ((ARAdjust)adj).CuryAdjgAmt))
					{
						if (adj.Selected == false || adj.CuryAdjgAmt == 0m)
							continue;

						decimal curyAdjgAmt = adj.CuryAdjgAmt.Value;
						decimal maxCorrectionAmount = Math.Min(curyAdjgAmt, totalCuryAdjgAmt);
						adj.CuryAdjgAmt -= maxCorrectionAmount;
						totalCuryAdjgAmt -= maxCorrectionAmount;
						adj.Selected = adj.CuryAdjgAmt != 0m;
		...
		protected virtual void ReverseApplicationProc(ARAdjust application, ARPayment payment)
		{
			...
			CheckDocumentBeforeReversing(this, application);
					...
					|| application.AdjgDocType == ARDocType.Refund))
			{
				...
				if (Document.Current.OpenDoc == false)
				{
					...
					ARPayment.Events.Select(ev => ev.OpenDocument).FireOn(this, payment);
		...
		protected virtual IEnumerable adjustments()
		{
			...
			else
			{
				PXResultMapper mapper = new PXResultMapper(this, 
					...
					typeof(ARAdjust), typeof(ARPayment), typeof(Standalone.ARRegisterAlias), typeof(ARInvoice));
				var result = mapper.CreateDelegateResult(true);
						...
						ref totalRows))
				{
					...
					ARInvoice inv = new ARInvoice() { DocType = fullPayment.DocType, RefNbr = fullPayment.RefNbr };
					if (fullPayment.DocType == ARDocType.CreditMemo)
					{
						fullPayment.DiscDate = ARInvoice.PK.Find(this, fullPayment.DocType, fullPayment.RefNbr)?.DiscDate;
					}
					...
					result.Add(
						mapper.CreateResult(new PXResult<ARAdjust, ARPayment, Standalone.ARRegisterAlias, ARInvoice>
						(res, fullPayment, res, inv))
						);
			}
			PXView.StartRow = 0;
				return result;
		...
		{
			return doc.DocType == ARDocType.Refund && ARSetupApproval.EPSetting.ApprovableDocTypes.Contains(doc.DocType);;
		}
		#endregion
		...
		protected virtual void _(Events.RowSelecting<ARAdjust2> e)
		{
			if (e.Row == null)
				return;
			ARAdjust2 adjust = e.Row;
			if (adjust.PPDVATAdjRefNbr != null)
			{
				adjust.PPDVATAdjDescription = ARDocType.GetDisplayName(adjust.PPDVATAdjDocType) + ", " + adjust.PPDVATAdjRefNbr;
			}
		}
		...
		protected virtual void ARAdjust_Selected_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			*/
			...
			bool checkBalance = !(payment.Status.IsIn(ARDocStatus.Hold, ARDocStatus.Balanced, ARDocStatus.CCHold));   // Payment Amount is editable
		...
		protected virtual void ARPaymentChargeTran_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			var projectsEnabled = PM.ProjectAttribute.IsPMVisible(BatchModule.AR);
			PXDefaultAttribute.SetPersistingCheck<ARPaymentChargeTran.projectID>(sender, e.Row, projectsEnabled ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
		...
		{
			ARPayment doc = (ARPayment)e.Row;
			bool checkingIsNotRequired = doc.DocType == ARDocType.CreditMemo
										|| doc.DocType == ARDocType.SmallBalanceWO
										|| doc.IsPrepaymentInvoiceDocument();

			if (!checkingIsNotRequired && doc.CashAccountID == null)
			{
			...
			if (!checkingIsNotRequired && String.IsNullOrEmpty(doc.PaymentMethodID))
			{
			...
			if(!PXAccess.FeatureInstalled<FeaturesSet.integratedCardProcessing>())
				...
				PXDefaultAttribute.SetPersistingCheck<ARPayment.pMInstanceID>(sender, doc, !checkingIsNotRequired && isPMInstanceRequired ? 
		...
		protected virtual void ARPayment_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			else
			{
				...
				PXUIFieldAttribute.SetEnabled<ARPayment.docDate>(cache, doc, false);
				PXUIFieldAttribute.SetEnabled<ARPayment.finPeriodID>(cache, doc, false);
			}
			...
			PXUIFieldAttribute.SetVisible<ARPaymentChargeTran.projectID>(PaymentCharges.Cache, null, PM.ProjectAttribute.IsPMVisible(BatchModule.AR));
			if (doc.DocType == ARDocType.PrepaymentInvoice && doc.Status == ARDocStatus.PendingPayment)
			{
				Adjustments.Cache.AllowInsert = false;
			}
		...
		public virtual void ARPayment_RowSelecting(PXCache sender, PXRowSelectingEventArgs e)
		{
			...
			if (row != null && !e.IsReadOnly)
			{
				using (new PXConnectionScope())
				{
					...
					if (sender.GetStatus(e.Row) == PXEntryStatus.Notchanged && (row.Status == ARDocStatus.Open || row.Status == ARDocStatus.Unapplied)
						&& row.VoidAppl == false && row.AdjDate != null && ((DateTime)row.AdjDate).CompareTo((DateTime)Accessinfo.BusinessDate) < 0
						&& IsImport != true)
					{
		...
		public virtual void VoidCheckProc(ARPayment doc)
		{
			...
			foreach (PXResult<ARPayment, CurrencyInfo, Currency, Customer> res in ARPayment_CurrencyInfo_Currency_Customer.Select(this, (object)doc.DocType, doc.RefNbr))
			{
				...
				payment.CuryApplAmt = null;
				payment.SOApplAmt = -1 * payment.SOApplAmt;
				payment.CurySOApplAmt = -1 * payment.CurySOApplAmt;
				payment.CuryUnappliedBal = null;
				payment.CuryWOAmt = null;
				...
				if (paymentmethod.Current?.PaymentType != PaymentMethodType.CreditCard && cashaccount.Current?.Reconcile == false)
				{
					payment.Cleared = true;
					payment.ClearDate = payment.DocDate;
				}
				...
				if (payment.DepositAsBatch == true && !string.IsNullOrEmpty(payment.DepositNbr))
				{
					PaymentMethod pm = PaymentMethod.PK.Find(this, payment.PaymentMethodID);
					...
					payment.DepositAsBatch = voidOnDepositAccount;
				...
				if (origDocument.DocType == ARPaymentType.Payment)
				{
					this.Document.Cache.SetValueExt<ARPayment.extRefNbr>(payment, origDocument.ExtRefNbr);
				}
