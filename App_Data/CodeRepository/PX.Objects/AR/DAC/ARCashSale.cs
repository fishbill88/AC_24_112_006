...
namespace PX.Objects.AR.Standalone
{
	...
	public class ARCashSale : ARRegister, ICCPayment, IApprovable, IApprovalDescription
	{
		...
		/// <inheritdoc cref="ARInvoice.IsPaymentsTransferred"/>
		...
		[PXDefault(typeof(Coalesce<
			Search<CustomerPaymentMethod.cCProcessingCenterID,
				Where<CustomerPaymentMethod.pMInstanceID, Equal<Current2<ARCashSale.pMInstanceID>>>>,
			Search2<CCProcessingCenterPmntMethod.processingCenterID,
				...
				Where<CCProcessingCenterPmntMethod.paymentMethodID, Equal<Current<ARCashSale.paymentMethodID>>,
		...
		/// <inheritdoc cref="ARPayment.TerminalID"/>
		[PXDBString(36, IsUnicode = true, InputMask = "", BqlField = typeof(ARPayment.terminalID))]
		[PXUIField(DisplayName = "Terminal")]
		[PXSelector(typeof(Search<CCProcessingCenterTerminal.terminalID, Where<CCProcessingCenterTerminal.processingCenterID, Equal<Current<processingCenterID>>, And<CCProcessingCenterTerminal.isActive, Equal<True>>>>), typeof(CCProcessingCenterTerminal.displayName),
			SubstituteKey = typeof(CCProcessingCenterTerminal.displayName))]
		[PXDefault(typeof(Search2<DefaultTerminal.terminalID,
			InnerJoin<PaymentMethod, On<PaymentMethod.paymentMethodID, Equal<Current<paymentMethodID>>>,
			InnerJoin<CCProcessingCenterTerminal, On<CCProcessingCenterTerminal.processingCenterID, Equal<DefaultTerminal.processingCenterID>,
				And<CCProcessingCenterTerminal.terminalID, Equal<DefaultTerminal.terminalID>>>>>,
			Where<PaymentMethod.paymentType, Equal<PaymentMethodType.posTerminal>,
				And<DefaultTerminal.userID, Equal<Current<AccessInfo.userID>>,
				And<DefaultTerminal.branchID, Equal<Current<AccessInfo.branchID>>,
				And<DefaultTerminal.processingCenterID, Equal<Current<processingCenterID>>,
				And<CCProcessingCenterTerminal.isActive, Equal<True>>>>>>>), PersistingCheck = PXPersistingCheck.Nothing)]
		...
		#endregion
		#region CardPresent
		public abstract class cardPresent : PX.Data.BQL.BqlBool.Field<cardPresent> { }

		/// <inheritdoc cref="ARPayment.CardPresent"/>
		[PXDBBool(BqlField = typeof(ARPayment.cardPresent))]
		[PXDefault(false)]
		public virtual bool? CardPresent
		{
			get;
			set;
		}
		...
		[PXSelector(typeof(Search2<ExternalTransaction.tranNumber,
			InnerJoin<AR.ARPayment, On<ExternalTransaction.docType, Equal<AR.ARPayment.docType>, And<ExternalTransaction.refNbr, Equal<AR.ARPayment.refNbr>>>>,
			Where<ExternalTransaction.procStatus, Equal<ExtTransactionProcStatusCode.captureSuccess>,
				And2<Where<Current<ARCashSale.pMInstanceID>, IsNull,
						And<AR.ARPayment.customerID, Equal<Current<ARCashSale.customerID>>,
						And<AR.ARPayment.paymentMethodID, Equal<Current<ARCashSale.paymentMethodID>>>>>,
					Or<Where<Current<ARCashSale.pMInstanceID>, IsNotNull,
						And<ExternalTransaction.pMInstanceID, Equal<Current<ARCashSale.pMInstanceID>>>>>>>,
		...
		[PXFormula(typeof(Switch<Case<Where<isMigratedRecord, Equal<False>,
				And<Selector<paymentMethodID, PaymentMethod.paymentType>, In3<PaymentMethodType.creditCard, PaymentMethodType.eft, PaymentMethodType.posTerminal>,
