...
namespace PX.Objects.AR
{
	...
	public class ARDocumentRelease : PXGraph<ARDocumentRelease>
	{
		...
		public static void ReleaseDoc(List<ARRegister> list, bool isMassProcess, List<Batch> externalPostList, ARMassProcessDelegate onsuccess, ARMassProcessReleaseTransactionScopeDelegate onreleasecomplete)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				try
				{
					...
					try
					{
						doc.ReleasedToVerify = ((doc.Status == ARDocStatus.Open || doc.Status == ARDocStatus.Unapplied)
							&& doc.OpenDoc == true && doc.Released == true) ? (bool?)null : false;
						...
						if (childs != null)
						{
							foreach (ARRegister child in childs)
							{
								...
								var isOpenDoc = (child.Status == ARDocStatus.Open || child.Status == ARDocStatus.Unapplied) && child.OpenDoc == true && child.Released == true;
			...
			PXSelectBase<BalancedARDocument> selectDocumentsCommand = new PXSelectJoinGroupBy<BalancedARDocument,
				...
				Where2<
					...
					And2<Where<BalancedARDocument.status, Equal<ARDocStatus.balanced>, 
						...
						Or<BalancedARDocument.status, Equal<ARDocStatus.unapplied>,
	   ...
	   // [PXFormula(typeof(Add<ARRegister.curyOrigDocAmt, ARRegister.curyRetainageTotal>))]
		...
		[PXHidden()]
		[Serializable()]
		public partial class ARInvoice : PXBqlTable, IBqlTable
		{
			#region DocType
		...
		[PXHidden()]
		[Serializable()]
		public partial class ARPayment : PXBqlTable, IBqlTable
		{
			#region DocType
			public abstract class docType : PX.Data.BQL.BqlString.Field<docType> { }
			[PXDBString(3, IsKey = true, IsFixed = true)]
	...
	public class ARReleaseProcess : PXGraph<ARReleaseProcess>
	{
		...
		public SelectFrom<SVATConversionHist>
			.Where<SVATConversionHist.module.IsEqual<BatchModule.moduleAR>
				.And<SVATConversionHist.adjdDocType.IsEqual<@P.AsString.ASCII>>
				.And<SVATConversionHist.adjdRefNbr.IsEqual<@P.AsString>>
				.And<SVATConversionHist.taxID.IsEqual<@P.AsString>>
				.And<Brackets<SVATConversionHist.adjdDocType.IsNotEqual<SVATConversionHist.adjgDocType>
							.Or<SVATConversionHist.adjdRefNbr.IsNotEqual<SVATConversionHist.adjgRefNbr>>>>>
			.View SVATRecognitionRecords;
		...
		private List<ARRegister> CreateInstallments(PXResult<ARInvoice, CurrencyInfo, Terms, Customer> res)
		{
			...
			foreach (TermsInstallments inst in installments)
			{
				...
				docgraph.GetExtension<ARInvoiceEntry.ARInvoiceEntryDocumentExtension>().SuppressApproval();
			...
			List<PMRegister> pmDocs)
		{
			...
			if (doc.Released != true)
			{
				...
				else if (ardoc.DocType != ARDocType.SmallCreditWO)
				{
					...
					{
						GLTran prepaymentTran = (GLTran)je.GLTranModuleBatNbr.Cache.CreateCopy(tran);
						prepaymentTran.SummPost = true;
						prepaymentTran.ReclassificationProhibited = true;
						prepaymentTran.AccountID = ardoc.PrepaymentAccountID;
						prepaymentTran.SubID = ardoc.PrepaymentSubID;
						prepaymentTran.BranchID = ardoc.BranchID;
						prepaymentTran.CreditAmt = tran.DebitAmt;
						prepaymentTran.CuryCreditAmt = tran.CuryDebitAmt;
						prepaymentTran.DebitAmt = tran.CreditAmt;
						prepaymentTran.CuryDebitAmt = tran.CuryCreditAmt;
						prepaymentTran.TranClass = GLTran.tranClass.PrepaymentInvoice;

						if (doc.OpenDoc == true)
						{
							UpdateHistory(prepaymentTran, customer);
							UpdateHistory(prepaymentTran, customer, info);
						}

						InsertInvoiceTransaction(je, prepaymentTran,
							new GLTranInsertionContext { ARRegisterRecord = doc });
					}
...
;						}
					...
					foreach (PXResult<ARTran, ARTax, Tax, DRDeferredCode, SO.SOOrderType, ARTaxTran> r in group)
					{
						...
						if (isFirstARTaxRow)
						{
							GLTran tran = new GLTran
							{
								...
								TranDate = n.TranDate,
								ProjectID = ardoc.IsRetainageDocument == true ? ProjectDefaultAttribute.NonProject() : n.ProjectID,
								TaskID = ardoc.IsRetainageDocument == true ? null : n.TaskID,
								CostCodeID = ardoc.IsRetainageDocument == true ? null : n.CostCodeID,
						...
						n.Released = true;
							n.ReleasedToVerify = doc.ReleasedToVerify;
				...
				if (docInclTaxDiscrepancy != 0 && !(ardoc.IsPrepaymentInvoiceDocument() || ardoc.IsPrepaymentInvoiceDocumentReverse()))
				{
		...
		{
			GLTran tran = new GLTran();
			tran.SummPost = this.SummPost || doc.IsPrepaymentInvoiceDocument();
			tran.ReclassificationProhibited = doc.IsPrepaymentInvoiceDocument();
			...
			if (ardoc.IsPrepaymentInvoiceDocument() && x.TaxType == TX.TaxType.PendingSales)
			{
				UpdateHistory(tran, customer);
		...
		public void SetProjectAndTaxID(GLTran tran, Account account, ARInvoice ardoc)
		{
		...
		public virtual void ProcessOriginTranPost(ARInvoice doc, Amount itemDiscount, bool masterInstallment)
		{
			...
			{
				post.TranClass = String.Concat(ARDocType.Prepayment, GLTran.tranClass.Payment);
			}
			...
			if (doc.IsPrepaymentInvoiceDocumentReverse() == true)
			{
				ARTranPost post1 = (ARTranPost)TranPost.Cache.CreateCopy(post);
				post1.ID = null;
				post1.AccountID = doc.PrepaymentAccountID;
				post1.SubID = doc.PrepaymentSubID;
				post1.GLSign = (short?)(-post.GLSign);
				TranPost.Insert(post1);
			}
		}
		...
		{
			ARTranPost post = CreateTranPost(doc);
			post.TranClass = String.Concat(doc.DocType, GLTran.tranClass.Payment);
			post.Type = ARTranPost.type.Origin;
		...
		public virtual void ProcessAdjustmentTranPost(ARTran tran, ARAdjust adj, ARRegister doc, ARRegister pmt, bool adjustedOnly = false)
		{
			...
			{
				adjd.TranClass = String.Concat(adjg.DocType, GLTran.tranClass.PrepaymentInvoice);
			}
			else if (doc.IsPrepaymentInvoiceDocument())
			{
				adjd.TranClass = String.Concat(ARDocType.Prepayment, GLTran.tranClass.Payment);
			}
				...
				IsNeedUpdateHistoryForTransaction(adjd.FinPeriodID))
			{
				...
				if (pmt.IsPrepaymentInvoiceDocumentReverse())
				{
					ARTranPost adjdReversePrepayment = (ARTranPost)TranPost.Cache.CreateCopy(adjd);
					adjdReversePrepayment.ID = null;
					adjdReversePrepayment.AccountID = pmt.PrepaymentAccountID;
					adjdReversePrepayment.SubID = pmt.PrepaymentSubID;
					adjdReversePrepayment.GLSign = -1;
					TranPost.Insert(adjdReversePrepayment);

					ARTranPost adjgReversePrepayment = (ARTranPost)TranPost.Cache.CreateCopy(adjg);
					adjgReversePrepayment.ID = null;
					adjgReversePrepayment.AccountID = pmt.PrepaymentAccountID;
					adjgReversePrepayment.SubID = pmt.PrepaymentSubID;
					adjgReversePrepayment.GLSign = -1;
					TranPost.Insert(adjgReversePrepayment);
				}
			}
		...
		{
			// Skip a balance update for applications of
			// Credit Memo to Prepayment Invoice (reversing a Prepayment Invoice)
			if (adj.AdjgDocType == ARDocType.CreditMemo && adj.AdjdDocType == ARDocType.PrepaymentInvoice)
			{
				return;
			}
				...
				string.Equals(ardoc.RefNbr, adj.AdjdRefNbr, StringComparison.OrdinalIgnoreCase))
			{
				if (ardoc.CustomerID != null && ardoc.CustomerLocationID != null)
				{
					...
					decimal signPPI = 1m;
					if (adj.AdjgDocType == ARDocType.PrepaymentInvoice)
					{
						ARRegister adjgDoc = (ARRegister)ARDocument.Cache.Locate(
							new ARRegister { DocType = adj.AdjgDocType, RefNbr = adj.AdjgRefNbr });
						if (adjgDoc.PendingPayment == false)
							signPPI = -1m;
					}

					arbal.CurrentBal += signPPI * adj.AdjdTBSign * (adj.AdjAmt + adj.AdjDiscAmt + adj.AdjWOAmt) - adj.RGOLAmt;
					if (ardoc.Released == true)
		...
		public virtual void VoidOrigAdjustment(ARAdjust adj)
		{
			...
			if (voidadj != null)
			{
				...
				if (voidadj.AdjgDocType == ARDocType.CreditMemo && voidadj.AdjdHasPPDTaxes == true)
				{
					...
					{
						PXUpdate<Set<ARAdjust.pPDVATAdjRefNbr, Null>, ARAdjust,
						Where<ARAdjust.pendingPPD, Equal<True>,
							And<ARAdjust.adjdDocType, Equal<Required<ARAdjust.adjdDocType>>,
 							And<ARAdjust.adjdRefNbr, Equal<Required<ARAdjust.adjdRefNbr>>,
							And<ARAdjust.pPDVATAdjRefNbr, Equal<Required<ARAdjust.pPDVATAdjRefNbr>>>>>>>
		...
		public virtual void UpdateBalances(ARAdjust adj, ARRegister adjddoc, ARTran adjdtran)
		{
			...
			if (ardoc.IsPrepaymentInvoiceDocument() && ardoc.Status == ARDocStatus.Unapplied)
			{
				ardoc.CuryDocBal = 0m;
				ardoc.DocBal = 0m;
			}
			...
			{
				decimal? discrepancy = adj.ReverseGainLoss == false ? ardoc.DocBal : -ardoc.DocBal;
				adj.RGOLAmt += discrepancy;
				ardoc.RGOLAmt += discrepancy;
			}
		...
		public virtual void CloseInvoiceAndRecoverBalances(ARRegister ardoc)
		{
			...
			ardoc.CuryDocBal = ardoc.CuryOrigDocAmt - CuryAdjustedBalance;
			ardoc.DocBal = ardoc.OrigDocAmt - AdjustedBalance;
			ardoc.CuryDiscBal = ardoc.CuryOrigDiscAmt;
			ardoc.DiscBal = ardoc.OrigDiscAmt;
			ardoc.CuryDiscTaken = 0m;
			ardoc.DiscTaken = 0m;
			...
			ardoc.OpenDoc = true;
			if (ardoc.PendingPayment != true || ardoc.DocBal == 0 && ardoc.CuryDocBal == 0)
				SetClosedPeriodsFromLatestApplication(ardoc);
			ARDocument.Cache.Update(ardoc);

			if (ardoc.DocBal == 0 && ardoc.CuryDocBal == 0 || ardoc.Voided == true)
			{
			...
			PXResult<ARPayment, CurrencyInfo, Currency, Customer, CashAccount> res)
		{
			...
			if (ardoc.IsPrepaymentInvoiceDocument() && ardoc.PendingPayment == false && ardoc.Released == false)
			{
				ardoc.PostponePendingPaymentFlag = true;
			}
		}
		...
		private ARStatementDetail GetRelatedStatementDetail(ARTranPost tranPost)
		{
			if (tranPost.Type.IsIn(ARTranPostType.Origin, ARTranPostType.Application, ARTranPostType.Adjustment))
			{
				...
				return StatementDetailsView.Select(tranPost.DocType, tranPost.RefNbr)
					.RowCast<ARStatementDetail>()
					.Where(_ => _.RefNoteID == tranPost.RefNoteID)
					.OrderBy(_=>_.StatementDate)//Normally are already sorted like this, just for case
					.FirstOrDefault();
			}
			else return null;
		}
			...
			Currency paycury)
		{
				...
				.GroupBy(res => ((ARRegister)res).AreAllKeysFilled(ARDocument.Cache) ? (ARRegister)res : (ARPayment)res, ARDocument.Cache.GetComparer()))
			{
						...
						&& adj.Voided != true && adj.VoidAppl != true)
				{
					...
					if (adj.CuryAdjgAmt == 0m && adj.CuryAdjgDiscAmt == 0m && adj.CuryAdjgPPDAmt == 0m
		...
		protected virtual void ProcessSVATAdjustments(JournalEntry je, ARAdjust adj, ARInvoice adjddoc, ARRegister adjgdoc, CurrencyInfo new_info)
		{
				...
				&& PXAccess.FeatureInstalled<FeaturesSet.vATReporting>())
			{
					...
					.SelectMultiBound(this, new object[] { adj }))
				{
					...
					{
						adjSVAT = ProcessLastSVATRecord(adjdoc, docSVAT, adjSVAT, percent);
					}
					...
					if (_IsIntegrityCheck == false)
					{
						AfterSVATConversionHistoryInserted(je, adj, adjddoc, adjgdoc, docSVAT, adjSVAT);
					}
				}
			}
		}

		protected virtual SVATConversionHist ProcessLastSVATRecord(ARRegister adjddoc, SVATConversionHist docSVAT, SVATConversionHist adjSVAT, decimal percent)
		{
			...
			{
				foreach (SVATConversionHist row in SVATRecognitionRecords.Select(docSVAT.AdjdDocType, docSVAT.AdjdRefNbr, docSVAT.TaxID))
				{
		...
		protected virtual void AfterSVATConversionHistoryInserted(JournalEntry je, ARAdjust adj, ARInvoice adjddoc, ARRegister adjgdoc, SVATConversionHist docSVAT, SVATConversionHist adjSVAT)
		{
		...
		private void ProcessAdjustmentAdjusting(JournalEntry je, ARAdjust adj, ARPayment payment, ARRegister adjustedDocument, Customer paymentCustomer, CurrencyInfo new_info)
		{
			...
			GLTran tran = new GLTran
			{
				...
				AccountID = (isPrepaymentInvoice) ? payment.PrepaymentAccountID : payment.ARAccountID,
				ReclassificationProhibited = true,
				SubID = (isPrepaymentInvoice) ? payment.PrepaymentSubID : payment.ARSubID,
			...
			if (isPrepaymentInvoiceReverse)
			{
				GLTran prepaymentInvoiceReverseTran = (GLTran)je.GLTranModuleBatNbr.Cache.CreateCopy(tran);
				prepaymentInvoiceReverseTran.AccountID = payment.PrepaymentAccountID;
				prepaymentInvoiceReverseTran.SubID = payment.PrepaymentSubID;
				prepaymentInvoiceReverseTran.TranClass = GLTran.tranClass.PrepaymentInvoice;

				InsertAdjustmentsAdjustingTransaction(je, prepaymentInvoiceReverseTran,
					new GLTranInsertionContext { ARRegisterRecord = payment, ARAdjustRecord = adj });
			}
		}
		...
		private void ProcessAdjustmentAdjusted(JournalEntry je, ARAdjust adj, ARRegister adjustedDocument, ARPayment payment, CurrencyInfo vouch_info, CurrencyInfo new_info)
		{
			...
			if (isPrepaymentInvoiceReverse)
			{
				...
				InsertAdjustmentsAdjustedTransaction(je, prepaymentInvoiceReverseTran,
					new GLTranInsertionContext { ARRegisterRecord = payment, ARAdjustRecord = adj });
			}
		}
		...
		protected virtual void PerformBasicReleaseChecks(PXGraph selectGraph, ARRegister document)
		{
			...
			if (documentAsPayment != null
				...
				&& documentAsPayment.DocType != ARDocType.VoidPayment
		...
		public virtual List<ARRegister> ReleaseDocProc(JournalEntry je, ARRegister ardoc, List<Batch> pmBatchList, ARDocumentRelease.ARMassProcessReleaseTransactionScopeDelegate onreleasecomplete)
		{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				{
					if (doc.DocType == ARDocType.PrepaymentInvoice && doc.Released == false) continue;
					ARPayment payment = res;
					CurrencyInfo info = res;
					Currency paycury = res;
					Customer customer = res;
				...
				if (ardoc.IsPrepaymentInvoiceDocument() && ardoc.PendingPayment == false && ardoc.Released == false)
				{
					doc.PostponePendingPaymentFlag = true;
				}
				...
				if (doc.DocType == ARDocType.CreditMemo)
				{
					...
					CreditMemoProcessingBeforeSave(ardoc);
				}
				else
				{
		...
		protected virtual void ProcessPostponedFlags()
		{
			...
			foreach (ARPayment item in ARPayment_DocType_RefNbr.Cache.Cached.Cast<ARPayment>().Where(p
				...
				|| p.PostponePendingPaymentFlag == true))
			{
				...
				if (item.PostponePendingPaymentFlag == true)
				{
					ARPayment_DocType_RefNbr.Cache.RaiseEventsOnFieldChanging<ARPayment.pendingPayment>(item, true);
		...
		{
			persister.Insert(ARPayment_DocType_RefNbr.Cache);
			persister.Update(ARPayment_DocType_RefNbr.Cache);

			persister.Update(ARDocument.Cache);
			persister.Insert(ARTran_TranType_RefNbr.Cache);
			persister.Update(ARTran_TranType_RefNbr.Cache);
			persister.Update(ARTaxTran_TranType_RefNbr.Cache);
			persister.Insert(SVATConversionHistory.Cache);
			persister.Update(SVATConversionHistory.Cache);
			persister.Update<INTran>();
			persister.Update(ARPaymentChargeTran_DocType_RefNbr.Cache);

			persister.Insert(ARAdjust_AdjgDocType_RefNbr_CustomerID.Cache);
			persister.Update(ARAdjust_AdjgDocType_RefNbr_CustomerID.Cache);
			persister.Delete(ARAdjust_AdjgDocType_RefNbr_CustomerID.Cache);

			persister.Insert(ARDoc_SalesPerTrans.Cache);
			persister.Update(ARDoc_SalesPerTrans.Cache);
			...
			persister.Insert<ItemCustSalesStats>(); //see ProcessInventory extension

			persister.Update(soOrder.Cache);
			persister.Update(soAdjust.Cache);
			...
			using (new PXConnectionScope())
			{
				...
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					foreach (ARRegister custdoc in invoices.Union(payments))
					{
						...
						ARRegister doc = ARDocument.Cache.Locate(custdoc) as ARRegister ?? custdoc;
						ARDocument.Cache.SetStatus(doc, PXEntryStatus.Updated);
		...
		protected virtual HashedList<ARRegister> GetDocumentsForIntegrityCheckProc(string minPeriod)
		{
			...
			{
				GetReleasedOriginalDocumentsWithAdjustments(custdocs, minPeriod);
				GetReleasedRetainageDocumentsWithAdjustments(custdocs, minPeriod);
			}
		...
		protected virtual void GetReleasedOriginalDocumentsWithAdjustments(HashedList<ARRegister> documents, string minPeriod)
		{
			HashSet<(string OrigDocType, string OrigRefNbr)> retainageDocuments = documents
				.RowCast<ARRegister>()
				.Where(adj => adj.IsRetainageDocument == true)
				.Select(_ => (OrigDocType: _.OrigDocType, OrigRefNbr: _.OrigRefNbr))
				.ToHashSet();

			foreach ((string OrigDocType, string OrigRefNbr) retainageDoc in retainageDocuments)
			{
				PXResultset<ARRegister> originalDocuments = new SelectFrom<ARRegister>
					.LeftJoin<ARInvoice>
						.On<ARInvoice.refNbr.IsEqual<ARRegister.refNbr>
						.And<ARInvoice.docType.IsEqual<ARRegister.docType>>>
					.Where<ARRegister.docType.IsEqual<@P.AsString>
						...
						.And<ARRegister.tranPeriodID.IsLess<@P.AsString>>
						.And<ARRegister.closedTranPeriodID.IsNull.Or<ARRegister.closedTranPeriodID.IsLess<@P.AsString>>>>
					.View(this)
					.Select(retainageDoc.OrigDocType, retainageDoc.OrigRefNbr, minPeriod, minPeriod);

				foreach (PXResult<ARRegister> originalDoc in originalDocuments)
				{
					ARInvoice doc = PXResult.Unwrap<ARInvoice>(originalDoc);
					PXCache<ARRegister>.RestoreCopy(doc, PXResult.Unwrap<ARRegister>(originalDoc));
					if (!documents.Contains(doc))
					{
						documents.Add(new PXResult<ARRegister>(doc));
					}
				}
				GetAllReleasedAdjustments(documents, originalDocuments, minPeriod);
			}
		}

		protected virtual void GetReleasedRetainageDocumentsWithAdjustments(HashedList<ARRegister> documents, string minPeriod)
		{
			HashSet<(string DocType, string RefNbr)> originalDocuments = documents
				.RowCast<ARRegister>()
				.Where(adj => adj.RetainageApply == true)
				.Select(_ => (DocType: _.DocType, RefNbr: _.RefNbr))
				.ToHashSet();

			foreach ((string DocType, string RefNbr) originalDoc in originalDocuments)
			{
				PXResultset<ARRegister> retainageDocuments = new SelectFrom<ARRegister>
					.LeftJoin<ARInvoice>
						.On<ARInvoice.refNbr.IsEqual<ARRegister.refNbr>
						.And<ARInvoice.docType.IsEqual<ARRegister.docType>>>
					.Where<ARRegister.origDocType.IsEqual<@P.AsString>
						.And<ARRegister.origRefNbr.IsEqual<@P.AsString>>
						.And<ARRegister.isRetainageDocument.IsEqual<True>>
						.And<ARRegister.released.IsEqual<True>>
						.And<ARRegister.tranPeriodID.IsLess<@P.AsString>>
						.And<ARRegister.closedTranPeriodID.IsNull.Or<ARRegister.closedTranPeriodID.IsLess<@P.AsString>>>>
					.View(this)
					.Select(originalDoc.DocType, originalDoc.RefNbr, minPeriod, minPeriod);

				foreach (PXResult<ARRegister> retainageDoc in retainageDocuments)
				{
					ARInvoice doc = PXResult.Unwrap<ARInvoice>(retainageDoc);
					PXCache<ARRegister>.RestoreCopy(doc, PXResult.Unwrap<ARRegister>(retainageDoc));
					if (!documents.Contains(doc))
					{
						documents.Add(new PXResult<ARRegister>(doc));
					}
				}
				GetAllReleasedAdjustments(documents, retainageDocuments, minPeriod);
			}
		}
...
namespace PX.Objects.AR.Overrides.ARDocumentRelease
{
	...
	public class ARHistoryDetDeleted : PXBqlTable, IBqlTable
	{
