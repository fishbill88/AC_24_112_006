...
namespace PX.Objects.AR
{
	public class ARCashSaleEntry : ARDataEntryGraph<ARCashSaleEntry, ARCashSale>, IGraphWithInitialization
	{
		...
		protected virtual void ARCashSale_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetVisible<ARTran.defScheduleID>(Transactions.Cache, null, false);
			PXUIFieldAttribute.SetVisibility<ARTran.defScheduleID>(Transactions.Cache, null, PXUIVisibility.Invisible);
			...
			UIState.RaiseOrHideErrorByErrorLevelPriority<ARPayment.status>(cache, doc, doc.Status == ARDocStatus.CCHold,
			...
			EnableVoidIfPossible(doc, cache, false);
		}
		...
		public virtual void EnableVoidIfPossible(ARCashSale doc, PXCache cache, bool docStatusIsCCHold)
		{
			...
			{
				bool extTransExists = ExternalTranHelper.HasTransactions(this.ExternalTran);
				if (extTransExists)
				{
					ExternalTransactionState tranState = ExternalTranHelper.GetActiveTransactionState(this, this.ExternalTran);
					bool isCCStateClear = !tranState.IsCaptured && !tranState.IsPreAuthorized;
					if (isCCStateClear || docStatusIsCCHold)
					{
						enableVoidCheck = true;
					}
				}
				cache.AllowDelete = !extTransExists;
			}
		...
		public virtual void VoidCheckProc(ARCashSale doc)
		{
			...
			foreach (PXResult<ARCashSale, CurrencyInfo> res in PXSelectJoin<ARCashSale, InnerJoin<CurrencyInfo, On<CurrencyInfo.curyInfoID, Equal<ARCashSale.curyInfoID>>>, Where<ARCashSale.docType, Equal<Required<ARCashSale.docType>>, And<ARCashSale.refNbr, Equal<Required<ARCashSale.refNbr>>>>>.Select(this, (object)doc.DocType, doc.RefNbr))
			{
				...
				ARCashSale newdocument = Document.Insert(new ARCashSale
				{
					...
					OrigDocDate = ((ARCashSale)res).DocDate,
				...
				newdocument.OrigDocDate = Document.Current.OrigDocDate;
			...
			{
				List<ARTaxTran> taxUpdates = new List<ARTaxTran>();
