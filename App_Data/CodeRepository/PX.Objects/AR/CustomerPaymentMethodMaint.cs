	...
	public class CustomerPaymentMethodMaint : PXGraph<CustomerPaymentMethodMaint, CustomerPaymentMethod>
	{
		...
		protected virtual void CustomerPaymentMethod_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			if (!string.IsNullOrEmpty(row.PaymentMethodID))
			{
				...
				bool visibleCustId = !string.IsNullOrEmpty(row.CCProcessingCenterID) && (isTokenizedMethodActive || row.CustomerCCPID != null) && isCCProcessingPM;
				...
				PXUIFieldAttribute.SetVisible<CustomerPaymentMethod.expirationDate>(cache, row, pmDef.PaymentType == PaymentMethodType.CreditCard);
			}
		...
		protected virtual void CustomerPaymentMethod_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
		{
			if ((e.Operation & PXDBOperation.Command) != PXDBOperation.Delete)
			{
				...
				if (row != null)
				{
					if (row.CCProcessingCenterID != null && row.IsActive == true)
					{
		...
		protected virtual void CustomerPaymentMethod_PaymentMethodID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			...
			{
				cache.SetValueExt<CustomerPaymentMethod.cCProcessingCenterID>(e.Row, null);
			}
			...
			{
				cache.SetDefaultExt<CustomerPaymentMethod.descr>(row);
			}

			if (pmDef?.PaymentType == PaymentMethodType.CreditCard && pmDef?.ARIsProcessingRequired == false)
			{
				cache.SetValueExt<CustomerPaymentMethod.cardType>(row, CardType.OtherCode);
			}
			else if (pmDef?.PaymentType == PaymentMethodType.EFT)
			{
				cache.SetValueExt<CustomerPaymentMethod.cardType>(row, CardType.EftCode);
			}
			else
			{
				cache.SetDefaultExt<CustomerPaymentMethod.cardType>(row);
			}
		...
		public static string FormatDescription(string cardType, string aMaskedID)
		{
			return string.IsNullOrEmpty(cardType)
				? aMaskedID
				: String.Format("{0}:{1}", CardType.GetDisplayName(cardType), aMaskedID);
		}
