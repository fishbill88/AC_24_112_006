...
namespace PX.Objects.AR
{
	/// <exclude />
	[Serializable]
	public partial class ARRetainageFilter : PXBqlTable, IBqlTable
	{
	...
	public class ARRetainageRelease : PXGraph<ARRetainageRelease>
	{
		...
		protected virtual IEnumerable documentList()
		{
			...
			docList.WhereAnd<Where<NotExists<Select2<ARRetainageInvoice, InnerJoin<ARTran,
					...
					Where<ARRetainageInvoice.isRetainageDocument, Equal<True>,
						And<ARTran.origDocType, Equal<ARInvoiceExt.docType>,
						And<ARTran.origRefNbr, Equal<ARInvoiceExt.refNbr>,
						And<ARRetainageInvoice.released, NotEqual<True>,
						And<Where<ARTran.origLineNbr, Equal<ARInvoiceExt.aRTranLineNbr>,
							Or<ARTran.origLineNbr, Equal<int0>>>>>>>>>>>>();

			foreach (ARInvoiceExt doc in docList.SelectWithViewContext())
				{
