...
namespace PX.Objects.AR
{
	namespace Override
	{
        ...
        [Serializable]
        public partial class Customer : PXBqlTable, IBqlTable
        {
            #region BAccountID
            public abstract class bAccountID : PX.Data.BQL.BqlInt.Field<bAccountID> { }
            [PXDBInt(IsKey = true)]
	...
	public class CustomerMaint : PXGraph<CustomerMaint, Customer>
	{
		...
		public partial class CustomerBalanceSummary : PXBqlTable, IBqlTable
		{
			#region CustomerID
		...
		[Serializable]
		[PXHidden]
		public class OnDemandStatementParameters : PXBqlTable, IBqlTable
		{
		...
		[PXHidden]
		public PXSetupOptional<CRSetup> CRSetup;
		...
		[PXProjection(typeof(Select4<CuryARHistory,
		 ...
		 >>>>>>>>))]
		public partial class ARLatestHistory : PXBqlTable, PX.Data.IBqlTable
		{
			#region BranchID
		...
		>>>>>))]
		[PXHidden]
		public partial class CustomerPrepaymentBalances : PXBqlTable, PX.Data.IBqlTable
		{
			#region CustomerID
			public abstract class customerID : PX.Data.BQL.BqlInt.Field<customerID> { }
		...
		[PXHidden]
		public partial class CustomerBalances : PXBqlTable, PX.Data.IBqlTable
			{
			#region CustomerID
			public abstract class customerID : PX.Data.BQL.BqlInt.Field<customerID> { }
		...
		>>>), Persistent = false)]
		[PXHidden]
		public partial class ChildCustomerBalanceSummary : PXBqlTable, IBqlTable
		{
			#region CustomerID
			public abstract class customerID : PX.Data.BQL.BqlInt.Field<customerID> { }
		...
		public class PaymentDetailsExt : PXGraphExtension<CustomerMaint>
		{
			...
			protected virtual void _(Events.RowPersisting<CustomerPaymentMethod> e)
			{
				...
				else if(SkipInsertingCustomerWithNewCpm(currentCustomer, row))
				{
					PaymentMethod defPM = PaymentMethodDef.Select();

					if (defPM != null)
					{
						var storedCpm = PaymentMethods.Select().RowCast<CustomerPaymentMethodInfo>()
							.Where(i => i.PMInstanceID == defPM.PMInstanceID).FirstOrDefault();
						if (storedCpm != null)
						{
							PaymentMethods.SetValueExt<CustomerPaymentMethodInfo.isDefault>(storedCpm, true);
							e.Cancel = true;
						}
					}
				}
			...
			{
				var ret = false;
				var status = Base.BAccount.Cache.GetStatus(customer);
				if (string.IsNullOrEmpty(cpm.CustomerCCPID) && status == PXEntryStatus.Inserted
					&& CCProcessingHelper.IsTokenizedPaymentMethod(Base, cpm.PMInstanceID))
				{
					PaymentMethod defPM = PaymentMethodDef.Select();
					if (defPM != null && defPM.PMInstanceID != customer.DefPMInstanceID)
					{
						ret = true;
					}
				}
		...
		public class LocationDetailsExt : LocationDetailsExt<CustomerMaint, Customer, Customer.bAccountID>
		{
			...
			public virtual void ChangeBAccountType(BAccount descendantEntity, string type, Action<BAccount, string> del)
			{
				...
				foreach (CRLocation location in this.Locations.Select())
				{
					...
					if (type != BAccountType.VendorType)
					{
						location.VExpenseAcctID = null;
						location.VExpenseSubID = null;
					}
					if (type != BAccountType.CustomerType)
					{
						location.CSalesAcctID = null;
						location.CSalesSubID = null;
					}
