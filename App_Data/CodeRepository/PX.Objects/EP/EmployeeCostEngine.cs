...
namespace PX.Objects.EP
{
    ...
    public class EmployeeCostEngine
	{
		...
		{
			PMLaborCostRate employeeLaborCostRate = SelectFrom<PMLaborCostRate>
				...
				.OrderBy<PMLaborCostRate.effectiveDate.Desc>
				.View
				.ReadOnly
				.SelectSingleBound(graph, null, employeeID, effectiveDate);

			if (employeeLaborCostRate != null && employeeLaborCostRate.RegularHours != null)
			{
				return (int) (employeeLaborCostRate.RegularHours.Value * 60);
			}

			return GetEmployeeWeeklyMinutesFromCalendar(employeeID);
		}

		public virtual int GetEmployeeWeeklyMinutesFromCalendar(int? employeeID)
		{
			int weeklyMinutes = 40 * 60; // Default 40 hours (or 2400 minutes) per week.

			EPEmployee employee = EPEmployee.PK.Find(graph, employeeID);

			if (employee != null && employee.CalendarID != null)
			{
				CSCalendar calendar = CSCalendar.PK.Find(graph, employee.CalendarID);
				if (calendar != null)
				{
					weeklyMinutes = 0;
					...
					{
						weeklyMinutes += CalendarHelper.GetMinutesWorkedOnDay(calendar, dayOfWeek);
					}
				}
			}
