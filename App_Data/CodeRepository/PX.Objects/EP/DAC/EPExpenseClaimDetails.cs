...
namespace PX.Objects.EP
{
	...
	public partial class EPExpenseClaimDetails : PXBqlTable, IBqlTable, PX.Data.EP.IAssign, INotable
	{
		...
		public abstract class claimDetailCD : PX.Data.BQL.BqlString.Field<claimDetailCD>
		{
			public class NumberingAttribute : AutoNumberAttribute
			{
				...
				private void CheckDuplicateAndAssignNextNumber(PXCache sender,
					EPExpenseClaimDetails row, string newValue)
						{
					EPExpenseClaimDetails duplicate = SelectFrom<EPExpenseClaimDetails>
						.Where<claimDetailCD.IsEqual<P.AsString>>
						.View.ReadOnly.Select(sender.Graph, newValue);
					if (duplicate == null)
						return;

					EPExpenseClaimDetails maxCDReceipt = SelectFrom<EPExpenseClaimDetails>
						.OrderBy<claimDetailCD.Desc>
						.View.ReadOnly.Select(sender.Graph);
					EPSetup setup = SelectFrom<EPSetup>.View.ReadOnly.Select(sender.Graph);
					var nextCD = GetNextNumber(sender, row, setup.ReceiptNumberingID, row.ExpenseDate,
						maxCDReceipt.ClaimDetailCD, out _, out _, out _);
					sender.SetValue(row, _FieldName, nextCD);
				}
			}
		}
		/// <summary>
		...
		[PXDefault(TypeCode.Decimal, "0.0")]
		[PXUIField(DisplayName = "Tip Amount", ErrorHandling = PXErrorHandling.Always)]
		[EPTaxTip]
		[PXUIVerify(
			typeof(Where<curyTipAmt, Equal<decimal0>,
						Or<curyTipAmt, Greater<decimal0>,
						And<curyExtCost, Greater<decimal0>,
						Or<curyTipAmt, Less<decimal0>,
						And<curyExtCost, Less<decimal0>>>>>>), PXErrorLevel.Error, Messages.TipSign)]
		...
		[PXDefault(TypeCode.Decimal, "0.0")]
		[PXUIEnabled(typeof(Where<paidWith, NotEqual<paidWith.cash>, Or<curyExtCost, GreaterEqual<decimal0>>>))]
		[PXFormula(typeof(Switch<Case<Where<paidWith, NotEqual<paidWith.cash>, Or<curyExtCost, LessEqual<decimal0>>>, decimal0>, curyEmployeePart>))]
		...
		[PXForeignReference(typeof(Field<corpCardID>.IsRelatedTo<CACorpCard.corpCardID>))]
