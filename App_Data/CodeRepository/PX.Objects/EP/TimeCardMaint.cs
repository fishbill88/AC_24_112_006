...
namespace PX.Objects.EP
{
	...
	public class TimeCardMaint : PXGraph<TimeCardMaint, EPTimeCard>, PXImportAttribute.IPXPrepareItems
	{
		...
		public virtual void PreloadHolidays()
		{
			...
			int regularWeeklyMinutes = CostEngine.GetEmployeeRegularWeeklyMinutes(Document.Current.EmployeeID, Document.Current.WeekStartDate);
			int minutesPerday = regularWeeklyMinutes / 5;
			EPTimeCardSummaryWithInfo summary = (EPTimeCardSummaryWithInfo)Summary.Cache.CreateInstance();
		...
		public virtual void NormalizeTimecard()
		{
			...
			string workCodeID = PXAccess.FeatureInstalled<FeaturesSet.payrollModule>() ? PREmployee.PK.Find(this, Document.Current.EmployeeID)?.WorkCodeID : null;
			...
			int delta = regularWeeklyMinutes - (monMinutes + tueMinutes + wedMinutes + thuMinutes + friMinutes + satMinutes + sunMinutes);
			...
			if (monQuota > 0)
			{
				...
				detailNormalize_Mon.WorkCodeID = workCodeID;
			...
			if (delta >= 0)
			{
				...
				if (tueQuota > 0)
				{
					...
					detailNormalize_Tue.WorkCodeID = workCodeID;
			...
			if (delta >= 0)
			{
				...
				if (wedQuota > 0)
				{
					...
					detailNormalize_Wed.WorkCodeID = workCodeID;
			...
			if (delta >= 0)
			{
				...
				if (thuQuota > 0)
				{
					...
					detailNormalize_Thu.WorkCodeID = workCodeID;
			...
			if (delta >= 0)
			{
				...
				if (friQuota > 0)
				{
					...
					detailNormalize_Fri.WorkCodeID = workCodeID;
		...
		protected virtual void EPTimeCardSummaryWithInfo_RowInserting(PXCache sender, PXRowInsertingEventArgs e)
		{
			...
			if (earningType != null && !skipProjectDefaultingFromEarningType)
			{
				...
				{
					sender.SetValueExt<EPTimeCardSummaryWithInfo.projectID>(row, earningType.ProjectID);
				}
				...
				if (earningType.TaskID != null && row.ProjectTaskID == null && !ProjectDefaultAttribute.IsNonProject( row.ProjectID))
				{
					...
					{
						sender.SetValueExt<EPTimeCardSummaryWithInfo.projectTaskID>(row, earningType.TaskID);
					}
				}
			}
			...
			if (row.ProjectID == null && e.ExternalCall)
			{
				...
				{
					sender.SetValueExt<EPTimeCardSummaryWithInfo.projectID>(row, previousRecord.ProjectID);
					sender.SetValueExt<EPTimeCardSummaryWithInfo.projectTaskID>(row, previousRecord.ProjectTaskID);
				}
			}

			if (row.ProjectID == null && earningType != null && earningType.TypeCD == EpSetup.Current.HolidaysType)
			{
				sender.SetValueExt<EPTimeCardSummaryWithInfo.projectID>(row, PM.ProjectDefaultAttribute.NonProject());
			}
		}
			...
			if (earningType != null)
			{
				...
				if (earningType.ProjectID != null)
				{
						...
						)
					{
						...
						{
							sender.SetValueExt<EPTimeCardSummaryWithInfo.projectTaskID>(row, null);
						}
					}
				}
		...
		public virtual void ProcessRegularTimecard(PM.RegisterEntry releaseGraph, EPTimeCard timecard)
		{
			...
			foreach (EPTimecardDetail act in Activities.Select())
			{
				...
				bool postPMTransaction = EPSetupMaint.GetPostPMTransaction(this, EpSetup.Current, timecard.EmployeeID) && !ProjectDefaultAttribute.IsNonProject(act.ProjectID);
		...
		public virtual bool ValidateTotals(EPTimeCard timecard, out string errorMsg)
		{
			...
			int regularWeeklyMinutes = CostEngine.GetEmployeeRegularWeeklyMinutes(Document.Current.EmployeeID, Document.Current.WeekStartDate);
			decimal? regularHours = regularWeeklyMinutes / 60m;

			if (isFullWeek && (timecard.TimeSpentCalc > regularWeeklyMinutes))
			{
				valid = false;
			...
			else if (isFullWeek && (timecard.TimeSpentCalc < regularWeeklyMinutes))
			{
				valid = false;
		...
		public class EPTimecardDetail : PMTimeActivity
		{
			...
			/// <summary>
			/// The workgroup to which the employee belongs.
			/// </summary>
			[PXDBInt]
			...
			[PXDefault(typeof(SearchFor<EPEmployee.defaultWorkgroupID>
				.Where<EPEmployee.defaultWorkgroupID.IsEqual<EPEmployee.defaultWorkgroupID.FromCurrent>>)
				, PersistingCheck = PXPersistingCheck.Nothing)]
	...
	public class TimecardApprovalAction : EPApprovalAutomation<EPTimeCard, EPTimeCard.isApproved, EPTimeCard.isRejected, EPTimeCard.isHold, EPSetupTimeCardApproval>
	{
		...
		public override bool IsApprover(EPTimeCard source)
		{
			...
			{
				if (ValidateApprovalAccess(item) || IsTaskApprover(item.RefNoteID))
				{
					return true;
				}
			}
		...
		{
			BAccount account = new SelectFrom<BAccount>.Where<BAccount.defContactID.IsEqual<@P.AsInt>>.View(_Graph).Select(PXAccess.GetContactID()).FirstOrDefault();
			...
			return new SelectFrom<EPTimeCard>
				...
				.Where<EPTimeCard.noteID.IsEqual<@P.AsGuid>
					.And<PMTask.approverID.IsEqual<@P.AsInt>>>.View(_Graph).Select(refNoteID, account.BAccountID).Any();
		}
		...
		{
			return IsTaskApprover(refNoteID);
		}
