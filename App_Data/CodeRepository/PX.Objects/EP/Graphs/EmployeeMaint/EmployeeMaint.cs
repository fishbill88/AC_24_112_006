...
namespace PX.Objects.EP
{
	public class EmployeeMaint : PXGraph<EmployeeMaint>
	{
        ...
        public PXSelectJoin<EPRule,
			...
			LeftJoin<EPRuleApprover, On<EPRuleApprover.ruleID, Equal<EPRule.ruleID>>>>,
             Where<EPRule.ownerID, Equal<Current<EPEmployee.defContactID>>, Or<Current<EPEmployee.defContactID>, Equal<EPRuleApprover.ownerID>>>> AssigmentAndApprovalMaps;
		...
		{
			foreach (object res in (new PXCancel<EPEmployee>(this, "Cancel")).Press(a))
			{
				EPEmployee e = PXResult.Unwrap<EPEmployee>(res);
		...
		protected virtual void _(Events.FieldVerifying<EPEmployeePosition, EPEmployeePosition.startDate> e)
		{
			...
			if (row == null || e.NewValue == null || e.OldValue == e.NewValue ||
				(row.StartReason != EPStartReason.New && row.StartReason != EPStartReason.Rehire) || months == 0)
			{
				return;
			}
		...
		protected virtual void _(Events.FieldVerifying<EPEmployeePosition, EPEmployeePosition.startReason> e)
		{
			...
			if (row == null || row.StartDate == null || e.OldValue == e.NewValue ||
				((string)e.NewValue != EPStartReason.New && (string)e.NewValue != EPStartReason.Rehire) || months == 0)
			{
				return;
			}
		...
		protected void SetDefContactJobTitle()
		{
			...
			if (activePosition?.PositionID != null)
			{
		...
		protected virtual void EPEmployeePosition_RowInserting(PXCache sender, PXRowInsertingEventArgs e)
		{
			...
			if (row != null)
			{
				...
				UpdateIfRequiredProbationPeriodEndDate(row, sender);
			}
		}
		...
		{
			if (employeePosition == null || employeePosition.StartDate == null)
			{
				return;
			}
    ...
    [Serializable]
	[PXHidden]
	public class GenTimeCardFilter : PXBqlTable, IBqlTable
    {
	...
	[Serializable]
	[PXHidden]
	public class RedirectEmployeeParameters : PXBqlTable, IBqlTable
	{
