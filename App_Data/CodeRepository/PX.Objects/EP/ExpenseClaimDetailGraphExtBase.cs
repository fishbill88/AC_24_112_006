...
namespace PX.Objects.EP
{
		...
		where TGraph : PXGraph
	{
		...
		protected virtual decimal? GetUnitCostByExpenseItem(PXCache cache, EPExpenseClaimDetails receipt)
		{
			...
			decimal curyStdCost = 0;
			if (item != null && currencyInfo != null && currencyInfo.CuryRate != null)
			{
				InventoryItemCurySettings curySettings = InventoryItemCurySettings.PK.Find(Base, item.InventoryID, currencyInfo.BaseCuryID);
				if (curySettings != null)
				{
					var cost = receipt.ExpenseDate >= curySettings.StdCostDate ? curySettings.StdCost.GetValueOrDefault() : curySettings.LastStdCost.GetValueOrDefault();
		...
		{
			if (paidWith == EPExpenseClaimDetails.paidWith.CardPersonalExpense
		...
		public virtual void VerifyEmployeePartNotExceedTotal(decimal? totalAmount, decimal? employeeAmount)
		{
			if (totalAmount * totalAmount - totalAmount * employeeAmount < 0)
				throw new PXSetPropertyException(Messages.EmployeePartMustBeZero);
		}

		public virtual void VerifyEmployeePartSign(decimal? totalAmount, decimal? employeeAmount)
		{
			if (totalAmount * employeeAmount < 0)
				throw new PXSetPropertyException(Messages.EmployeePartSign);
		}
