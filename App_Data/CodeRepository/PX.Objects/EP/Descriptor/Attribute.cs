...
namespace PX.TM
{
	...
	public class OwnerAttribute : PXEntityAttribute, IPXFieldUpdatingSubscriber
	{
		...
		public OwnerAttribute(
			...
			PXSelectorMode selectorMode = PXSelectorMode.DisplayModeText)
		{
			_workgroupType = workgroupType;
			_substituteKey = substituteKey;
			_Attributes.Add(CreateSelector(search, fieldList, headerList, validateValue, selectorMode));
		...
		protected virtual PXSelectorAttribute CreateSelector(Type search, Type[] fieldList, string[] headerList, bool validateValue, PXSelectorMode selectorMode)
		{
	...
	{
		protected OwnerEmployeeAttribute(Type workgroupType,
			Type search,
			bool validateValue = true,
			bool inquiryMode = false,
			Type[] fieldList = null,
			string[] headerList = null,
			PXSelectorMode selectorMode = PXSelectorMode.Undefined)
			: base(workgroupType, search, validateValue, inquiryMode, fieldList, headerList, null, selectorMode)
		{ }

		protected override PXSelectorAttribute CreateSelector(Type search, Type[] fieldList, string[] headerList, bool validateValue, PXSelectorMode selectorMode)
		{
			PXSelectorAttribute selector = new PXSelectorAttribute(
				search,
			...
			selector.ValidateValue = validateValue;
			if(selectorMode != PXSelectorMode.Undefined)
			{
				selector.SelectorMode = selectorMode;
			}			
			if (headerList != null)
	...
	public class PXOwnerSelectorAttribute : PXAggregateAttribute
	{
		...
		[PXHidden]
		[Obsolete]
		public class EPEmployee : PXBqlTable, IBqlTable
		{
			#region PKID
...
namespace PX.Objects.EP
{
	...
	[Serializable]
	[PXHidden]
	public class ReasonApproveRejectFilter : PXBqlTable, IBqlTable
	{
		#region Reason
		...
		where Rejected : class, IBqlField
	{
		...
		protected override void Init(PXGraph graph)
		{
			...
			_RelatedApprovalsToDelete = CreateView($"{viewNamePrefix}_RelatedApprovalsToDelete", graph, BqlCommand.Compose(
				typeof(Select<,>), typeof(EPApproval),
				typeof(Where<,,>),
				typeof(EPApproval.refNoteID), typeof(Equal<>), typeof(Required<>), SourceNoteID,
				typeof(And<Where<
					EPApproval.status, Equal<EPApprovalStatus.approved>,
					Or<EPApproval.status, Equal<EPApprovalStatus.pending>,
					Or<Where<
						EPApproval.status, Equal<EPApprovalStatus.rejected>,
						And<EPApproval.reason, IsNull>>>>>>)));
		...
		{
			if (SuppressInFull) return false;
		...
		{
			foreach (EPApproval item in this._RelatedApprovalsToDelete.SelectMulti(GetSourceNoteID(doc)))
			{
				this.Cache.Delete(item);
			}
		}
						...
						if (!reasonSettings.Equals(EPReasonSettings.NotRequired))
						{
							if (reasonSettings.Equals(EPReasonSettings.Required))
							{
								if (this._Graph.UnattendedMode)
									throw new PXException(status.Equals(EPApprovalStatus.Approved) ?
										Messages.RequireCommentForApprove : Messages.RequireCommentForReject);
							}
							if (!this._Graph.UnattendedMode)
							{
									...
									if (attr != null)
									{
										...
										if (attr.PersistingCheck == PXPersistingCheck.Null && string.IsNullOrWhiteSpace(current.Reason))
										{
											PXUIFieldAttribute.SetError(cache, current, nameof(ReasonApproveRejectFilter.Reason),
												PXMessages.LocalizeFormat(Messages.ApprovalRuleReasonCannotBeEmpty, rule.Name));
										}
										else
										{
		...
		protected virtual void RowUpdated(PXCache cache, PXRowUpdatedEventArgs e)
		{
			...
			if (oldValue != null)
			{
				if (hold == true)
				{
					if (oldValue == false)
					{
						this.ClearRelatedApprovals(doc);
	...
	public class PXWeekSelectorAttribute : PXCustomSelectorAttribute, IPXFieldDefaultingSubscriber
	{
		...
		[Serializable]
		[PXHidden]
		public partial class EPWeek : PXBqlTable, IBqlTable
		{
			#region WeekID
