	...
	public class EPReleaseProcess : PXGraph<EPReleaseProcess>
	{
			...
			where TAPDocumentGraphExtension : InvoiceBaseGraphExtension<TGraph, TAPDocument, TInvoiceMapping>
		{
			...
			foreach (var receiptGroup in receiptsForDocument)
			{
				...
				invoice.Printed = invoice.DocType == APDocType.QuickCheck || invoice.DocType == APDocType.CashReturn;
				...
				bool reversedDocument = (invoice.DocType == APInvoiceType.DebitAdj && receiptGroupPaidWithType == EPExpenseClaimDetails.paidWith.PersonalAccount) || invoice.DocType == APInvoiceType.CashReturn;
...

		{
			...
			foreach (EPTaxTran epTaxTran in cmdEPTaxTran.Select(claimdetail.ClaimDetailID))
			{
				...
				EPTax epTax = cmdEPTax.Select(claimdetail.ClaimDetailID, epTaxTran.TaxID);
				if (epTax == null)
					continue;
