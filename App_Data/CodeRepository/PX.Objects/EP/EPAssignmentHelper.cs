...
namespace PX.Objects.EP
{
		...
		where Table : class, IBqlTable, new()
	{
			...
			foreach (EPRule rule in rules)
			{
				...
				switch (rule.RuleType)
				{
					case EPRuleType.Direct:
						...
						ApproveInfo approval;
						if (rule.OwnerID == null)
						{
							PXResultset<EPRuleApprover> approvers = PXSelect<EPRuleApprover, Where<EPRuleApprover.ruleID, Equal<Required<EPRule.ruleID>>>>
														.Select(this, rule.RuleID);
							if (approvers.Any())
							{
								foreach (EPRuleApprover approver in approvers)
								{
									OwnerID = approver.OwnerID;
									WorkgroupID = rule.WorkgroupID;

									approval = CreateApproveInfo(OwnerID, WorkgroupID, rule);
					...
					if (isAttributeField)
					{
						...
						string AttrValue(string value)
						{
							if (DateTime.TryParse(value, out DateTime date))
								return date.ToString(CSAttributeSelectorAttribute.AttributeDateTimeFormat, CultureInfo.InvariantCulture);
		...
		private ApproveInfo CreateApproveInfo(int? OwnerID, int? WorkgroupID, EPRule rule)
		{
			...
			if (OwnerID == null && WorkgroupID == null) return null;
