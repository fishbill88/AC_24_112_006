...
{ 
	...
	public partial class APInvoiceEntry : APDataEntryGraph<APInvoiceEntry, APInvoice>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		[PXCustomizeBaseAttribute(typeof(OwnerAttribute), nameof(PXUIFieldAttribute.Visibility), PXUIVisibility.Visible)]
		[PXCustomizeBaseAttribute(typeof(OwnerAttribute), nameof(PXUIFieldAttribute.Visible), false)]
		public virtual void _(Events.CacheAttached<EPApproval.origOwnerID> e) { }
		...
		public virtual void _(Events.CacheAttached<EPApproval.assignmentMapID> e) { }

		[PXCustomizeBaseAttribute(typeof(PXUIFieldAttribute), nameof(PXUIFieldAttribute.Visibility), PXUIVisibility.Visible)]
		[PXCustomizeBaseAttribute(typeof(PXUIFieldAttribute), nameof(PXUIFieldAttribute.Visible), false)]
		public virtual void _(Events.CacheAttached<EPApproval.ruleID> e) { }

		[PXCustomizeBaseAttribute(typeof(PXUIFieldAttribute), nameof(PXUIFieldAttribute.Visibility), PXUIVisibility.Visible)]
		[PXCustomizeBaseAttribute(typeof(PXUIFieldAttribute), nameof(PXUIFieldAttribute.Visible), false)]
		public virtual void _(Events.CacheAttached<EPApproval.stepID> e) { }

		[PXCustomizeBaseAttribute(typeof(PXUIFieldAttribute), nameof(PXUIFieldAttribute.Visibility), PXUIVisibility.Visible)]
		[PXCustomizeBaseAttribute(typeof(PXUIFieldAttribute), nameof(PXUIFieldAttribute.Visible), false)]
		public virtual void _(Events.CacheAttached<EPApproval.createdDateTime> e) { }
		...
		public virtual void ReverseInvoiceProc(APRegister doc)
		{
			...
			foreach (PXResult<APInvoice, CurrencyInfo, Terms, Vendor> res in APInvoice_CurrencyInfo_Terms_Vendor.Select(this, (object)doc.DocType, doc.RefNbr))
			{
				...
				invoice.IntercompanyInvoiceNoteID = null;
				...
				invoice = this.Document.Insert(invoice);
					...
					invoice.CuryOrigDiscAmt = APSetup.Current?.TermsInDebitAdjustments == false ? 0m : doc.CuryOrigDiscAmt;
        ...
        protected virtual void APInvoice_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			reverseInvoice.SetEnabled(Document.Current.OrigModule != BatchModule.TX);
		}
		...
		{
			return !string.IsNullOrEmpty(tran.PONbr) ||
				!string.IsNullOrEmpty(tran.ReceiptNbr) ||
				!string.IsNullOrEmpty(tran.LCDocType) && !string.IsNullOrEmpty(tran.LCRefNbr);
		}
		...
		protected virtual void APTran_SubID_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			if (vendor.Current?.Type == null || RelatedWithPOOrLC(documentLine) ||
				IsReverseContext)
			{
				return;
			}
		...
		protected virtual void APTran_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			bool isDocumentPrebookedOrReleased = doc?.Released == true || doc?.Prebooked == true;
			...
			PXUIFieldAttribute.SetEnabled<APTran.retainagePct>(cache, row, Document.Current?.RetainageApply == true && !isDocumentPrebookedOrReleased);
			PXUIFieldAttribute.SetEnabled<APTran.curyRetainageAmt>(cache, row, Document.Current?.RetainageApply == true && !isDocumentPrebookedOrReleased) ;

			// reset values to enable state  
			PXUIFieldAttribute.SetEnabled<APTran.manualDisc>(cache, row, !isDocumentPrebookedOrReleased);
			PXUIFieldAttribute.SetEnabled<APTran.curyDiscAmt>(cache, row, !isDocumentPrebookedOrReleased);
			PXUIFieldAttribute.SetEnabled<APTran.discPct>(cache, row, !isDocumentPrebookedOrReleased);
			PXUIFieldAttribute.SetEnabled<APTran.deferredCode>(cache, row, !isDocumentPrebookedOrReleased);
			PXUIFieldAttribute.SetEnabled<APTran.discountID>(cache, row, !isDocumentPrebookedOrReleased);
			...
			if (!isLCBasedTranAP)
			{
				if (isPOOrderRelated || isPOReceiptRelated)
				{
					...
					PXUIFieldAttribute.SetEnabled<APTran.uOM>(cache, row, isPOOrderBasedBilling && !isReverseTran && !isDocumentPrebookedOrReleased);
			...
			PXUIFieldAttribute.SetEnabled<APTran.projectID>(cache, row, isProjectEditable && APSetup.Current.RequireSingleProjectPerDocument != true && !(isDocumentReleased && vendor.Current?.Vendor1099 == true && doc.Status != APDocStatus.UnderReclassification));
			PXUIFieldAttribute.SetEnabled<APTran.taskID>(cache, row, isProjectEditable && !(isDocumentReleased && vendor.Current?.Vendor1099 == true && doc.Status != APDocStatus.UnderReclassification));
		...
		protected virtual void APTran_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
		{
			...
			{
				PMTask oldTask = PMTask.PK.Find(this, tran.ProjectID, tran.TaskID);
				PMTask newTask = PMTask.UK.Find(this, doc.ProjectID, oldTask.TaskCD);

				tran.ProjectID = doc.ProjectID;

				if (newTask != null)
				{
					tran.TaskID = newTask.TaskID;
				}
				else
				{
					tran.TaskID = null;
		...
		protected virtual void _(Events.FieldUpdated<APTaxTran, APTaxTran.taxID> e)
		{
			...
			{
				Taxes.Cache.RaiseFieldDefaulting<APTaxTran.accountID>(apTaxTran, out object newAccountID);
				apTaxTran.AccountID = (int?)newAccountID;
				Taxes.Cache.RaiseFieldDefaulting<APTaxTran.taxType>(apTaxTran, out object newTaxType);
				apTaxTran.TaxType = (string)newTaxType;
				Taxes.Cache.RaiseFieldDefaulting<APTaxTran.taxBucketID>(apTaxTran, out object newTaxBucket);
				apTaxTran.TaxBucketID = (int?)newTaxBucket;
			}
		}
		...
		protected virtual void _(Events.FieldDefaulting<APAdjust, APAdjust.adjgDocDate> e)
		{
			if (e.Row.DisplayDocDate < Document.Current.DocDate)
				e.NewValue = GetAdjgDate();
			else e.NewValue = Document.Current.DocDate;
		}

		protected virtual void _(Events.RowPersisting<APAdjust> e)
		{
			if (e.Row.DisplayDocDate < Document.Current.DocDate)
			{
				e.Row.AdjgDocDate = GetAdjgDate();
				e.Cache.RaiseFieldDefaulting<APAdjust.adjgFinPeriodID>(e.Row, out var adjgFinPeriodID);
				e.Row.AdjgFinPeriodID = FinPeriodIDFormattingAttribute.FormatForStoringNoTrim((string)adjgFinPeriodID);

				e.Cache.RaiseFieldDefaulting<APAdjust.adjgTranPeriodID>(e.Row, out var adjgTranPeriodID);
				e.Row.AdjgTranPeriodID = FinPeriodIDFormattingAttribute.FormatForStoringNoTrim((string)adjgTranPeriodID);
			}
		}
		protected virtual DateTime? GetAdjgDate()
		{	
			DateTime? earliestPaymentDate = Adjustments_Raw.Select()
				.Where(_ => (_.GetItem<APAdjust>().AdjAmt ?? 0m) != 0)
				.Select(_ => _.GetItem<APPayment>().AdjDate)
				.Where(_ => _.HasValue)
				.OrderBy(_ => _.Value)
				.FirstOrDefault();

			if (earliestPaymentDate.HasValue && earliestPaymentDate.Value > Document.Current.DocDate)
		...
		{
			return
				APSetupApproval.EPSetting.ApprovableDocTypes.Contains(doc.DocType) ||
				(doc.DocType == APDocType.Prepayment && APSetupApproval.EPSetting.ApprovableDocTypes.Contains(APDocType.PrepaymentRequest));
		}
