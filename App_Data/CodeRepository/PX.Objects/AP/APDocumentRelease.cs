...
namespace PX.Objects.AP
{
		...
		public static void ReleaseDoc(List<APRegister> list, bool isMassProcess, bool isPrebooking, List<Batch> externalPostList)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				try
				{
					...
					doc.ReleasedToVerify = (doc.Status == APDocStatus.Open && doc.OpenDoc == true && doc.Released == true) ? (bool?)null : false;
					...
					if (childs != null)
					{
						for (int j = 0; j < childs.Count; j++)
						{
							...
							var isOpenDoc = childs[j].Status == APDocStatus.Open && childs[j].OpenDoc == true && childs[j].Released == true;
	...
	public class APReleaseProcess : PXGraph<APReleaseProcess>
	{
		...
		public bool IsMigratedDocumentForProcessing(APRegister doc)
        {
			...
			//
			bool isQuickCheckOrVoidQuickCheckOrCashReturnDocument = doc.DocType == APDocType.QuickCheck ||
				doc.DocType == APDocType.VoidQuickCheck || doc.DocType == APDocType.CashReturn;

			return
				doc.IsMigratedRecord == true &&
				doc.Released != true &&
				doc.CuryInitDocBal != doc.CuryOrigDocAmt &&
				!isQuickCheckOrVoidQuickCheckOrCashReturnDocument;
        }
		...
		/// <summary>
		/// The method to fill a self document application (the same adjusted and adjusting documents)
		...
		{
			APAdjust adj = FillSelfApplicationForDocument(doc);
		...
		{
			// Find or create special application to update balances with proper amounts.
			// The current method can be called when recalculating vendor balances when the application already exists and is in the cache.
			// Therefore, before creating, we need to try to search for the application using key fields.
			// For the search, we lack the value of the key field AdjdLineNbr.
			// We use the InitNewRow method to fill AdjdLineNbr with the actual default value.
			APAdjust filledAdj = FillSelfApplicationForDocument(doc);
			filledAdj = APAdjust_AdjgDocType_RefNbr_VendorID.Cache.InitNewRow<APAdjust>(filledAdj);
			APAdjust initAdj = APAdjust_AdjgDocType_RefNbr_VendorID.Cache.Locate(filledAdj) as APAdjust;

			if (initAdj != null)
			{
				APAdjust_AdjgDocType_RefNbr_VendorID.Cache.SetStatus(initAdj, PXEntryStatus.Inserted);
			}
			else
			{
				initAdj = APAdjust_AdjgDocType_RefNbr_VendorID.Cache.Insert(filledAdj) as APAdjust;
			}
			...
			out List<INRegister> inDocs)
		{
			...
			if (doc.Released != true && (!isPrebooking || doc.Prebooked != true))
			{
				...
				bool isQuickCheckOrVoidQuickCheckCashReturnDocument = apdoc.DocType == APDocType.QuickCheck ||
					apdoc.DocType == APDocType.VoidQuickCheck || apdoc.DocType == APDocType.CashReturn;

				if (isPrebookCompletion == false)
				{
					if (isQuickCheckOrVoidQuickCheckCashReturnDocument)
					{
				...
				else
				{
					...
					else
					{
						foreach (PXResult<APTaxTran, Tax, APInvoice> r in APTaxTran_TranType_RefNbr.Select(apdoc.DocType, apdoc.RefNbr))
						{
							...
							if (apdoc.DocType == APDocType.QuickCheck || apdoc.DocType == APDocType.VoidQuickCheck || apdoc.DocType == APDocType.CashReturn)
							{
								...
								(x.TaxType == TX.TaxType.PendingPurchase || x.TaxType == TX.TaxType.PendingSales))
							{
								...
								string reversalMethod = x.TranType == APDocType.QuickCheck || x.TranType == APDocType.VoidQuickCheck || x.TranType == APDocType.CashReturn
				...
				if (!_IsIntegrityCheck && !IsInvoiceReclassification)
				{
					foreach (PXResult<APAdjust, APPayment> appres in PXSelectJoin<APAdjust, InnerJoin<APPayment, On<APPayment.docType, Equal<APAdjust.adjgDocType>, And<APPayment.refNbr, Equal<APAdjust.adjgRefNbr>>>>, Where<APAdjust.adjdDocType, Equal<Required<APAdjust.adjdDocType>>, And<APAdjust.adjdRefNbr, Equal<Required<APAdjust.adjdRefNbr>>, And<APAdjust.released, Equal<False>, And<APPayment.released, Equal<True>>>>>>.Select(this, doc.DocType, doc.RefNbr))
					{
						...
						if (((APAdjust)appres).CuryAdjdAmt > 0m)
						{
							...
							//are NOT always greater then payments period
							if (payment.AdjDate < adj.AdjdDocDate)
							{
			...
			bool isReversedTran = false)
		{
			...
			if (isOriginalRetainageDocumentWithTax)
			{
				...
				else if (!isUseTax)
				{
					...
					GLTran retainedTaxTran = PXCache<GLTran>.CreateCopy(origTran);
					retainedTaxTran.ReclassificationProhibited = true;
					retainedTaxTran.AccountID = !isReversedTax ? salestax.RetainageTaxClaimableAcctID : salestax.RetainageTaxPayableAcctID;
					retainedTaxTran.SubID = !isReversedTax ? salestax.RetainageTaxClaimableSubID : salestax.RetainageTaxPayableSubID;
			...
			else if (isChildRetainageDocumentWithTax)
			{
				...
				else
				{
					...
					GLTran extraGLTran = je.GLTranModuleBatNbr.Cache.Inserted.Cast<GLTran>().FirstOrDefault(d => d.RefNbr == apdoc.RefNbr && d.AccountID == apdoc.RetainageAcctID);
					Batch batch = je.Caches[typeof(Batch)].Current as Batch;
					decimal diff = (decimal)(batch?.DebitTotal - batch?.CreditTotal);

					if (extraGLTran == null || (extraGLTran.ReclassificationProhibited != true) || Math.Abs(Math.Round((decimal)(apdoc.CuryTaxRoundDiff) - diff, 4)) >= 0.00005m)
					{
		...
		public virtual void ProcessAdjustmentTranPost(APAdjust adj, APRegister doc, APRegister pmt, bool adjustedOnly = false)
		{
			...
			FinPeriodIDAttribute.SetPeriodsByMaster<APTranPost.finPeriodID>(TranPost.Cache, adjd, adj.AdjgTranPeriodID);
			...
			FinPeriodIDAttribute.SetPeriodsByMaster<APTranPost.finPeriodID>(TranPost.Cache, adjg, adj.AdjgTranPeriodID);
			...
			{
				if(!adjd.DocType.IsIn(APDocType.QuickCheck, APDocType.VoidQuickCheck, APDocType.CashReturn))
		...
		public virtual bool IsPPVCalcNeeded(POReceiptLine rctLine, APTran tran)
		{
			return
		...
		public static Amount GetExpensePostingAmount(PXGraph graph, POLine documentLine)
		{
			...
			PXResult<POLine, POTax, Tax, POOrder> queryResult = documentLineWithTaxes
					.Select(documentLine.OrderType, documentLine.OrderNbr, documentLine.LineNbr).AsEnumerable()
					.Cast<PXResult<POLine, POTax, Tax, POOrder>>()
					.First();

			return GetExpensePostingAmountBase(
				(POLine)queryResult,
				(POTax)queryResult,
				queryResult,
				 queryResult.GetItem<POOrder>()?.TaxCalcMode,
				currencyInfo.RoundCury,
				currencyInfo.RoundBase);
		}
		...
		public static Amount GetExpensePostingAmount(PXGraph graph, APTran documentLine)
		{
			...
			return GetExpensePostingAmount(graph, documentLine, queryResult, queryResult, queryResult, roundingFunction, roundingFunction);
		}
		...
		public static void AdjustTaxCalculationLevelForNetGrossEntryMode(APInvoice document, APTran documentLine, ref Tax taxCorrespondingToLine)
		{
			if (taxCorrespondingToLine?.TaxCalcType == CSTaxCalcType.Item
				&& PXAccess.FeatureInstalled<FeaturesSet.netGrossEntryMode>() && taxCorrespondingToLine.DirectTax != true)
			{
			...
			Func<decimal, decimal> roundBase)
		{
			...
			{
				Amount postingAmount = GetExpensePostingAmountBase(documentLine, lineTax, salesTax, document?.TaxCalcMode, roundCury, roundBase);
		...
		{
			return GetExpensePostingAmountBase(documentLine, lineTax, salesTax, document?.TaxCalcMode, round, round);
		}
		...
		public static Amount GetExpensePostingAmountBase(PXGraph graph,
			...
			Func<decimal, decimal> roundBase)
		{
			return GetExpensePostingAmountBase(documentLine, lineTax, salesTax, document?.TaxCalcMode, roundCury, roundBase);
		}
		...
		/// <returns></returns>
		public static Amount GetExpensePostingAmountBase(
			ITaxableDetail documentLine,
			ITaxDetailWithAmounts lineTax,
			Tax salesTax,
			string TaxCalcMode,
			...
			Func<decimal, decimal> roundBase)
		{
			if (lineTax?.TaxID != null && (salesTax?.IsRegularInclusiveTax() == true || TaxCalcMode == TaxCalculationMode.Gross))
			{
		...
		public static void Update1099Hist(PXGraph graph, decimal histMult, APAdjust adj, APTran tran, APRegister apdoc)
		{
			...
			if (adj.AdjgDocType == APDocType.VoidQuickCheck || adj.AdjgDocType == APDocType.Refund || adj.AdjgDocType == APDocType.VoidRefund || adj.AdjdDocType == APDocType.DebitAdj ||
				adj.AdjgDocType == APDocType.CashReturn)
            {
                histMult = -histMult;
            }
		...
		public virtual void UpdateBalances(APAdjust adj, APRegister adjddoc, Vendor vendor, APTran adjdtran)
		{
			...
			{
				decimal? discrepancy = adj.ReverseGainLoss == false ? apdoc.DocBal : -apdoc.DocBal;
				adj.RGOLAmt += discrepancy;
				apdoc.RGOLAmt += discrepancy;
			}
			...
			PXResult<APPayment, CurrencyInfo, Currency, Vendor, CashAccount> res)
		{
			...
			bool isQuickCheckOrVoidQuickCheckOrCashReturnDocument = apdoc.DocType == APDocType.QuickCheck ||
				apdoc.DocType == APDocType.VoidQuickCheck || apdoc.DocType == APDocType.CashReturn;
			...
			if (doc.Released != true)
			{
                ...
                {
					bool isCADebit = doc.DocType == APDocType.CashReturn ? charge.DrCr == GL.DrCr.Debit : charge.GetCASign() == 1;
                ...
                if(!isQuickCheckOrVoidQuickCheckOrCashReturnDocument)
			...
			Currency paycury)
		{
				...
				.GroupBy(res => ((APInvoice)res), APDocument.Cache.GetComparer()))
			{
					...
					|| AP1099Tran_Select.Any(adjddoc.DocType, adjddoc.RefNbr))
				{
					...
					if (adj.CuryAdjgAmt == 0m && adj.CuryAdjgDiscAmt == 0m && adj.CuryAdjgWhTaxAmt == 0m && adj.CuryAdjgPPDAmt == 0m
					...
					if (adjddoc.RefNbr != null)
					{
					if (adjddoc.ReleasedOrPrebooked != true && adjddoc.DocType != APDocType.QuickCheck && adjddoc.DocType != APDocType.VoidQuickCheck && adjddoc.DocType != APDocType.CashReturn)
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				foreach (PXResult<APPayment, CurrencyInfo, Currency, Vendor, CashAccount> res in APPayment_DocType_RefNbr.Select(doc.DocType, doc.RefNbr))
				{
					...
					if (doc.Prebooked == true && doc.Released != true &&
						(doc.DocType == APDocType.QuickCheck || doc.DocType == APDocType.VoidQuickCheck || doc.DocType == APDocType.DebitAdj || doc.DocType == APDocType.CashReturn))
					{
					...
					{
						if (doc.DocType != APDocType.QuickCheck && doc.DocType != APDocType.VoidQuickCheck && doc.DocType != APDocType.CashReturn)
						{
			...
			if (payment != null && string.IsNullOrEmpty(payment.ExtRefNbr))
			{
				...
				if (PaymentRefAttribute.PaymentRefMustBeUnique(paymentMethod))
				{
					...
					if (payment.DocType == APDocType.Check
							...
							|| payment.DocType == APDocType.CashReturn)
					{
		...
		public virtual void IntegrityCheckProc(Vendor vend, string startPeriod)
		{
			...
			using (new PXConnectionScope())
			{
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					{
						APRegister doc = APDocument.Cache.Locate(venddoc) as APRegister ?? venddoc;
						         ...
						         APPayment_DocType_RefNbr.Select(doc.DocType, doc.RefNbr, doc.VendorID))
						{
							...
							while (doc.AdjCntr < OrigAdjCntr)
							{
								...
								if (doc.AdjCntr == 0 || (doc.DocType != APDocType.QuickCheck &&
														 ...
														 doc.DocType != APDocType.CashReturn))
								{
		...
		protected virtual HashedList<APRegister> GetDocumentsForIntegrityCheckProc(string minPeriod)
		{
			...
			{
				GetReleasedOriginalDocumentsWithAdjustments(venddocs, minPeriod);
				GetReleasedRetainageDocumentsWithAdjustments(venddocs, minPeriod);
			}
		...
		protected virtual void GetReleasedOriginalDocumentsWithAdjustments(HashedList<APRegister> documents, string minPeriod)
		{
			...
			{
				PXResultset<APRegister> originalDocuments = new SelectFrom<APRegister>
					.LeftJoin<APInvoice>
						.On<APInvoice.refNbr.IsEqual<APRegister.refNbr>
						.And<APInvoice.docType.IsEqual<APRegister.docType>>>
					.Where<APRegister.docType.IsEqual<@P.AsString>
						...
						.And<APRegister.tranPeriodID.IsLess<@P.AsString>>
						.And<APRegister.closedTranPeriodID.IsNull.Or<APRegister.closedTranPeriodID.IsLess<@P.AsString>>>>
					.View(this)
					.Select(retainageDoc.OrigDocType, retainageDoc.OrigRefNbr, minPeriod, minPeriod);

				foreach (PXResult<APRegister> originalDoc in originalDocuments)
				{
					APInvoice doc = PXResult.Unwrap<APInvoice>(originalDoc);
					PXCache<APRegister>.RestoreCopy(doc, PXResult.Unwrap<APRegister>(originalDoc));
					if (!documents.Contains(doc))
					{
						documents.Add(new PXResult<APRegister>(doc));
					}
				}
		...
		protected virtual void GetReleasedRetainageDocumentsWithAdjustments(HashedList<APRegister> documents, string minPeriod)
		{
			...
			{
				PXResultset<APRegister> retainageDocuments = new SelectFrom<APRegister>
					.LeftJoin<APInvoice>
						.On<APInvoice.refNbr.IsEqual<APRegister.refNbr>
						.And<APInvoice.docType.IsEqual<APRegister.docType>>>
					.Where<APRegister.origDocType.IsEqual<@P.AsString>
						.And<APRegister.origRefNbr.IsEqual<@P.AsString>>
						.And<APRegister.isRetainageDocument.IsEqual<True>>
						.And<APRegister.released.IsEqual<True>>
						.And<APRegister.tranPeriodID.IsLess<@P.AsString>>
						.And<APRegister.closedTranPeriodID.IsNull.Or<APRegister.closedTranPeriodID.IsLess<@P.AsString>>>>
					.View(this)
				...
				foreach (PXResult<APRegister> retainageDoc in retainageDocuments)
				{
					APInvoice doc = PXResult.Unwrap<APInvoice>(retainageDoc);
					PXCache<APRegister>.RestoreCopy(doc, PXResult.Unwrap<APRegister>(retainageDoc));
					if (!documents.Contains(doc))
					{
						documents.Add(new PXResult<APRegister>(doc));
					}
		...
		public virtual void RetrievePPVAccount(PXGraph aOpGraph, POReceiptLine aLine, ref int? aPPVAcctID, ref int? aPPVSubID) 
		{
			aPPVAcctID = null;
			aPPVSubID = null;
			var invItem = InventoryItem.PK.Find(this, aLine.InventoryID);
			var postClass = INPostClass.PK.Find(this, invItem?.PostClassID);
			var invSite = INSite.PK.Find(this, aLine.SiteID);
			if (invItem != null && postClass != null && invSite != null)
			{
		...
		public virtual bool IsFullyProcessedOriginalRetainageDocument(APRegister origRetainageInvoice)
		{
			...
			//
			foreach (APRegister childRetainageBill in SelectRetainageDocuments(origRetainageInvoice))
			{
		...
		public class GLTranInsertionContext
		{
			...
			public virtual POReceiptLine POReceiptLineRecord { get; set; }
		}
