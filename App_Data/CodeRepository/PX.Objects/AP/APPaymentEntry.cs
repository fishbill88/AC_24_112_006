	...
	public partial class APPaymentEntry : APDataEntryGraph<APPaymentEntry, APPayment>
	{
		...
		public override IEnumerable VoidCheck(PXAdapter adapter)
		{
				...
				APPaymentType.VoidEnabled(Document.Current.DocType))
			{
				...
				if (!AskUserApprovalToVoidPayment(Document.Current))
				{
					return adapter.Get();
				}
		...
		{
			int?[] branches = this.CurrentUserInformationProvider.GetAllBranches().Select(b => (int?)b.Id).Distinct().ToArray();
			using (var scope = new PXReadBranchRestrictedScope(PXAccess.GetAvailableOrganizationIDs(), branches, true, true))
			{
		...
		public static PXResultset<APInvoice> GetVendDocs(APPayment currentAPPayment, PXGraph graph, APSetup currentAPSetup)
		{
			var cmd = new PXSelectReadonly2<APInvoice,
							...
							LeftJoin<APPayment, On<APPayment.docType, Equal<APInvoice.docType>, And<APPayment.refNbr, Equal<APInvoice.refNbr>, And<APPayment.docType, Equal<APDocType.prepayment>>>>>>>>>,
								...
								And<CurrencyInfo.baseCuryID, Equal<Required<CurrencyInfo.baseCuryID>>,
			...
			string documentBranchBaseCuryId = PXAccess.GetBranch(currentAPPayment.BranchID).BaseCuryID;
		...
		public virtual IEnumerable ReverseApplication(PXAdapter adapter)
		{
					...
					|| application.AdjgDocType == APDocType.Check))
			{
				...
				if (payment.OpenDoc != true)
				{
					...
					APPayment.Events.Select(ev => ev.OpenDocument).FireOn(this, payment);
			...
			else
			{
						...
						ref totalRows))
				{
					...
					ret.Add(mapper.CreateResult(
						new PXResult<APAdjust, APPayment, Standalone.APRegisterAlias, APInvoice>(res, fullPayment, (Standalone.APRegisterAlias)res, invoice)));
				}
				PXView.StartRow = 0;
				return ret;
			}	
		...
		public void OnBeforeCommitPayment(PXGraph graph)
		{
			if (!(Document?.Current?.VoidAppl == true || Document?.Current?.DocType == APDocType.QuickCheck))
			{
				...
				.LeftJoin<APInvoice>
				.On<APInvoice.refNbr.IsEqual<APAdjust.adjdRefNbr>
				...
				.View(this)
				.SelectSingle(Document?.Current?.RefNbr, Document?.Current?.DocType, Document?.Current?.DocType, Document?.Current?.RefNbr);
		...
		public static bool MustPrintCheck(IPrintCheckControlable payment, PaymentMethod paymentMethod)
			...
			return payment != null
				...
				&& !APDocType.CashReturn.Equals(payment.DocType)
		...
		protected virtual void APPayment_AdjDate_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if (payment.VoidAppl != true)
			{
				if (vendor.Current != null && vendor.Current.Vendor1099 == true)
				{
					...
					{
						throw new PXSetPropertyException(Messages.AP1099PaymentApplicationDateNotInOpenYear);
					}
				}
			}
		...
		protected virtual void _(Events.FieldVerifying<APPayment, APPayment.branchID> e)
		{
			APPayment payment = (APPayment)e.Row;

			if (payment.VoidAppl != true && vendor.Current != null && vendor.Current.Vendor1099 == true)
			{
				string year1099 = ((DateTime)payment.AdjDate).Year.ToString();
				AP1099Year year = PXSelect<AP1099Year,
											Where<AP1099Year.finYear, Equal<Required<AP1099Year.finYear>>,
													And<AP1099Year.organizationID, Equal<Required<AP1099Year.organizationID>>>>>
		...
		protected virtual void APPayment_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			else
			{
				...
				PXUIFieldAttribute.SetEnabled<APPayment.docDate>(cache, doc, false);
				PXUIFieldAttribute.SetEnabled<APPayment.finPeriodID>(cache, doc, false);
