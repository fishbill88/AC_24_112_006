...
namespace PX.Objects.AP
{
	...
	public class APPrintChecks : PXGraph<APPrintChecks>
	{
		...
		public virtual void PrintPayments(List<APPayment> list, PrintChecksFilter filter, PaymentMethod paymentMethod)
		{
				       ...
				       OrganizationLocalizationHelper.GetCurrentLocalizationCodeForBranch(cashAccount?.BranchID)))
			{
				...
				if (paymentMethod.APCreateBatchPayment == true)
				{
					...
					bool failedBecauseOfAddendaCalculation = false;
					...
					using (new CABatchEntry.PrintProcessContext(be))
					{
						...
						if (batch != null)
						{
							...
							{
								PXProcessing<APPayment>.SetError(e);
								failedBecauseOfAddendaCalculation = true;
								innerErrorMessage = e.MessageNoPrefix;
								failed = true;
							}
							catch (PXException e)
							{
						...
						if (failed && !batchHasPayments)
						{
							if(failedBecauseOfAddendaCalculation)
							{
								throw new PXOperationCompletedWithErrorException(CA.Messages.BatchPaymentCannotBeCreated, innerErrorMessage);
							}
			...
			where T : IBqlField
		{
			...
			else if (string.IsNullOrEmpty(existingError) && !string.IsNullOrEmpty(row.NextCheckNbr) && !AutoNumberAttribute.TryToGetNextNumber(row.NextCheckNbr))
			{
				string message = AutoNumberAttribute.CheckIfNumberEndsDigit(row.NextCheckNbr) ? Messages.NextCheckNumberLastReached : Messages.NextCheckNumberMustEndDigit;
				sender.RaiseExceptionHandling<T>(row, row.NextCheckNbr,
					new PXSetPropertyException(message, PXErrorLevel.Warning, PXUIFieldAttribute.GetDisplayName<T>(sender)));
			}
	...
	[Serializable]
	public partial class PrintChecksFilter : PXBqlTable, IBqlTable
	{
		#region BranchID
