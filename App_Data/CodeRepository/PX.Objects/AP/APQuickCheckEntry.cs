...
namespace PX.Objects.AP
{
	...
	{			
		...
		{
			if (apQuickCheck.Deposited == true)
			{
				string localizedMsg = PXMessages.LocalizeNoPrefix(Messages.PaymentDepositedConfirmation);
				return Document.View.Ask(localizedMsg, MessageButtons.OKCancel) == WebDialogResult.OK;
			}
			return true;
		}
		...
		public override IEnumerable VoidCheck(PXAdapter adapter)
		{
			if (Document.Current != null && (Document.Current.Released == true || Document.Current.Prebooked == true) && Document.Current.Voided == false && Document.Current.DocType == APDocType.QuickCheck)
			{
				...
				APQuickCheck voidcheck = Document.Search<APQuickCheck.refNbr>(Document.Current.RefNbr, APPaymentType.GetVoidingAPDocType(Document.Current.DocType));
				...
				if (!AskUserApprovalToVoidQuickCheck(Document.Current))
				{
					return adapter.Get();
				}
				try
		...
		{
			if (Document.Current != null && (Document.Current.Released == true) && Document.Current.Voided == false && Document.Current.DocType == APDocType.QuickCheck)
			{
				APQuickCheck doc = PXCache<APQuickCheck>.CreateCopy(Document.Current);
				FinPeriodUtils.VerifyAndSetFirstOpenedFinPeriod<APQuickCheck.finPeriodID, APQuickCheck.branchID>(Document.Cache, doc, finperiod, typeof(OrganizationFinPeriod.aPClosed));

				if (!AskUserApprovalIfCashReturnDocumentAlreadyExists(Document.Current))
				{
					return adapter.Get();
				}
				...
				try
				{
					_IsCashReturnInProgress = true;
					this.CashReturnProc(doc);
				}
				catch (PXSetPropertyException)
				{
					this.Clear();
				...
				{
					throw new PXRedirectRequiredException(this, Messages.CashReturn);
				}
			}
		...
		public override void Persist()
		{
			...
			Transactions.View.Clear();

			if (aptran != null)
				Transactions.Current = APTran.PK.Find(this, aptran.TranType, aptran.RefNbr, aptran.LineNbr);
		}
		...
		protected virtual void _(Events.FieldDefaulting<APQuickCheck, APQuickCheck.depositAfter> e)
		{
			if (e.Row.DocType == APDocType.CashReturn && e.Row.DepositAsBatch == true)
			{
				e.NewValue = e.Row.AdjDate;
				e.Cancel = true;
			}
		}

		protected virtual void _(Events.FieldUpdated<APQuickCheck, APQuickCheck.depositAsBatch> e)
		{
			if (e.Row.DocType == APDocType.CashReturn)
			{
				e.Cache.SetDefaultExt<APQuickCheck.depositAfter>(e.Row);
			}
		}
		...
		protected virtual void APQuickCheck_AdjDate_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if (quickCheck.VoidAppl != true)
			{
				if (vendor.Current != null && (bool)vendor.Current.Vendor1099)
				{
					...
					{
						throw new PXSetPropertyException(Messages.AP1099CashPurchaseDateNotInOpenYear);
					}
				}
			}
		}

		protected virtual void _(Events.FieldVerifying<APQuickCheck, APQuickCheck.branchID> e)
		{
			APQuickCheck quickCheck = (APQuickCheck)e.Row;
			...
			if (quickCheck.VoidAppl != true && vendor.Current != null && vendor.Current.Vendor1099 == true)
			{
				string year1099 = ((DateTime)quickCheck.AdjDate).Year.ToString();
				AP1099Year year = PXSelect<AP1099Year,
											Where<AP1099Year.finYear, Equal<Required<AP1099Year.finYear>>,
													And<AP1099Year.organizationID, Equal<Required<AP1099Year.organizationID>>>>>
											.Select(this, year1099, PXAccess.GetParentOrganizationID((int?)e.NewValue));

				if (year != null && year.Status != "N")
				{
					Branch branch = Branch.PK.Find(this, (int?)e.NewValue);
		...
		{
			return APSetupApproval.EPSetting.ApprovableDocTypes.Contains(doc.DocType);
		}
		...
		protected virtual void APQuickCheck_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			cashReturn.SetEnabled(doc.Released == true && doc.Voided == false && doc.DocType == APDocType.QuickCheck);
			printAPPayment.SetEnabled( doc.DocType != APDocType.CashReturn || ( doc.DocType == APDocType.CashReturn && doc.Released == true));
			...
			PXUIFieldAttribute.SetVisible<APTran.defScheduleID>(Transactions.Cache, null, false);
			PXUIFieldAttribute.SetVisibility<APTran.defScheduleID>(Transactions.Cache, null, PXUIVisibility.Invisible);

			this.PaymentCharges.Cache.AllowInsert = ((doc.DocType == APDocType.QuickCheck || doc.DocType == APDocType.VoidQuickCheck || doc.DocType == APDocType.CashReturn)
				&& doc.Released != true && doc.Prebooked != true);
			this.PaymentCharges.Cache.AllowUpdate = ((doc.DocType == APDocType.QuickCheck || doc.DocType == APDocType.VoidQuickCheck || doc.DocType == APDocType.CashReturn)
				&& doc.Released != true && doc.Prebooked != true);
			this.PaymentCharges.Cache.AllowDelete = ((doc.DocType == APDocType.QuickCheck || doc.DocType == APDocType.VoidQuickCheck || doc.DocType == APDocType.CashReturn)
				&& doc.Released != true && doc.Prebooked != true);
			...
			if (!_IsVoidCheckInProgress && !_IsCashReturnInProgress)
			{
			...
			if (doc.DocType == APDocType.CashReturn)
			{
			...
			bool isDepositAfterEditable = doc.DocType == APDocType.CashReturn && doc.DepositAsBatch == true;
			PXDefaultAttribute.SetPersistingCheck<APQuickCheck.depositAfter>(cache, doc, isDepositAfterEditable ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
			PXUIFieldAttribute.SetVisible<APQuickCheck.depositAfter>(cache, doc, isDepositAfterEditable);
			PXUIFieldAttribute.SetRequired<APQuickCheck.depositAfter>(cache, isDepositAfterEditable);
			PXUIFieldAttribute.SetEnabled<APQuickCheck.depositAfter>(cache, doc, isDepositAfterEditable);

			bool isDeposited = string.IsNullOrEmpty(doc.DepositNbr) == false && string.IsNullOrEmpty(doc.DepositType) == false;
			CashAccount cashAccount = PXSelectorAttribute.Select<APQuickCheck.cashAccountID>(cache, doc) as CashAccount;
			bool isClearingAccount = cashAccount != null && cashAccount.ClearingAccount == true;
			bool enableDepositEdit = !isDeposited && cashAccount != null && isClearingAccount;

			if (enableDepositEdit)
			{
				PXSetPropertyException exception = doc.DepositAsBatch != isClearingAccount ? new PXSetPropertyException(Messages.DocsDepositAsBatchSettingDoesNotMatchClearingAccountFlag, PXErrorLevel.Warning): null;
				cache.RaiseExceptionHandling<APQuickCheck.depositAsBatch>(doc, doc.DepositAsBatch, exception);
			}
			PXUIFieldAttribute.SetEnabled<APQuickCheck.depositAsBatch>(cache, doc, enableDepositEdit);
			PXUIFieldAttribute.SetEnabled<APQuickCheck.deposited>(cache, null, false);
			PXUIFieldAttribute.SetEnabled<APQuickCheck.depositType>(cache, null, false);
			PXUIFieldAttribute.SetEnabled<APQuickCheck.depositNbr>(cache, null, false);
		}
		...
		#region CashReturn
		private bool _IsCashReturnInProgress = false;
		...
		public virtual void CashReturnProc(APQuickCheck doc)
		{
					...
					And<APQuickCheck.refNbr, Equal<Required<APQuickCheck.refNbr>>>>>.Select(this, doc.DocType, doc.RefNbr))
			{
				...
				APQuickCheck cashReturn = Document.Insert(new APQuickCheck
				{
					DocType = APDocType.CashReturn,
					RefNbr = null,
					CuryInfoID = info.CuryInfoID
				});

				if (cashReturn.RefNbr == null)
				{
					//manual numbering, check for duplicate
					APQuickCheck duplicate = PXSelect<APQuickCheck>.Search<APQuickCheck.docType, APQuickCheck.refNbr>(this, APDocType.CashReturn, doc.RefNbr);
					if (duplicate != null)
					{
						throw new PXException(ErrorMessages.RecordExists);
					}

					cashReturn.RefNbr = doc.RefNbr;
					this.Document.Cache.Normalize();
					cashReturn = this.Document.Update(cashReturn);
				}

				cashReturn = PXCache<APQuickCheck>.CreateCopy(doc);

				cashReturn.DocType = APDocType.CashReturn;
				cashReturn.RefNbr = Document.Current.RefNbr;
				cashReturn.CuryInfoID = info.CuryInfoID;
				cashReturn.CATranID = null;
				cashReturn.NoteID = null;
				cashReturn.OpenDoc = true;
				cashReturn.Released = false;
				cashReturn.PrebookBatchNbr = null;
				cashReturn.Prebooked = false;
				Document.Cache.SetDefaultExt<APQuickCheck.hold>(cashReturn);
				Document.Cache.SetDefaultExt<APQuickCheck.isMigratedRecord>(cashReturn);
				Document.Cache.SetDefaultExt<APQuickCheck.status>(cashReturn);
				cashReturn.LineCntr = 0;
				cashReturn.AdjCntr = 0;
				cashReturn.BatchNbr = null;
				cashReturn.AdjDate = doc.DocDate;
				cashReturn.AdjFinPeriodID = doc.AdjFinPeriodID;
				cashReturn.AdjTranPeriodID = doc.AdjTranPeriodID;
				cashReturn.CuryDocBal = cashReturn.CuryOrigDocAmt + cashReturn.CuryOrigDiscAmt;
				cashReturn.CuryLineTotal = 0m;
				cashReturn.CuryTaxTotal = 0m;
				cashReturn.CuryOrigWhTaxAmt = 0m;
				cashReturn.CuryChargeAmt = 0;
				cashReturn.CuryVatExemptTotal = 0;
				cashReturn.CuryVatTaxableTotal = 0;
				cashReturn.ClosedDate = null;
				cashReturn.ClosedFinPeriodID = null;
				cashReturn.ClosedTranPeriodID = null;
				cashReturn.Printed = true;
				cashReturn.PrintCheck = false; 
				cashReturn.CashAccountID = null;
				cashReturn.ExtRefNbr = null;
				cashReturn.EmployeeID = null;
				cashReturn.EmployeeWorkgroupID = null;
				cashReturn.Cleared = false;
			    cashReturn.ClearDate = null;

				cashReturn.OrigDocType = doc.DocType;
				cashReturn.OrigRefNbr = doc.RefNbr;
				cashReturn.OrigModule = GL.BatchModule.AP;

				Document.Update(cashReturn);

				cashReturn.CashAccountID = doc.CashAccountID;
				Document.Update(cashReturn);
		...
		protected virtual bool AskUserApprovalIfCashReturnDocumentAlreadyExists(APQuickCheck origDoc)
		{
			APRegister reversingDoc = PXSelect<APRegister,
				Where<APRegister.docType, Equal<APDocType.cashReturn>,
