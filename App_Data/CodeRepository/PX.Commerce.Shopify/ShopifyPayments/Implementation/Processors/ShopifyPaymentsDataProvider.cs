...
namespace PX.Commerce.Shopify.ShopifyPayments
{
	public abstract class ShopifyPaymentsDataProvider : OrderRestDataProvider
	{
		...
		public static IShopifyRestClient GetRestClient(IEnumerable<SettingsValue> settingValues)
		{
			...
			var restclient=((SPConnector)ConnectorHelper.GetConnector(SPConnector.TYPE)).shopifyRestClientFactory.GetRestClient(shopifyStore);
			return restclient;
		}
		...
		public async Task PrepareBCSyncStatusUpdate(BCSyncStatus bcSyncStatus)
		{
			...
			if (bcSyncStatus != null && bcSyncStatus.ExternID != null)
			{
				...
				OriginalOrderData =await GetByID(bcSyncStatus.ExternID);
			}
			else
			{
		...
		public async Task UpdateBCSyncStatus(PXGraph graph)
		{
			...
			OrderData newOrderData = await GetByID(bcSyncStatus.ExternID);
