...
{
	public class AmazonFBMSalesOrderValidator : BCBaseValidator, ISettingsValidator, ILocalValidator
	{
		public int Priority { get { return 0; } }

		public virtual void Validate(IProcessor iproc)
		{
			Validate<AmazonFBMSalesOrderProcessor>(iproc, (processor) =>
			{
		...
		{
			Validate<AmazonFBMSalesOrderProcessor, SalesOrder>(iproc, ilocal, (processor, entity) =>
			{
				BCBinding store = processor.GetBinding();
				BCBindingExt storeExt = processor.GetBindingExt<BCBindingExt>();
				...
				if (string.IsNullOrEmpty(bindingAmazon.SellerFulfilledOrderType))
                    throw new PXException(AmazonMessages.CannotProcessBecauseofSellerFulfilledOrderTypeIsEmpty);

                var orderType = SOOrderType.PK.Find(processor, bindingAmazon.SellerFulfilledOrderType);
