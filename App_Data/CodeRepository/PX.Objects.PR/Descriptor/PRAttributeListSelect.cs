...
namespace PX.Objects.PR
{
	...
	public abstract class PRAttributeSelectBase<TEntity, TSelect, TDynamicType, TTypeMeta> : PXSelectBase<TEntity>, IDynamicSettingsUpdateObserver
		...
		where TTypeMeta : SettingDefinitionListAttribute
	{
		...
		private static ConcurrentDictionary<string, ISettingDefinition> _SettingDefinitionCache = new ConcurrentDictionary<string, ISettingDefinition>();

		public void DynamicSettingsUpdated()
		{
			_SettingDefinitionCache.Clear();
		}
		...
		protected ISettingDefinition GetSettingDefinition(string settingName)
		{
			...
			if (_SettingDefinitionCache.TryGetValue(settingName, out settingDefinition))
			{
			...
			_SettingDefinitionCache[settingName] = settingDefinition;
			return settingDefinition;
		}
	...
	public class PRAttributeValuesSelect<TEntity, TSelect, TDefinitionEntity, TDefinitionSelect, TTypeSettingSelect, TDynamicType, TTypeMeta> : PRAttributeValuesSelectBase<TEntity, TSelect, TDefinitionEntity, TDefinitionSelect, TDynamicType, TTypeMeta>
		where TEntity : class, IBqlTable, IPRSetting, new()
		where TSelect : class, IBqlSelect, new()
		where TDefinitionEntity : class, IBqlTable, IPRSetting, new()
		where TDefinitionSelect : class, IBqlSelect, new()
		...
		where TTypeMeta : SettingDefinitionListAttribute
	{
		...
		protected override IEnumerable<IPRReflectiveSettingMapper<TTypeMeta>> GetMetaTypes()
		{
			...
			return settingView.SelectMulti().Select(x => new PRTaxCodeTaxSettingMapper(x as PRTaxCode) as IPRReflectiveSettingMapper<TTypeMeta>);
		}

		protected override IEnumerable<IPRReflectiveSettingMapper<TTypeMeta>> GetAllMetaTypes()
		{
			return SelectFrom<PRTaxCode>.View.Select(_Graph).FirstTableItems.Select(x => new PRTaxCodeTaxSettingMapper(x) as IPRReflectiveSettingMapper<TTypeMeta>);
		}
	...
	public class PRAttributeDefinitionSelect<TEntity, TSelect, TDynamicType, TTypeMeta> : PRAttributeDefinitionSelectBase<TEntity, TSelect, TDynamicType, TTypeMeta>
		where TEntity : class, IBqlTable, IPRSetting, new()
		where TSelect : class, IBqlSelect, new()
		where TDynamicType : DynamicEntity<TDynamicType, TTypeMeta>
		where TTypeMeta : SettingDefinitionListAttribute
	{
		...
		protected override IEnumerable<IPRReflectiveSettingMapper<TTypeMeta>> GetMetaTypes()
		{
			PRTaxCode current = _Graph.Caches[typeof(PRTaxCode)].Current as PRTaxCode;
			if (current != null)
			{
				yield return new PRTaxCodeTaxSettingMapper(current) as IPRReflectiveSettingMapper<TTypeMeta>;
			}
			else
			{
				yield break;
		...
		protected override IEnumerable<TaxSetting> FilterTaxSettingsInView(IEnumerable<TaxSetting> unfiltered)
		{
			ITaxCode current = _Graph.Caches[typeof(PRTaxCode)].Current as ITaxCode;
			return unfiltered.Where(setting => !setting.IsEmployeeSetting
				&& !string.IsNullOrWhiteSpace(setting.ApplicableTaxUniqueID)
