...
namespace PX.Objects.PR
{
	...
	public abstract class PRAccountAttribute : AccountAttribute
	{
		...
		public Type PayrollTypeField { get; set; } = typeof(PRPayment.docType);
		...
		{
			if (row == null || IsVoidCheck(sender, row))
			{
				return;
			}
		...
		protected bool IsVoidCheck(PXCache sender, object row)
		{
			string payrollType = CacheHelper.GetCurrentValue(sender.Graph, PayrollTypeField) as string;
			return payrollType == PayrollType.VoidCheck;
		}
	}
	...
	public abstract class PRExpenseAccountAttribte : PRAccountAttribute
	{
		...
		{
			if (row == null || IsVoidCheck(sender, row) || (sender.Graph.IsImportFromExcel || sender.Graph.IsContractBasedAPI) && sender.GetValue(row, FieldName) != null)
			{
				return;
			}
		...
		protected override int? GetAccountValue(PXCache cache, object setupFieldValue, object row)
		{
			switch (setupFieldValue)
			{
				case PRTaxAcctSubDefault.MaskTaxCode:
					...
					var taxCode = (PRTaxCode)SelectFrom<PRTaxCode>
						.Where<PRTaxCode.taxID.IsEqual<P.AsInt>
							.And<PRTaxCode.isDeleted.IsEqual<False>>>.View.Select(cache.Graph, taxCodeID);
