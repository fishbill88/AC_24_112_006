...
namespace PX.Objects.PR
{
	...
	public partial class PRCalculationEngine : PXGraph<PRCalculationEngine>
	{
		...
		public PaymentCalculationInfoCollection PaymentsToProcess { get; protected set; }
		...
		public SelectFrom<PRTaxCode>
			...
			.Where<PRTaxCode.countryID.IsEqual<PRPayment.countryID.FromCurrent>
				.And<PRTaxCode.isDeleted.IsEqual<False>>>.View EarningTaxList;
		...
		public SelectFrom<PREmployeeTax>
			...
			.Where<PREmployeeTax.bAccountID.IsEqual<PRPayment.employeeID.FromCurrent>
				...
				.And<PRTaxCode.countryID.IsEqual<PRPayment.countryID.FromCurrent>>
				.And<PRTaxCode.isDeleted.IsEqual<False>>>.View TaxSettings;
		...
		public SelectFrom<PRYtdTaxes>
			...
			.Where<PRYtdTaxes.employeeID.IsEqual<PRPayment.employeeID.FromCurrent>
				...
				.And<PRTaxCode.countryID.IsEqual<PRPayment.countryID.FromCurrent>>
				.And<PRTaxCode.isDeleted.IsEqual<False>>>.View EmployeeYTDTaxes;
		...
		public SelectFrom<PRTaxCodeAttribute>
			...
			.Where<PREmployeeTax.bAccountID.IsEqual<PRPayment.employeeID.FromCurrent>
				.And<PRTaxCode.countryID.IsEqual<PRPayment.countryID.FromCurrent>>
				.And<PRTaxCode.isDeleted.IsEqual<False>>>.View TaxSettingAttributes;
		...
		public SelectFrom<PRPTOAdjustmentDetail>
			.Where<PRPTOAdjustmentDetail.paymentDocType.IsEqual<PRPayment.docType.FromCurrent>
				.And<PRPTOAdjustmentDetail.paymentRefNbr.IsEqual<PRPayment.refNbr.FromCurrent>>>.View PTOAdjustmentDetails;
		...
		public static void Run(List<PRPayment> payments, bool isMassProcess = true)
		{
			...
			try
			{
				...
				{
					validationGraph.ValidateEarningDetails(payment);
				}

				calculationEngine.Run();

				validationGraph = CreateInstance<PRValidatePaycheckTotals>();
				foreach (PRPayment payment in calculationEngine.PaymentsToProcess.GetAllPayments())
				{
					calculationEngine.Payments.Current = payment;
			...
			foreach (PRPayment payment in _PaymentList)
			{
				...
				else
				{
					...
					foreach (PXResult<PREarningDetail, PRLocation, Address> result in PaymentEarningDetails.Select())
					{
						...
						{
							errorMessage = DateInPeriodAttribute.GetIncorrectDateMessage();
							break;
						}
					...
					if (errorMessage == null)
					{
						...
						{
							errorMessage = string.Format(Messages.EmployeeAddressNotRecognized,
								string.Format("{0} ({1})", employee.AcctName, employee.AcctCD));
						}
					}
		...
		protected virtual Dictionary<int?, List<ISettingDefinition>> CreateTaxSettingDictionary()
		{
				...
				.GroupBy(x => ((PRTaxCodeAttribute)x).TaxID))
			{
				...
				foreach (PXResult<PRTaxCodeAttribute, PREmployeeTaxAttribute> result in grouping)
				{
					...
					{
						PRTaxCode taxCode = SelectFrom<PRTaxCode>
							.Where<PRTaxCode.isDeleted.IsEqual<False>>.View
							.Search<PRTaxCode.taxID>(this, companyTaxSettingAttribute.TaxID);
		...
		protected virtual IEnumerable<PRWage> PrepareWageData()
		{
			List<PXResult<PREarningTypeDetail, PRTaxCode>> earningTypeTaxabilityRecords = SelectFrom<PREarningTypeDetail>
				.InnerJoin<PRTaxCode>.On<PRTaxCode.taxID.IsEqual<PREarningTypeDetail.taxID>>
				.Where<PRTaxCode.isDeleted.IsEqual<False>>.View
				.Select(this)
			...
			where TBenefitTypeField : IBqlField
		{
			...
			SetBenefitPercentages(benefit, deductionCode, employeeDeduction);
		...
		protected virtual IEnumerable<string> GetBenefitTaxList(int? codeID, bool useReturnList)
		{
			var deductionTaxList = new SelectFrom<PRTaxCode>
				...
				.Where<PRTaxCode.countryID.IsEqual<PRPayment.countryID.FromCurrent>
					.And<PRTaxCode.isDeleted.IsEqual<False>>>.View(this);
		...
		protected virtual void PopulateTaxSettingData(string referenceNbr, PXPayrollAssemblyScope<PayrollCalculationProxy> payrollAssemblyScope)
		{
			...
			foreach (PXResult<PREmployeeTax, PRTaxCode> taxEmployeeCompanySetting in TaxSettings.Select())
			{
				...
				payrollAssemblyScope.Proxy.AddTaxSetting(
					...
					compagnyWagesQtd,
					compagnyWagesYtd,
					new PRTaxCodeTaxSettingMapper(companyTaxSetting),
		...
		protected virtual void SaveTaxCalculations(PRPayrollCalculation payrollCalculation)
		{
			...
			{
				Dictionary<string, int?> taxUniqueIDs = SelectFrom<PRTaxCode>
					.Where<PRTaxCode.isDeleted.IsEqual<False>>.View
					.Select(this).FirstTableItems.ToDictionary(k => k.TaxUniqueCode, v => v.TaxID);
			...
			List<FringeSourceEarning> fringeRates)
		{
			...
			{
				decimal totalProjectFringeAmount = 0m;
				foreach (IEnumerable<FringeSourceEarning> projectTaskGroup in projectGroup.GroupBy(x => new { x.Earning.LabourItemID, x.Earning.ProjectTaskID }))
				{
					totalProjectFringeAmount += Math.Max(projectTaskGroup.Sum(x => x.CalculatedFringeAmount), 0m);
				}
					...
					if (fringeAmountsPerProject.ContainsKey(projectGroup.Key))
					{
						...
						if (fringeAmountAsEarning > 0 && totalFringeAmount != 0)
						{
							...
							decimal amountToSubstract = 0;
								foreach (FringeSourceEarning sourceEarningInfo in projectTaskGroup.Where(x => x.CalculatedFringeAmount != 0).OrderBy(x => x.CalculatedFringeAmount))
							{
		...
		protected virtual void CalculateTaxesOnFringeEarnings(IEnumerable<PRPayrollCalculation> payrollCalculations)
		{
			using (var payrollAssemblyScope = new PXPayrollAssemblyScope<PayrollCalculationProxy>())
			{
				...
				foreach (var payrollCalculation in payrollCalculations)
				{
					PaymentCalculationInfo paymentCalculationInfo = PaymentsToProcess[payrollCalculation.ReferenceNbr];
					PRPayment payment = paymentCalculationInfo.Payment;
					Payments.Current = payment;

					bool adjustedFringePayoutBenefits =
						paymentCalculationInfo.FringeAmountsPerProject.Values.Any(y => y.DeductCode?.AffectsTaxes == true && y.ProjectFringeAmountAsBenefit > 0);

					if ((FringeBenefitCalculatedEarnings.Select().Any() || adjustedFringePayoutBenefits) && payment.CountryID == LocationConstants.USCountryCode)
					{
			...
			using (PXTransactionScope transactionScope = new PXTransactionScope())
			{
				foreach (OvertimeDistribution overtimeDistribution in overtimeDistributions)
				{
					...
					foreach (string field in cache.Fields)
					{
						...
						{
						if (!cache.GetAttributesOfType<PXDefaultAttribute>(overtimeEarningDetailRecord, field)
							.Any(x => x.PersistingCheck == PXPersistingCheck.Null || x.PersistingCheck == PXPersistingCheck.NullOrBlank))
							{
		...
		protected virtual void PerformPTOCalculations()
		{
			foreach (PRPayment payment in PaymentsToProcess.GetAllPayments())
			{
				Payments.Current = payment;

				ValidatePTOUsageRestrictionsDuringProbationPeriod();
		...
		protected virtual void ProcessCarryover(PREmployeePTOBank sourceBank, PRPaymentPTOBank effectiveBank, DateTime targetDate)
		{
			...
			{
				effectiveBank.CarryoverAmount = PTOHelper.CalculateHoursToCarryover(this, Payments.Current, sourceBank, yearSummary.StartDate);
				effectiveBank.ProcessedCarryover = true;

				PTOAmountOfMoneyHelper.UpdateCarryOverMoney(effectiveBank, PTOHelper.CalculateMoneyToCarryover(this, Payments.Current, sourceBank, yearSummary.StartDate));
			}
		}

		protected virtual void ProcessFrontLoading(PREmployeePTOBank sourceBank, PRPaymentPTOBank effectiveBank, DateTime targetDate)
		{
			PTOHelper.PTOYearSummary yearSummary = GetPTOYearAndPaymentSummary(this, targetDate, Payments.Current.EmployeeID.Value, sourceBank);
			...
			if (yearSummary.ProcessedFrontLoading == false &&
				(effectiveBank.AccrualMethod == PTOAccrualMethod.FrontLoading ||
				effectiveBank.AccrualMethod == PTOAccrualMethod.FrontLoadingAndHoursPerYear ||
				effectiveBank.AccrualMethod == PTOAccrualMethod.FrontLoadingAndPercentage))
			{
		...
		protected virtual void CalculateCarryover()
		{
			...
			{
				PREmployeePTOBank sourceBank = PTOHelper.GetBankSettings(this, effectiveBank.BankID, Payments.Current.EmployeeID.Value, effectiveBank.EffectiveStartDate.Value);
				...
				{
					if (IsPaycheckCalculatedDuringProbationPeriod())
					{
						if (sourceBank.ProbationPeriodBehaviour == ProbationPeriodBehaviour.NotAccrued && !PaycheckContainsEarningsPastProbationDate())
						{
							continue;
						}
					}
		...
		protected virtual void RegularizeAccrualAmount(PREmployeePTOBank sourceBank, PRPaymentPTOBank effectiveBank, DateTime targetDate)
		{
			...
			{
				carryoverAmount = PTOHelper.CalculateHoursToCarryover(this, Payments.Current, sourceBank, updatedYearSummary.StartDate);
			}
			...
			if (sourceBank.AccrualLimit != null)
			{
				...
				{
					balance += effectiveBank.DisbursementAmount.GetValueOrDefault();
				}
				...
				if (balance > sourceBank.AccrualLimit)
				{
					...
					// Round the available amount and limited accrual hours to two decimal places
					decimal roundedAvailableAmount = Math.Round(effectiveBank.AvailableAmount.GetValueOrDefault(), 2, MidpointRounding.AwayFromZero);
					decimal roundedLimitedAccrualHours = Math.Round(limitedAccrualHours, 2, MidpointRounding.AwayFromZero);

					// Check if the rounded sum of available amount and limited accrual hours exceeds the accrual limit
					if (roundedAvailableAmount + roundedLimitedAccrualHours > sourceBank.AccrualLimit)
					{
						// Adjust limited accrual hours to not exceed the accrual limit
						limitedAccrualHours = Math.Floor(limitedAccrualHours * 100) / 100;
					}
					...
					PTOAmountOfMoneyHelper.UpdateAccrualMoney(effectiveBank, Math.Round(effectiveBank.AccrualMoney.GetValueOrDefault() * limitedAccrualFactor, 2, MidpointRounding.AwayFromZero));
				}
			}
		}
		...
		protected virtual void CalculatePTO(PREarningDetail earningDetail)
		{
			foreach (IGrouping<string, PRPaymentPTOBank> bankGroup in PaymentPTOBanks.Select().FirstTableItems.GroupBy(x => x.BankID))
			{
				...
				PREmployeePTOBank sourceBank = PTOHelper.GetBankSettings(this, effectiveBank.BankID, Payments.Current.EmployeeID.Value, earningDetail.Date.Value);
				if (sourceBank?.StartDate == null)
				{
					throw new PXException(Messages.BankNotFound, effectiveBank.BankID);
				}
				...
				if (IsPaycheckCalculatedDuringProbationPeriod() &&
					sourceBank.ProbationPeriodBehaviour == ProbationPeriodBehaviour.NotAccrued)
				{
					DateTime? probationEndDate = EmploymentHistoryHelper.GetEmploymentDates(this, Payments.Current.EmployeeID, Payments.Current.TransactionDate).ProbationPeriodEndDate;
					if (earningDetail.Date <= probationEndDate)
					{
						continue;
					}
				}
				...
				{
					//percentage PTO calculations
					if (effectiveBank.AccrualMethod == PTOAccrualMethod.Percentage || effectiveBank.AccrualMethod == PTOAccrualMethod.FrontLoadingAndPercentage)
					{
						EPEarningType earningType = new SelectFrom<EPEarningType>.Where<EPEarningType.typeCD.IsEqual<P.AsString>>.View(this).SelectSingle(earningDetail.TypeCD);
						PREarningType prEarningType = earningType?.GetExtension<PREarningType>();

						if (prEarningType?.AccruePTO == true)
						{
						...
						PTOAmountOfMoneyHelper.AddAccrualMoney(effectiveBank, accrualMoney);
					}
				}

				//disbursement amounts calculations
				if (earningDetail.TypeCD == effectiveBank.EarningTypeCD)
				{
					effectiveBank.DisbursementAmount += earningDetail.Hours.GetValueOrDefault();

					PTOAmountOfMoneyHelper.AddDisbursementMoney(effectiveBank, earningDetail.Amount.GetValueOrDefault());
					...
					decimal carryoverAmount = PTOHelper.CalculateHoursToCarryover(this, Payments.Current, sourceBank, updatedYearSummary.StartDate);
					decimal carryoverMoney = PTOHelper.CalculateMoneyToCarryover(this, Payments.Current, sourceBank, updatedYearSummary.StartDate);
					...
					PTOHelper.PTOHistoricalAmounts history =
						PTOHelper.GetPTOHistoryForPaymentPTOBank(this, earningDetail.Date.Value, Payments.Current.EmployeeID.GetValueOrDefault(), sourceBank, Payments.Current.TransactionDate);
					...
					{
						decimal currentAvailableHours = history.AvailableHours - history.UsedHours + carryoverAmount;
						decimal currentAvailableMoney = history.AvailableMoney - history.UsedMoney + effectiveBank.CarryoverMoney.GetValueOrDefault();

						if (newDisbursementHours > currentAvailableHours)
						{
							throw new PXException(Messages.NotEnoughLastYearCarryover, effectiveBank.BankID);
						}
						else if (sourceBank.CreateFinancialTransaction == true && newDisbursementMoney > currentAvailableMoney)
						{
							throw new PXException(Messages.NotEnoughLastYearCarryoverMoney, effectiveBank.BankID);
						}
					}

					if (sourceBank.AllowNegativeBalance == false && effectiveAvailableHours < 0)
					{
		...
		protected virtual void CreatePTODetails()
		{
			// This method is overridden in the 'CanadianPTOCalculationEngineExt' graph extension.
		}
		...
		protected virtual void CalculateAmountPerYearPTO()
		{
			if (Payments.Current.DocType == PayrollType.Regular)
			{
				...
				IEnumerable<PRPaymentPTOBank> ptoBanks = PaymentPTOBanks.Select().FirstTableItems
					.Where(x => (x.AccrualMethod == PTOAccrualMethod.TotalHoursPerYear || x.AccrualMethod == PTOAccrualMethod.FrontLoadingAndHoursPerYear) && x.IsActive == true);

				foreach (PRPaymentPTOBank bank in ptoBanks)
				{
					PREmployeePTOBank sourceBank = PTOHelper.GetBankSettings(this, bank.BankID, Payments.Current.EmployeeID.Value, bank.EffectiveStartDate.Value);
					if (sourceBank?.StartDate == null)
					{
					...
					bank.AccruingDays = CalculateAccruingDays(sourceBank, bank);
		...
		protected virtual void CalculatePercentagePTO(PRPaymentPTOBank paymentBank, PREarningDetail earningDetail, PREmployeePTOBank sourceBank, out decimal accrualHours, out decimal accrualMoney)
		{
			accrualHours = 0;
			accrualMoney = 0;
			...
			if (prEarningType?.AccruePTO == true && (paymentBank.AccrualMethod == PTOAccrualMethod.Percentage || paymentBank.AccrualMethod == PTOAccrualMethod.FrontLoadingAndPercentage)
				&& earningDetail.IsPayingSettlement == false)
			{
				PRPTOBank ptoBank = PRPTOBank.PK.Find(this, sourceBank.BankID);
				accrualHours = (accuralPct * earningDetail.Hours) ?? 0;
				if (sourceBank.CreateFinancialTransaction != true || ptoBank.DisbursingType != PTODisbursingType.AverageRate)
				{
		...
		protected virtual void ResetPTOInfo()
		{
			...
			foreach (PRPaymentPTOBank bankToReset in banks)
			{
				...
				bankToReset.AdjustmentHours = 0;
				bankToReset.AdjustmentCarryoverHours = 0;

				PREmployeePTOBank bankInfo = PTOHelper.GetBankSettings(this, bankToReset.BankID, Payments.Current.EmployeeID.Value, Payments.Current.EndDate.Value);
				...
				if (lastEffectiveBanks.Contains(bankToReset))
				{
					...
					PTOHelper.PTOHistoricalAmounts history =
						PTOHelper.GetPTOHistoryForPaymentPTOBank(this, Payments.Current.EndDate.Value, Payments.Current.EmployeeID.Value, bankInfo, Payments.Current.TransactionDate.Value);
					...
					bankToReset.AdjustmentHours = history.PTOAdjustmentAmounts.AdjustmentHours;
					bankToReset.AdjustmentCarryoverHours = history.PTOAdjustmentAmounts.AdjustmentCarryoverHours;
					if (bankToReset.AdjustmentCarryoverHours > 0)
					{
						bankToReset.CarryoverAmount = bankToReset.AdjustmentCarryoverHours;
					}

					PTOAmountOfMoneyHelper.UpdateAccumulatedMoney(bankToReset, history.AccumulatedMoney);
					PTOAmountOfMoneyHelper.UpdateUsedMoney(bankToReset, history.UsedMoney);
					PTOAmountOfMoneyHelper.UpdateAvailableMoney(bankToReset, history.AvailableMoney);

					foreach (PRPTOAdjustmentDetail ptoAdjustmentDetail in history.PTOAdjustmentAmounts.PTOAdjustmentDetails)
					{
						if (ptoAdjustmentDetail.PaymentDocType == null || ptoAdjustmentDetail.PaymentRefNbr == null)
						{
							ptoAdjustmentDetail.PaymentDocType = Payments.Current.DocType;
							ptoAdjustmentDetail.PaymentRefNbr = Payments.Current.RefNbr;
							PTOAdjustmentDetails.Update(ptoAdjustmentDetail);
						}
					}
				}
		...
		protected virtual void AdjustPTODisbursementRateAndAcctSub(PREmployeePTOBank bank, PREarningDetail row, decimal availableHours, decimal availableMoney)
		{
			...
			PRPTOBank ptoBank = PRPTOBank.PK.Find(this, bank.BankID);
		...
		public PTOHelper.PTOYearSummary GetPTOYearAndPaymentSummary(PXGraph graph, DateTime targetDate, int employeeID, PREmployeePTOBank bank)
		{
		...
		public void InsertSettlementEarnings(PRPayment payment)
		{
			if (Payments.Current.DocType == PayrollType.Final)
			{
				...
				{
					PREmployeePTOBank bankSettings = PTOHelper.GetBankSettings(this, paymentBank.BankID, Payments.Current.EmployeeID.Value, paymentBank.EffectiveStartDate.Value);
					if (bankSettings.SettlementBalanceType == SettlementBalanceType.Pay)
					{
		...
		protected virtual void ProcessPTOSettlement(PRPayment payment)
		{
			...
			foreach (PREmployeePTOBank ptoBankSettings in employeePTOBanks)
			{
				switch (ptoBankSettings.SettlementBalanceType)
				{
		...
		protected virtual void ProcessSettlementPTODiscard(PRPayment payment, PREmployeePTOBank ptoBankSettings)
		{
		...
		private decimal GetDedBenApplicableHours(PRDeductCode deductCode, string contributionType)
		{
			...
			foreach (PXResult<PREarningDetail, PRLocation, Address, EPEarningType> record in PaymentEarningDetails.Select())
			{
				PREarningDetail earningDetail = record;
				EPEarningType earningType = record;

				decimal applicableHours = _CalculationUtils.Value.GetDedBenApplicableHours(this, deductCode, contributionType, earningDetail);

				if (earningType.IsOvertime == true && earningType.OvertimeMultiplier.HasValue)
				{
					applicableHours = applicableHours * earningType.OvertimeMultiplier.Value;
				}

				result += applicableHours;
			}
		...
		protected virtual void SetBenefitPercentages(PRBenefit benefit, PRDeductCode deductionCode, PREmployeeDeduct employeeDeduction)
		{
			if (IsBenefitPassedAsPercentage(deductionCode))
			{
				SetBenefitEmployeePercentage(benefit, deductionCode, employeeDeduction);
				SetBenefitEmployerPercentage(benefit, deductionCode, employeeDeduction);
			}
		}

		protected virtual bool IsBenefitPassedAsPercentage(PRDeductCode deductionCode)
		{
			if (deductionCode.BenefitTypeCD != null)
			{
				return deductionCode.BenefitCalculationMethod == (int?)CalculationType.Percentage;
			}

			return false;
		}

		protected virtual void SetBenefitEmployeePercentage(PRBenefit benefit, PRDeductCode deductionCode, PREmployeeDeduct employeeDeduction)
		{
			if (deductionCode.DedCalcType == DedCntCalculationMethod.PercentOfGross && deductionCode.DedApplicableEarnings == DedBenApplicableEarningsAttribute.TotalEarnings)
			{
				benefit.Percent = employeeDeduction.DedPercent;
			}
		}

		protected virtual void SetBenefitEmployerPercentage(PRBenefit benefit, PRDeductCode deductionCode, PREmployeeDeduct employeeDeduction)
		{
			if (deductionCode.CntCalcType == DedCntCalculationMethod.PercentOfGross && deductionCode.CntApplicableEarnings == DedBenApplicableEarningsAttribute.TotalEarnings)
			{
				benefit.EmployerPercent = employeeDeduction.CntPercent;
			}
		}

		protected virtual void ValidatePTOUsageRestrictionsDuringProbationPeriod()
		{
			if (!IsPaycheckCalculatedDuringProbationPeriod())
			{
				return;
			...
			foreach (PRPaymentPTOBank bank in PaymentPTOBanks.Select())
			{
				foreach (PREarningDetail earningDetail in PaymentEarningDetails.Select())
				{
					if (earningDetail.TypeCD == bank.EarningTypeCD)
					{
						PREmployeePTOBank sourceBank = PTOHelper.GetBankSettings(this, bank.BankID, Payments.Current.EmployeeID.Value, earningDetail.Date.Value);
						if (sourceBank.ProbationPeriodBehaviour != ProbationPeriodBehaviour.AccruedAndAvailable)
						{
							DateTime? probationEndDate = EmploymentHistoryHelper.GetEmploymentDates(this, Payments.Current.EmployeeID, Payments.Current.TransactionDate).ProbationPeriodEndDate;
							if (earningDetail.Date <= probationEndDate)
							{
								throw new PXException(Messages.PTOCannotBeUsedDuringProbationPeriod, bank.BankID);
							}
						}
					}
				}
		...
		/// <summary>
		/// This check is specifically designed to address the case where the probation period concludes during the pay period.
		/// </summary>
		protected virtual bool PaycheckContainsEarningsPastProbationDate()
		{
			DateTime? probationEndDate = EmploymentHistoryHelper.GetEmploymentDates(this, Payments.Current.EmployeeID, Payments.Current.TransactionDate).ProbationPeriodEndDate;
			if (Payments.Current.StartDate <= probationEndDate)
			{
				if (Payments.Current.EndDate <= probationEndDate)
				{
					return false;
				}

				return PaymentEarningDetails.Select().FirstTableItems.Where(x => x.Date > probationEndDate).Any();
			}

			return true;
		}

		protected virtual bool IsPaycheckCalculatedDuringProbationPeriod()
		{
			DateTime? probationEndDate = EmploymentHistoryHelper.GetEmploymentDates(this, Payments.Current.EmployeeID, Payments.Current.TransactionDate).ProbationPeriodEndDate;
			return Payments.Current.StartDate <= probationEndDate;
		}

		protected virtual int CalculateAccruingDays(PREmployeePTOBank sourceBank, PRPaymentPTOBank bank)
		{
			if (IsPaycheckCalculatedDuringProbationPeriod())
			{
				DateTime? probationEndDate = EmploymentHistoryHelper.GetEmploymentDates(this, Payments.Current.EmployeeID, Payments.Current.TransactionDate).ProbationPeriodEndDate;
				if (sourceBank.ProbationPeriodBehaviour == ProbationPeriodBehaviour.NotAccrued && PaycheckContainsEarningsPastProbationDate())
				{
					return (Payments.Current.EndDate - probationEndDate).Value.Days;
				}
			}
			...
			return (bankEffectiveEndForPayment - bankEffectiveStartForPayment).Value.Days + 1;
		}
