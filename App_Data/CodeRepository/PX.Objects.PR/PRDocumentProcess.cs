...
namespace PX.Objects.PR
{
		...
		protected IEnumerable documentList()
		{
			...
			switch (filter?.Operation)
			{
				...
				case PRDocumentProcessFilter.operation.Recalculate:
					// Should satisfy all conditions from the 'PRPayChecksAndAdjustments.IsCalculateActionEnabled' method
					return SelectFrom<PRPayment>.
						Where<PRPayment.calculated.IsEqual<P.AsBool>.
							...
							And<PRPayment.docType.IsEqual<PayrollType.final>.
								Or<PRPayment.grossAmount.IsGreater<Zero>>>.
							And<PRPayment.transactionDate.IsNotNull>.
	...
	public class PRReleaseProcess : PXGraph<PRReleaseProcess>
	{
		...
		public SelectFrom<PRTaxDetail>.
			...
			Where<PRTaxDetail.employeeID.IsEqual<P.AsInt>
				...
				.And<PRTaxCode.isDeleted.IsEqual<False>>>>.View TaxDetails;
			...
			if (doc.Released != true)
			{
				...
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					if (doc.DocType == PayrollType.VoidCheck)
					{
						...
						UnlinkPTOAdjustments(doc.OrigDocType, doc.OrigRefNbr);
					}
		...
		protected virtual void PostReleaseForSettlementPayments(PRPayment payment)
		{
			if (payment.DocType == PayrollType.Final)
			{
				...
				var activePosition = EmployeePositions.Select().FirstTableItems.Where(x => x.IsActive == true).OrderByDescending(x => x.StartDate).FirstOrDefault();
		...
		protected virtual GLTran WriteReverseGLTran(PRPayment payment, GLTran glTran, string earningTypeCD)
		{
			...
			reverseTransaction.TranDesc = string.Format(Messages.TimeTransactionReverse, glTran.TranDesc).Truncate(Common.Constants.TranDescLength);
		...
		public virtual void ProcessPTO(PRPayment doc)
		{
			...
			{
				PREmployeePTOBank sourceBank = PTOHelper.GetBankSettings(this, bankGroup.Key, doc.EmployeeID.Value, doc.EndDate.Value);
				...
				PTOHelper.PTOHistoricalAmounts historyToDate =
					PTOHelper.GetPTOHistoryForPaymentPTOBank(this, doc.EndDate.Value, doc.EmployeeID.Value, sourceBank, doc.TransactionDate.Value);
				...
				PTOAmountOfMoneyHelper.UpdateAccumulatedMoney(lastEffectiveBank, historyToDate.AccumulatedMoney);
				PTOAmountOfMoneyHelper.UpdateUsedMoney(lastEffectiveBank, historyToDate.UsedMoney);
				PTOAmountOfMoneyHelper.UpdateAvailableMoney(lastEffectiveBank, historyToDate.AvailableMoney);
	...
	[Serializable]
	[PXHidden]
	public class PRDocumentProcessFilter : PXBqlTable, IBqlTable
	{
		#region Operation
