...
namespace PX.Objects.PR
{
	public class PRPayChecksAndAdjustments : PXGraph<PRPayChecksAndAdjustments, PRPayment>
	{
		...
		public SelectFrom<PRPTOAdjustmentDetail>
			.Where<PRPTOAdjustmentDetail.paymentDocType.IsEqual<P.AsString>
				.And<PRPTOAdjustmentDetail.paymentRefNbr.IsEqual<P.AsString>>>.View PTOAdjustmentDetails;
		...
		public SelectFrom<CABatch>
			.Where<CABatch.batchNbr.IsEqual<P.AsString>>.View PaymentBatch;
		...
		public SelectFrom<PRPaymentPTOBank>
			...
			.Where<PRPaymentPTOBank.FK.Payment.SameAsCurrent
				...
				.And<PRPTOBank.createFinancialTransaction.IsEqual<True>>
				.And<PRPTOBank.earningTypeCD.IsEqual<P.AsString>>>.View FinancialPTOBanks;
		#endregion
		...
		protected virtual IEnumerable earnings()
		{
			...
			earningDetails.Values.ForEach(earningDetail => SetEarningFinancialPTOFlags(earningDetail));
		...
		protected IEnumerable paymentPTOBanks()
		{
			...
			if (CurrentDocument.Current?.DocType == PayrollType.Adjustment)
			{
				...
				{
					PXUIFieldAttribute.SetEnabled<PRPaymentPTOBank.isActive>(PaymentPTOBanks.Cache, record, false);
					PXUIFieldAttribute.SetEnabled<PRPaymentPTOBank.hoursPerYear>(PaymentPTOBanks.Cache, record, false);
					PXUIFieldAttribute.SetEnabled<PRPaymentPTOBank.accrualRate>(PaymentPTOBanks.Cache, record, false);
					PXUIFieldAttribute.SetEnabled<PRPaymentPTOBank.isCertifiedJob>(PaymentPTOBanks.Cache, record, false);
		...
		[PXRemoveBaseAttribute(typeof(PXSelectorAttribute))]
		[PXSelector(
			typeof(SearchFor<PRTaxCode.taxID>
			.Where<PRTaxCode.countryID.IsEqual<PRPaymentTax.paymentCountryID.FromCurrent>
		...
		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXDBDefault(typeof(PRPayment.docType), DefaultForUpdate = true, PersistingCheck = PXPersistingCheck.Nothing)]
		protected virtual void _(Events.CacheAttached<PRPTOAdjustmentDetail.paymentDocType> e) { }

		// _(Events.CacheAttached<PRPTOAdjustmentDetail.paymentRefNbr> e) handler code is needed for one specific edge case:
		// PTO Adjustments should be applied on Paycheck creation.
		// The user is able to start creating a Paycheck (e.g.choose an Employee add some Earning Details) without clicking the save button.
		// In that case the PRPayment.RefNbr field will be shown as ' <NEW>'.
		// At this moment the PTO Adjustments should be linked to the Paycheck.
		// The 'PXDBDefaultAttribute' attribute is needed to ensure that the 'PRPTOAdjustmentDetail.PaymentRefNbr' field
		// will be saved with a real 'PRPayment.RefNbr' value that will be known after saving the paycheck (and not with ' <NEW>' text).
		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXDBDefault(typeof(PRPayment.refNbr), DefaultForUpdate = true, PersistingCheck = PXPersistingCheck.Nothing)]
		protected virtual void _(Events.CacheAttached<PRPTOAdjustmentDetail.paymentRefNbr> e) { }
		#endregion
		...
		protected void _(Events.RowSelected<PRPayment> e)
		{
			...
			VoidPayment.SetEnabled(e.Row.Voided == false && (e.Row.Released == true
			...
			if (isReadyForInput && !PaymentPTOBanks.Select().FirstTableItems.Where(x => x.IsActive == true).Any())
			{
				PXUIFieldAttribute.SetWarning<PRPayment.employeeID>(e.Cache, Document.Current, Messages.PTOBankTabIsEmpty);
			}
		}
		...
		protected void _(Events.RowDeleted<PRPayment> e)
		{
			if (e.Row != null && e.Row.DocType != PayrollType.VoidCheck)
			{
				UnlinkPTOAdjustments(e.Row.DocType, e.Row.RefNbr);
			}

			if (e.Row != null)
			{
				RemoveCurrentPaymentFromPaymentBatch(e.Row.NetAmount);
			}
		}
		...
		protected virtual void _(Events.RowUpdated<PRPayment> e)
		{
			if (e.Row.DocType == PayrollType.Adjustment)
			{
				if (DirectDepositSplits.Select(e.Row.DocType, e.Row.RefNbr).Any())
				{
					if (e.Row.NetAmount != e.OldRow?.NetAmount)
					{
						DirectDepositSplits.Select().ForEach(x => DirectDepositSplits.Delete(x));
						RemoveCurrentPaymentFromPaymentBatch(e.OldRow?.NetAmount);
					}
					else if (!e.Cache.ObjectsEqual<PRPayment.paymentMethodID, PRPayment.cashAccountID>(e.Row, e.OldRow))
					{
						DirectDepositSplits.Select().ForEach(x => DirectDepositSplits.Update(x));
					}
				}
				else
				{
					if (e.Row.NetAmount != e.OldRow?.NetAmount)
					{
						CABatch paymentBatch = PaymentBatch.SelectSingle(e.Row.PaymentBatchNbr);

						if (paymentBatch != null)
						{
							AdjustPaymentBatchTotals(paymentBatch, e.OldRow?.NetAmount - e.Row.NetAmount);
						}
					}
				}
			}
		...
		protected virtual void _(Events.FieldUpdated<PRPayment, PRPayment.transactionDate> e)
		{
			// If 'PRPayment.DocType' is not equal 'PayrollType.Special', 'PRPayment.TransactionDate' will be disabled
			// and everything will be handled in the scope of '_(Events.FieldUpdated<PRPayment.payPeriodID> e)' handler.
			// Thus, this handler is needed for Special Paychecks only to handle the cases when 'TransactionDate' is changed manually
			if (e.Row != null && e.Row.DocType == PayrollType.Special && !IsVoidCheckInProgress)
			{
				UpdateChildrenRecords(e.Row);
			}
		}
		...
		protected virtual void _(Events.FieldVerifying<PRPayment.employeeID> e)
		{
			...
			CheckUnreleasedPTOAdjustments(e);
		}
		...
		public void _(Events.RowSelected<PRPaymentPTOBank> e)
		{
			...
			else if (row.AvailableMoney < 0 && row.CreateFinancialTransaction == true)
			{
				PXUIFieldAttribute.SetWarning<PRPaymentPTOBank.availableMoney>(e.Cache, row, Messages.PTOUsedExceedsAvailable);
			}
		}

		protected virtual void _(Events.FieldSelecting<PRPaymentPTOBank, PRPaymentPTOBank.calculationFormula> e)
		{
			PRPaymentPTOBank row = e.Row;

			if (row == null || row.EffectiveStartDate == null)
			{
				return;
			}

			var sourceBank = PTOHelper.GetBankSettings(this, row.BankID, CurrentDocument.Current.EmployeeID.Value, row.EffectiveStartDate.Value);
			if (sourceBank != null)
			{
				e.ReturnValue = GetPaycheckAccrualCalculationFormula(row, sourceBank.ProbationPeriodBehaviour);
			}
		}
		...
		protected virtual void _(Events.FieldUpdated<PMTimeActivity.selected> e)
		{
			...
			ImportTimeActivitiesFilter.Current.SelectedTimeActivityExist =
				e.NewValue as bool? == true ||
				 TimeActivities.Select().FirstTableItems.Any(item => item.Selected == true);
		}
		...
		protected void _(Events.RowSelected<PREarningDetail> e)
		{
			...
			else if (Document.Current.CountryID == LocationConstants.CanadaCountryCode)
			{
			...
			if (!warningOnHoursField)
			{
				...
				if (e.Row.IsPayingSettlement == true)
				{
		...
		protected virtual void _(Events.RowUpdating<PRPaymentPTOBank> e)
		{
			...
			if (IsImport && !IsCopyPasteContext)
			{
				throw new PXException(Messages.RestrictModificationOfPTOBanksViaImport);
			}
		}
		...
		private void RecreateWorkCompensationDeductionSummaryAndDetails()
		{
			...
			PXFormulaAttribute.CalcAggregate<PRPaymentDeduct.dedAmount>(Deductions.Cache, Document.Current);
			PXFormulaAttribute.CalcAggregate<PRPaymentDeduct.cntAmount>(Deductions.Cache, Document.Current);
		}
		...
		private void CreatePTOBanks(PRPayment row)
		{
			if (row.EmployeeID != null)
			{
				...
				UnlinkPTOAdjustments(Document.Current.DocType, Document.Current.RefNbr);
				var banksToInsert = new List<PREmployeePTOBank>();
				foreach (IGrouping<string, PREmployeePTOBank> group in PTOHelper.GetEmployeeBanks(this, Document.Current).GroupBy(x => x.BankID))
				{
				...
				foreach (PREmployeePTOBank bank in banksToInsert)
				{
					...
					foreach ((DateTime, DateTime) yearSpan in ptoYears)
					{
						...
						var day = PTOHelper.GetPTOEffectiveDay(bank.StartDate, bank.PTOYearStartDate, ptoYearStart, ptoYearEnd);
						var effectiveStartDate = PTOHelper.GetPTOEffectiveStartDate(bank.StartDate, ptoYearStart, day);
						...
						PREmployeePTOBank nextEffectiveBankInTheSameYear = PTOHelper.GetNextEffectiveBankInTheSameYear(this, Document.Current, bank.BankID, ptoYearStart, effectiveStartDate);
						...
						PRPaymentPTOBank paymentBank = new PRPaymentPTOBank();
						...
						paymentBank.AdjustmentHours = 0;
						paymentBank.AdjustmentCarryoverHours = 0;

						PTOHelper.PTOHistoricalAmounts history =
							PTOHelper.GetPTOHistoryForPaymentPTOBank(this, ptoYearStart, row.EmployeeID.Value, bank, row.TransactionDate, includePTOAdjustments);
						// PTO Adjustment hours from the current and previous cycles
						// should be included to the PRPaymentPTOBank with the latest effective date.
						// The GetPTOHistory method will be called for such a bank first.
						includePTOAdjustments = false;
						...
						paymentBank.AdjustmentHours = history.PTOAdjustmentAmounts.AdjustmentHours;
						paymentBank.AdjustmentCarryoverHours = history.PTOAdjustmentAmounts.AdjustmentCarryoverHours;
						if (paymentBank.AdjustmentCarryoverHours > 0)
						{
							paymentBank.CarryoverAmount = paymentBank.AdjustmentCarryoverHours;
						}

						PTOAmountOfMoneyHelper.UpdateAccumulatedMoney(paymentBank, history.AccumulatedMoney);
						PTOAmountOfMoneyHelper.UpdateUsedMoney(paymentBank, history.UsedMoney);
						PTOAmountOfMoneyHelper.UpdateAvailableMoney(paymentBank, history.AvailableMoney);
						...
						foreach (PRPTOAdjustmentDetail ptoAdjustmentDetail in history.PTOAdjustmentAmounts.PTOAdjustmentDetails)
						{
							...
							{
								ptoAdjustmentDetail.PaymentDocType = Document.Current.DocType;
								ptoAdjustmentDetail.PaymentRefNbr = Document.Current.RefNbr;
		...
		private void CheckUnreleasedPTOAdjustments(Events.FieldVerifying<PRPayment.employeeID> e)
		{
			int? employeeID = e.NewValue as int?;
			if (Document.Current.DocType == PayrollType.VoidCheck || employeeID == null)
			{
				return;
			}

			string[] unreleasedPTOAdjustments = SelectFrom<PRPTOAdjustmentDetail>
				.InnerJoin<PRPTOAdjustment>.On<PRPTOAdjustmentDetail.FK.PTOAdjustment>
				.Where<PRPTOAdjustment.status.IsNotEqual<PTOAdjustmentStatus.released>
					.And<PRPTOAdjustment.status.IsNotEqual<PTOAdjustmentStatus.voided>>
					.And<PRPTOAdjustmentDetail.bAccountID.IsEqual<P.AsInt>>>
				.View
				.Select(this, employeeID)
				.FirstTableItems
				.Select(item => item.RefNbr)
				.ToArray();

			if (unreleasedPTOAdjustments.Length > 0)
			{
				e.NewValue = null;
				e.Cancel = true;

				foreach (string unreleasedPTOAdjustment in unreleasedPTOAdjustments)
				{
					PXTrace.WriteError(Messages.PTOAdjustmentIsNotReleased, unreleasedPTOAdjustment);
				}

				throw new PXSetPropertyException<PRPayment.employeeID>(Messages.PaycheckCannotBeCreated);
			}
		}
		...
		protected virtual void SetEarningFinancialPTOFlags(PREarningDetail row)
		{
			...
			if (Document.Current.CountryID != LocationConstants.CanadaCountryCode || string.IsNullOrEmpty(row.TypeCD))
			{
				return;
			}

			foreach (PXResult<PRPaymentPTOBank, PRPTOBank, PREmployee> bankResult in
				FinancialPTOBanks.Select(row.TypeCD))
			{
				PRPaymentPTOBank paymentPTOBank = bankResult;

				if (row.Date < paymentPTOBank.EffectiveStartDate || row.Date > paymentPTOBank.EffectiveEndDate)
				{
					continue;
				}

				PRPTOBank ptoBank = bankResult;
				row.PTODisbursementWithAverageRate = ptoBank.DisbursingType == PTODisbursingType.AverageRate;
		...
		public virtual void UnlinkPTOAdjustments(string docType, string refNbr)
		{
			foreach (PRPTOAdjustmentDetail ptoAdjustmentDetail in PTOAdjustmentDetails.Select(docType, refNbr))
			{
				ptoAdjustmentDetail.PaymentDocType = null;
				ptoAdjustmentDetail.PaymentRefNbr = null;
				PTOAdjustmentDetails.Update(ptoAdjustmentDetail);
			}
		}

		protected virtual void RemoveCurrentPaymentFromPaymentBatch(decimal? netAmountDifference)
		{
			Dictionary<string, PRCABatch> affectedPaymentBatches = new Dictionary<string, PRCABatch>();

			foreach (PXResult<CABatchDetail, PRCABatch> record in DirectDepositBatchAndDetails.Select())
			{
				PRCABatch paymentBatch = record;
				CABatchDetail caBatchDetail = record;

				if (!affectedPaymentBatches.ContainsKey(paymentBatch.BatchNbr))
				{
					affectedPaymentBatches[paymentBatch.BatchNbr] = paymentBatch;
				}

				DirectDepositBatchAndDetails.Delete(caBatchDetail);
			}

			foreach (PRCABatch paymentBatch in affectedPaymentBatches.Values)
			{
				AdjustPaymentBatchTotals(paymentBatch, netAmountDifference);
			}
		}

		protected virtual void AdjustPaymentBatchTotals(CABatch paymentBatch, decimal? netAmountDifference)
		{
			paymentBatch.CuryDetailTotal = paymentBatch.CuryDetailTotal - netAmountDifference;
			paymentBatch.DetailTotal = paymentBatch.DetailTotal - netAmountDifference;
			paymentBatch.Total = paymentBatch.Total - netAmountDifference;

			PaymentBatch.Update(paymentBatch);
		}

		protected virtual string GetPaycheckAccrualCalculationFormula(PRPaymentPTOBank bank, string probationPeriod)
		{
			DateTime? probationEndDate = EmploymentHistoryHelper.GetEmploymentDates(this, Document.Current.EmployeeID, Document.Current.TransactionDate).ProbationPeriodEndDate;

			if (probationPeriod == ProbationPeriodBehaviour.NotAccrued &&
				Document.Current.Calculated == true &&
				Document.Current.StartDate <= probationEndDate &&
				bank.TotalAccrual == 0)
			{
				return string.Format(Messages.HoursCannotBeAccruedDuringProbationPeriod, bank.BankID);
			}

			switch (bank.AccrualMethod)
			{
				case PTOAccrualMethod.Percentage:
					decimal pctRate = bank.AccrualRate / 100 ?? 0;
					decimal pctCalculation = (pctRate * bank.AccruingHours) + bank.CarryoverAmount ?? 0;

					if (Math.Round(pctCalculation, 2, MidpointRounding.AwayFromZero) == bank.TotalAccrual)
					{
						return $"({pctRate:0.00} * {bank.AccruingHours:0.00}) + {bank.CarryoverAmount:0.00} = {bank.TotalAccrual:0.00}";
					}

					break;

				case PTOAccrualMethod.FrontLoadingAndPercentage:
					decimal fapRate = bank.AccrualRate / 100 ?? 0;
					decimal fapCalculation = (fapRate * bank.AccruingHours) + bank.FrontLoadingAmount + bank.CarryoverAmount ?? 0;

					if (Math.Round(fapCalculation, 2, MidpointRounding.AwayFromZero) == bank.TotalAccrual)
					{
						return $"({fapRate:0.00} * {bank.AccruingHours:0.00}) + {bank.FrontLoadingAmount:0.00} + " +
							$"{bank.CarryoverAmount:0.00} = {bank.TotalAccrual:0.00}";
					}

					break;

				case PTOAccrualMethod.TotalHoursPerYear:
					decimal hpyCalculation = (bank.HoursPerYear / bank.NbrOfPayPeriods * bank.EffectiveCoefficient) + bank.CarryoverAmount ?? 0;

					if (Math.Round(hpyCalculation, 2, MidpointRounding.AwayFromZero) == bank.TotalAccrual)
					{
						return $"{bank.HoursPerYear:0.00} / {bank.NbrOfPayPeriods} * {bank.AccruingDays} / " +
							$"{bank.DaysInPeriod} + {bank.CarryoverAmount:0.00} = {bank.TotalAccrual:0.00}";
					}

					break;

				case PTOAccrualMethod.FrontLoadingAndHoursPerYear:
					decimal fhpCalculation = (bank.HoursPerYear / bank.NbrOfPayPeriods * bank.EffectiveCoefficient) + bank.FrontLoadingAmount + bank.CarryoverAmount ?? 0;

					if (Math.Round(fhpCalculation, 2, MidpointRounding.AwayFromZero) == bank.TotalAccrual)
					{
						return $"{bank.HoursPerYear:0.00} / {bank.NbrOfPayPeriods} * {bank.AccruingDays} / {bank.DaysInPeriod} + " +
							$"{bank.FrontLoadingAmount:0.00} + {bank.CarryoverAmount:0.00} = {bank.TotalAccrual:0.00}";
					}

					break;

				case PTOAccrualMethod.FrontLoading:
					decimal fldCalculation = bank.FrontLoadingAmount + bank.CarryoverAmount ?? 0;

					if (Math.Round(fldCalculation, 2, MidpointRounding.AwayFromZero) == bank.TotalAccrual)
					{
						return $"{bank.FrontLoadingAmount:0.00} + {bank.CarryoverAmount:0.00} = {bank.TotalAccrual:0.00}";
					}
		...
		public virtual void VoidCheckProc(PRPayment doc)
		{
					...
					.And<PRPaymentPTOBank.refNbr.IsEqual<P.AsString>>>.View.Select(this, doc.DocType, doc.RefNbr))
			{
				...
				copy.AdjustmentHours = -1 * copy.AdjustmentHours;
				copy.AdjustmentCarryoverHours = -1 * copy.AdjustmentCarryoverHours;
		...
		protected virtual void SetEarningFinancialPTOFlags(
			PREarningDetail row,
			ref Dictionary<string, PXResult<PRPaymentPTOBank, PRPTOBank, PREmployee>> financialPTOBanks)
		{
			SetEarningFinancialPTOFlags(row);
		}
		#endregion
	...
	[Serializable]
	[PXHidden]
	public class ExistingPayment : PXBqlTable, IBqlTable
	{
		#region DocType
	...
	[Serializable]
	[PXHidden]
	public class ExistingPayrollBatch : PXBqlTable, IBqlTable
	{
		#region BatchNbr
