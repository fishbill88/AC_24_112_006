...
namespace PX.Objects.PR
{
	...
	public class PRAufGenerationEngine : PXGraph<PRAufGenerationEngine>
	{
		...
		// ToDo: If any optimizations of this method are needed, please first consider the following slow places in the 'CreateGeneralEmployeeRecords' method:
		// var employeePayments = EmployeePayments.Select(...).Select(...).GroupBy().OrderBy()		- execution time about 3 minutes.
		// List<PXResult<...>> paymentTaxSplits = PaymentTaxSplits.Select().Select(...).ToList();	- execution time about 3 minutes.
		// CreateEsiRecords(employeeAttributes, paymentTaxSplits).OrderBy(x => x.PimID).ToList();	- execution time about 7 minutes.
		// These two methods are called from the 'CreateEsiRecords' method:
		//			CreateBenefitEsiRecords(ref esiRecords, employeeAttributes, paymentEarnings);	- execution time about 4.5 minutes.
		//			CreateWCEsiRecords(ref esiRecords, employeeAttributes);							- execution time about 1 minute.
		// The execution time given above is the total execution time of all iterations the relevant place of code is executed
		// during one AUF file generation by the company with ~600 employees and ~16700 paychecks in the given year.
		protected virtual byte[] GenerateAufInternal(int orgBAccountID, PRGovernmentReport reportToGenerate)
		{
			using (var prTypeCache = new PRTypeSelectorBaseAttribute.PRTypeSelectorCache(new Type[] { typeof(PRWage), typeof(PRBenefit) }))
			{
				...
				{
					throw new PXException(keyNotFoundException, Messages.ReportingTypesMissing);
				}
		...
		public SelectFrom<EPEarningType>
			.InnerJoin<PRPaymentEarning>.On<PRPaymentEarning.FK.EarningType>
			.InnerJoin<PRPayment>.On<PRPaymentEarning.FK.Payment>
			.Where<Brackets<PRPayment.transactionDate.IsGreaterEqual<P.AsDateTime>
				...
				.And<PRPayment.docType.IsNotEqual<PayrollType.voidCheck>>
				.And<Where<PRPayment.branchID, Inside<BAccountR.bAccountID.FromCurrent>>>>
			.AggregateTo<GroupBy<EPEarningType.typeCD>>.View.ReadOnly EarningTypesUsed;

		public SelectFrom<PRDeductCode>
			.InnerJoin<PRPaymentDeduct>.On<PRPaymentDeduct.FK.DeductionCode>
			.InnerJoin<PRPayment>.On<PRPaymentDeduct.FK.Payment>
			.Where<Brackets<PRPayment.transactionDate.IsGreaterEqual<P.AsDateTime>
				.And<PRPayment.transactionDate.IsLessEqual<P.AsDateTime>>
					.And<PRGovernmentReport.reportType.FromCurrent.IsNotEqual<certifiedReportType>>
					.Or<PRPayment.startDate.IsGreaterEqual<P.AsDateTime>
				...
				.And<PRPayment.docType.IsNotEqual<PayrollType.voidCheck>>
				.And<Where<PRPayment.branchID, Inside<BAccountR.bAccountID.FromCurrent>>>>
			.AggregateTo<GroupBy<PRDeductCode.codeID>>.View.ReadOnly DeductCodesUsed;

		public SelectFrom<PRTaxCode>
			.InnerJoin<PRPaymentTax>.On<PRPaymentTax.FK.TaxCode
				.And<MatchPRCountry<PRTaxCode.countryID>>>
			.InnerJoin<PRPayment>.On<PRPaymentTax.FK.Payment>
			.Where<Brackets<PRPayment.transactionDate.IsGreaterEqual<P.AsDateTime>
				.And<PRPayment.transactionDate.IsLessEqual<P.AsDateTime>>
					.And<PRGovernmentReport.reportType.FromCurrent.IsNotEqual<certifiedReportType>>
					.Or<PRPayment.startDate.IsGreaterEqual<P.AsDateTime>
				...
				.And<PRPayment.docType.IsNotEqual<PayrollType.voidCheck>>
				.And<Where<PRPayment.branchID, Inside<BAccountR.bAccountID.FromCurrent>>>>
			.AggregateTo<GroupBy<PRTaxCode.taxID>>.View.ReadOnly TaxCodesUsed;
		...
		public SelectFrom<PRPaymentEarning>
			...
			.Where<PRPaymentEarning.refNbr.IsEqual<PRPayment.refNbr.FromCurrent>
				.And<PRPaymentEarning.docType.IsEqual<PRPayment.docType.FromCurrent>>>.View.ReadOnly PaymentEarnings;
		...
		public SelectFrom<PRPaymentDeduct>
			...
			.Where<PRPaymentDeduct.refNbr.IsEqual<PRPayment.refNbr.FromCurrent>
				.And<PRPaymentDeduct.docType.IsEqual<PRPayment.docType.FromCurrent>>>.View.ReadOnly PaymentDeductions;
		...
		public SelectFrom<PRPaymentTaxSplit>
			...
			.Where<PRPaymentTaxSplit.docType.IsEqual<PRPayment.docType.FromCurrent>
				.And<PRPaymentTaxSplit.refNbr.IsEqual<PRPayment.refNbr.FromCurrent>>>.View.ReadOnly PaymentTaxSplits;
		...
		public SelectFrom<PRPaymentWCPremium>
			...
			.Where<PRPaymentWCPremium.docType.IsEqual<PRPayment.docType.FromCurrent>
				...
				.And<PRPaymentWCPremium.amount.IsNotEqual<decimal0>
					.Or<PRPaymentWCPremium.deductionAmount.IsNotEqual<decimal0>>>>.View.ReadOnly PaymentWCPremiums;
		...
		public SelectFrom<PRTaxDetail>
			...
			.Where<PRPayment.startDate.IsEqual<P.AsDateTime.UTC>
				.And<PRPayment.endDate.IsEqual<P.AsDateTime.UTC>>
				.And<PRPayment.transactionDate.IsEqual<P.AsDateTime.UTC>>
				.And<APAdjust.adjgDocType.IsEqual<APDocType.check>>
		...
		protected virtual Dictionary<int?, decimal?> GetTaxRates()
		{
			Dictionary<int?, decimal?> taxRates = new Dictionary<int?, decimal?>();
			
			var query = SelectFrom<PRTaxCodeAttribute>
				.Where<PRTaxCodeAttribute.aatrixMapping.IsEqual<P.AsInt>>
				.View.Select(this, AatrixField.ELI.Rate);

			foreach (PRTaxCodeAttribute taxCodeAttribute in query)
			{
				if (decimal.TryParse(taxCodeAttribute.Value, out decimal taxRate))
				{
					taxRates[taxCodeAttribute.TaxID] = taxRate;
				}
			}
		...
		protected virtual DatRecord CreateDateRecord()
		{
			...
			int? reportYear = PRUtils.ParseNullableInteger(reportToGenerate.Year);
		...
		protected virtual AleRecord CreateCompanyAleRecord()
		{
			...
			if (companyYear != null)
			{
				...
				{
					IsDesignatedGovernmentEntity = PRUtils.ParseNullableBoolean(isDesignatedGovernmentEntity),
		...
		protected virtual IEnumerable<EmpRecord> CreateEmpRecords(DatRecord reportPeriod)
		{
				...
				in periodEmployees)
			{
				...
				AatrixAttributes aatrixAttributes = AatrixAttributes.GetAatrixAttributes(prEmployee.AcctCD?.Trim(), employeeAttributes);

				string socialSecurityNumber = aatrixAttributes.GetString(AatrixField.EMP.SocialSecurityNumber);
				bool? isFemale = aatrixAttributes.GetBoolean(AatrixField.EMP.IsFemale);
				bool? isDisable = aatrixAttributes.GetBoolean(AatrixField.EMP.IsDisabled);
				decimal? payRate = EmployeeEarningRates.SelectSingle(reportPeriod.LastDate, reportPeriod.LastDate, regularWageType)?.PayRate.Value;
				int? federalExemptions = aatrixAttributes.GetInt(AatrixField.EMP.FederalExemptions);
				string stateOfHireAbbr = OrgAddress.SelectSingle()?.State;
				bool? isFullTime = aatrixAttributes.GetBoolean(AatrixField.EMP.IsFullTime);
				bool? hasHealthBenefits = aatrixAttributes.GetBoolean(AatrixField.EMP.HasHealthBenefits);
				bool? isSeasonal = aatrixAttributes.GetBoolean(AatrixField.EMP.IsSeasonal);
				string maritalStatus = aatrixAttributes.GetString(AatrixField.EMP.MaritalStatus);
				bool? isStatutoryEmployee = aatrixAttributes.GetBoolean(AatrixField.EMP.IsStatutoryEmployee);
				bool? hasRetirementPlan = aatrixAttributes.GetBoolean(AatrixField.EMP.HasRetirementPlan);
				bool? hasThirdPartySickPay = aatrixAttributes.GetBoolean(AatrixField.EMP.HasThirdPartySickPay);
				bool? hasElectronicW2 = aatrixAttributes.GetBoolean(AatrixField.EMP.HasElectronicW2);
				string employmentCode = aatrixAttributes.GetString(AatrixField.EMP.EmploymentCode);
				string standardOccupationalClassification = aatrixAttributes.GetString(AatrixField.EMP.StandardOccupationalClassification);
				int? stateExemptions = employeeAttributes
						...
						.Sum(x => PRUtils.ParseInteger(GetAttributeValue(x), 0));
				string ethnicity = aatrixAttributes.GetString(AatrixField.EMP.Ethnicity);
				string eeoEthnicity = aatrixAttributes.GetString(AatrixField.EMP.EEOEthnicity);
				int? eeoJobCategory = aatrixAttributes.GetInt(AatrixField.EMP.EEOJobCategory);
				...
				EmpRecord record = new EmpRecord(employee.AcctCD)
				{
					...
					Title = title,
					StateOfHireAbbr = stateOfHireAbbr,
				...
				if (shouldGenerateAcaRecords)
				{
					(record.Ecv, record.Eci) = CreateEmployeeAcaRecords(record, aatrixAttributes);
				}
			...
			List<PXResult<PREmployeeAttribute, PRCompanyTaxAttribute>> employeeAttributes)
		{
			...
			// ToDo: Optimize (priority 3). Check the comment on top of the GenerateAufInternal method for the reference.
			...
			foreach (var paymentResult in employeePayments)
			{
				...
				// ToDo: Optimize (priority 2). Check the comment on top of the GenerateAufInternal method for the reference.
		...
		protected virtual IEnumerable<GtoRecord> CreateGeneralCompanyRecords(IEnumerable<EmpRecord> empList)
		{
			...
			{
				DateTime periodStart = paymentGroup.Key.PeriodStart.Value;
				DateTime periodEnd = paymentGroup.Key.PeriodEnd.Value;
				DateTime checkDate = paymentGroup.Key.CheckDate;

				GtoRecord record = new GtoRecord(checkDate)
				{
					GrossPay = 0m,
					NetPay = 0m,
					SSWages = 0m,
					...
					// Children records
					CsiList = CreateCsiRecords(empList, periodStart, periodEnd, checkDate)
						.OrderBy(x => x.PimID)
						.ToList(),
					CliList = CreateCliRecords(empList, periodStart, periodEnd, checkDate)
						.OrderBy(x => x.PimID)
						.ToList(),
					CspList = CreateCspList(periodStart, periodEnd, checkDate).ToList(),
					ClpList = CreateClpList(periodStart, periodEnd, checkDate).ToList()
				};
		...
		{
			Dictionary<int, PimRecord> pimRecords = GetEarningPimRecords(reportPeriod);

			foreach (KeyValuePair<int, PimRecord> kvp in GetBenefitPimRecords(reportPeriod))
			{
				pimRecords[kvp.Key] = kvp.Value;
		}

			foreach (KeyValuePair<int, PimRecord> kvp in GetTaxPimRecords(reportPeriod))
		{
				pimRecords[kvp.Key] = kvp.Value;
			}

			return pimRecords.Values;
		}

		protected virtual Dictionary<int, PimRecord> GetEarningPimRecords(DatRecord reportPeriod)
			{
			Dictionary<int, PimRecord> result = new Dictionary<int, PimRecord>();
			Dictionary<int, PRTypeMeta> wageReportingTypes = PRReportingTypeSelectorAttribute
				.GetAll<PRWage>(PRCountryAttribute.GetPayrollCountry())
				.ToDictionary(k => k.ID, v => v);

			// Acuminator disable once PX1015 IncorrectNumberOfSelectParameters [Number of parameters is correct. Acuminator issue https://jira.acumatica.com/browse/ATR-600]
			foreach (EPEarningType epEarningType in EarningTypesUsed.Select(reportPeriod.FirstDate, reportPeriod.LastDate, reportPeriod.FirstDate, reportPeriod.LastDate))
				{
				int? reportType = PXCache<EPEarningType>.GetExtension<PREarningType>(epEarningType)?.ReportType;
				if (reportType != null)
					{
					PRTypeMeta prType = wageReportingTypes[reportType.Value];
					if (prType.AatrixMapping?.IsDirectMapping == true)
					{
						PimRecord record = CreatePimRecord(prType);
						result[record.PimID] = record;
					}
				}
			}
		...
		protected virtual Dictionary<int, PimRecord> GetBenefitPimRecords(DatRecord reportPeriod)
		{
			Dictionary<int, PimRecord> result = new Dictionary<int, PimRecord>();
			Dictionary<int, PRTypeMeta> deductionReportingTypes = PRReportingTypeSelectorAttribute
				.GetAll<PRBenefit>(PRCountryAttribute.GetPayrollCountry())
				.ToDictionary(k => k.ID, v => v);

			HashSet<string> statesWithWC = new HashSet<string>();
			// Acuminator disable once PX1015 IncorrectNumberOfSelectParameters [Number of parameters is correct. Acuminator issue https://jira.acumatica.com/browse/ATR-600]
			foreach (PRDeductCode deductCode in DeductCodesUsed.Select(reportPeriod.FirstDate, reportPeriod.LastDate, reportPeriod.FirstDate, reportPeriod.LastDate))
			{
					if (deductCode?.DedReportType != null)
					{
					PRTypeMeta prType = deductionReportingTypes[deductCode.DedReportType.Value];
					if (prType.AatrixMapping?.IsDirectMapping == true)
					{
						PimRecord record = CreatePimRecord(prType);
						result[record.PimID] = record;
					...
					{
					PRTypeMeta prType = deductionReportingTypes[deductCode.CntReportType.Value];
					if (prType.AatrixMapping?.IsDirectMapping == true)
					{
						PimRecord record = CreatePimRecord(prType);
						result[record.PimID] = record;
			...
			foreach (KeyValuePair<string, SymmetryToAatrixTaxMapping> kvp in
				_TaxIDMapping.Where(kvp => kvp.Value.IsWorkersCompensation && statesWithWC.Contains(kvp.Value.SubnationalEntity?.Abbr)))
			{
		...
		protected virtual Dictionary<int, PimRecord> GetTaxPimRecords(DatRecord reportPeriod)
		{
			Dictionary<int, PimRecord> result = new Dictionary<int, PimRecord>();
			// Acuminator disable once PX1015 IncorrectNumberOfSelectParameters [Number of parameters is correct. Acuminator issue https://jira.acumatica.com/browse/ATR-600]
			foreach (PRTaxCode tax in TaxCodesUsed.Select(reportPeriod.FirstDate, reportPeriod.LastDate, reportPeriod.FirstDate, reportPeriod.LastDate))
			{
		...
		protected PimRecord CreatePimRecord(PRTypeMeta prType)
		{
			return new PimRecord(prType.Name)
			{
		...
		protected virtual IEnumerable<CsiRecord> CreateCsiRecords(IEnumerable<EmpRecord> empList, DateTime periodStart, DateTime periodEnd, DateTime checkDate)
		{
				...
				.GroupBy(x => x.PimID))
			{
				...
				foreach (EsiRecord esi in esiGroup)
				{
					...
					if (record.Rate == null && esi.Rate != null)
					{
						record.Rate = esi.Rate;
					}
				}

				if (record.Rate == null && record.TaxableWagesAndTips.GetValueOrDefault() != 0)
				{
		...
		protected virtual IEnumerable<CliRecord> CreateCliRecords(IEnumerable<EmpRecord> empList, DateTime periodStart, DateTime periodEnd, DateTime checkDate)
		{
				...
				.GroupBy(x => x.PimID))
			{
				...
				foreach (EliRecord eli in eliGroup)
				{
					...
					if (record.Rate == null && eli.Rate != null)
					{
						record.Rate = eli.Rate;
					}
				}
			...
			foreach (PRTypeMeta prType in PRReportingTypeSelectorAttribute.GetAll<PRWage>(PRCountryAttribute.GetPayrollCountry()).Where(x => x.AatrixMapping?.IsDirectMapping == true))
			{
				...
				if (earningsByReportType.TryGetValue(prType.ID, out earningsForReportType))
				{
					...
					if (!records.TryGetValue(pimID, out EsiRecord record))
					{
						...
						{
							Allowances = GetPuertoRicoTotalAllowances(employeeAttributes)
						};
		...
		// ToDo: Optimize (priority 1). Check the comment on top of the GenerateAufInternal method for the reference.
			...
			List<PXResult<PRPaymentTaxSplit, PRPaymentTax, PRTaxCode>> paymentTaxSplits)
		{
				...
				.GroupBy(x => ((PRTaxCode)x).TaxID))
			{
				...
				_TaxRates.TryGetValue(tax.TaxID, out decimal? taxRate);
		...
		protected virtual IEnumerable<EliRecord> CreateTaxEliRecords(List<PXResult<PRPaymentTaxSplit, PRPaymentTax, PRTaxCode>> paymentTaxSplits)
		{
				...
				.GroupBy(x => ((PRTaxCode)x).TaxID))
			{
				PRPaymentTax paymentTax = taxGroup.First();
				PRTaxCode tax = taxGroup.First();
				bool isResident = PRUtils.ParseBoolean(GetAttributeValue(employeeTaxAttributes, AatrixField.ELI.IsResidentTax, tax.TaxID), false);
		...
		protected virtual IEnumerable<CspRecord> CreateCspList(DateTime periodStart, DateTime periodEnd, DateTime checkDate)
		{
			Dictionary<int, CspRecord> records = new Dictionary<int, CspRecord>(); ;
			foreach (PXResult<PRTaxDetail, APInvoice, APAdjust, APPayment, PRTaxCode> result in
				PaymentSettledTaxes.Select(periodStart, periodEnd, checkDate))
			{
		...
		protected virtual IEnumerable<ClpRecord> CreateClpList(DateTime periodStart, DateTime periodEnd, DateTime checkDate)
		{
			Dictionary<int, ClpRecord> records = new Dictionary<int, ClpRecord>(); ;
			foreach (PXResult<PRTaxDetail, APInvoice, APAdjust, APPayment, PRTaxCode, PRPayment> result in
				PaymentSettledTaxes.Select(periodStart, periodEnd, checkDate))
			{
				APPayment apPayment = (APPayment)result;
				PRTaxCode tax = (PRTaxCode)result;
				...
				{
					bool residentSplitTax = TaxIsSplitByResidency(tax.TaxUniqueCode);
					bool isEmployeeResident = PRUtils.ParseBoolean(GetAttributeValue(
		...
		protected virtual (EcvRecord, EciRecord) CreateEmployeeAcaRecords(EmpRecord emp, AatrixAttributes aatrixAttributes)
		{
			...
			{
				string policyOriginCode = aatrixAttributes.GetString(AatrixField.ECV.AcaPolicyOrigin);
				EcvRecord ecv = new EcvRecord()
				{
					ElectronicOnly = aatrixAttributes.GetBoolean(AatrixField.ECV.ElectronicAcaForms) ?? false,
					PolicyOriginCode = policyOriginCode != null && policyOriginCode.Length > 0 ? policyOriginCode[0] : (char?)null,
					SelfInsuredEmployee = aatrixAttributes.GetBoolean(AatrixField.ECV.EmployeeSelfInsured) ?? false
				};
		...
		/// <summary>
		/// Even though the generic 'IsApplicableAatrixPredefinedField' method (above) is suitable
		/// for the 'PRTaxCodeAttribute' type, this method is needed because the unbound 'PRTaxCodeAttribute.State' field 
		/// has the 'TaxAttributeStateAttribute' attribute with the 'FieldSelecting' handler which is supposed
		/// to initialize the value of the 'PRTaxCodeAttribute.State' field.
		/// However, the 'TaxAttributeStateAttribute.FieldSelecting' handler is not triggered when the 'PRTaxCodeAttribute' DAC
		/// is used in the context of the 'PRAufGenerationEngine.EmployeeTaxAttributes' view, since it is a joined DAC in the view.
		/// That is why it is necessary to retrieve the Tax State by the 'PRTaxCodeAttribute.TaxID' field.
		/// </summary>
		protected virtual bool IsApplicableAatrixPredefinedField(PRTaxCodeAttribute taxCodeAttribute)
		{
			if (taxCodeAttribute.AatrixMapping >= WebserviceContants.FirstAatrixPredefinedField
				&& taxCodeAttribute.AatrixMapping <= WebserviceContants.LastAatrixPredefinedField)
			{
				string taxState = taxCodeAttribute.State;

				if (string.IsNullOrWhiteSpace(taxState))
				{
					PRTaxCode taxCode = PRTaxCode.PK.Find(this, taxCodeAttribute.TaxID);
					taxState = TaxAttributeStateAttribute.GetTaxState(taxCode);
				}

				return taxState == Report.Current.State; 
			}
		...
		protected virtual decimal GetPuertoRicoTotalAllowances(List<PXResult<PREmployeeAttribute, PRCompanyTaxAttribute>> employeeAttributes)
		{
			string attributeValue = GetAttributeValue(employeeAttributes, AatrixField.ESI.PuertoRicoTotalAllowances);

			return string.IsNullOrWhiteSpace(attributeValue) ? 0m : PRUtils.ParseDecimal(attributeValue);
		}
		#endregion Helpers
		...
		public class AatrixAttributes : Dictionary<int, List<IPRSetting>>
		{
			private string _EmployeeCD;

			private AatrixAttributes(string employeeCD)
			{
				_EmployeeCD = employeeCD;
			}

			public IPRSetting GetAatrixAttribute(int aatrixMapping)
			{
				if (!TryGetValue(aatrixMapping, out List<IPRSetting> settings))
				{
					return null;
				}
			...
			public string GetString(int aatrixMapping)
			{
				return GetAatrixAttribute(aatrixMapping)?.Value;
			}

			public bool? GetBoolean(int aatrixMapping)
			{
				IPRSetting aatrixAttribute = GetAatrixAttribute(aatrixMapping);
				string attributeValue = aatrixAttribute?.Value;

				try
				{
					return PRUtils.ParseNullableBoolean(attributeValue);
				}
				catch (PXException exception)
				{
					if (aatrixAttribute == null)
					{
						throw new PXException(exception, Messages.BooleanParseError, attributeValue);
					}

					string attributeName = aatrixAttribute.Description ?? aatrixAttribute.SettingName;
					if (aatrixAttribute.UseDefault == true)
					{
						throw new PXException(exception, Messages.BooleanTaxSettingParseError, attributeValue, attributeName);
					}

					throw new PXException(exception, Messages.BooleanEmployeeTaxSettingParseError, attributeValue, attributeName, _EmployeeCD);
				}
			}

			public int? GetInt(int aatrixMapping)
			{
				IPRSetting aatrixAttribute = GetAatrixAttribute(aatrixMapping);
				string attributeValue = aatrixAttribute?.Value;
				...
				try
				{
					return PRUtils.ParseNullableInteger(attributeValue);
				}
				catch (PXException exception)
				{
					if (aatrixAttribute == null)
					{
						throw new PXException(exception, Messages.IntegerParseError, attributeValue);
					}

					string attributeName = aatrixAttribute.Description ?? aatrixAttribute.SettingName;
					if (aatrixAttribute.UseDefault == true)
					{
						throw new PXException(exception, Messages.IntegerTaxSettingParseError, attributeValue, attributeName);
					}

					throw new PXException(exception, Messages.IntegerEmployeeTaxSettingParseError, attributeValue, attributeName, _EmployeeCD);
				}
			}

			public static AatrixAttributes GetAatrixAttributes<TEmployee, TCompany>(string employeeCD, List<PXResult<TEmployee, TCompany>> employeeAttributes)
				where TEmployee : class, IPRSetting, IBqlTable, new()
				where TCompany : class, IPRSetting, IBqlTable, new()
			{
				AatrixAttributes aatrixAttributes = new AatrixAttributes(employeeCD);

				foreach (PXResult<TEmployee, TCompany> employeeAttribute in employeeAttributes)
				{
					IPRSetting payrollSetting = GetAttribute(employeeAttribute);

					if (payrollSetting.AatrixMapping == null)
					{
						continue;
					}

					if (!aatrixAttributes.TryGetValue(payrollSetting.AatrixMapping.Value, out List<IPRSetting> settings))
					{
						settings = new List<IPRSetting>();
						aatrixAttributes[payrollSetting.AatrixMapping.Value] = settings;
					}

					settings.Add(payrollSetting);
				}

				return aatrixAttributes;
			}

			private static IPRSetting GetAttribute<TEmployee, TCompany>(PXResult<TEmployee, TCompany> attributeResult)
				where TEmployee : class, IPRSetting, IBqlTable, new()
				where TCompany : class, IPRSetting, IBqlTable, new()
			{
				TEmployee employeeAttribute = attributeResult;
				...
				if (employeeAttribute?.UseDefault == false)
				{
		...
		#region Avoid breaking changes in 2023R2
		[Obsolete(Common.Messages.ItemIsObsoleteAndWillBeRemoved2024R2)]
		public SelectFrom<PRTaxDetail>
			.InnerJoin<APInvoice>.On<APInvoice.docType.IsEqual<PRTaxDetail.apInvoiceDocType>
				.And<APInvoice.refNbr.IsEqual<PRTaxDetail.apInvoiceRefNbr>>>
			...
			.Where<PRPayment.transactionDate.IsEqual<P.AsDateTime>
				.And<APInvoice.released.IsEqual<True>>>
			.AggregateTo<GroupBy<PRTaxCode.taxID>, GroupBy<APPayment.adjDate>, Sum<PRTaxDetail.amount>>.View PaymentSettledTaxesOld;

		[Obsolete(Common.Messages.ItemIsObsoleteAndWillBeRemoved2024R2)]
		protected virtual IEnumerable<CspRecord> CreateCspList(DateTime checkDate)
		{
			Dictionary<int, CspRecord> records = new Dictionary<int, CspRecord>(); ;
			foreach (PXResult<PRTaxDetail, APInvoice, APAdjust, APPayment, PRTaxCode> result in PaymentSettledTaxesOld.Select(checkDate))
			{
				APPayment apPayment = (APPayment)result;
				PRTaxCode tax = (PRTaxCode)result;
				PRTaxDetail taxDetail = (PRTaxDetail)result;
		...
		{
			Dictionary<int, ClpRecord> records = new Dictionary<int, ClpRecord>(); ;
			foreach (PXResult<PRTaxDetail, APInvoice, APAdjust, APPayment, PRTaxCode, PRPayment> result in PaymentSettledTaxesOld.Select(checkDate))
			{
				APPayment apPayment = (APPayment)result;
				PRTaxCode tax = (PRTaxCode)result;
				PRTaxDetail taxDetail = (PRTaxDetail)result;
