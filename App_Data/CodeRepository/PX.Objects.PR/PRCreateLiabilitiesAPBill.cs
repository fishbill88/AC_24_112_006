...
namespace PX.Objects.PR
{
	public class PRCreateLiabilitiesAPBill : PXGraph<PRCreateLiabilitiesAPBill>
	{
		...
		public IEnumerable details()
		{
			if (!Details.Cache.Inserted.Any_())
			{
				...
				{
					Details.Cache.SetStatus(CreateDeduction(liability), PXEntryStatus.Inserted);
				}
				foreach (PRBenefitDetailLiability liability in BenefitDetails.Select())
				{
					Details.Cache.SetStatus(CreateBenefit(liability), PXEntryStatus.Inserted);
				}
				foreach (PRTaxDetailLiability liability in TaxDetails.Select())
				{
					Details.Cache.SetStatus(CreateTax(liability), PXEntryStatus.Inserted);
				}
			}
		...
		private static string GetLineDescription(PXGraph graph, PayCheckDetail detail)
		{
			...
			switch (detail.DetailType)
			{
				...
				case DetailTypeListAttribute.Type.Tax:
					code = SelectFrom<PRTaxCode>
						.Where<PRTaxCode.taxID.IsEqual<P.AsInt>
							.And<PRTaxCode.isDeleted.IsEqual<False>>>.View
						.Select(graph, detail.CodeID).FirstTableItems.FirstOrDefault();
	...
	public class PayCheckDetailFilter : PXBqlTable, IBqlTable
	{
		#region DocDate
	...
	public class PayCheckDetail : PXBqlTable, IBqlTable
	{
		#region Selected
	...
	[PXHidden]
	public class PRDeductionDetailLiability : PXBqlTable, IBqlTable
	{
		#region RecordID
		public abstract class recordID : PX.Data.BQL.BqlInt.Field<recordID> { }
	...
	[PXHidden]
	public class PRBenefitDetailLiability : PXBqlTable, IBqlTable
	{
		#region RecordID
		public abstract class recordID : PX.Data.BQL.BqlInt.Field<recordID> { }
	...
	[PXHidden]
	public class PRTaxDetailLiability : PXBqlTable, IBqlTable
	{
		#region RecordID
		public abstract class recordID : PX.Data.BQL.BqlInt.Field<recordID> { }
