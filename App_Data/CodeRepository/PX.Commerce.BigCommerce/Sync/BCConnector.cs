...
namespace PX.Commerce.BigCommerce
{
	...
	{
		#region IConnector
		public const string TYPE = BigCommerceConstants.BigCommerceConnector;
		...
		[InjectDependency]
		public IBCRestClientFactory BCRestClientFactory { get; set; }
		...
		public async Task<DateTime> GetSyncTime(ConnectorOperation operation)
		{
			...
			//Big Commerce Time
			StoreTimeRestDataProvider storeTime = new StoreTimeRestDataProvider(BCRestClientFactory.GetRestClient(binding));
			DateTime syncTime = (await storeTime.Get())?.CurrentDateTime ?? default(DateTime);
		...
		public override async Task StartWebHook(String baseUrl, BCWebHook hook,CancellationToken cancellationToken= default)
		{
			...
			WebHookRestDataProvider restClient = new WebHookRestDataProvider(BCRestClientFactory.GetRestClient(storeBigCommerce));
			...
			if (hook.HookRef != null)
			{
				...
				if (data != null)
				{
					if (data.IsActive != true
						...
						await restClient.Delete(hook.HookRef?.ToString());
					else
					{
						hook.IsActive = true;
			...
			else
			{
				...
				await Task.Yield();
				await foreach (WebHookData data in restClient.GetAll())
				{
					if (data.Scope == hook.Scope && data.Destination == url)
					{
						...
						if (data.IsActive == false || validation != hashcode)
							await restClient.Delete(data.Id?.ToString());
						else
						{
							//Saving missing hook
							hook.IsActive = true;
