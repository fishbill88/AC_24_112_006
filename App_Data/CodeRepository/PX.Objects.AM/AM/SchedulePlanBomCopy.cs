    ...
    public class SchedulePlanBomCopy : ProductionBomCopyBase
    {
        ...
        protected virtual AMWCSchd GetWorkCenterSchd(string workCenterID, string shiftCD, int? siteID, AMDateInfo dateInfo)
        {
            ...
            if (workCenterSchd == null)
            {
				...
				{
#if DEBUG
					AMDebug.TraceWriteMethodName($"Error inserting AMWCSchd record.");
#endif
					return null;
				}
            }
		...
		public List<object> MaterialCache;

		protected virtual PXResultset<AMBomMatl> GetMaterialResults(string bomID, string revisionID)
		{
			var results = new PXResultset<AMBomMatl>();
			var detail = MaterialCache.Cast<PXResult<AMBomMatl, AMBomItem, AMBomMatlCury>>()
				.Where(r => ((AMBomMatl)r).BOMID == bomID && ((AMBomMatl)r).RevisionID == revisionID);
			if (detail != null)
			{
				results.AddRange(detail);
			}
			return results;
		}
        ...
        protected virtual List<AMPlanMaterial> GetOrderMaterial(DateTime? materialDateTime)
        {
            ...
            foreach (var bomRevision in uniqueBomRevisions)
            {
                ...
                foreach (PXResult<AMBomMatl, AMBomItem, AMBomMatlCury> result in GetMaterialResults(bomRevision.Item1, bomRevision.Item2))
				{
                    var bomMatl = (AMBomMatl)result;
                    var bomItem = (AMBomItem)result;
					var bomMatlCury = (AMBomMatlCury)result;

                    if (bomMatl?.RevisionID == null || bomMatl.IsStockItem != true || 
            ...
            foreach (var operationDetail in _operationDetails.OrderedList)
            {
                ...
                {
					var amSchdOper = ProcessingGraph.Caches[typeof(AMSchdOper)].Cached
						.RowCast<AMSchdOper>()
						.Where(r =>
							r.OrderType == Order.Current.OrderType
							&& r.ProdOrdID == Order.Current.OrderNbr
							&& r.SortOrder == operationDetail.SortOrder)
						.FirstOrDefault();
