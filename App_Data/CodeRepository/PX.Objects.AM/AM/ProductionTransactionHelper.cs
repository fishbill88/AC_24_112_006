...
namespace PX.Objects.AM
{
    public class ProductionTransactionHelper
    {
		...
		public virtual void BuildRelatedTransactions(AMMTran moveTransaction, MoveOperationQtyTotals moveOperQtyTotals, AMProdItem prodItem)
        {
            ...
            var lastOperQtyComplete = prodItem.QtytoProd.GetValueOrDefault();
            var lastOperQtyCompleteBase = prodItem.BaseQtytoProd.GetValueOrDefault();
            ...
            foreach (var operationQtyTotal in moveOperQtyTotals.OperationTotals.Values.OrderBy(x => x.ProdOper.OperationCD))
            {
				...
				var oldOperStatus = amProdOper.StatusID;
                ...
                if (operationQtyTotal.CurrentMoveBaseQty != 0)
                {
                    BackflushLabor(amProdOper, moveTransaction, operationQtyTotal.CurrentMoveBaseQty,
                        operationQtyTotal.CurrentMoveVarLaborTime + operationQtyTotal.CurrentMoveFixLaborTime, orderUpdateOnly, reverseCost, prodItem);
                    Machine(amProdOper, moveTransaction, operationQtyTotal.CurrentMoveBaseQty, orderUpdateOnly, reverseCost, prodItem);
                    if (!orderUpdateOnly && !hasReleasedBfMaterial && prodItem.Function == OrderTypeFunction.Regular && !BackflushMaterialReleased && !BackflushMaterialINRegisterReleased)
                    {
                        BackflushMaterial(amProdOper, moveTransaction, operationQtyTotal.CurrentMoveBaseQty, prodItem);
                    }
                    Tool(amProdOper, moveTransaction, operationQtyTotal.CurrentMoveBaseQty, orderUpdateOnly, reverseCost, prodItem);
                    Overhead(amProdOper, moveTransaction, operationQtyTotal.CurrentMoveBaseQty, orderUpdateOnly, reverseCost, prodItem);
                }
                    ...
                    || OperationHasMaterialIssued(amProdOper))
                {
                    ...
                    if (IsOperationComplete(amProdOper, prodItem))
                    {
                        amProdOper.StatusID = ProductionOrderStatus.Completed;
						if (!oldOperStatus.Equals(ProductionOrderStatus.Completed))
						{
				...
				if (prodItem.AutoBackwardReporting.GetValueOrDefault())
				{
        ...
        private void CreateScrapWipAdjustment(AMMTran productionAmmTran, MoveOperationQtyTotals moveOperQtyTotals)
        {
			...
			if(AmReleaseGraph.ProductionOvhd.Cache.Dirty.Any_())
			{
				var operations = moveOperQtyTotals.OperationsList.OrderBy(x => x.OperationCD).ToList();
				foreach (var prodOper in operations)
				{
					PXResultset<AMProdOvhd> overheads = ProductionCostCalculator.GetOverheadCostsByOper(AmReleaseGraph, prodOper);
					if(overheads.Any_()) {
						foreach (AMProdOvhd item in overheads)
						{
							WIPAdjustmentGraph.Caches<AMProdOvhd>().SetStatus(item, PXEntryStatus.Held);
						}
					}
				}
			}
        ...
        protected virtual bool IsOperationComplete(AMProdOper amProdOper, AMProdItem prodItem)
        {
            decimal? prodItemQtyComplete = IncludeScrap
                ? prodItem.QtyComplete.GetValueOrDefault() + prodItem.QtyScrapped.GetValueOrDefault()
                : prodItem.QtyComplete.GetValueOrDefault();
            ...
            return prodItemQtyComplete.GetValueOrDefault() >= prodItem.QtytoProd.GetValueOrDefault()
                || operQtyRemaining <= 0m;
        }
        ...
        protected virtual void BackflushLabor(AMProdOper amProdOper, AMMTran productionAmmTran, decimal? currentFgQty, int laborTime,
            bool orderUpdateOnly, bool reverseCost, AMProdItem prodItem)
        {
            ...
            if (string.IsNullOrWhiteSpace(amwc?.WcID))
            {
				...
				// Acuminator disable once PX1050 HardcodedStringInLocalizationMethod [A legacy string, doesn't contain any message which can be localized. All are property values]
            ...
            if (!orderUpdateOnly)
            {
                ...
                ProductionGlTranBuilder.CreateAmGlLine(prodItem, productionAmmTran, tranAmount,
        ...
        protected virtual void Machine(AMProdOper amProdOper, AMMTran productionAmmTran, decimal currentFgQty, bool orderUpdateOnly,
            bool reverseCost, AMProdItem prodItem)
        {
        ...
        protected virtual void BackflushMaterial(AMProdOper amProdOper, AMMTran productionAmmTran, decimal? currentFgQty, AMProdItem prodItem)
        {
            ...
            bool reverseCost, AMProdItem prodItem)
        {
                ...
                >>.Select(AmReleaseGraph, productionAmmTran.OrderType, productionAmmTran.ProdOrdID, amProdOper.OperationID))
            {
				...
				if(toolCurySettings?.CuryID == null)
				{
                ...
                if (tranAmount != 0)
                {
                    if (!orderUpdateOnly)
                    {
                        ProductionGlTranBuilder.CreateAmGlLine(prodItem, productionAmmTran,
		...
		public static void CheckControlPointQuantity(AMMTran tran, MoveOperationQtyTotals operationTotals, AMProdItem prodItem, out string errorMessage)
		{
			errorMessage = null;
			...
			if (!prodItem.AutoBackwardReporting.GetValueOrDefault() && tran.LastOper == false)
			{
				return;
			}
			else if(!prodItem.AutoBackwardReporting.GetValueOrDefault() && tran.LastOper == true)
			{
				tranTotal.ProdOper.BaseQtytoProd = operTotalList.Select(s=>s.ProdOper).Where(t => t.ControlPoint == true).Min(m => m.BaseQtyComplete);
			}

			if (!prodItem.AutoBackwardReporting.GetValueOrDefault() && operationTotals.HasControlPoints && tran.LastOper == true
				&& tranTotal.ProdOper.BaseQtyComplete.GetValueOrDefault() > tranTotal.ProdOper.BaseQtytoProd.GetValueOrDefault())
			{
				errorMessage = Messages.GetLocal(Messages.MoveControlPointQtyAvailableWithoutBackwardReporting, tranTotal.ProdOper?.OrderType, tranTotal.ProdOper?.ProdOrdID, tranTotal.ProdOper?.OperationCD);
				return;
			}
			...
			if (isNegativeTransaction && tranTotal.ProdOper.BaseQtyComplete + tranTotal.ProdOper.BaseQtyScrapped >= controlPointOper.ProdOper.BaseQtyComplete + controlPointOper.ProdOper.BaseQtyScrapped)
			{
				return;
			}
			else if(!isNegativeTransaction && tranTotal.ProdOper.BaseQtyComplete + tranTotal.ProdOper.BaseQtyScrapped <= controlPointOper.ProdOper.BaseQtyComplete + controlPointOper.ProdOper.BaseQtyScrapped)
			{
				return;
			}
        ...
        public static void UpdateOperationQty(PXGraph graph, AMProdItem amProdItem, List<AMProdOper> amProdOpers, bool includeScrap)
        {
			...
			UpdateOperationQtyWithoutMaterial(graph, amProdItem, amProdOpers, includeScrap);
		...
		/// Material total updated as operations are updated.
		/// </summary>
		public static void UpdateOperationQtyWithoutMaterial(PXGraph graph, AMProdItem amProdItem, List<AMProdOper> amProdOpers, bool includeScrap)
		{
        ...
        {
			return SetOperationTotalQty(amProdItem, SetOperationQtyToProd(amProdItem, amProdOpers, includeScrap), includeScrap);
        }

		private static List<AMProdOper> SetOperationTotalQty(AMProdItem amProdItem, List<AMProdOper> amProdOpers, bool includeScrap)
        {
            var list = new List<AMProdOper>();
            var scrapTotal = 0m;
            var scrapBaseTotal = 0m;
			var scrapMultipler = includeScrap ? -1 : 1;
			var sortedOpers = includeScrap ? amProdOpers.OrderBy(x => x.OperationCD) : amProdOpers.OrderByDescending(x => x.OperationCD);
			foreach (var prodOper in sortedOpers)
            {
				if (!includeScrap)
				{
					if (!prodOper.AutoReportQty.GetValueOrDefault())
					{
						scrapTotal = 0m;
						scrapBaseTotal = 0m;
					}
				...
				var calculatedTotalQty = amProdItem.QtytoProd.GetValueOrDefault() + (scrapTotal * scrapMultipler);
				var calculatedBaseTotalQty = amProdItem.BaseQtytoProd.GetValueOrDefault() + (scrapBaseTotal * scrapMultipler);
			...
			return includeScrap ? list : list.OrderBy(x => x.OperationCD).ToList();
        }

        private static List<AMProdOper> SetOperationQtyToProd(AMProdItem amProdItem, List<AMProdOper> amProdOpers, bool includeScrap)
        {
            var list = new List<AMProdOper>();
			if(!amProdOpers.Any())
				return list;
            var nextOperQty = amProdItem.QtytoProd.GetValueOrDefault();
            var nextOperQtyBase = amProdItem.BaseQtytoProd.GetValueOrDefault();
			var orderedOpeations = amProdOpers.OrderBy(x => x.OperationCD).ToList();

			var hasControlPoints = false;
			var minControlQtyInitialized = false;
			var minControlQty = 0m;
			var minControlBaseQty = 0m;
			var lastOperationID = orderedOpeations.Last().OperationID;
			foreach (var prodOper in orderedOpeations)
			{
				var isLastOper = prodOper.OperationID == lastOperationID;
				var isControlPoint = prodOper.ControlPoint.GetValueOrDefault() && !isLastOper;
				hasControlPoints |= isControlPoint;

				if (isControlPoint)
            {
					minControlQty = minControlQtyInitialized ? Math.Min(minControlQty, prodOper.QtyComplete.GetValueOrDefault()) : prodOper.QtyComplete.GetValueOrDefault();
					minControlBaseQty = minControlQtyInitialized ? Math.Min(minControlBaseQty, prodOper.BaseQtyComplete.GetValueOrDefault()) : prodOper.BaseQtyComplete.GetValueOrDefault();
					minControlQtyInitialized = true;
				}

				prodOper.QtytoProd = nextOperQty;
				prodOper.BaseQtytoProd = nextOperQtyBase;

				if (isLastOper && hasControlPoints && !amProdItem.AutoBackwardReporting.GetValueOrDefault())
                {
					prodOper.QtytoProd = minControlQty - prodOper.QtyComplete;
					prodOper.BaseQtytoProd = minControlBaseQty - prodOper.BaseQtyComplete;
                }

                list.Add(prodOper);

				if (prodOper.AutoReportQty.GetValueOrDefault() || (amProdItem.AutoBackwardReporting.GetValueOrDefault() && prodOper.ControlPoint.GetValueOrDefault()))
				{
					nextOperQty = prodOper.QtyComplete.GetValueOrDefault();
					nextOperQtyBase = prodOper.BaseQtyComplete.GetValueOrDefault();
					continue;
        }

				if (includeScrap)
        {
					nextOperQty -= prodOper.QtyScrapped.GetValueOrDefault();
					nextOperQtyBase -= prodOper.BaseQtyScrapped.GetValueOrDefault();
				}
            }
        ...
        public class UnreleasedMaterialTran : PXBqlTable, IBqlTable
        {
            #region DocType
		...
		[PXHidden]
		[Serializable]
		public partial class AMMTranCostTotals : PXBqlTable, IBqlTable, IProdCostTran
		{
			#region OrderType
			public abstract class orderType : PX.Data.BQL.BqlString.Field<orderType> { }
        ...
        public static bool TryCheckNegativeMoveQty(PXCache cache, AMMTran ammTran, decimal moveQty, List<AMMTran> allRows, out Exception exception)
        {
            ...
            if (item != null && (oper.AutoReportQty.GetValueOrDefault() || oper.ControlPoint.GetValueOrDefault())
				&& absTotalNegQty > oper.QtyComplete.GetValueOrDefault() - item.QtyComplete.GetValueOrDefault())
            {
                exception = new PXSetPropertyException(
		...
		/// <summary>
		/// Correct Production Event Line Counter
		/// </summary>
		/// <param name="graph"></param>
		/// <param name="prodOrderCache"></param>
		/// <param name="evntCache"></param>
		/// <param name="prodItem"></param>
		/// <returns></returns>
		private static bool TryCorrectProdEvntLineCounters(PXGraph graph, PXCache prodOrderCache, PXCache evntCache, AMProdItem prodItem)
        {
            ...
            var maxEvent = (AMProdEvnt)SelectFrom<AMProdEvnt>.
                ...
                View.SelectWindowed(graph, 0, 1, prodItem.OrderType, prodItem.ProdOrdID);
		...
		/// <summary>
		/// Run Persist with Production Event Line counter
		/// </summary>
		/// <param name="graph"></param>
		/// <param name="prodOrderCache"></param>
		/// <param name="evntCache"></param>
		/// <param name="persist"></param>
		public static void PersistWithCorrectProdEvntLineCounters(PXGraph graph, PXCache prodOrderCache, PXCache evntCache, Action persist)
		{
			var retryCount = 0;
			do
			{
				...
				{
					var prodItem = (AMProdItem)prodOrderCache?.Current;

					// Only retry on production event lock
					if (prodItem?.ProdOrdID == null || retryCount > 0 || lve.Table != typeof(AMProdEvnt))
					{
						throw;
					}

					if (!TryCorrectProdEvntLineCounters(graph, prodOrderCache, evntCache, prodItem))
					{
						throw;
					}
				}
