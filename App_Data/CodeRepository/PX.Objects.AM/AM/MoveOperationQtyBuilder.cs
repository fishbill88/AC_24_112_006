    ...
    public class MoveOperationQtyBuilder
    {
        ...
        private void ProcessAllTranOperations(bool postOperUpdate)
        {
            foreach (PXResult<AMMTran, InventoryItem, AMOrderType, AMProdItem, AMProdOper> result in AllTranRows)
            {
                ...
                if (ammTran?.LineNbr == null || tranProdOper?.OperationID == null || amProdItem == null || amProdItem.IsProcessMFG == true)
                {
                    continue;
                }
        ...
        private void ProcessPositiveMove(AMMTran ammTran, AMProdOper tranProdOper, bool postOperupdate)
        {
			...
			var tranOperReached = false;
			var skipAutoReportQtyReached = false;
            decimal maxOperBaseQty = 0;
			bool anyAllowBackwardReport = operationResults.Select(s => s.GetItem<AMProdOper>()).Any(a => a.AutoReportQty.GetValueOrDefault());
			bool allControlPoints = operationResults.Select(s => s.GetItem<AMProdOper>()).All(a => a.ControlPoint.GetValueOrDefault());
            foreach (PXResult<AMProdOper, AMWC> operResult in operationResults)
            {
				...
				tranOperReached |= sameOper;
				if (!sameOper && !anyAllowBackwardReport && !allControlPoints)
					continue;

				if (tranOperReached && !sameOper && (operation.AutoReportQty != true || skipAutoReportQtyReached))
				{
					skipAutoReportQtyReached = true;
					continue;
				}
