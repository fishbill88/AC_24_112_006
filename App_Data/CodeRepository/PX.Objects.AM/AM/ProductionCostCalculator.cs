...
namespace PX.Objects.AM
{
    ...
    public class ProductionCostCalculator
    {
        ...
        protected decimal? GetStandardUnitCost(AMMTran ammTran, InventoryItem inventoryItem)
        {
			...
			var baseUnitCost = 0m;
			if (inItemSite != null && inItemSite.StdCost.GetValueOrDefault() != 0)
				baseUnitCost = inItemSite.StdCost.GetValueOrDefault();
			else
			{
				var itemCury = InventoryItemCurySettings.PK.Find(Graph, inventoryItem.InventoryID, Graph.Accessinfo.BaseCuryID);
				baseUnitCost = itemCury != null ? itemCury.StdCost.GetValueOrDefault() : 0m;
			}
        ...
        protected virtual PlannedCosts GetActualCostByOperation(AMMTran ammTran, InventoryItem inventoryItem, AMProdOper amProdOper, decimal transactionBaseQty, decimal additionalWip)
        {
            ...
            var unitCostByBase = transactionBaseQty == 0 ? 0 : (wipBalance / transactionBaseQty);
            var unitCost = ConvertUnitCost(ammTran, inventoryItem.BaseUnit, unitCostByBase).GetValueOrDefault();
            var totalCost = wipBalance;
#if DEBUG
            ...
            decimal? operMaterialUnitCost, decimal? operLaborHours, decimal? operMachHours)
        {
            return GetPlannedVariableOverheadCost(amProdOper,
                ...
                operLaborHours,
                operMachHours,
				GetOverheadCostsByOper(graph, amProdOper));
		}
		...
		/// <returns></returns>
		public static PXResultset<AMProdOvhd> GetOverheadCostsByOper(PXGraph graph, AMProdOper amProdOper)
		{
			return PXSelectJoin<AMProdOvhd,
                InnerJoin<AMOverhead, On<AMProdOvhd.ovhdID, Equal<AMOverhead.ovhdID>>,
				LeftJoin<AMOverheadCurySettings, On<AMOverheadCurySettings.ovhdID, Equal<AMOverhead.ovhdID>,
					And<AMOverheadCurySettings.curyID, Equal<Current<AccessInfo.baseCuryID>>>>>>,
				Where<AMProdOvhd.orderType, Equal<Required<AMProdOvhd.orderType>>,
        ...
        public static PlannedCosts GetPlannedToolCost(PXGraph graph, AMProdOper amProdOper, decimal? productionQty)
        {
            ...
            >.Select(graph, amProdOper.OrderType, amProdOper.ProdOrdID, amProdOper.OperationID))
            {
				...
				var qty = amProdOper.BaseQtyComplete > amProdOper.BaseTotalQty ? amProdOper.BaseQtyComplete : productionQty;
                decimal? operToolQtyRem = amProdTool.QtyReq.GetValueOrDefault() * qty.GetValueOrDefault() - amProdTool.TotActUses.GetValueOrDefault();
            ...
            List<AMMTran> releasedTransactions, List<AMMTran> operWipCompleteResults)
        {
            ...
            foreach (var releasedTran in releasedTransactions.OrderByDescending(tran => tran.LastModifiedDateTime))
            {
                ...
                if (currentQty != 0 && operWipCompleteResults != null)
                {
                    ...
                    {
                        var unitAmountByBase = GetUnitCostFromOperWipComplete(operWipComplete, Math.Abs(releasedTran.BaseQty.GetValueOrDefault()));
						var unitAmount = unitAmountByBase;
						if (!inventoryItem.BaseUnit.Equals(ammTran.UOM))
						{
							unitAmount = ConvertUnitCost(ammTran, inventoryItem.BaseUnit, unitAmount).GetValueOrDefault();
						}

						if (unitAmountByBase == null)
                        {
                            continue;
                        }

                        var processQtyByBase = currentQty * -1;
						UomHelper.TryConvertFromBaseQty<AMMTran.inventoryID>(Graph.Caches[typeof(AMMTran)], ammTran, ammTran.UOM, processQtyByBase,out var processQty);
						processQty = processQty?? processQtyByBase;
                        //Potential add for multiple WIP complete for same oper... this is ok
                        operCostResults.AddOperationCost(GetProdOper(operWipComplete),
							unitAmount.GetValueOrDefault(),
							unitAmount.GetValueOrDefault() * processQty.GetValueOrDefault(),
							processQty.GetValueOrDefault(),
							processQtyByBase,
							false);
                    }
                }
			...
			operCostResults.CalculateTotal();
