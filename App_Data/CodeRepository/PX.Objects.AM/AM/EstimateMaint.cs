...
namespace PX.Objects.AM
{
    ...
    public class EstimateMaint : PXRevisionableGraph<EstimateMaint, AMEstimateItem, AMEstimateItem.estimateID, AMEstimateItem.revisionID>
    {
        ...
        /// </summary>
        protected virtual void CopyWCOverheads()
        {
            foreach (AMEstimateOper result in EstimateOperRecords.Select())
            {
                if (string.IsNullOrWhiteSpace(result?.WcID))
                {
                    continue;
                }
				InsertEstiamteWorkcenterOverheads(EstimateOperRecords.Cache, result);
            }
        }
        ...
        protected virtual void viewOperation()
        {
            ...
            if (EstimateOperRecords.Current != null)
            {
                ...
                {
					if (EstimateOperRecords.Cache.GetStatus(EstimateOperRecords.Current).IsIn(PXEntryStatus.Inserted, PXEntryStatus.Updated))
					{
						Save.Press();
					}
        ...
        protected virtual void CreateBomDetails(AMEstimateItem estimateItem, BOMMaint bomMaintGraph)
        {
            ...
            >.Select(this, estimateItem.EstimateID, estimateItem.RevisionID))
            {
				...
				// Insert the steps for the current operation
					...
					>.Select(this, estimateItem.EstimateID, estimateItem.RevisionID, estimateOper.OperationID))
				{
					try
					{
						bomMaintGraph.BomStepRecords.Insert(new AMBomStep
						{
							OperationID = estimateStep.OperationID,
							NoteID = estimateStep.NoteID,							
							Descr = estimateStep.Description,							
							LineID = estimateStep.LineID,
						});
					}
					catch (Exception e)
					{
						...
						throw new PXException(Messages.GetLocal(Messages.UnableToCreateBomFromEstimateStep),
							estimateItem.EstimateID, estimateItem.RevisionID, estimateStep.LineID, e.Message);
					}
				}
			}
		}
        ...
        protected virtual void AMEstimateItem_RowDeleting(PXCache sender, PXRowDeletingEventArgs e)
        {
			...
			if(amEstimateItem.QuoteSource == EstimateSource.Opportunity)
			{
            var oppMsg = Messages.GetLocal(Messages.UnableToDeleteEstimate,
                Messages.GetLocal(Messages.OpportunityRef), amEstimateReference.OpportunityID);
			...
			EstimateReferenceRecord.Delete(amEstimateReference);  
        }
		...
		protected virtual void _(Events.FieldUpdated<AMEstimateItem, AMEstimateItem.uOM> e)
		{
			if (e.Row != null && !e.Row.IsNonInventory.GetValueOrDefault() && e.OldValue != null && e.OldValue != e.NewValue)
			{
				e.Cache.RaiseFieldUpdated<AMEstimateOper.baseOrderQty>(e.Row, e.OldValue);
			}
		}
	...
	/// <summary>
	/// The settings of the Add to Order dialog box on the Estimate (AM303000) form (which corresponds to the <see cref="EstimateMaint"/> graph).
	/// </summary>
	[Serializable]
    [PXCacheName("Add to Order Filter")]
    public class AddToOrderFilter : PXBqlTable, IBqlTable
    {
        #region OrderType
	...
	/// <summary>
	/// The settings of the Add Comment dialog box on the History tab of the Estimate (AM303000) form (which corresponds to the <see cref="EstimateMaint"/> graph). 
	/// </summary>
	[Serializable]
    [PXCacheName("History Filter")]
    public class HistoryFilter : PXBqlTable, IBqlTable
    {
        #region EstimateID
	...
	/// <summary>
	/// The settings of the Create Production Order dialog box on the Estimate (AM303000) form (which corresponds to the <see cref="EstimateMaint"/> graph). 
	/// </summary>
	//SHARED WITH ESTIMATING AND CRITICAL MATERIALS!!!
	[Serializable]
    [PXCacheName("Create Production Order Filter")]
    public class CreateProdOrderFilter : PXBqlTable, IBqlTable
    {
        #region OrderType
        public abstract class orderType : PX.Data.BQL.BqlString.Field<orderType> { }
	...
	/// <summary>
	/// The settings of the Create BOM dialog box on the Estimate (AM303000) form (which corresponds to the <see cref="EstimateMaint"/> graph). 
	/// </summary>
	[Serializable]
    [PXCacheName("Create BOM Filter")]
    public class CreateBOMFilter : PXBqlTable, IBqlTable, IBomRevision
    {
        #region BOMID
