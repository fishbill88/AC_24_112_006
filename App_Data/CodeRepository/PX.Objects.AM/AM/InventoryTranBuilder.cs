...
namespace PX.Objects.AM
{
    ...
    public class InventoryTranBuilder
    {
		...
		protected virtual void ValidateControlCost()
		{
			IEnumerable<INRegister> updatedList = _inGraph.Caches<INRegister>().Updated.RowCast<INRegister>();
			if (updatedList.Any())
			{
				foreach (INRegister register in updatedList)
				{
					if (register.TotalCost != register.ControlCost)
					{
						register.ControlCost = register.TotalCost;
						_inGraph.Caches<INRegister>().Update(register);
					}
				}
			}
		}
        ...
        protected virtual AMMTran CreateNonStockTranLine(AMMTran ammTran, InventoryItem inventoryItem, bool wipIsDebit)
        {
			...
			var itemCury = InventoryItemCurySettings.PK.Find(_inGraph, inventoryItem.InventoryID, _inGraph.Accessinfo.BaseCuryID);
            var stdCost = itemCury?.StdCost;
