...
namespace PX.Objects.AM
{
    public class ProdDetail : ProdMaintBase<ProdDetail>
    {
        ...
        {
			return GetCurrentWorkcenter(this.ProdOperRecords.Current?.WcID);
        }

        protected virtual AMWC GetCurrentWorkcenter(string wcID)
        {
			AMWC workCenter = AMWC.PK.Find(this, wcID);
        ...
        protected virtual void AMProdOper_WcID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            ...
            var amWC = GetCurrentWorkcenter(row.WcID);
            if (string.IsNullOrWhiteSpace(row.Descr))
            {
        ...
        public void AMProdMatl_InventoryID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            ...
            if (row != null)
            {
				var prodOper = PXParentAttribute.SelectParent<AMProdOper>(cache, row);
				...
				if (prodOper != null)
					row.BFlush = GetCurrentWorkcenter(prodOper.WcID).BflushMatl;
            }
        }
            ...
            foreach (PXResult<INItemPlan, INTranSplit, INTran, INPlanType, INItemPlanDemand> res in list)
            {
                ...
                if (origSchedule != null && (origSchedule.Completed == false || prodMatlSplit.Cache.GetStatus(origSchedule) == PXEntryStatus.Updated))
                {
                    ...
                    else
                    {
						...
						if (schedule.SiteID != origSchedule.SiteID)
						{
        ...
        internal static void TriggerItemPlanUpdates(PXGraph graph, AMProdItem prodItem)
		{
			...
			var splitsCommand = PXSelectJoin<AMProdMatlSplit, InnerJoin<INItemPlan, On<AMProdMatlSplit.FK.ItemPlan>>,
			...
			foreach (PXResult<AMProdMatlSplit, INItemPlan> record in splitsCommand)
			{
				var row = (AMProdMatlSplit)record;
				var plan = (INItemPlan)record;

				graph.Caches[typeof(AMProdMatlSplit)].Hold(row);
				graph.Caches[typeof(INItemPlan)].Hold(plan);
			}

			foreach (PXResult<AMProdMatl, InventoryItem, INSiteStatusByCostCenter> item in SelectFrom<AMProdMatl>
				.InnerJoin<InventoryItem>.On<AMProdMatl.FK.InventoryItem>
				.InnerJoin<INSiteStatusByCostCenter>.On<INSiteStatusByCostCenter.inventoryID.IsEqual<AMProdMatl.inventoryID>
					.And<INSiteStatusByCostCenter.subItemID.IsEqual<AMProdMatl.subItemID>
					.And<INSiteStatusByCostCenter.siteID.IsEqual<AMProdMatl.siteID>
					.And<INSiteStatusByCostCenter.costCenterID.IsEqual<CostCenter.freeStock>>>>>
				.Where<AMProdMatl.orderType.IsEqual<@P.AsString>
					.And<AMProdMatl.prodOrdID.IsEqual<@P.AsString>>>.View.Select(graph, prodItem.OrderType, prodItem.ProdOrdID))
			{
				var matl = (AMProdMatl)item;
				var inventory = (InventoryItem)item;
				var costCenter = (INSiteStatusByCostCenter)item;
				graph.Caches<AMProdMatl>().Update(matl);
				graph.Caches<InventoryItem>().Hold(inventory);
				graph.Caches<INSiteStatusByCostCenter>().Hold(costCenter);
			}

			graph.Caches[typeof(AMProdMatlSplit)].DisableReadItem = true;
			graph.Caches[typeof(INItemPlan)].DisableReadItem = true;
			graph.Caches[typeof(InventoryItem)].DisableReadItem = true;
			foreach (PXResult<AMProdMatlSplit, INItemPlan> record in splitsCommand)
			{
				var row = (AMProdMatlSplit)record;
    ...
    [PXHidden]
    public partial class POLineMatl : PXBqlTable, PX.Data.IBqlTable, ISortOrder
    {
        #region Selected
