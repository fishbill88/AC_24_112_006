...
namespace PX.Objects.AM
{
    ...
    public class AMReleaseProcess : PXGraph<AMReleaseProcess>
    {
        ...
        public override void Persist()
        {
            ...
            ProductionTransactionHelper.PersistWithCorrectProdEvntLineCounters(this, ProductionItems.Cache, ProductionEvents.Cache, base.Persist);
        ...
        protected virtual void UpdateProductionOrdersForMoveLabor(AMBatch doc)
        {
            ...
            UpdateProductionOrders(doc, GetTranDetailsByBatch(doc));
        }

            protected virtual PXResultset<AMMTran> GetTranDetailsByBatch(AMBatch doc)
		{
			return PXSelectJoin<
                AMMTran,
                LeftJoin<InventoryItem, 
        ...
        protected virtual void UpdateProductionOrders(AMBatch doc, PXResultset<AMMTran> results)
        {
			...
			foreach (PXResult<AMMTran, InventoryItem, AMOrderType, AMProdItem, AMProdOper> result in results)
            {
                ...
                if (ammTran.QtyScrapped.GetValueOrDefault() != 0)
                {
                    // Required for scrap to drive material qty
				...
				ProductionItems.Current = amProdItem;
                productionTranHelper.BuildRelatedTransactions(ammTran, moveOperationQtyTotals, amProdItem);
            }
            ...
            foreach (PXResult<AMMTran, InventoryItem, AMOrderType, AMProdItem, AMProdOper> result in results)
            {
				...
				if (!amProdItem.AutoBackwardReporting.GetValueOrDefault())
				{
					ProductionTransactionHelper.UpdateOperationQtyWithoutMaterial(this, amProdItem, moveOperationQtyBuilder?.GetMoveOperationTotals(result.GetItem<AMMTran>())?.OperationsList.LocateElseCopy(this), productionTranHelper.IncludeScrap);
				}
        ...
        protected virtual void ProcessDisassembly(AMBatch doc)
        {
           ...
           >
            ...
            using (var ts = new PXTransactionScope())
            {
                ...
                if (issueTranBuilder?.CurrentGlDocument != null && !GeneralLedgerBatches.Contains(issueTranBuilder?.CurrentGlDocument))
                {
                    GeneralLedgerBatches.Add(issueTranBuilder.CurrentGlDocument);
                }
                if (receiptTranBuilder?.CurrentGlDocument != null && !GeneralLedgerBatches.Contains(receiptTranBuilder?.CurrentGlDocument))
                {
                    GeneralLedgerBatches.Add(receiptTranBuilder.CurrentGlDocument);
                }
                if (adjustmentBuilder?.CurrentGlDocument != null && !GeneralLedgerBatches.Contains(adjustmentBuilder?.CurrentGlDocument))
                {
        ...
        protected virtual void ProcessDisassembleBatchTran(AMBatch doc, AMMTran tranBatch, List<AMMTranSplit> splits, InventoryItem inventoryItem, AMProdItem prodItem, ProductionTransactionHelper productionTranHelper, ref InventoryTranBuilder inventoryBuilder)
        {
            ...
            productionTranHelper.BuildRelatedTransactions(tranBatch, moveOperationQtyTotals, prodItem);
        ...
        protected virtual MaterialEntry FindResetBackflushMaterialBatch(AMBatch doc, out bool materialReleased, out bool inRegisterReleased)
        {
			...
			var inDocuments = new HashSet<string>();
			var glDocuments = new HashSet<string>();
            ...
            foreach (AMMTran row in materialEntry.transactions.Select())
            {
				...
				if (!string.IsNullOrWhiteSpace(row.INBatNbr))
				{
					inDocuments.Add(string.Join(",",row.INDocType, row.INBatNbr)); 
				}
				if (!string.IsNullOrWhiteSpace(row.GLBatNbr))
				{
					glDocuments.Add(row.GLBatNbr);
				}

                materialEntry.transactions.Delete(row);
            }

			foreach (var glDoc in glDocuments)
			{
				FindResetRelatedGLBatch(glDoc);
			}

			ResetINDocuments(inDocuments);
            ...
            if (materialEntry.transactions.Cache.Deleted.Any_())
            {
				...
				materialEntry.SkipReferenceDelete = true;
		...
		protected virtual void ResetINDocuments(IEnumerable<string> documents)
		{
			var receiptEntryGraph = new Lazy<INReceiptEntry>(() => PXGraph.CreateInstance<INReceiptEntry>());
			var issueEntryGraph = new Lazy<INIssueEntry>(() => PXGraph.CreateInstance<INIssueEntry>());
			var adjustmentEntryGraph = new Lazy<INAdjustmentEntry>(() => PXGraph.CreateInstance<INAdjustmentEntry>());

			foreach (var inDoc in documents)
			{
				var inDocKeys = inDoc.Split(',');

				if (inDocKeys.Length != 2)
				{
					continue;
				}

				var docType = inDocKeys[0];
				var inRefNbr = inDocKeys[1];

				var inRegister = INRegister.PK.Find(this, docType, inRefNbr);
				if (inRegister == null || inRegister.Released == true)
				{
					continue;
				}
				...
				{
					receiptEntryGraph.Value.Clear();
					receiptEntryGraph.Value.receipt.Current = inRegister;
					foreach (var inTran in receiptEntryGraph.Value.transactions.Select())
					{
						receiptEntryGraph.Value.transactions.Delete(inTran);
					}
					receiptEntryGraph.Value.Persist();
				...
				{
					issueEntryGraph.Value.Clear();
					issueEntryGraph.Value.issue.Current = inRegister;
					foreach (var inTran in issueEntryGraph.Value.transactions.Select())
					{
						issueEntryGraph.Value.transactions.Delete(inTran);
					}
					issueEntryGraph.Value.Persist();
				...
				{
					adjustmentEntryGraph.Value.Clear();
					adjustmentEntryGraph.Value.adjustment.Current = inRegister;
					foreach (var inTran in adjustmentEntryGraph.Value.transactions.Select())
					{
						adjustmentEntryGraph.Value.transactions.Delete(inTran);
					}
					adjustmentEntryGraph.Value.Persist();
				}
			}
		}
            ...
            AMProdOper amprodoper, MoveOperationQtyTotals moveOperationQtyTotals)
        {
            ...
            if (!tran.Released.GetValueOrDefault())
            {
                ...
                // + tool
                // + overhead
                productionTranHelper.BuildRelatedTransactions(tran, moveOperationQtyTotals, amproditem);
        ...
        protected virtual void CreateWipCompleteCostEntries(AMMTran tran, ProductionTransactionHelper productionTranHelper, ProductionOperationCostResults operationResults)
        {
            foreach (var operationResult in operationResults.OperationResults)
            {
                CreateWipCompleteCostEntry(tran, productionTranHelper, operationResult, operationResult.ProcessBaseQty);
            }
        }
        ...
        protected virtual void ProcessMoveTran(AMBatch doc)
        {
            ...
            PXResultset<AMMTran> results = GetTranDetailsByBatch(doc);
            ...
            foreach (PXResult<AMMTran, InventoryItem, AMOrderType, AMProdItem, AMProdOper> result in results)
            {
				...
				if (tran.Released == true || prodItem?.IsProcessMFG == true) continue;
        ...
        protected virtual void CheckUnderIssuedMaterial(MoveOperationQtyBuilder builder)
        {
            ...
            foreach (var result in builder.AllTranRows)
            {
                ...
                if (ammTran == null
                    ...
                    || ammTran.Released == true || prodItem.IsProcessMFG == true)
                {
                    continue;
                }
		...
		protected virtual void CheckControlPointQty(MoveOperationQtyBuilder builder)
		{
			...
			foreach (PXResult<AMMTran, InventoryItem, AMOrderType, AMProdItem, AMProdOper> result in builder.AllTranRows)
			{
				...
				var prodItem = result.GetItem<AMProdItem>();
				if (prodOper == null
					|| !orderHash.Add(string.Join(":", prodOper.OrderType, prodOper.ProdOrdID, prodOper.OperationID))
					|| tran.Released == true
					|| prodItem?.IsProcessMFG == true)
				{
					continue;
				}
				...
				ProductionTransactionHelper.CheckControlPointQuantity(tran, operTotals, result.GetItem<AMProdItem>(), out var eMessage);
        ...
        protected virtual void CheckDuplicateSerialNumber(AMBatch doc)
        {
                ...
                >>>.Select(this, doc.DocType, doc.BatNbr))
            {
                ...
                if (isIssue && (duplicateResult.LotSerAssign == INLotSerAssign.WhenReceived && duplicateResult.QtyOnHand.GetValueOrDefault() <= 0 ||
                                duplicateResult.LotSerAssign == INLotSerAssign.WhenUsed && duplicateResult.QtyOnHand.GetValueOrDefault() > 0))
                {
                    sb.AppendLine(Messages.GetLocal(
        ...
        protected virtual void ProcessMaterialTran(AMBatch doc)
        {
			...
			var allTrans = PXSelectJoin<
				...
				InnerJoin<AMProdItemStatus,
					On<AMProdItemStatus.orderType, Equal<AMMTran.orderType>,
						And<AMProdItemStatus.prodOrdID, Equal<AMMTran.prodOrdID>>>>>,
				Where<AMMTran.docType, Equal<Required<AMBatch.docType>>,
					And<AMMTran.batNbr, Equal<Required<AMBatch.batNbr>>>>>
				.Select(this, doc.DocType, doc.BatNbr);

			foreach (PXResult<AMMTran, InventoryItem, AMProdItemStatus> result in allTrans)
            {
                var tran = (AMMTran) result;
                var inventoryItem = (InventoryItem) result;
				var prodItemStatus = (AMProdItemStatus) result;

				if (tran.Released == true || prodItemStatus.IsProcessMFG == true)
				{
					continue;
				}
		...
		protected virtual void SetMaterialLinkedItems(List<AMProdItemSplit> aMProdItems)
		{
			...
			foreach (AMProdItemSplit item in aMProdItems)
			{
				...
				{
					var matlLotSerialCache = this.Caches<AMProdMatlLotSerial>();
					...
					var collection = matlLotSerialCache.Cached.RowCast<AMProdMatlLotSerial>().Where(l => l.OrderType == item.OrderType
																											&& l.ProdOrdID == item.ProdOrdID
																											&& l.ParentLotSerialNbr == item.LotSerialNbr);
					decimal deltaQtyIssued = CalculateCachedIssueDelta(matlLotSerialCache, collection);

					item.IsMaterialLinked = ((results?.QtyIssued.GetValueOrDefault() + deltaQtyIssued) > 0);
		...
		protected virtual decimal CalculateCachedIssueDelta(PXCache<AMProdMatlLotSerial> matlLotSerialCache, IEnumerable<AMProdMatlLotSerial> collection)
		{
			decimal deltaQtyIssued = 0m;
			foreach (AMProdMatlLotSerial uItem in collection)
			{
				switch (matlLotSerialCache.GetStatus(uItem))
				{
					case PXEntryStatus.Updated:
						var original = matlLotSerialCache.GetOriginal(uItem);
						deltaQtyIssued += (uItem.QtyIssued - original?.QtyIssued).GetValueOrDefault();
						break;
					case PXEntryStatus.Inserted:
						deltaQtyIssued += uItem.QtyIssued.GetValueOrDefault();
						break;
			...
			foreach (AMProdMatl amProdMatl in resultset)
			{
				...
				if (counter == 1)
				{
					...
					if (ammTran.MatlLineId == null)
						TranRecs.Cache.SetValueExt<AMMTran.matlLineId>(ammTran, amProdMatl.LineID);
				}
        ...
        protected virtual void UpdateProductionMaterialSplits(AMMTran ammTran, AMProdMatl prodMatl, List<AMProdMatlSplit> prodMatlSplits)
        {
			...
			List<AMMTranSplit> tranSplits = PXParentAttribute.SelectChildren(SplitRecs.Cache, ammTran, typeof(AMMTran)).RowCast<AMMTranSplit>().ToList();

			UpdateProductionMaterialSplits(ammTran, tranSplits, prodMatl, prodMatlSplits);			
        }

		protected virtual void UpdateProductionMaterialSplits(AMMTran ammTran, List<AMMTranSplit> tranSplits, AMProdMatl prodMatl, List<AMProdMatlSplit> prodMatlSplits)
		{
			...
			//first, reduce prodmatl with matching lot/serial as the tran
			foreach(var transplit in tranSplits)
			{
				var matchSplit = prodMatlSplits.FirstOrDefault(x => x.LotSerialNbr == transplit.LotSerialNbr);
				if(matchSplit != null)
				{
					var tsallocatedIssueQty = isIssue ? transplit.Qty.GetValueOrDefault() : 0m;
					var tsallocatedIssueBaseQty = isIssue ? transplit.BaseQty.GetValueOrDefault() : 0m;
					var tsissueQty = transplit.Qty.GetValueOrDefault();
					var tsissueBaseQty = transplit.BaseQty.GetValueOrDefault();
					UpdateProductionMaterialSplit(matchSplit, ref tsallocatedIssueQty, ref tsallocatedIssueBaseQty, ref tsissueQty, ref tsissueBaseQty);
					if(matchSplit.IsAllocated.GetValueOrDefault())
					{
						allocatedIssueQty -= isIssue ? transplit.Qty.GetValueOrDefault() - tsallocatedIssueQty : 0;
						allocatedIssueBaseQty -= isIssue ? transplit.BaseQty.GetValueOrDefault() - tsallocatedIssueBaseQty : 0;
						if(matchSplit.ParentSplitLineNbr == null)
						{
							issueQty -= transplit.Qty.GetValueOrDefault() - tsissueQty;
							issueBaseQty -= transplit.BaseQty.GetValueOrDefault() - tsissueBaseQty;
						}
						continue;
					}
					issueQty -= transplit.Qty.GetValueOrDefault() - tsissueQty;
			...
			//second, if it's not lot/serial or there is remaining amount, reduce oldest planid first 
            foreach (var split in prodMatlSplits.OrderByDescending(x => x.IsAllocated).ThenBy(x => x.PlanID))
            {
				UpdateProductionMaterialSplit(split, ref allocatedIssueQty, ref allocatedIssueBaseQty, ref issueQty, ref issueBaseQty);
			}
		}

		protected virtual void UpdateProductionMaterialSplit(AMProdMatlSplit split, ref decimal allocatedIssueQty, ref decimal allocatedIssueBaseQty, ref decimal issueQty, ref decimal issueBaseQty)
                {
			// Need to find cached value in case already updated or deleted
			var splitCopy = ProdMatlSplitsLocate(split);

			if (splitCopy?.ProdOrdID == null || ProdMatlSplitsGetStatus(splitCopy) == PXEntryStatus.Deleted)
			{
				return;
                }
        ...
        protected virtual void UpdateProductionMaterialSplit(AMProdMatlSplit split, decimal qty, decimal baseQty)
        {
            ...
            if (split.Qty.GetValueOrDefault() <= 0 && split.POLineNbr == null)
            {
                ...
                ProdMatlSplitsDelete(ProdMatlSplitsUpdate(split));
                return;
            }

            ProdMatlSplitsUpdate(split);
        }

		protected virtual AMProdMatlSplit ProdMatlSplitsUpdate(AMProdMatlSplit split)
		{
			return ProdMatlSplits.Update(split);
		}

		protected virtual AMProdMatlSplit ProdMatlSplitsDelete(AMProdMatlSplit split)
		{
			return ProdMatlSplits.Delete(split);
		}

		protected virtual AMProdMatlSplit ProdMatlSplitsLocate(AMProdMatlSplit split)
		{
			return ProdMatlSplits.Cache.LocateElseCopy(split);
		}

		protected virtual PXEntryStatus ProdMatlSplitsGetStatus(AMProdMatlSplit split)
		{
			return ProdMatlSplits.Cache.GetStatus(split);
        }
        ...
        protected virtual void AddMaterialToOrder(AMBatch doc)
        {
                    ...
                    And<AMMTran.batNbr, Equal<Required<AMMTran.batNbr>>>>>.Select(this, doc.DocType, doc.BatNbr))
            {
                ...
                if (amProdMatl != null && cacheStatus == PXEntryStatus.Inserted)
                {
                    ...
                    matlTran.MatlLineId = amProdMatl.LineID;
    ...
    public class AMMTranMoveByLotSerial : PXBqlTable, IBqlTable
    {
