...
namespace PX.Objects.AM
{
    ...
    public class GenerateForecastProcess : PXGraph<GenerateForecastProcess>
    {
        ...
        public void ProcessHistory()
        {
            ...
            try
            {
				using (new DisableFormulaCalculationScope(SelectedRecs.Cache, typeof(AMForecastStaging.changeUnits), typeof(AMForecastStaging.percentChange)))
				{
					...
					var includeNSKits = InventoryHelper.KitAssembliesFeatureEnabled;
					var queryRestriction = new QueryRestriction(siteId != null, itemClassId != null, customerId != null, inventoryId != null, subItemId != null, includeNSKits);

					// Get the Command to select relevant sales History by Customer as needed
					AllSalesCommands allCmds = new AllSalesCommands();

					bool groupByCustomer = customerId != null || Settings.Current.ProcessByCustomer == true;

					if (groupByCustomer)
					{
						allCmds = GetAllSalesHistoryByCustomerCommand(queryRestriction);
					}
					else
					{
						allCmds = GetAllSalesHistoryCommand(queryRestriction);
					}

					List<INTranAllSalesHistory> resultset = GetAllSalesHistory(allCmds, queryRestriction, inventoryId, subItemId, siteId, itemClassId, customerId);
					...
					foreach (var aggregate in resultset)
					{
						...
						var qr = new QueryRestriction(AM.InventoryHelper.MultiWarehousesFeatureEnabled,
							AM.InventoryHelper.SubItemFeatureEnabled, groupByCustomer, includeNSKits);

						var cmds = GetSalesHistoryCommand(qr);

						try
						{
							ProcessBuckets(cmds, qr, buckets);
						}
						catch (Exception e)
						{
		...
		protected virtual decimal GetSalesHistoryTotal(SalesCommands cmds, QueryRestriction queryRestriction, DateTime beginDate, DateTime endDate, Bucket bucket, int? bAccountId)
        {
            var qty = 0m;
            foreach (INTranSalesHistory row in GetSalesHistory(cmds, queryRestriction, beginDate, endDate, bucket, bAccountId))
		{
				if (row?.InventoryID == null)
				{
					continue;
        ...
        protected virtual List<INTranSalesHistory> GetSalesHistory(SalesCommands cmds, QueryRestriction queryRestriction, DateTime beginDate, DateTime endDate,
			Bucket bucket, int? bAccountId) 
		{
            return GetSalesHistory(cmds, queryRestriction, beginDate, endDate, bucket?.InventoryID, bucket?.SiteID, bucket?.SubItemID, bAccountId);
        }

        protected virtual List<INTranSalesHistory> GetSalesHistory(SalesCommands cmds, QueryRestriction queryRestriction, DateTime beginDate, DateTime endDate,
			int? inventoryId, int? siteId, int? subItemId, int? bAccountId)
		{
            if (inventoryId == null)
            {
            ...
            return GetSalesHistory(cmds, parms, queryRestriction.IncludeNSKits);
        }

        protected virtual List<INTranSalesHistory> GetSalesHistory(SalesCommands cmds, List<object> parms, bool includeNSKits) 
		{
			var cmd = cmds.BaseCmdTran;
            if (cmd == null)
            {
			...
			var tranResults = cmd.Select(parms.ToArray()).RowCast<INTranSalesHistory>().ToList();
			if(includeNSKits)
			{
				var cmdKits = cmds.BaseCmdNSKit;
				if(cmdKits == null)
				{
					throw new ArgumentNullException(nameof(cmdKits));
				}
				var kitResult = cmdKits.Select(parms.ToArray());
				var convtoTran = kitResult.RowCast<NSKitSalesHistory>().Select(x => (INTranSalesHistory)x).ToList();
				tranResults.AddRange(convtoTran);
			}
			return tranResults;
        }

        protected virtual List<INTranAllSalesHistory> GetAllSalesHistory(AllSalesCommands cmds, QueryRestriction queryRestriction, 
            int? inventoryId, int? subItemId, int? siteId, int? itemClassId, int? bAccountId)
        {
            ...
            return GetAllSalesHistory(cmds, parms, queryRestriction.IncludeNSKits);
        }

        protected virtual List<INTranAllSalesHistory> GetAllSalesHistory(AllSalesCommands cmds, List<object> parms, bool includeNSKits)
		{
			var cmdTran = cmds.BaseCmdTran;
            if (cmdTran == null)
            {
			...
			var tranResults = cmdTran.Select(parms.ToArray()).RowCast<INTranAllSalesHistory>().ToList();
			if(includeNSKits)
			{
				var cmdKits = cmds.BaseCmdNSKit;
				if (cmdKits == null)
				...
				var kitResult = cmdKits.Select(parms.ToArray());
				var convtoTran = kitResult.RowCast<NSKitAllSalesHistory>().Select(x => (INTranAllSalesHistory)x).ToList();
				tranResults.AddRange(convtoTran);
			}
        ...
        protected struct QueryRestriction
        {
			...
			public bool IncludeNSKits { get; private set; }

			public QueryRestriction(bool byWarehouse, bool bySubItem, bool byCustomer, bool includeNSKits)
                : this(byWarehouse, bySubItem, byCustomer, false, false, includeNSKits)
            {
            }

            public QueryRestriction(bool byWarehouse, bool byItemClass, bool byCustomer, bool byInventoryID, bool bySubItem, bool includeNSKits)
            {
                ...
                Customer = byCustomer;
				IncludeNSKits = includeNSKits;
            }
        }

		protected struct AllSalesCommands
		{
			public PXSelectBase<INTranAllSalesHistory> BaseCmdTran { get; set; }
			public PXSelectBase<NSKitAllSalesHistory> BaseCmdNSKit { get; set; }
		}

		protected struct SalesCommands
		{
			public PXSelectBase<INTranSalesHistory> BaseCmdTran { get; set; }
			public PXSelectBase<NSKitSalesHistory> BaseCmdNSKit { get; set; }
		}

		protected virtual SalesCommands GetSalesHistoryCommand(QueryRestriction queryRestriction)
        {
			...
			PXSelectBase<NSKitSalesHistory> cmdKit = new PXSelect<NSKitSalesHistory,
				Where<NSKitSalesHistory.inventoryID, Equal<Required<NSKitSalesHistory.inventoryID>>,
					And<NSKitSalesHistory.tranDate, Between<Required<SOLine.shipDate>, Required<SOLine.shipDate>>>>>(this);

			if (queryRestriction.Warehouse)
            {
                cmd.WhereAnd<Where<INTranSalesHistory.siteID, Equal<Required<INTranSalesHistory.siteID>>>>();
				cmdKit.WhereAnd<Where<NSKitSalesHistory.siteID, Equal<Required<NSKitSalesHistory.siteID>>>>();
			}

            if (queryRestriction.SubItem)
            {
                cmd.WhereAnd<Where<INTranSalesHistory.subItemID, Equal<Required<INTranSalesHistory.subItemID>>>>();
				cmdKit.WhereAnd<Where<NSKitSalesHistory.subItemID, Equal<Required<NSKitSalesHistory.subItemID>>>>();
			}

            if (queryRestriction.Customer)
            {
                cmd.WhereAnd<Where<INTranSalesHistory.bAccountID, Equal<Required<INTranSalesHistory.bAccountID>>>>();
				cmdKit.WhereAnd<Where<NSKitSalesHistory.bAccountID, Equal<Required<NSKitSalesHistory.bAccountID>>>>();
			}

            return new SalesCommands
			{
				BaseCmdTran = cmd,
				BaseCmdNSKit = cmdKit
			};
		...
		protected virtual AllSalesCommands GetAllSalesHistoryCommand(QueryRestriction queryRestriction)
        {
			...
			// Create Base select command for selecting sales history records
			PXSelectBase<NSKitAllSalesHistory> cmdKit = new PXSelectGroupBy<NSKitAllSalesHistory,
				Where<NSKitAllSalesHistory.tranDate, Between<Required<SOLine.shipDate>, Required<SOLine.shipDate>>>,
				Aggregate<
					GroupBy<NSKitAllSalesHistory.inventoryID,
					GroupBy<NSKitAllSalesHistory.subItemID,
					GroupBy<NSKitAllSalesHistory.siteID,
					Sum<NSKitAllSalesHistory.tranAmt,
					Sum<NSKitAllSalesHistory.baseQty>>>>
				>>>(this);
			...
			if (queryRestriction.InventoryID)
			{
				...
				cmdKit.WhereAnd<Where<NSKitAllSalesHistory.inventoryID, Equal<Current<ForecastSettings.inventoryID>>>>();
			}
			#endregion
			...
			if (queryRestriction.SubItem)
			{
				...
				cmdKit.WhereAnd<Where<NSKitAllSalesHistory.subItemID, Equal<Current<ForecastSettings.subItemID>>>>();
			}
			#endregion
			...
			if (queryRestriction.Warehouse)
			{
				...
				cmdKit.WhereAnd<Where<NSKitAllSalesHistory.siteID, Equal<Current<ForecastSettings.siteID>>>>();
			}
			...
			if (queryRestriction.ItemClass)
			{
				...
				cmdKit.WhereAnd<Where<NSKitAllSalesHistory.itemClassID, Equal<Current<ForecastSettings.itemClassID>>>>();
			}
			#endregion

			return new AllSalesCommands
			{
				BaseCmdTran = cmdTran,
				BaseCmdNSKit = cmdKit
			};
		...
		protected virtual AllSalesCommands GetAllSalesHistoryByCustomerCommand(QueryRestriction queryRestriction)
        {
			...
			// Create Base select command for selecting sales history records Customer
			PXSelectBase<NSKitAllSalesHistory> cmdKit = new PXSelectJoinGroupBy<NSKitAllSalesHistory,
				InnerJoin<Customer, On<Customer.bAccountID, Equal<NSKitAllSalesHistory.bAccountID>>>,
				Where<NSKitAllSalesHistory.tranDate, Between<Required<INTran.tranDate>, Required<INTran.tranDate>>>,
				Aggregate<
					GroupBy<NSKitAllSalesHistory.bAccountID,
					GroupBy<NSKitAllSalesHistory.inventoryID,
					GroupBy<NSKitAllSalesHistory.subItemID,
					GroupBy<NSKitAllSalesHistory.siteID,
					Sum<NSKitAllSalesHistory.tranAmt>>>
			...
			if (queryRestriction.Customer)
			{
				...
				cmdKit.WhereAnd<Where<NSKitAllSalesHistory.bAccountID, Equal<Required<ForecastSettings.bAccountID>>>>();
			}
			#endregion
		...
		[PXProjection(typeof(Select<
            INTran, 
            Where2<Where<INTran.tranType, Equal<INTranType.invoice>,
                Or<INTran.tranType, Equal<INTranType.creditMemo>,
                Or<INTran.tranType, Equal<INTranType.debitMemo>>>>,
				And<INTran.isComponentItem,Equal<False>>>>), Persistent = false)]
        [PXHidden]
        [Serializable]
        public class INTranSalesHistory : PXBqlTable, IBqlTable
		{
            #region BranchID
			...
			public static explicit operator INTranSalesHistory(NSKitSalesHistory kit)
			{
				return new INTranSalesHistory
				{
					TranDate = kit.TranDate,
					InvtMult = kit.InvtMult,
					InventoryID = kit.InventoryID,
					SubItemID = kit.SubItemID,
					SiteID = kit.SiteID,
					BAccountID = kit.BAccountID,
					BaseQty = kit.BaseQty
				};
			}
		}
		...
		[PXProjection(typeof(Select2<
			...
			Where2<Where<InventoryItem.itemStatus, Equal<InventoryItemStatus.active>,
				...
				And<Where2<Where<INTran.tranType, Equal<INTranType.invoice>,
					Or<INTran.tranType, Equal<INTranType.creditMemo>,
					Or<INTran.tranType, Equal<INTranType.debitMemo>>>>,
					And<INTran.isComponentItem, Equal<False>>>>>>), Persistent = false)]
		[PXHidden]
        [Serializable]
        public class INTranAllSalesHistory : PXBqlTable, IBqlTable
		{
			#region BranchID
			public abstract class branchID : PX.Data.BQL.BqlInt.Field<branchID> { }
			protected Int32? _BranchID;
			...
			#endregion
			public static explicit operator INTranAllSalesHistory(NSKitAllSalesHistory kit)
			{
				return new INTranAllSalesHistory
				{
					TranDate = kit.TranDate,
					InvtMult = kit.InvtMult,
					InventoryID = kit.InventoryID,
					...
					BaseQty = kit.BaseQty,
					TranAmt = kit.TranAmt,
					ItemStatus = kit.ItemStatus,
					ItemClassID = kit.ItemClassID,
					BaseUnit = kit.BaseUnit,
					SalesUnit = kit.SalesUnit,
					QtyRoundUp = kit.QtyRoundUp
				};
			}
		...
		[PXProjection(typeof(Select2<SOLine,
			...
			Where2<Where2<Where<InventoryItem.itemStatus, Equal<InventoryItemStatus.active>,
						Or<InventoryItem.itemStatus, Equal<InventoryItemStatus.noPurchases>>>,
					And<Where<InventoryItem.kitItem, Equal<True>,And<InventoryItem.stkItem,Equal<False>>>>>,
				And<SOLine.completed,Equal<True>>>>), Persistent = false)]
		[PXHidden]
		[Serializable]
		public class NSKitAllSalesHistory : PXBqlTable, IBqlTable
		{
			#region TranDate
			public abstract class tranDate : PX.Data.BQL.BqlDateTime.Field<tranDate> { }
			protected DateTime? _TranDate;
			...
			public abstract class invtMult : PX.Data.BQL.BqlShort.Field<invtMult> { }
			protected Int16? _InvtMult;
			[PXDBShort(BqlField = typeof(SOLine.invtMult))]
			[PXUIField(DisplayName = "Multiplier")]
			public virtual Int16? InvtMult
			{
				get
			...
			public abstract class tranAmt : PX.Data.BQL.BqlDecimal.Field<tranAmt> { }
			protected Decimal? _TranAmt;
			[PXDBBaseCury(BqlField = typeof(SOLine.extCost))]
			[PXDefault(TypeCode.Decimal, "0.0")]
			[PXUIField(DisplayName = "Tran Amount")]
			public virtual Decimal? TranAmt
			{
		...
		[PXProjection(typeof(Select2<SOLine,
			InnerJoin<InventoryItem, On<InventoryItem.inventoryID, Equal<SOLine.inventoryID>>>,
			Where2<Where<InventoryItem.kitItem, Equal<True>, And<InventoryItem.stkItem, Equal<False>>>,
				And<SOLine.completed, Equal<True>>>>), Persistent = false)]
		[PXHidden]
		[Serializable]
		public class NSKitSalesHistory : PXBqlTable, IBqlTable
		{
			#region TranDate
			public abstract class tranDate : PX.Data.BQL.BqlDateTime.Field<tranDate> { }
			protected DateTime? _TranDate;
        ...
        protected virtual void ProcessBuckets(SalesCommands cmds, QueryRestriction queryRestriction, BucketCollection buckets) 
        {
            if (buckets == null)
            {
                return;
            ...
            foreach (Bucket bucket in buckets)
            {
                ...
                {
                    totalForecastQty += GetSalesHistoryTotal(cmds, queryRestriction, bucket.BeginDate.AddYears(-i), bucket.EndDate.AddYears(-i), bucket, bucket.BAccountID);
                }

                //Get previous year sales
                var previousYearQty = GetSalesHistoryTotal(cmds, queryRestriction, bucket.BeginDate.AddYears(-1), bucket.EndDate.AddYears(-1), bucket, bucket.BAccountID);
	...
	/// <summary>
	/// The filter for the Generate Forecast (AM502000) form (which corresponds to the <see cref="GenerateForecastProcess"/> graph).
	/// </summary>
	#region ForecastSettings
	[Serializable]
    [PXCacheName(AM.Messages.ForecastSettings)]
    public class ForecastSettings : PXBqlTable, IBqlTable
    {
        #region ForecastDate
