...
namespace PX.Objects.AM
{
    ...
    public class MRPDisplay : PXGraph<MRPDisplay>
    {
		...
		public PXFilter<PlanKitFilter> PlanKitAssemblyFilter;
		...
		[PXHidden]
		public PXSelect<Numbering, Where<Numbering.numberingID, Equal<Current<INSetup.kitAssemblyNumberingID>>>> KitAssemblyNumbering;
		...
		protected Numbering CurrentKitAssemblyNumbering
		{
			get
			{
				KitAssemblyNumbering.Current = PXSelect
					<Numbering, Where<Numbering.numberingID, Equal<Required<Numbering.numberingID>>>
						>.Select(this, INSetup.Current.KitAssemblyNumberingID);
				return KitAssemblyNumbering.Current;
			}
		}
		...
		public PXAction<AMRPDetail> KitAssemblyButton;
		[PXUIField(DisplayName = "Assemble Kit", Enabled = true)]
		[PXButton]
		public virtual IEnumerable kitAssemblyButton(PXAdapter adapter)
		{
			if (!HasCrossRefRecords)
			{
				try
				{
					GenerateKitAssemblyProcessRecords();
				}
				catch
				{
					ResetOrderCrossRef();
			...
			if (PlanKitAssemblyFilter.AskExt(true) == WebDialogResult.OK)
			{
				try
				{
					this.Persist();
					GenerateKitAssemblies();
				}
				catch
				{
					ResetOrderCrossRef();
		...
		public MRPDisplay()
        {
			...
			if (!InventoryHelper.KitAssembliesFeatureEnabled)
			{
				KitAssemblyButton.SetVisible(false);
				KitAssemblyButton.SetEnabled(false);
			}
		}
		...
		protected virtual void GenerateKitAssemblyProcessRecords()
		{
			int groupNumber = 0;
			int inventoryID = 0;
			int? subItemID = null;
			...
			KitAssemble(Detailrecs.Cache.Cached.Cast<AMRPDetail>().Where(row => row.Selected == true).ToList());
                ...
                foreach (var orderCrossRef in list)
                {
                    ...
                    try
                    {
                        ...
                        if (order.OrderNbr == null)
                        {
                            ...
                            poOrder = poOrderEntryGraph.Document.Insert(poOrder);

							if (PXAccess.FeatureInstalled<FeaturesSet.multicurrency>())
							{
								// Redefault the base currency and use currency from vendor.
								poOrder.OverrideCurrency = true;
							}
							poOrder.VendorID = filter.VendorID;
                            poOrder.VendorLocationID = filter.VendorLocationID;
                            poOrder.SiteID = orderCrossRef.SiteID;

                            poOrderEntryGraph.Document.Update(poOrder);
                        }
                        else
                        {
                            //  PO Exists
                        ...
                        ordered.Add(line);
						if (order.SiteID != null && order.SiteID != line.SiteID)
							poOrderEntryGraph.Document.SetValueExt<POOrder.siteID>(order, null);
        ...
        private static POLine InsertPOLine(POOrderEntry graph, AMOrderCrossRef amOrderCrossRef)
        {
            ...
            copyX.LineType = lineType;
            copyX.InventoryID = amOrderCrossRef.InventoryID;

            graph.Transactions.Cache.RaiseFieldUpdated<POLine.inventoryID>(copyX, null);

            copyX.SubItemID = amOrderCrossRef.SubItemID;

            copyX.UOM = amOrderCrossRef.UOM;
            graph.Transactions.Cache.RaiseFieldUpdated<POLine.uOM>(copyX, null);
            graph.Transactions.Cache.SetDefaultExt<POLine.rcptQtyAction>(copyX);

            copyX = graph.Transactions.Update(copyX);
            if (amOrderCrossRef.SiteID.HasValue)
                graph.Transactions.Cache.RaiseExceptionHandling<POLine.siteID>((object)copyX, (object)null, (Exception)null);
            if (amOrderCrossRef.SiteID.HasValue)
            {
                graph.Transactions.Cache.RaiseExceptionHandling<POLine.siteID>((object)copyX, (object)null, (Exception)null);
                copyX.SiteID = amOrderCrossRef.SiteID;
            }
            copyX.OrderQty = amOrderCrossRef.Qty;
            graph.Transactions.Cache.RaiseFieldUpdated<POLine.uOM>(copyX, null);

            if (DateTime.Compare((amOrderCrossRef.PlanDate ?? Common.Dates.BeginOfTimeDate), (copyX.PromisedDate ?? Common.Current.BusinessDate(graph))) > 0)
            {
                //only set when date is greater than calculated promised date 
                copyX.PromisedDate = amOrderCrossRef.PlanDate;
            }

            POLine poLine3 = graph.Transactions.Update(copyX);
		...
		public void KitAssemble(List<AMRPDetail> list)
		{
			int? lineNbrCntr = 0;
			...
			foreach (AMRPDetail detail in list)
			{
				...
				var processRecord = new AMOrderCrossRef
				{
					...
					Source = detail.ReplenishmentSource,
					UOM = detail.BaseUOM,
					KitRevisionID = detail.KitRevisionID
				};
				processRecord.Qty = detail.BaseQty;
				processRecord.ReferenceLineNbr = detail.RecordID;

				Numbering numbering = CurrentKitAssemblyNumbering;

				if (numbering != null && !numbering.UserNumbering.GetValueOrDefault())
				{
					processRecord.ReferenceNbr = numbering.NewSymbol;
					processRecord.ManualNumbering = false;
				}
				else
				{
                    ...
                    try
                    {
                        ...
                        if (order.OrderNbr == null)
                        {
                            ...
                            soOrder.OrderNbr = numbering != null && numbering.UserNumbering.GetValueOrDefault()
                                ? filter.OrderNbr : null;
                            soOrder = soOrderEntryGraph.Document.Insert(soOrder);
                            soOrder.DestinationSiteID = filter.ToSiteID;
                            soOrderEntryGraph.Document.Update(soOrder);
                        }
                        else
                        {
                            //  SO Exists
        ...
        public static void CreateProductionOrders(ProdMaint prodMaint, AMOrderCrossRef orderCrossRef)
        {
            ...
            amProdItem = prodMaint.ProdMaintRecords.Insert(amProdItem);
			...
			if (orderCrossRef.BOMID != null && amProdItem.BOMID != orderCrossRef.BOMID)
			{
				var oldValue = amProdItem.BOMID;
				amProdItem.BOMID = orderCrossRef.BOMID;
				prodMaint.ProdMaintRecords.Cache.RaiseFieldUpdated<AMProdItem.bOMID>(amProdItem, oldValue);
			}
            ...
            amProdItem = prodMaint.ProdMaintRecords.Update(amProdItem);
		...
		protected virtual void GenerateKitAssemblies()
		{
			GenerateKitAssemblies(GroupOrderCrossRef(PXSelect<AMOrderCrossRef,
					Where<AMOrderCrossRef.userID, Equal<Current<AccessInfo.userID>>>,
					OrderBy<Asc<AMOrderCrossRef.siteID, Asc<AMOrderCrossRef.inventoryID,
						Asc<AMOrderCrossRef.subItemID, Asc<AMOrderCrossRef.groupNumber>>>>>>.Select(this)
				.ToFirstTableList()));
		...
		protected virtual void GenerateKitAssemblies(List<AMOrderCrossRef> list)
		{
			if (list == null || list.Count == 0)
			{
				return;
			...
			PXLongOperation.StartOperation(this, () =>
			{
				CreateKitAssemblies(CreateInstance<KitAssemblyEntry>(), list);
			});
		}

		public static void CreateKitAssemblies(KitAssemblyEntry kitAssemblyEntry, List<AMOrderCrossRef> list)
		{
			if (list == null || list.Count == 0 || kitAssemblyEntry == null)
			{
				return;
			}
			...
			foreach (var orderCrossRef in list)
			{
				try
				{
					CreateKitAssemblies(kitAssemblyEntry, orderCrossRef);
				}
				catch (Exception exception)
				{
					var recordNbrs = string.IsNullOrWhiteSpace(orderCrossRef.DmdDetGroupedRecNbrs)
						? orderCrossRef.DmdDetRecNbr.ToString()
						: orderCrossRef.DmdDetGroupedRecNbrs;
					PXTrace.WriteError($"Unable to create Kit Assembly entry for AMRPDetail Record ID {recordNbrs}. {exception.Message}");
					throw exception;
				}
			}
		}

		public static void CreateKitAssemblies(KitAssemblyEntry kitAssemblyEntry, AMOrderCrossRef orderCrossRef)
		{
			if (orderCrossRef == null) return;
			kitAssemblyEntry.Clear();
			kitAssemblyEntry.Document.Current = null;

			PXSelectBase<INKitSpecHdr> iNKitSpecHdrCmd = new PXSelect<INKitSpecHdr,
				Where<INKitSpecHdr.kitInventoryID, Equal<Required<INKitSpecHdr.kitInventoryID>>,
					And<INKitSpecHdr.isActive, Equal<True>>>,
				OrderBy<Desc<INKitSpecHdr.createdDateTime>>>(kitAssemblyEntry);

			INKitSpecHdr iNKitSpecHdr;
			if (orderCrossRef.KitRevisionID != null)
			{
				iNKitSpecHdrCmd.WhereAnd<Where<INKitSpecHdr.revisionID, Equal<Required<INKitSpecHdr.revisionID>>>>();
				iNKitSpecHdr = iNKitSpecHdrCmd.Select(orderCrossRef.InventoryID, orderCrossRef.KitRevisionID);
			}
			else
			{
				iNKitSpecHdr = iNKitSpecHdrCmd.Select(orderCrossRef.InventoryID);
			}

			if (iNKitSpecHdr == null)
			{
				throw new PXException(Messages.GetLocal(Messages.NoActiveKitSpec));
			}

			INKitRegister kitRegister = new INKitRegister(){ DocType = INDocType.Production };

			kitRegister = kitAssemblyEntry.Document.Insert(kitRegister);

			if (orderCrossRef.ManualNumbering == true)
			{
				INKitRegister iNKitRegister = PXSelect
					<INKitRegister, Where<INKitRegister.docType, Equal<Required<INKitRegister.docType>>,
						And<INKitRegister.refNbr, Equal<Required<INKitRegister.refNbr>>>
						>>.Select(kitAssemblyEntry, INDocType.Production, orderCrossRef.ReferenceNbr);
				if (iNKitRegister != null)
				{
					throw new PXException(Messages.GetLocal(Messages.KitIDIsAlreadyUsed,
							orderCrossRef.ReferenceNbr));

				}
				kitRegister.RefNbr = orderCrossRef.ReferenceNbr;
			}

			kitRegister.Qty = orderCrossRef.Qty;
			kitRegister.UOM = orderCrossRef.UOM;
			kitRegister.SubItemID = orderCrossRef.SubItemID;
			kitRegister.SiteID = orderCrossRef.SiteID;
			kitRegister.LocationID = orderCrossRef.LocationID;
			kitRegister.KitInventoryID = iNKitSpecHdr.KitInventoryID;
			kitRegister.KitRevisionID = iNKitSpecHdr.RevisionID;
			kitRegister.KitRequestDate = orderCrossRef.PlanDate;

			kitAssemblyEntry.Document.Update(kitRegister);
			kitAssemblyEntry.Actions.PressSave();

			if (kitAssemblyEntry?.Document?.Current?.RefNbr == null)
			{
				PXTrace.WriteWarning("Kit not assembled");
				return;
			}

			PXTrace.WriteInformation(Messages.GetLocal(Messages.CreatedKitFromMrp, kitAssemblyEntry.Document.Current.DocType, kitAssemblyEntry.Document.Current.RefNbr));

			if (string.IsNullOrWhiteSpace(orderCrossRef.DmdDetGroupedRecNbrs))
			{
				MarkProcessed(kitAssemblyEntry, orderCrossRef.DmdDetRecNbr);
				return;
			}
			...
			foreach (var demandDetailNbr in orderCrossRef.DmdDetGroupedRecNbrs.Split(AMOrderCrossRef.DmdDetGroupedRecNbrsDelimiter.ToCharArray()))
			{
				if (int.TryParse(demandDetailNbr, out var asInt))
				{
					MarkProcessed(kitAssemblyEntry, asInt);
				}
			}
		}
	...
	/// which corresponds to the <see cref="MRPDisplay"/> graph.
	/// </summary>
	[Serializable]
    [PXCacheName("Plan Transfer Filter")]
    public class PlanTransferFilter : PXBqlTable, IBqlTable
    {
        #region OrderType
    ...
    public class PlanPurchaseFilter : PXBqlTable, IBqlTable
    {
        #region OrderType
        public abstract class orderType : PX.Data.BQL.BqlString.Field<orderType> { }
    ...
    public class PlanManufactureFilter : PXBqlTable, IBqlTable
    {
        #region OrderType
        public abstract class orderType : PX.Data.BQL.BqlString.Field<orderType> { }
	...
	/// </summary>
	[Serializable]
	[PXCacheName("Plan Kit Assembly Filter")]
	public class PlanKitFilter : PXBqlTable, IBqlTable {}

    /// <summary>
	/// The filter for the Inventory Planning Display (AM400000) form, which corresponds to the <see cref="MRPDisplay"/> graph.
	/// </summary>
	[Serializable]
	[PXCacheName("MRP Display Filters")]
	public class MRPDisplayFilters : PXBqlTable, IBqlTable
	{
