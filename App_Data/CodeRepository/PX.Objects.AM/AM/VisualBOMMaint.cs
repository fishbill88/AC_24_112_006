...
#pragma warning restore PX1043 // Changes cannot be saved to the database from the event handler
        ...
        protected virtual void CheckDuplicateEntry(PXFieldVerifyingEventArgs e, AMBomMatl matlRow, InventoryItem inventoryItem, int? subItemID)
        {
            ...
            if (IsImport && bomSetup.DuplicateItemOnBOM == SetupMessage.WarningMsg)
            {
                bomSetup.DuplicateItemOnBOM = SetupMessage.AllowMsg;
            }
            if (IsImport && bomSetup.DuplicateItemOnOper == SetupMessage.WarningMsg)
            {
...
//        }
		...
		/// <summary>
		/// The filter for the Visual BOM (AM216000) form, which corresponds to the <see cref="VisualBOMMaint"/> graph.
		/// </summary>
		[Serializable]
        [PXCacheName("Visual BOM Filter")]
        public class VisualBOMFilter : PXBqlTable, IBqlTable
        {
            #region BOMID
