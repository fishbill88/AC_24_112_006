...
namespace PX.Objects.AM
{
    ...
    public class ProductionScheduleEngineAdv : ProductionScheduleEngine
    {
        ...
        private void ProcessWorkCenterSchedule(WorkCenterCalendarHelper workCenterCalendar, ReadDirection scheduleDirection, ref AMSchdOper schdOper, ref DateTime currentSchedDateTime, ref int remainingSchdTime, bool isFirstAttempt)
        {
            ...
            {
				//if current date was already incremented by CalculateUsedTime don't increment it again, but do reset it to 12:00 time
				if(wcDateInfo.WorkCenterDateInfo.Date == currentSchedDateTime.Date)
                currentSchedDateTime = currentSchedDateTime.AddDays(AddDays(scheduleDirection)).Date;
				else
					currentSchedDateTime = currentSchedDateTime.Date;
            }
        }

        protected bool IsCurrentDateUsedUp(AMDateInfo di, DateTime dateTime, ReadDirection direction)
        {
        ...
        protected void CalculateUsedTime(List<AMWCSchdDetail> wcSchdDetailList, ReadDirection scheduleDirection, DateTime currentSchedDateTime, out int usedTime, out DateTime schedDateTime)
        {
            usedTime = 0;
			...
			if (scheduleDirection == ReadDirection.Forward)
				schedDateTime = currentSchedDateTime.Date.AddDays(start > end ? 1 : 0) + end.GetValueOrDefault().TimeOfDay;
            else
				schedDateTime = currentSchedDateTime.Date.AddDays(start > end ? -1 : 0) + start.GetValueOrDefault().TimeOfDay;
        }
    ...
    public class INItemPlanHardSupplyPlanAggregate : PXBqlTable, IBqlTable
    {
        #region InventoryID
        ...
        Aggregate<
            GroupBy<INItemPlan.inventoryID,
                GroupBy<INItemPlanAMExtension.aMSoftSupplyPlanID,
                    Sum<INItemPlan.planQty>>>>>))]
    public class INItemPlanSoftSupplyPlanAggregate : PXBqlTable, IBqlTable
    {
        #region InventoryID
        public abstract class inventoryID : PX.Data.BQL.BqlInt.Field<inventoryID> { }
