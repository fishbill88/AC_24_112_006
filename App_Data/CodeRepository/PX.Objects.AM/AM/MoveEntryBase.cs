...
namespace PX.Objects.AM
{
    ...
    {		
		...
		/// </summary>
		public PXAction<AMBatch> ProductionAttributes;
		/// <summary>
		/// Button delegate for production attributes panel
		/// </summary>
		[PXUIField(DisplayName = "Attributes", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		[PXButton]
		public virtual IEnumerable productionAttributes(PXAdapter adapter)
		{
			TransactionAttributes.AskExt();
			return adapter.Get();
		}
        ...
        {
            // Empty intentionally to avoid breaking change
        }
        ...
        protected virtual void AMMTran_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
        {
			...
			if (row != null && row.Qty.GetValueOrDefault() < 0 && !row.IsScrap.GetValueOrDefault()
				&& ProductionTransactionHelper.TryCheckNegativeMoveQty(sender, row, (decimal)row.Qty, transactions.Select().FirstTableItems.ToList(), out var exception))
			{
				// Acuminator disable once PX1050 HardcodedStringInLocalizationMethod [Justification]
				// Acuminator disable once PX1051 NonLocalizableString [Justification]
				throw new PXRowPersistingException(typeof(AMMTran.qty).Name, row.Qty, exception.Message);
			}
		...
		protected virtual INCostStatus GetINCostStatus(AMMTran row)
		{
			...
			return PXSelect<INCostStatus,
					Where<INCostStatus.inventoryID, Equal<Required<INCostStatus.inventoryID>>,
						...
						And<Where<INCostStatus.costSiteID, Equal<Required<AMMTran.siteID>>,
							Or<INCostStatus.costSiteID, Equal<Required<AMMTran.locationID>>>>>>>>>
					.Select(this, row?.InventoryID, row?.SubItemID, row?.ReceiptNbr, row?.SiteID, row?.LocationID);
        }
