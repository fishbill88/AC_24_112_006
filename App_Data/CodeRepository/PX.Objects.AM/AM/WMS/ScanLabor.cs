	...
	public class ScanLabor : WMSBase
	{
		...
		public sealed class LaborMode : ScanMode
		{
			...
			protected override IEnumerable<ScanState<ScanLabor>> CreateStates()
			{
                ...
                yield return new AMLotSerialState().Intercept.IsStateActive.ByConjoin(basis => basis.SelectedLotSerialClass.LotSerAssign == INLotSerAssign.WhenReceived && basis.IsLastOperation() && basis.IsLotSerialRequired());
			...
			public sealed class ConfirmState : ConfirmationState
			{
				...
				public class Logic : ScanExtension
				{
					...
					protected virtual bool CanConfirm(out FlowStatus error)
					{
						...
						if (Basis.CurrentMode.HasActive<AMLotSerialState>() && !Basis.IsIndirectLabor && Basis.LotSerialTrack.IsTrackedSerial && Basis.Qty != 1)
						{
