...
namespace PX.Objects.AM
{
		...
		where TDocType : IConstant, IBqlOperand, new()
	{
		...
		[PXMergeAttributes]
		[PXUnboundDefault(typeof(INTranType.issue.When<RegisterScanHeader.docType.FromCurrent.IsEqual<AMDocType.material>>.Else<INTranType.receipt>))]
		protected virtual void _(Events.CacheAttached<WMSScanHeader.tranType> e) { }

		[PXMergeAttributes]
		[PXUnboundDefault(typeof(InventoryMultiplicator.decrease.When<ProdScanHeader.docType.FromCurrent.IsEqual<AMDocType.material>>.Else<InventoryMultiplicator.increase>))]
		...
		/// <inheritdoc cref="ProdScanHeader.AllowPreAssigned"/>
	...
	public sealed class ProdScanHeader : PXCacheExtension<WMSScanHeader, QtyScanHeader, ScanHeader>
	{
		...
		/// <inheritdoc cref="AllowPreAssigned"/>
		public abstract class allowPreAssigned : PX.Data.BQL.BqlInt.Field<allowPreAssigned> { }

		/// <summary>
		/// Is the production order (<see cref="AMProdItem"/>) configured for preassigned lot/serial (<see cref="AMProdItem.PreassignLotSerial"/>).
		/// </summary>
		[PXDBBool]
