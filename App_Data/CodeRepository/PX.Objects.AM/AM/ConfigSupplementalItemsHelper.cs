...
namespace PX.Objects.AM
{
    public static class ConfigSupplementalItemsHelper
    {
        ...
        public static void AddSupplementalProductLineItems(CROpportunityProducts parentConfigProducts, PXGraph graph, PXResultset<AMConfigResultsOption> configResultOptions)
        {
			...
			int sortOrder = parentConfigProducts.SortOrder.GetValueOrDefault();
            foreach (PXResult<AMConfigResultsOption, AMConfigurationOption> result in configResultOptions)
            {
                var amConfigResultsOption = (AMConfigResultsOption)result;
                var amConfigurationOption = (AMConfigurationOption)result;
        ...
        /// </summary>
		public static void ResetOpportunityDetailsLineSortOrder(CROpportunityProducts parentConfigProduct, PXGraph graph, PXSelectBase<CROpportunityProducts> detailLines)
        {
            int sortOrder = 0;
            foreach (CROpportunityProducts line in detailLines.Select())
            {
                if (line.SortOrder.GetValueOrDefault() <= parentConfigProduct.SortOrder.GetValueOrDefault()
					&& !IsSupplementalLine(parentConfigProduct,line))
                {
					sortOrder = line.SortOrder.GetValueOrDefault();
                    continue;
                }

                line.SortOrder = ++sortOrder;
				graph.Caches<CROpportunityProducts>().Update(line);
            }
        }
        ...
        /// </summary>
		public static bool IsSupplementalLine(CROpportunityProducts parentLine, CROpportunityProducts childLine)
        {
            var childExt = childLine.GetExtension<CROpportunityProductsExt>();
            if (childExt?.AMParentLineNbr == null || parentLine == null)
            {
                return false;
            }
            ...
            return parentLine.LineNbr.GetValueOrDefault() == childExt.AMParentLineNbr.GetValueOrDefault()
                   && parentLine.LineNbr.GetValueOrDefault() != childLine.LineNbr.GetValueOrDefault()
				   && childExt.AMIsSupplemental.GetValueOrDefault();
        }
