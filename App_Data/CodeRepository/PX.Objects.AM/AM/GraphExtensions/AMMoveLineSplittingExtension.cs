...
namespace PX.Objects.AM
{
		...
		where TMoveGraph : AMBatchEntryBase
	{
		...
		public override void CreateNumbers(AMMTran Row, decimal BaseQty) => base.CreateNumbers(Row, BaseQty);

		public override void CreateNumbers(AMMTran Row, decimal BaseQty, bool forceAutoNextNbr)
		{
			if (Row?.InventoryID == null) return;
			(var _, var lsClass) = ReadInventoryItem(Row.InventoryID);
			var isPreassignedLotSerialDefaulting = Row.IsLotSerialPreassigned == true && lsClass.LotSerTrack != INLotSerTrack.NotNumbered && lsClass.LotSerAssign != INLotSerAssign.WhenUsed;
			...
			{
				ProdItemSplitHelper.BuildPreassignNumbers(Base, Row, (AMMTranSplit)Row);
			}
		}
		...
		[Obsolete("Moved to ProdItemSplitHelper as a generic method")]
		public void BuildPreassignNumbers(AMMTran Row)
		{
			...
			if (AssignedSplits.Count > 0)
			{
				foreach (AMMTranSplit detail in PXParentAttribute.SelectSiblings(SplitCache, (AMMTranSplit)Row, typeof(AMMTran)))
				{
					...
					if (AssignedSplits.ContainsKey(detail.LotSerialNbr) && AssignedSplits[detail.LotSerialNbr].qty > detail.Qty)
					{
						continue;
					}
