    ...
    public class QuoteMaintAMExtension : OpportunityBaseAMExtension<QuoteMaint, CRQuote>
    {
        ...
        public virtual void CopyToQuote(CRQuote currentquote, QuoteMaint.CopyQuoteFilter param, Action<CRQuote, QuoteMaint.CopyQuoteFilter> del)
        {
            ...
            catch (PXRedirectRequiredException redirect)
            {
				...
				bool hasConfiguredItem = ItemConfiguration.Cache.Inserted.Any_();
                if (hasConfiguredItem && (param.RecalculatePrices == true || param.RecalculateDiscounts == true))
                {
                	var Discount = quoteMaintGraph.GetExtension<QuoteMaint.Discount>();
                	...
                	quoteMaintGraph.Actions[nameof(Discount.RecalculateDiscountsAction)].Press();
                }
        ...
        [Serializable]
        [PXHidden]
        public class AMEstimateReferenceQuote : PXBqlTable, IBqlTable
        {
            #region Estimate ID
