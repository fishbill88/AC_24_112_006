            ...
            if (paramExtension != null && paramExtension.AMCopyConfigurations.GetValueOrDefault() && CreateSalesOrderFilterAllowsConfigurations)
            {
                ...
                PXGraph.InstanceCreated.AddHandler<SOOrderEntry>(graph =>
                {
                    graph.RowUpdated.AddHandler<SOLine>((cache, args) =>
                    {
						...
						if (!cache.ObjectsEqual<SOLine.curyLineAmt>(row, args.OldRow))
						{
							var parent = PXParentAttribute.SelectParent(cache, row, typeof(SOOrder));
							PXFormulaAttribute.CalcAggregate<SOLine.taxCategoryID>(cache, parent);
						}
                    });
                });
            }
