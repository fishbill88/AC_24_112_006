...
#pragma warning restore PX1043
		...
		{
			if (e.Row?.OrderType == SOOrderTypeConstants.CreditMemo && e.Row.GetExtension<SOLineExt>()?.AMProdCreate == true)
			{
				e.Cache.SetValueExt<SOLineExt.aMProdCreate>(e.Row, false);
			}
		}
		...
		/// <summary>
		/// Overrides <see cref="SOOrderEntry.CheckPOLinked(SOLineSplit, PXCache)"/>
		/// </summary>
		[PXOverride]
		public virtual bool CheckPOLinked(SOLineSplit line, PXCache cache, Func<SOLineSplit, PXCache, bool> del)
		{
			SOLineSplitExt splitExt = cache.GetExtension<SOLineSplitExt>(line);
			if (line != null && line.ReceivedQty > 0 && splitExt?.AMProdOrdID != null)
			{
				return false;
			}
			return (del?.Invoke(line, cache)).GetValueOrDefault();
		}
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public virtual void CopyOrderProc(SOOrder order, CopyParamFilter copyFilter, Action<SOOrder, CopyParamFilter> del)
        {
			...
			bool disableTaxRecalculation = order.DisableAutomaticTaxCalculation ?? false;
			if (!disableTaxRecalculation)
			{
				TaxBaseAttribute.SetTaxCalc<SOLine.taxCategoryID>(Base.Transactions.Cache, null, TaxCalc.ManualLineCalc);
			}
        ...
        public static void AddEstimateAsSalesLine(SOOrderEntry orderEntryGraph, AMEstimateItem estimateItem, int newEstimateRefStatus)
        {
			...
			if(orderEntryGraph.Transactions.Cache.Inserted.Count() > 1)
			{
				newLine.ShipComplete = (orderEntryGraph.Transactions.Cache.Inserted.RowCast<SOLine>().FirstOrDefault())?.ShipComplete;
			}
			else
			{
				var sourceEstReference = AMEstimateReference.PK.Find(orderEntryGraph, estimateItem.EstimateID, estimateItem.RevisionID);
				newLine.ShipComplete = CRQuote.PK.Find(orderEntryGraph, sourceEstReference?.OpportunityID, sourceEstReference?.QuoteNbr)?.ShipComplete ??
					CROpportunity.PK.Find(orderEntryGraph, sourceEstReference?.OpportunityID)?.ShipComplete ?? newLine.ShipComplete;
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public static void RefreshConfiguredSalesLine(SOOrderEntry graph, SOLine line, AMConfigurationResults configResults, bool calledFromSalesOrder)
        {
            ...
            var locatedLine = (SOLine)graph.Transactions.Cache.Locate(line) ?? line;
        ...
        [Serializable]
        [PXHidden]
        public class AMEstimateReferenceSO : PXBqlTable, IBqlTable
        {
            #region Estimate ID
		...
		/// which corresponds to the <see cref="SOOrderEntry"/> graph.
		/// </summary>
		[Serializable]
        [PXCacheName("Link Prod. Orders Filter")]
        public partial class LinkProdOrderSelectFilter : PXBqlTable, IBqlTable
        {
            #region OrderType
            public abstract class orderType : PX.Data.BQL.BqlString.Field<orderType> { }
	...
	public class SoOrderEntryDRPExtension: PXGraphExtension<SOOrderEntry>
	{
		...
		protected virtual void _(Events.RowInserted<INItemSite> e)
		{
			...
			var itemCurySettings = InventoryItemCurySettings.PK.Find(Base, row.InventoryID, Base.Accessinfo.BaseCuryID);
			if (itemCurySettings != null)
			{
				var itemCurySettingsExt = itemCurySettings.GetExtension<InventoryItemCurySettingsExt>();
				if (itemCurySettingsExt != null)
				{
					row.ReplenishmentSourceSiteID = itemCurySettingsExt.AMSourceSiteID;
				}
			}
		}
	}
