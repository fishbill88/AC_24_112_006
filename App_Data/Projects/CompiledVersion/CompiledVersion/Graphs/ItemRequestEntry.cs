    ...
    public class ItemRequestEntry : PXGraph<ItemRequestEntry>
    {
        ...
        public virtual IEnumerable createStockItem(PXAdapter adapter)
        {
            ...
            foreach (InventoryRequest ardoc in adapter.Get<InventoryRequest>())
            {
                ...
                {
                    throw new PXException(Messages.RequiredFields);
                }
                list.Add(ardoc);
                //}
            }
        ...
        public string ReleaseProcess(List<InventoryRequest> list)
        {
            ...
            foreach (var request in list)
            {
                ...
                using (PXTransactionScope ts = new PXTransactionScope())
                {
                    ...
                    {
                        POVendorInventory vendorInventory = new POVendorInventory();
                        vendorInventory.IsDefault = true;
                        vendorInventory.VendorID = request.VendorID;
                        vendorInventory.VendorLocationID = request.VendorLocationID;
                        vendorInventory.VendorInventoryID = request.VendorInventoryID;
                        vendorInventory.VendorLocationID = request.VendorLocationID;
                        itemGraph.Caches[typeof(POVendorInventory)].Insert(vendorInventory);
                    }
                    ...
                    #region Save Note
                    string noteText = PXNoteAttribute.GetNote(Caches[typeof(InventoryRequest)], request);
                    ...
                    inventoryItemExt.UsrItemSpecs = request.ItemSpecs;
                    itemGraph.Item.Update(item);
                    itemGraph.Save.Press();


                    #region Create new INItemSiteMaint
                    INItemSite itemSite = INItemSite.PK.Find(this, item.InventoryID, request.VendorSiteID,PKFindOptions.IncludeDirty);
                    INItemSiteMaint iNItemSiteMaint = PXGraph.CreateInstance<INItemSiteMaint>();
                    iNItemSiteMaint.Clear();
                    if (itemSite == null)
                    {
                        PXCache cache = iNItemSiteMaint.itemsiterecord.Cache;
                        INItemSite rec = (INItemSite)cache.CreateCopy(cache.Insert());
                        rec.InventoryID = item.InventoryID;
                        rec.SiteID = request.VendorSiteID;
                        rec.PreferredVendorOverride = true;
                        rec.PreferredVendorID = request.VendorID;
                        rec.PreferredVendorLocationID = request.VendorLocationID;
                        //rec.ValMethod = item.ValMethod;
                        //rec.PlanningMethod = item.PlanningMethod;
                        //iNItemSiteMaint.itemsiterecord.SetValueExt<INItemSite.inventoryID>(itemSite, item.InventoryID);
                        //iNItemSiteMaint.itemsiterecord.SetValueExt<INItemSite.siteID>(itemSite, request.VendorSiteID);
                        //iNItemSiteMaint.itemsiterecord.SetValueExt<INItemSite.preferredVendorOverride>(itemSite, true);
                        //iNItemSiteMaint.itemsiterecord.SetValueExt<INItemSite.preferredVendorID>(itemSite, request.VendorID);
                        //iNItemSiteMaint.itemsiterecord.SetValueExt<INItemSite.preferredVendorLocationID>(itemSite, request.VendorLocationID);
                        //iNItemSiteMaint.itemsiterecord.SetValueExt<INItemSite.valMethod>(itemSite, item.ValMethod);
                        //iNItemSiteMaint.itemsiterecord.SetValueExt<INItemSite.planningMethod>(itemSite, item.PlanningMethod);

                        cache.Update(rec);
                        cache.Persist(PXDBOperation.Insert);
                        //iNItemSiteMaint.itemsiterecord.Current = itemSite;
                        //iNItemSiteMaint.itemsiterecord.Update(itemSite);
                        //iNItemSiteMaint.Save.Press();
                    }
                    #endregion
        ...
        public bool ItemClassHasActiveProductBrand(int? itemClassID)
        {
            ...
            if (INSetup.Current == null) INSetup.Current = INSetup.Select();
        ...
        protected virtual void _(Events.FieldDefaulting<InventoryRequest, InventoryRequest.vendorSiteID> e)
        {
            if (e.Row == null) return;
        ...
        protected virtual void _(Events.FieldUpdated<InventoryRequest, InventoryRequest.vendorLocationID> e)
        {
            var row = (InventoryRequest)e.Row;
            if (row == null) return;

            if (row.VendorLocationID == null)
            {
                // Clear vendor site and reset default warehouse to setup default
                e.Cache.SetValueExt<InventoryRequest.vendorSiteID>(row, null);
                ResetDefaultWarehouseToSetup(e.Cache, row);
                return;
            }

            //TrySetDefaultWarehouseFromVendorLocation(e.Cache, row);
        }

        protected virtual void _(Events.FieldUpdated<InventoryRequest, InventoryRequest.vendorID> e)
        {
            var row = (InventoryRequest)e.Row;
            if (row == null) return;
            ...
            {
                e.Cache.SetValueExt<InventoryRequest.vendorLocationID>(row, null);
                e.Cache.SetValueExt<InventoryRequest.vendorSiteID>(row, null);
                ResetDefaultWarehouseToSetup(e.Cache, row);
                return;
            }

            // LocationID may have defaulted; try to set warehouse if we have one
            //TrySetDefaultWarehouseFromVendorLocation(e.Cache, row);
        }

        private void TrySetDefaultWarehouseFromVendorLocation(PXCache cache, InventoryRequest row)
        {
            ...
            if (location?.VSiteID != null)
            {
                // set both DefaultWarehouse and VendorSiteID to the vendor location's site
                cache.SetValueExt<InventoryRequest.defaultWarehouse>(row, location.VSiteID);
                cache.SetValueExt<InventoryRequest.vendorSiteID>(row, location.VSiteID);
            }
            else
            {
                // no site on the location; fall back to setup default
                ResetDefaultWarehouseToSetup(cache, row);
            }
        }

        private void ResetDefaultWarehouseToSetup(PXCache cache, InventoryRequest row)
        {
            var setupExt = INSetup.Cache.GetExtension<INSetupExt>(INSetup.Current);
            cache.SetValueExt<InventoryRequest.defaultWarehouse>(row, setupExt?.UsrDefaultWarehouse);
            cache.SetValueExt<InventoryRequest.vendorSiteID>(row, setupExt?.UsrDefaultWarehouse);
        }
    }
}
