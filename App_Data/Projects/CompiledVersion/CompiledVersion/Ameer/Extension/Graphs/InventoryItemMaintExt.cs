...
{
    /// <summary>
    /// Extension for InventoryItemMaint (Stock Items) to implement cascading attributes from item class hierarchy
    /// </summary>
    public class InventoryItemMaintExt : PXGraphExtension<InventoryItemMaint>
    {
        public static bool IsActive() => true;
        private bool itemClassChanged = false;
        private int? previousItemClassID = null;
        ...
        protected virtual void _(Events.FieldUpdated<InventoryItem, InventoryItem.itemClassID> e)
        {
            ...
            // Track that item class changed and store the new value
            if (e.ExternalCall && e.OldValue != e.NewValue)
            {
                itemClassChanged = true;
                previousItemClassID = (int?)e.OldValue;
            }
        }
        ...
        protected virtual void _(Events.RowUpdated<InventoryItem> e)
        {
            ...
            {
                throw new PXException(string.Format(ACustom.Messages.ErrorPopulatingAttributes, ex.Message));
            }
        }

        /// <summary>
        /// Display hierarchy information when item is selected
        /// </summary>
        ...
        private void PopulateCascadingAttributes(InventoryItem item)
        {
            ...
            try
            {
                // Use the helper to populate attributes
        ...
        /// <summary>
        /// Gets the count of attributes from the item class hierarchy
        /// </summary>
        /// <param name="itemClassID">The item class ID</param>
        /// <returns>Count of cascading attributes</returns>
        private int GetCascadingAttributeCount(int? itemClassID)
        {
            if (itemClassID == null)
                return 0;

            var attributes = ItemClassHierarchyHelper.GetCascadingAttributes(Base, itemClassID);
        ...
        protected virtual void refreshCascadingAttributes()
        {
            ...
            {
                throw new PXException(string.Format(ACustom.Messages.ErrorRefreshingAttributes, ex.Message));
            }
        }
        ...
        protected virtual void viewAttributeHierarchy()
        {
            ...
            if (current == null || current.ItemClassID == null)
            {
                throw new PXException(ACustom.Messages.SelectInventoryItemFirst);
            }
