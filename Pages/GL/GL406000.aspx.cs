...
using static PX.GLAnomalyDetection.AnomalyTransactionEnq;

public partial class Page_GL406000 : PX.Web.UI.PXPage
{
	...
	{
		PXResult<GLTranScoreCurrent> item1 = e.Row.DataItem as PXResult<GLTranScoreCurrent>;
		if (item1 == null) return;
		GLTranScoreCurrent item = (GLTranScoreCurrent)item1;

		if (item.MLScoreColor == GLTranScoreCurrent.StringColor.Light)
		{
			e.Row.Style.CssClass = TranCss.Light;
		}
		else if (item.MLScoreColor == GLTranScoreCurrent.StringColor.Regular)
		{
			e.Row.Style.CssClass = TranCss.Regular;
		}
		else if (item.MLScoreColor == GLTranScoreCurrent.StringColor.Dark)
		{
