...
<asp:Content ID="cont1" ContentPlaceHolderID="phDS" runat="Server">
	<pxa:DynamicDataSource ID="ds" runat="server" AutoCallBack="True" Visible="True"
		...
		<CallbackCommands>
			...
			<px:PXDSCallbackCommand Name="CheckEMailAccount" PopupVisible="true" Visible="False" CommitChanges="True" DependOnGrid="gridEMailAccounts" />
			<px:PXDSCallbackCommand Name="SignIn" Visible="False" CommitChanges="True" DependOnGrid="gridEMailAccounts" StateColumn="CanSignIn" />
			<px:PXDSCallbackCommand Name="UpdatePassword" Visible="False" CommitChanges="True" DependOnGrid="gridEMailAccounts" StateColumn="CanUpdatePassword" />
		</CallbackCommands>
...
<asp:Content ID="cont2" ContentPlaceHolderID="phF" runat="Server">
	<px:PXTab ID="tab" runat="server" DataSourceID="ds" Height="400px" Style="z-index: 100"
		...
		<Items>
				...
				<Template>
					...
					<px:PXFormView ID="formUserPrefs" runat="server" DataMember="UserPrefs" RenderStyle="Simple">
						<Template>
							...
							<px:PXCheckBox ID="chkSmartSuggest" runat="server" DataField="EnableSmartSuggest" />
			...
			<px:PXTabItem Key="EmailSettings" Text="Email Settings">
				<Template>
					<px:PXFormView ID="frmEMailAccounts" runat="server" Width="100%" DataSourceID="ds" CaptionVisible="False" SkinID="Transparent">
						<Template>
							<px:PXLayoutRule ID="PXLayoutRule8" runat="server" StartGroup="true" StartColumn="True" GroupCaption="Personal Email Accounts" />
							<px:PXGrid ID="gridEMailAccounts" runat="server" DataSourceID="ds" Height="120px" Style="z-index: 100"
								Width="100%" Heigth="100%" ActionsPosition="Top" SkinID="Details" AllowPaging="false">
								<Levels>
									<px:PXGridLevel DataMember="EMailAccounts">
										<Columns>
											...
											<px:PXGridColumn DataField="CanSignIn" TextAlign="Center" Type="CheckBox"/>
											<px:PXGridColumn DataField="CanUpdatePassword" TextAlign="Center" Type="CheckBox"/>
										</Columns>
									</px:PXGridLevel>
								</Levels>
								...
								<ActionBar>
									...
									<CustomItems>
										<px:PXToolBarButton Text="Test" CommandName="CheckEmailAccount" CommandSourceID="ds" />
										<px:PXToolBarButton Text="Sign In" CommandName="SignIn" CommandSourceID="ds" StateColumn="CanSignIn" />
										<px:PXToolBarButton Text="Update Password" CommandName="UpdatePassword" CommandSourceID="ds" StateColumn="CanUpdatePassword" />
									</CustomItems>
								</ActionBar>
							</px:PXGrid>
						</Template>
					</px:PXFormView>
					<px:PXFormView ID="form2" runat="server" Width="100%" DataMember="UserPrefs" DataSourceID="ds" SkinID="Transparent" Style="margin-top: -20px;">
						<Template>
							<px:PXLayoutRule ID="PXLayoutRuleEmailService" runat="server" StartGroup="True" GroupCaption="Email Service" LabelsWidth="SM" ControlSize="M" StartColumn="True" />
							<px:PXSelector ID="edDefaultEMailAccountID" runat="server" DataField="DefaultEMailAccountID" DataSourceID="ds" ValueField="emailAccountID" TextField="Address" MaxLength="30" TextMode="Search"/>
							
							<px:PXLayoutRule ID="PXLayoutRuleEmailSignature" runat="server" StartGroup="True" GroupCaption="Email Signature" LabelsWidth="SM" ControlSize="XM" />
							<px:PXCheckBox ID="chkSignatureToNewEmail" runat="server" DataField="SignatureToNewEmail" AlignLeft="True" SuppressLabel="True" />
							<px:PXCheckBox ID="chkSignatureToReplyAndForward" runat="server" DataField="SignatureToReplyAndForward" AlignLeft="True" SuppressLabel="True" />

							<px:PXLayoutRule ID="PXLayoutRuleCalendar" runat="server" StartColumn="true" StartGroup="True" GroupCaption="Calendar" LabelsWidth="SM" ControlSize="XM" ColumnWidth="L" />
							<px:PXCheckBox ID="chbIsPublic" runat="server" DataField="CalendarSettings.IsPublic" AlignLeft="True" />
							<px:PXButton ID="cmdCheckMailSettings" runat="server" Text="Synchronization URL" OnCallBack="cmdCheckMailSettings_CallBack" AlignLeft="True" >
								<AutoCallBack Command="X"></AutoCallBack>
							</px:PXButton>
				
							<px:PXLayoutRule ID="PXLayoutRuleOutlook" runat="server" LabelsWidth="SM" ControlSize="XM" GroupCaption="Outlook" StartGroup="True" Merge="True"/>
							<asp:HyperLink CssClass="ordinary_link" Style="margin-left: 10px; margin-top:5px;" Height="15px" runat="server" ID="OutlookAddin" NavigateUrl="~/OutlookAddinManifest">Download Outlook Add-In Manifest</asp:HyperLink>
							<px:PXButton ID="btnHelp" runat="server" Text="Help" CommandSourceID="ds" 
			...
			<px:PXTabItem Text="Teams Settings">
				<Template>
					<px:PXFormView runat="server" SkinID="Transparent" ID="formOAuthTokenSettings" DataMember="AccessToken">
						<Template>
							<px:PXLayoutRule runat="server" GroupCaption="Authentication Token" LabelsWidth="SM" ControlSize="XM" />
							<px:PXDropDown runat="server" ID="edGraphUserType" DataField="UserType" />
							<px:PXLayoutRule runat="server" Merge="True" />
							<px:PXButton runat="server" ID="edSignIn" CommandSourceID="ds" CommandName="SignIn" AlignLeft="True" Style="margin-left: 8px; margin-top:8px" />
							<px:PXButton runat="server" ID="edSignOut" CommandSourceID="ds" CommandName="SignOut" Style="margin-top:8px" />
							<px:PXButton runat="server" ID="edTestConnection" CommandSourceID="ds" CommandName="TestConnection" Style="margin-top:8px" />
						</Template>
					</px:PXFormView>
					<px:PXFormView runat="server" ID="formContactTeamsSettings" SkinID="Transparent" DataSourceID="ds" DataMember="Contact">
						<Template>
							<px:PXLayoutRule runat="server" StartColumn="True" GroupCaption="Teams Preferences" LabelsWidth="SM" ControlSize="XM" />
			...
			<!--#include file="~\Pages\Includes\EPEmployeeDelegateExtension.inc"-->
	...
	<!--#include file="~\Pages\CR\Includes\CREmailAccountUpdatePasswordExt.inc"-->

	<!--#include file="~\Pages\Includes\SendTestEmailPanel.inc"-->
