...
<asp:Content ID="cont2" ContentPlaceHolderID="phF" runat="Server">
	<script type="text/javascript">
		window.addEventListener('unload', function () {
            ...
            if (!window['fileLinkDialog'] || !origin) return;
			if (!tryRefreshFileDialogFor(origin)) {
                origin.document.querySelectorAll('iframe').forEach((frame) => {
                    tryRefreshFileDialogFor(frame.contentWindow);
                });
            }
		});
        function tryRefreshFileDialogFor(origin) {
            if (origin.px_all && origin.px_all[window['fileLinkDialog']]) {
