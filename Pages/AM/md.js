...
const Localizations = {
    CURRENT: {
        ...
        PRESET_NAME_monthAndYear: window.parent.ClientLocalizedStrings.AM.PRESET_NAME_monthAndYear || 'Months',
				ResourceHistogram_Item_Tooltip: window.parent.ClientLocalizedStrings.AM.ResourceHistogram_Item_Tooltip || '{0}BL of {1}BL allocated'
    }
};
		...
		"Customer"
			...
			listeners: {
				beforeCellEditStart(obj) {
					return obj.editorContext.record.get("editable") !== false;
				},
				beforePresetChange: function (cfg) {
			...
			workCenterScheduler = new ResourceHistogram({
				...
				getBarTip(series, rectConfig, datum, index) {
					...
					var dateFormat = HELPERS.DATA.formats.dateFormatL,
						resultFormatStart = '<b>{resource}</b> {startDate} - {endDate}<br>';
						//resultFormat = resourceHistogram.L("L{barTipInRange}");

					if (DH.compareUnits(timeAxis.unit, "day") == 0) {
						resultFormatStart = '<b>{resource}</b> {startDate}<br>'; //resourceHistogram.L("L{barTipOnDate}");
					} else if (DH.compareUnits(timeAxis.unit, "second") <= 0) {
						dateFormat = "HH:mm:ss A";
					} else if (DH.compareUnits(timeAxis.unit, "hour") <= 0) {
						dateFormat = "LT";
					}
					var resultFormat = resultFormatStart + Localizations.CURRENT.ResourceHistogram_Item_Tooltip;
					result = resultFormat
						.replace(
							"{resource}",
