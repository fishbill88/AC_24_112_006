 ...
 *
 */
import { Base, Delayable, Events, Identifiable, EventHelper, DomDataStore, DomHelper, Objects, Factoryable, ObjectHelper, Tooltip, StringHelper } from './Editor.js';
...
class AvatarRendering extends Base {
  ...
  getResourceAvatar({
    ...
    const config = this.getImageConfig(initials, color, imageUrl, defaultImageUrl, dataset, alt) || this.getIconConfig(iconCls, dataset) || this.getResourceInitialsConfig(initials, color, dataset),
      {
        size
      } = this;
...
export { ActionBase, AvatarRendering, DragContext, DragProxy, Draggable, Droppable, Finalizable, StateBase, StateTrackingManager, Transaction, UpdateAction };
