...
public partial class User_PageTitle : TitlePanel, ITitleModuleController
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!string.IsNullOrEmpty(screenTitle))
		{
			...
			if (Page != null && Page.GetType().Name == "wiki_showwiki_aspx")
			{
	...
	private void FillTitleCaption()
    {
		...
		if (ds.PrimaryView != null)
		{
			...
			if (pnlCaption.HasCaptionAndForm())
			{
				...
				var formFields = form?.DataControls.Values.OfType<IFieldEditor>()
					.Where(fieldEditor => fieldEditor.Visible == true).ToArray() ?? Array<IFieldEditor>.Empty;
				pnlCaption.Caption = ScreenGraphUtils.GetPrimaryKeysCaption(ds.DataGraph,
					formFields.Select(field=>field.DataField).ToArray(),
						formFields.Where(item=>(item as PXSelector) != null)
							.ToDictionary(item=>item.DataField, item=> (item as PXSelector)?.Text),
					ds.HeaderDescriptionField);
    ...
    void InitAUWorkflow()
    {
		...
		if (PXDynamicFormGenerator.CreateChildControls(graph, FormPreview, !(p.DefaultDataSource is AUDataSource)))
		{
