...
public partial class Customization_CustomizationDialogs : System.Web.UI.UserControl//, IDlgCallback
{
    ...
    protected override void OnInit(EventArgs e)
    {
        ...
        var menus = (PXToolBar1.Items[0] as PXToolBarButton);
        if (menus == null)
	        return;
        ...
        if (page !=null && page.DefaultDataSource != null && page.DefaultDataSource.EnableAttributes)
        {
            var menuItem = new PXMenuItem("Manage User-Defined Fields")
            {
	            AutoCallBack =
	            {
		            Command = "ManageAttributes",
		            Target = "DsControlProps"
	            }
            };
            menus.MenuItems.Insert(2, menuItem);
        }
        ...
        if (page != null && page.DefaultDataSource  != null && page.DefaultDataSource.DataGraph?.Actions["ShowWorkflow"] != null)
        {
	        ...
	        menus.MenuItems.Add(menuItem);
        }

        var currentNode = PXSiteMap.CurrentNode;
        if (page?.Master is BaseMasterPage { IsEnableSiteMapSwitchUI: true }
            && !string.IsNullOrEmpty(currentNode.Url))
        {
	        var graphType = GraphHelper.GetType(currentNode.GraphType);
	        if (graphType != null)
	        {
		        var role = ServiceLocator.Current.GetInstance<IOptions<CustomizationOptions>>().Value.CustomizerRole;
		        var userIsCustomizer = PXContext.PXIdentity.AuthUser.With(_ => _.IsInRole(role));
		        if (userIsCustomizer && graphType != typeof(PXGenericInqGrph))
		        {
			        // converter to modern UI
			        var converterMenuItem = new PXMenuItem(ActionsMessages.ConvertToModernScreen)
			        {
				        CommandName = "ConvertToModernScreen",
				        CommandArgument = currentNode.ScreenID,
				        CommandSourceID = "tlbDataView"
			        };
			        menus.MenuItems.Add(converterMenuItem);

			        if (menus.MenuItems.Count > 1)
				        menus.MenuItems[menus.MenuItems.Count - 2].ShowSeparator = true;
		        }
	        }
        }
    }
